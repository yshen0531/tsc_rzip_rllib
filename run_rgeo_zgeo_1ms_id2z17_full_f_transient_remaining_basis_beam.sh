...
SOURCE_REVISION="${ID2Z17_SOURCE_REVISION:-}"
OUTPUT="${ID2Z17_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z17_full_f_transient_remaining_basis_beam.json"

fail() { printf 'ID2Z17_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z17_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z17_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z17_full_f_transient_remaining_basis_beam.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z17_full_f_transient_remaining_basis_beam_independent.py \
