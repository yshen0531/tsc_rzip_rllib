...
RUN_DIR="${STAGE4_2R3C3T13S24D1R14R8R8_RUN_DIR:-$(stage4_2r3c3t13s24d1r14r8r8_new_run_dir)}"
STATE_FILE="${RUN_DIR}/stage4_2r3c3t13s24d1r14r8r8_causal_discrete_pulse_receding_horizon_mpc_core/stage_state.json"
if [[ "${STAGE4_2R3C3T13S24D1R14R8R8_COMMAND}" == offline ]]; then
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]
elif [[ "${STAGE4_2R3C3T13S24D1R14R8R8_COMMAND}" != self-test ]]; then
  [[ -f "${STATE_FILE}" ]] || { echo "ERROR: R8R8 frozen state missing" >&2; exit 1; }
fi
mkdir -p "${RUN_DIR}"
stage4_2r3c3t13s24d1r14r8r8_export
cd "${PROJECT_DIR}"
printf '[R8R8] user=%s host=%s command=%s run_dir=%s\n' "$(id -un)" "$(hostname)" "${STAGE4_2R3C3T13S24D1R14R8R8_COMMAND}" "${RUN_DIR}"
printf '[R8R8] maximum 16 fresh deterministic core trajectories; all are forbidden from expert and learning data.\n'
if [[ "${STAGE4_2R3C3T13S24D1R14R8R8_COMMAND}" == self-test ]]; then
  exec "${STAGE4_2R3C3T13S24D1R14R8R8_PYTHON}" -c "from pathlib import Path; from tsc_rzip_rllib.diagnostics.stage4_2r3c3t13s24d1r14r8r8_causal_discrete_pulse_mpc_core import self_test; print(self_test(Path(r'${STAGE4_2R3C3T13S24D1R14R8R8_CONFIG}')))"
fi
args=(
  --config "${STAGE4_2R3C3T13S24D1R14R8R8_CONFIG}" --run-dir "${RUN_DIR}" --r8r7-run "${STAGE4_2R3C3T13S24D1R14R8R8_R8R7_RUN}"
...
if [[ -n "${AUDIT_KIND}" ]]; then
  exec "${STAGE4_2R3C3T13S24D1R14R8R8_PYTHON}" docs/codex/audit_tools/stage4_2r3c3t13s24d1r14r8r8_independent_forensics.py "${args[@]}" --audit-kind "${AUDIT_KIND}"
fi
if [[ "${STAGE4_2R3C3T13S24D1R14R8R8_RESUME}" == 1 ]]; then args+=(--resume); fi
exec "${STAGE4_2R3C3T13S24D1R14R8R8_PYTHON}" scripts/stage4_2r3c3t13s24d1r14r8r8_causal_discrete_pulse_mpc_campaign.py "${args[@]}" --command "${STAGE4_2R3C3T13S24D1R14R8R8_COMMAND}" --backend "${STAGE4_2R3C3T13S24D1R14R8R8_BACKEND}"
