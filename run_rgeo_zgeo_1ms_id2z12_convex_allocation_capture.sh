...
SOURCE_REVISION="${ID2Z12_SOURCE_REVISION:-}"
OUTPUT="${ID2Z12_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z12_convex_allocation_capture.json"

fail() { printf 'ID2Z12_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z12_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z12_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z12_convex_allocation_capture.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z12_convex_allocation_capture_independent.py \
