...
RUN_DIR="${STAGE4_2R3C3T13S24D1R6_RUN_DIR:-$(stage4_2r3c3t13s24d1r6_new_run_dir)}"
STATE_FILE="${RUN_DIR}/stage4_2r3c3t13s24d1r6_recursive_split_return_safety_sentinel/stage_state.json"
if [[ "${STAGE4_2R3C3T13S24D1R6_COMMAND}" == offline || "${STAGE4_2R3C3T13S24D1R6_COMMAND}" == all ]] && [[ "${STAGE4_2R3C3T13S24D1R6_RESUME}" == 0 ]]; then
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]] || { echo "ERROR: fresh D1R6 run directory not empty" >&2; exit 1; }
else
  [[ -f "${STATE_FILE}" ]] || { echo "ERROR: frozen D1R6 state missing" >&2; exit 1; }
fi
mkdir -p "${RUN_DIR}"
stage4_2r3c3t13s24d1r6_export_runtime
printf '[T13S24D1R6] user=%s host=%s pwd=%s\n' "$(id -un)" "$(hostname)" "$(pwd)"
printf '[T13S24D1R6] run_dir=%s command=%s backend=%s resume=%s workers=%s\n' "${RUN_DIR}" "${STAGE4_2R3C3T13S24D1R6_COMMAND}" "${STAGE4_2R3C3T13S24D1R6_BACKEND}" "${STAGE4_2R3C3T13S24D1R6_RESUME}" "${STAGE4_2R3C3T13S24D1R6_WORKERS}"
printf '[T13S24D1R6] exact nine-case real-TSC recursive split-return safety sentinel.\n'
printf '[T13S24D1R6] arrival by 250 ms and hold through 350 ms unchanged; formal result diagnostic only.\n'
cd "${PROJECT_DIR}"
args=(
  ...
  --run-dir "${RUN_DIR}" --command "${STAGE4_2R3C3T13S24D1R6_COMMAND}" --backend "${STAGE4_2R3C3T13S24D1R6_BACKEND}"
)
[[ "${STAGE4_2R3C3T13S24D1R6_RESUME}" == 1 ]] && args+=(--resume)
exec "${STAGE4_2R3C3T13S24D1R6_PYTHON}" scripts/stage4_2r3c3t13s24d1r6_recursive_split_return_safety_sentinel.py "${args[@]}"
