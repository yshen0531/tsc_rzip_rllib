...
mkdir -p "$RAY_TMPDIR" "$RAY_TMPDIR/tmp" stage2_runs
...
cat <<EOF
[Stage2] user=$(whoami) host=$(hostname) pwd=$(pwd)
[Stage2] source_stage1_1_run=$SOURCE_STAGE1_1_RUN
[Stage2] run_dir=$STAGE2_RUN_DIR
[Stage2] config=$CONFIG
[Stage2] workers=$STAGE2_WORKERS
[Stage2] RAY_TMPDIR=$RAY_TMPDIR
[Stage2] TSC_WORKSPACE_ROOT=${STAGE2_TSC_WORKSPACE_ROOT:-<source-config>}
[Stage2] TSC_RUN_ROOT=${STAGE2_TSC_RUN_ROOT:-<source-config>}
[Stage2] 3 SVD modes x 5 time nodes = 15 optimization variables.
[Stage2] Linear model pre-screens R/Z/Ip only; real TSC ranks speed and hold.
[Stage2] Immediate TERM/KILL is not intercepted. Completed candidate JSON files remain resumable.
EOF

set +e
python -u scripts/stage2_trajectory_optimization.py all \
  --config "$CONFIG" \
