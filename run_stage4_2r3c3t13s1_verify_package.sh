...
if not t13s1.self_test()["passed"]:
    raise SystemExit("T13S1 self-test failed")
cfg = json.loads(
    (
        root
...
t13s1._validate_config(cfg)
for path in (
    root / "run_stage4_2r3c3t13s1_native.sh",
    root / "run_stop_stage4_2r3c3t13s1_now.sh",
):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[Stage4.2R3c3T13S1 verify] Python/JSON/scientific guards passed.")
PY
...
"${PYTHON_BIN}" -m unittest \
  tests.test_stage4_2r3c3t13s1_minimal_transition_sentinel
printf '[Stage4.2R3c3T13S1 verify] checksums, shell syntax and focused tests passed.\n'
