...
if not t6.self_test()["passed"]:
    raise SystemExit("T6 self-test failed")
cfg = json.loads(
    (
        root
        / "configs/stage4_2r3c3t6_target_residual_new_direction_identification_500ms.json"
    ).read_text(encoding="utf-8")
)
t6._validate_config(cfg)
for path in (
    root
    / "run_stage4_2r3c3t6_target_residual_new_direction_identification_native.sh",
    root / "run_stop_stage4_2r3c3t6_now.sh",
):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[Stage4.2R3c3T6 verify] Python/JSON/scientific guards passed.")
PY
...
"${PYTHON_BIN}" -m unittest \
  tests.test_stage4_2r3c3t6_target_residual_new_direction_identification
printf '[Stage4.2R3c3T6 verify] checksums, shell syntax and focused tests passed.\n'
