...
SOURCE_REVISION="${ID2Z18_SOURCE_REVISION:-}"
OUTPUT="${ID2Z18_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z18_full_horizon_token_development.json"

fail() { printf 'ID2Z18_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z18_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z18_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z18_full_horizon_token_development.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z18_full_horizon_token_development_independent.py \
