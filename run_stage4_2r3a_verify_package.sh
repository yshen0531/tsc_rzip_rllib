...
for path in \
  ...
  tests/test_stage4_2r3a_delayed_counterpulse_hidden_history_initial_state.py \
  ...
  run_stage4_2r3a_self_test.sh run_stage4_2r3a_verify_package.sh \
  run_stop_stage4_2r3a_now.sh; do
  [[ -f "${path}" ]] || {
...
printf '[Stage4.2R3a verify] packaged file checksums passed.\n'
...
expected = {
    ...
    "run_name": "stage4_2r3a_delayed_counterpulse_hidden_history_initial_state",
}
...
if not self_test.get("passed"):
    raise SystemExit("Stage4.2R3a self-test failed")
...
common_prefix = {
    ...
    "prefix_action_digests": {
        "4": "verify-prefix-digest-4",
        "8": "verify-prefix-digest-8",
    },
...
if len(specs) != 72 or len({row["pair_id"] for row in specs}) != 36:
    raise SystemExit("Stage4.2R3a state-generation matrix changed")
if r3._requested_workers(cfg) != 128:
    raise SystemExit("Stage4.2R3a Ray capacity changed")
...
for token in (
    ...
    if token not in module:
        raise SystemExit(f"Stage4.2R3a implementation guard missing: {token}")
for path in (
    root / "run_stage4_2r3a_delayed_counterpulse_hidden_history_initial_state_native.sh",
    root / "run_stop_stage4_2r3a_now.sh",
):
    if "ray stop --force" in path.read_text(encoding="utf-8"):
        raise SystemExit("Stage4.2R3a launcher contains broad Ray termination")
print(
    "[Stage4.2R3a verify] Python compile, JSON parse, import closure and "
...
printf '[Stage4.2R3a verify] declared shell scripts passed bash -n.\n'
"${PYTHON_BIN}" -m unittest discover -s tests -p 'test_*.py'
printf '[Stage4.2R3a verify] complete unittest discovery passed.\n'
