...
PYTHON_BIN="${STAGE4_2R3C3T13S24_PYTHON:-/home/yangshen0711/tsc_all/tsc_simulation/venv_simu/bin/python}"
...
from tsc_rzip_rllib.control import sequential_transition
from tsc_rzip_rllib.diagnostics import stage4_2r3c3t13s24_sequential_transition_identification as s24
config = root / "configs/stage4_2r3c3t13s24_sequential_transition_identification_370ms.json"
cfg = json.loads(config.read_text(encoding="utf-8"))
s24._validate_config(cfg, config)
if not s24.self_test(config)["passed"]:
    raise SystemExit("S24 self-test failed")
if sequential_transition.PREDICTOR_ITEM_FIELDS & set(cfg["transition_model"]["forbidden_predictor_fields"]):
    raise SystemExit("S24 predictor whitelist overlaps forbidden fields")
for path in (
    root / "run_stage4_2r3c3t13s24_native.sh",
    root / "run_stop_stage4_2r3c3t13s24_now.sh",
):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[T13S24 verify] Python/JSON/scientific guards passed.")
PY
mapfile -t SHELLS < <(find . -maxdepth 2 -type f -name '*.sh' -print | sort)
for script in "${SHELLS[@]}"; do bash -n "${script}"; done
"${PYTHON_BIN}" -m unittest -v \
  tests.test_sequential_transition \
  tests.test_stage4_2r3c3t13s24_sequential_transition_identification
printf '[T13S24 verify] checksums, shell syntax and focused tests passed.\n'
