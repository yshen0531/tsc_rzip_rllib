...
source "${PROJECT_DIR}/scripts/stage4_2r3c3t13s24d1_shell_common.sh"
...
OUTPUT_DIR="${STAGE4_2R3C3T13S24D1_OUTPUT_DIR:-$(stage4_2r3c3t13s24d1_new_output_dir)}"
[[ ! -e "${OUTPUT_DIR}" ]] || {
  echo "ERROR: S24D1 output must be a new path: ${OUTPUT_DIR}" >&2
  exit 1
}
for path in "${SOURCE_S23R1}" "${SOURCE_S24}" "${SOURCE_S24_FORENSICS}" "${SOURCE_S24_LOG}"; do
  [[ -e "${path}" ]] || { echo "ERROR: required immutable source missing: ${path}" >&2; exit 1; }
done
stage4_2r3c3t13s24d1_export_runtime
printf '[Stage4.2R3c3T13S24D1] user=%s host=%s pwd=%s\n' "$(id -un)" "$(hostname)" "$(pwd)"
printf '[Stage4.2R3c3T13S24D1] source_s24=%s\nsource_s23r1=%s\noutput=%s\n' \
  "${SOURCE_S24}" "${SOURCE_S23R1}" "${OUTPUT_DIR}"
printf '[Stage4.2R3c3T13S24D1] zero plant/controller/Ray/gotsc/TSC execution; fixed 0.25/0.25/0.225/0.225 replay.\n'
printf '[Stage4.2R3c3T13S24D1] Formal 250/270 ms arrival and 350/370 ms hold timing is unchanged.\n'
cd "${PROJECT_DIR}"
exec "${STAGE4_2R3C3T13S24D1_PYTHON}" \
  docs/codex/audit_tools/stage4_2r3c3t13s24d1_contracted_amplitude_preflight.py \
  --config "${STAGE4_2R3C3T13S24D1_CONFIG}" --design-document "${STAGE4_2R3C3T13S24D1_DESIGN}" \
  ...
  --source-s23r1-output "${SOURCE_S23R1}" \
  --source-s24-run "${SOURCE_S24}" \
