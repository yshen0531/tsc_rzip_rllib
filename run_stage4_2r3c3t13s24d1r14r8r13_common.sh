...
PYTHON="${STAGE4_2R3C3T13S24D1R14R8R13_PYTHON:-/home/yangshen0711/tsc_all/tsc_simulation/venv_simu/bin/python}"
CONFIG="${STAGE4_2R3C3T13S24D1R14R8R13_CONFIG:-${PROJECT_DIR}/configs/stage4_2r3c3t13s24d1r14r8r13_measured_additive_direction0_on_direction2_composition.json}"
COMMAND="${STAGE4_2R3C3T13S24D1R14R8R13_COMMAND:-primary}"
OUTPUT_ROOT="${STAGE4_2R3C3T13S24D1R14R8R13_OUTPUT_ROOT:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r13_runs}"
RUN_DIR="${STAGE4_2R3C3T13S24D1R14R8R13_RUN_DIR:-${OUTPUT_ROOT}/stage4_2r3c3t13s24d1r14r8r13_measured_additive_direction0_on_direction2_composition_$(date -u +%Y%m%d_%H%M%S)}"
R8R7_RUN="${STAGE4_2R3C3T13S24D1R14R8R13_R8R7_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r7_runs/stage4_2r3c3t13s24d1r14r8r7_fresh_multipulse_static_observer_interaction_sentinel_20260807_d8d231e_v1}"
R8R11_RUN="${STAGE4_2R3C3T13S24D1R14R8R13_R8R11_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r11_runs/stage4_2r3c3t13s24d1r14r8r11_sustained_exact_target_refresh_authority_sentinel_20260807_863692b_v1}"
R8R12_RUN="${STAGE4_2R3C3T13S24D1R14R8R13_R8R12_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r12_runs/stage4_2r3c3t13s24d1r14r8r12_causal_cumulative_direction2_staircase_authority_sentinel_20260808_523c706_v2}"
...
[[ -x "${PYTHON}" ]]
[[ -f "${CONFIG}" ]]
[[ -d "${R8R7_RUN}" && -d "${R8R11_RUN}" && -d "${R8R12_RUN}" ]]
...
STAGE_DIR="${RUN_DIR}/stage4_2r3c3t13s24d1r14r8r13_measured_additive_direction0_on_direction2_composition"
if [[ "${COMMAND}" == primary ]]; then
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]
else
  [[ -f "${STAGE_DIR}/analysis/primary_summary.json" ]]
  [[ "$("${PYTHON}" -c 'import json,sys; print(json.load(open(sys.argv[1]))["phase_status"])' "${STAGE_DIR}/stage_state.json")" == primary_ready ]]
fi
...
cd "${PROJECT_DIR}"
printf '[R8R13] user=%s host=%s command=%s run_dir=%s zero_new_tsc=true\n' "$(id -un)" "$(hostname)" "${COMMAND}" "${RUN_DIR}"

args=(
  --config "${CONFIG}" --run-dir "${RUN_DIR}" --r8r7-run "${R8R7_RUN}" --r8r11-run "${R8R11_RUN}" --r8r12-run "${R8R12_RUN}"
...
if [[ "${COMMAND}" == primary ]]; then
  exec "${PYTHON}" -m tsc_rzip_rllib.diagnostics.stage4_2r3c3t13s24d1r14r8r13_measured_additive_direction0_on_direction2_composition "${args[@]}"
fi
exec "${PYTHON}" docs/codex/audit_tools/stage4_2r3c3t13s24d1r14r8r13_independent_forensics.py "${args[@]}"
