...
PID_FILE="${PROJECT_DIR}/stage2_2_runs/stage2_2_driver.pid"
ALT_PID_FILE="${PROJECT_DIR}/logs/nohup/latest_stage2_2_svd3_corner.pid"
PID=""
...
if [[ -n "${PID}" ]] && kill -0 "${PID}" 2>/dev/null; then
  ...
  case "${PROCESS_ARGS}" in
    *run_stage2_2_svd3_corner_native.sh*|*scripts/stage2_2_trajectory_optimization.py*)
      ...
      ;;
    *) echo "WARNING: saved PID ${PID} no longer belongs to Stage2.2; refusing to signal it." >&2 ;;
  esac
fi

pkill -TERM -f '[s]cripts/stage2_2_trajectory_optimization.py' 2>/dev/null || true
pkill -TERM -f '[r]un_stage2_2_svd3_corner_native.sh' 2>/dev/null || true
sleep 2
pkill -KILL -f '[s]cripts/stage2_2_trajectory_optimization.py' 2>/dev/null || true
pkill -KILL -f '[r]un_stage2_2_svd3_corner_native.sh' 2>/dev/null || true
...
RAY_TMP="${RAY_TMPDIR:-/tmp/stage2_2_$(id -u)}"
...
echo "[Stage2.2] driver/process group stopped; Ray and temporary /tmp runtime directories cleaned."
echo "[Stage2.2] result directories under stage2_2_runs were not deleted."
