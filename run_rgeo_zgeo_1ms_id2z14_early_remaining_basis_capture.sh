...
SOURCE_REVISION="${ID2Z14_SOURCE_REVISION:-}"
OUTPUT="${ID2Z14_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z14_early_remaining_basis_capture.json"

fail() { printf 'ID2Z14_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z14_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z14_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z14_early_remaining_basis_capture.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z14_early_remaining_basis_capture_independent.py \
