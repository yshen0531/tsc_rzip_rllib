...
for path in \
  ...
  tests/test_stage4_2r3c1_authenticated_visible_manifold_phase_mpc.py \
  ...
  run_stage4_2r3c1_self_test.sh run_stage4_2r3c1_verify_package.sh \
  run_stop_stage4_2r3c1_now.sh; do
  [[ -f "${path}" ]] || {
...
printf '[Stage4.2R3c1 verify] packaged file checksums passed.\n'
...
expected = {
    ...
    "run_name": "stage4_2r3c1_authenticated_visible_manifold_phase_mpc",
}
...
if not self_test.get("passed"):
    raise SystemExit("Stage4.2R3c1 self-test failed")
...
if r3c._requested_workers(cfg) != 128:
    raise SystemExit("Stage4.2R3c1 Ray capacity changed")
...
if (
    ...
    raise SystemExit("Stage4.2R3c1 scientific scope changed")
...
if (
    ...
    raise SystemExit("Stage4.2R3c1 visible-manifold contract changed")
...
if (
    ...
    raise SystemExit("Stage4.2R3c1 failed-R3c evidence contract changed")
module = (
    root
...
for token in (
    ...
    "AuthenticatedVisibleManifoldPhaseTaskController",
    ...
    if token not in module:
        raise SystemExit(f"Stage4.2R3c1 implementation guard missing: {token}")
for path in (
    root / "run_stage4_2r3c1_authenticated_visible_manifold_phase_mpc_native.sh",
    root / "run_stop_stage4_2r3c1_now.sh",
):
    if "ray stop --force" in path.read_text(encoding="utf-8"):
        raise SystemExit("Stage4.2R3c1 launcher contains broad Ray termination")
print(
    "[Stage4.2R3c1 verify] Python compile, JSON parse, import closure and "
...
printf '[Stage4.2R3c1 verify] declared shell scripts passed bash -n.\n'
"${PYTHON_BIN}" - <<'PY'
import unittest
...
PY
printf '[Stage4.2R3c1 verify] complete unittest discovery passed.\n'
