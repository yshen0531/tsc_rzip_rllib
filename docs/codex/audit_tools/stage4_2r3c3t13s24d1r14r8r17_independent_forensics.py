#!/usr/bin/env python3
"""Structurally independent R8R17 adjacent-switch interaction audit."""
...
def _row(code: str) -> np.ndarray:
    if len(code) != 4 or any(symbol not in "UV" for symbol in code):
        raise ValueError("independent R8R17 code changed")
    q = np.asarray([1.0 if symbol == "U" else -1.0 for symbol in code])
    persistence = float(np.dot(q[:-1], q[1:]) / 3.0)
    return np.concatenate((np.asarray([1.0]), q, np.asarray([persistence])))
...
def _fit(codes: Sequence[str], values: Sequence[np.ndarray]) -> np.ndarray:
    ...
    if rank != 6 or not np.all(np.isfinite(flat)):
        raise ValueError("independent R8R17 fit changed")
    return flat.reshape(6, y.shape[1], y.shape[2])
...
def _response(result: Mapping[str, Any], baseline: Mapping[str, Any]) -> np.ndarray:
    ...
    if values.shape != center.shape or values.shape[0] <= 10 or not np.all(np.isfinite(values)) or not np.all(np.isfinite(center)):
        raise ValueError("independent R8R17 source trajectory changed")
...
def _prediction_result(baseline: Mapping[str, Any], delta: np.ndarray) -> dict[str, Any]:
    ...
    if delta.shape != (len(output["trajectory"]) - 10, 3):
        raise ValueError("independent R8R17 predicted horizon changed")
...
def _authenticate(args: argparse.Namespace, cfg: Mapping[str, Any], source_ctx: r15.Context) -> dict[str, Any]:
    ...
    if any(hashes[name] != str(expected[f"{name}_sha256"]) for name in hashes):
        raise ValueError("independent R8R17 source hash changed")
    for phase in ("safety", "qualification"):
        ...
        if (int(inventory["count"]), int(inventory["bytes"]), str(inventory["digest"])) != (
            ...
            raise ValueError("independent R8R17 source inventory changed")
...
def _authenticate_r8r16(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    hashes = {name: _sha(path) for name, path in paths.items()}
    if any(hashes[name] != str(expected[f"{name}_sha256"]) for name in hashes):
        raise ValueError("independent R8R17 source R8R16 hash changed")
...
def _collect(args: argparse.Namespace, cfg: Mapping[str, Any], source_ctx: r15.Context) -> tuple[list[dict[str, Any]], dict[tuple[str, str], dict[str, Any]]]:
    ...
    if len(groups) != 16 or any(set(group["candidates"]) != measured for group in groups.values()):
        raise ValueError("independent R8R17 measured coverage changed")
...
def recompute(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or tuple(cfg["model_contract"]["feature_order"]) != ("intercept", "q10", "q14", "q18", "q22", "normalized_adjacent_persistence")
        or cfg["model_contract"].get("adjacent_interaction_formula") != "(q10*q14+q14*q18+q18*q22)/3"
    ):
        raise ValueError("independent R8R17 frozen design changed")
    ...
    geometry = {
        ...
        "cube_rank": int(np.linalg.matrix_rank(_matrix(sorted({"".join(row) for row in __import__("itertools").product("UV", repeat=4)})))),
        "cube_condition": float(np.linalg.cond(_matrix(sorted({"".join(row) for row in __import__("itertools").product("UV", repeat=4)})))),
    }
    model_gate = bool(
        source["passed"] and r8r16_source["passed"]
        and geometry["development_rank"] == 6 and geometry["full_rank"] == 6 and geometry["cube_rank"] == 6
        ...
        and geometry["cube_condition"] <= float(cfg["model_contract"]["maximum_cube_condition"])
    ...
    return {
        ...
        "source_authenticated": bool(source["passed"] and r8r16_source["passed"]), "geometry": geometry,
