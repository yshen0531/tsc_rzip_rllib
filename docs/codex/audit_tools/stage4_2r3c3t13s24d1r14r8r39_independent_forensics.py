#!/usr/bin/env python3
"""Independent raw-bank and fixed equal cold-ensemble audit for R8R39."""
...
STAGE = r8r39.STAGE
IDENTITY = r8r39.IDENTITY
AUDIT_KIND = "fixed_equal_global_ridge_local_constant_cold_ensemble_independent"
...
def _coordinate(row: Mapping[str, Any]) -> np.ndarray:
    ...
    if coordinate.shape != (62,) or not np.isfinite(coordinate).all():
        raise ValueError("independent R8R39 coordinate invalid")
    return coordinate
...
def _fit_local(
    ...
    if not chosen:
        raise ValueError("independent R8R39 local fit selection empty")
    ...
    for interval_index in range(6):
        ...
        for membership, offsets in memberships.items():
            ...
            if (
                ...
                raise ValueError("independent R8R39 local training group invalid")
            groups.append(
                {
    ...
    return {"model_kind": "causal_local_constant_k64_cold", "intervals": intervals}
...
def _predict_local(
    ...
    for group in model["intervals"][interval]["groups"]:
        ...
        values = np.sum(target[chosen], axis=0) / len(chosen)
    ...
    if not filled.all() or not np.isfinite(output).all():
        raise ValueError("independent R8R39 local prediction coverage invalid")
...
def _predict(
    ...
    if ensemble.shape != global_prediction.shape or not np.isfinite(ensemble).all():
        raise ValueError("independent R8R39 ensemble prediction invalid")
...
def _schedule(
    ...
    for held_schedule in schedules:
        ...
        if len(held) != 16:
            raise ValueError("independent R8R39 schedule fold cardinality changed")
        residual_groups, residual_digest, predictions = _residuals(model, held, cfg)
        evidence = _model_evidence(model)
        folds.append(
            {
...
def _verify_bank(
    ...
    if (
        ...
        raise ValueError("independent R8R39 bank reproduction changed")
...
def _compute(
    ...
    if not cardinality["passed"]:
        raise ValueError("independent R8R39 cardinality failed")
    ...
    return r8r39._jsonable(detailed), r8r39._jsonable(artifact)
...
def _authenticate_final(stage: Path, contract: Mapping[str, Any], label: str) -> dict[str, Any]:
    ...
    if any(hashes[name] != contract[f"{name}_sha256"] for name in hashes):
        raise ValueError(f"independent R8R39 {label} source hash changed")
    ...
    if (
        ...
        raise ValueError(f"independent R8R39 {label} source state changed")
    return {"hashes": hashes, "passed": True}


def audit(args: argparse.Namespace) -> dict[str, Any]:
    ctx = r8r39.load_context(args)
    if ctx.cfg.get("stage") != STAGE or ctx.cfg.get("identity") != IDENTITY:
        raise ValueError("independent R8R39 configuration identity changed")
    ...
    r37_auth = _authenticate_final(ctx.r8r37_stage, ctx.cfg["source_r8r37"], "R8R37")
    ...
    independent_detailed, independent_model = _compute(
        ctx,
        {"r8r31_final": r31_auth, "r8r37_final": r37_auth, "independent_transitive": independent_transitive, "passed": True},
        trajectories,
        bank_evidence,
    )
    ...
    if not result["passed"]:
        ...
        raise ValueError("independent R8R39 preflight disagrees with primary")
...
def _parser() -> argparse.ArgumentParser:
    ...
    for name in r8r39.ARGUMENT_NAMES:
