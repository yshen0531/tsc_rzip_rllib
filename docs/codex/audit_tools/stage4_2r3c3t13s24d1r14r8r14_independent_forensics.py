#!/usr/bin/env python3
"""Structurally independent R8R14 source, action, raw, and formal forensics."""
...
def _r8r13_stage(args: argparse.Namespace, cfg: Mapping[str, Any]) -> Path:
    return args.r8r13_run.expanduser().resolve() / str(
        cfg["source_r8r13"]["stage_directory"]
    )
...
def _source_baselines(
    ...
    if len(specs) != 16 or {
        ...
        raise ValueError("independent R8R14 source coverage changed")
...
def _source_r8r12_candidates(
    ...
    if (
        ...
        raise ValueError("independent R8R14 source R8R12 candidate matrix changed")
    results = {
        str(row["experiment_id"]): _gzip(
...
def _expected_specs(
    ...
    for index, source in enumerate(baselines):
        ...
        for direction, sign in cfg["schedule_contract"]["new_candidates"]:
...
def _authenticate_r8r12(
    ...
    for name, key in (
        ...
        if hashes[name] != expected[key]:
            raise ValueError(f"independent R8R14 R8R12 {name} changed")
    ...
    for phase in PHASES:
        ...
        if (
            ...
            raise ValueError(f"independent R8R14 R8R12 {phase} raw changed")
    passed = bool(
...
def _authenticate_r8r13(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    stage = _r8r13_stage(args, cfg)
    expected = cfg["source_r8r13"]
    paths = {
        "primary_detailed": stage / "analysis/primary_detailed.json",
        "primary_summary": stage / "analysis/primary_summary.json",
        "final_independent": stage / "analysis/final_independent.json",
    ...
    for name, key in (
        ...
        if hashes[name] != expected[key]:
            raise ValueError(f"independent R8R14 R8R13 {name} changed")
    final, state = _read(paths["final_report"]), _read(paths["state"])
    passed = bool(
        final.get("route") == expected["required_route"]
        and final.get("passed") is False
        and state.get("finished") is True
        and state.get("new_raw_count") == 0
        and state.get("real_tsc_executed") is False
        and (state.get("verdict") or {}).get("route") == expected["required_route"]
    )
    return {"stage": str(stage), "hashes": hashes, "required_route": expected["required_route"], "passed": passed}
...
def _fixed_basis(result: Mapping[str, Any]) -> np.ndarray:
    ...
    if not values or any(value != values[0] for value in values[1:]):
        raise ValueError("independent R8R14 fixed basis changed")
    matrix = np.asarray(values[0], dtype=float).T
    if matrix.shape != (N_COILS, 4) or not np.all(np.isfinite(matrix)):
        raise ValueError("independent R8R14 fixed basis invalid")
    return matrix
...
def offline_audit(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    source_r8r13 = _authenticate_r8r13(args, cfg)
    ...
    specs_exact = saved_specs == expected_specs and len(specs) == 16 and len(expected_specs) == 112
    construction_exact = recomputed == primary_preflight
    outcome = bool(
        primary.get("passed") is True
        and int(primary.get("spec_count", -1)) == 112
        and int(primary.get("level_construction_pass_count", -1)) == 448
    ...
    result = {
        ...
        "source_r8r13_authenticated": bool(source_r8r13["passed"]),
...
def final_audit(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    source_specs, source_results = _source_r8r12_candidates(args, cfg)
    ...
    for key, group in sorted(groups.items()):
        ...
        if len(baseline) != 1 or len(candidates) != 8 or members != {
            ...
            raise ValueError("independent R8R14 formal context coverage changed")
        base = baseline[0]
        oracle = max(
            [base, *candidates],
            key=lambda row: (
...
def main() -> None:
    ...
    if cfg.get("stage") != STAGE or cfg.get("identity") != IDENTITY:
        raise ValueError("independent R8R14 configuration identity changed")
