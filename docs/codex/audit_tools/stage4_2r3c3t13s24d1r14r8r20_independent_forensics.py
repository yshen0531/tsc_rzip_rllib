#!/usr/bin/env python3
"""Structurally independent R8R20 source, action, raw, and formal forensics."""
...
def _source_baselines(
    ...
    if len(specs) != 16 or {
        ...
        raise ValueError("independent R8R20 source coverage changed")
...
def _source_r8r12_candidates(
    ...
    if (
        ...
        raise ValueError("independent R8R20 source R8R12 candidate matrix changed")
    results = {
        str(row["experiment_id"]): _gzip(
...
def _source_r8r14_candidates(
    ...
    if (
        ...
        raise ValueError("independent R8R20 source R8R14 VVVV matrix changed")
    results = {
        str(row["experiment_id"]): _gzip(
            stage / "raw" / str(row["partition"]) / f"{row['experiment_id']}.json.gz"
        )
...
def _source_r8r15_candidates(
    ...
    if (
        ...
        raise ValueError("independent R8R20 source R8R15 candidate matrix changed")
    results = {
        str(row["experiment_id"]): _gzip(
            stage / "raw" / str(row["partition"]) / f"{row['experiment_id']}.json.gz"
        )
...
def _expected_specs(
    ...
    for index, source in enumerate(baselines):
        ...
        for candidate_index, code in enumerate(cfg["schedule_contract"]["new_sequences"]):
            sequence = []
            for symbol in str(code):
                definition = cfg["schedule_contract"]["symbols"][symbol]
                direction, sign = int(definition["direction_index"]), int(definition["sign"])
                requested = np.asarray(
                    cfg["schedule_contract"]["canonical_matrix_columns"], dtype=float
                )[:, direction] * sign
                sequence.append(
                    {
                        "symbol": symbol,
...
def _authenticate_r8r12(
    ...
    for name, key in (
        ...
        if hashes[name] != expected[key]:
            raise ValueError(f"independent R8R20 R8R12 {name} changed")
    ...
    for phase in PHASES:
        ...
        if (
            ...
            raise ValueError(f"independent R8R20 R8R12 {phase} raw changed")
    passed = bool(
...
def _authenticate_r8r14(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for name, key in (
        ...
        if hashes[name] != expected[key]:
            raise ValueError(f"independent R8R20 R8R14 {name} changed")
    if _sha(stage / "specs" / "all_specs.json") != expected["all_specs_sha256"]:
        raise ValueError("independent R8R20 R8R14 specifications changed")
    inventories = {phase: _inventory(stage / "raw" / phase) for phase in PHASES}
    for phase in PHASES:
        current = inventories[phase]
        if (
            ...
            raise ValueError(f"independent R8R20 R8R14 {phase} raw changed")
...
def _authenticate_r8r15(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for name, key in (
        ...
        if hashes[name] != expected[key]:
            raise ValueError(f"independent R8R20 R8R15 {name} changed")
    if _sha(stage / "specs" / "all_specs.json") != expected["all_specs_sha256"]:
        raise ValueError("independent R8R20 R8R15 specifications changed")
    inventories = {phase: _inventory(stage / "raw" / phase) for phase in PHASES}
    for phase in PHASES:
        current = inventories[phase]
        if (
            ...
            raise ValueError(f"independent R8R20 R8R15 {phase} raw changed")
...
def _authenticate_r8r19(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for name, key in (
        ...
        if hashes[name] != expected[key]:
            raise ValueError(f"independent R8R20 R8R19 {name} changed")
    final = _read(paths["final_report"])
...
def _fixed_basis(result: Mapping[str, Any]) -> np.ndarray:
    ...
    if not values or any(value != values[0] for value in values[1:]):
        raise ValueError("independent R8R20 fixed basis changed")
    matrix = np.asarray(values[0], dtype=float).T
    if matrix.shape != (N_COILS, 4) or not np.all(np.isfinite(matrix)):
        raise ValueError("independent R8R20 fixed basis invalid")
    return matrix
...
def _recompute_preflight(
    ...
    return {
        ...
        "passed": bool(
            len(rows) == 96
            and len(issue_levels) == 384
            and len(refreshes) == 2112
...
def offline_audit(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    primary_preflight = _read(stage / "analysis" / "offline_direct_cube_preflight.json")
    primary_path = stage / "analysis" / "offline_primary.json"
    primary = _read(primary_path)
    specs_exact = saved_specs == expected_specs and len(specs) == 16 and len(expected_specs) == 96
    construction_exact = recomputed == primary_preflight
    outcome = bool(
        primary.get("passed") is True
        and int(primary.get("spec_count", -1)) == 96
        and int(primary.get("level_construction_pass_count", -1)) == 384
        and int(primary.get("refresh_construction_pass_count", -1)) == 2112
        ...
        sequence_exact = bool(
            ...
            and all(
                ...
                for level, (event, symbol) in enumerate(
                    zip(issues, spec["r8r20_sequence"])
                )
            )
        )
        issue_increment = max(
...
def final_audit(args: argparse.Namespace, cfg: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key, group in sorted(groups.items()):
        ...
        expected_codes = set(map(str, cfg["schedule_contract"]["reused_sequences"])) | set(
            map(str, cfg["schedule_contract"]["new_sequences"])
        )
        if len(baseline) != 1 or len(candidates) != 16 or members != expected_codes:
            raise ValueError("independent R8R20 formal context coverage changed")
        base = baseline[0]
        oracle = max(
            [base, *candidates],
            key=lambda row: (
    ...
    candidate_summaries = []
    for code in (*cfg["schedule_contract"]["reused_sequences"], *cfg["schedule_contract"]["new_sequences"]):
        selected = [
            row
            for row in rows
...
def main() -> None:
    ...
    if cfg.get("stage") != STAGE or cfg.get("identity") != IDENTITY:
        raise ValueError("independent R8R20 configuration identity changed")
