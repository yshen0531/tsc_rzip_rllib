#!/usr/bin/env python3
"""Independent raw-bank and training-only diagonal-gain audit for R8R37."""
...
STAGE = r8r37.STAGE
IDENTITY = r8r37.IDENTITY
AUDIT_KIND = "training_only_diagonal_innovation_gain_independent"
...
def _coordinate(row: Mapping[str, Any]) -> np.ndarray:
    ...
    if coordinate.shape != (62,) or not np.isfinite(coordinate).all():
        raise ValueError("independent R8R37 coordinate invalid")
    return coordinate
...
def _fit_model(
    ...
    if not chosen:
        raise ValueError("independent R8R37 fit selection empty")
    intervals = []
    for interval_index in range(6):
        ...
        for membership, offsets in memberships.items():
            ...
            if (
                ...
                raise ValueError("independent R8R37 training group invalid")
            groups.append(
                {
    ...
    numerator_terms = [[[] for _ in range(5)] for _ in range(6)]
    denominator_terms = [[[] for _ in range(5)] for _ in range(6)]
    sample_counts = np.zeros((6, 5), dtype=np.int64)
    for trajectory in chosen:
        cold = [_predict_base(model, row, cfg)[0] for row in trajectory["intervals"]]
        for interval_index in range(1, 6):
            previous_actual = np.asarray(
                trajectory["intervals"][interval_index - 1]["targets"],
                dtype=np.float64,
            ).reshape((-1, 5))
            current_actual = np.asarray(
                trajectory["intervals"][interval_index]["targets"],
                dtype=np.float64,
            ).reshape((-1, 5))
            innovation = (
                previous_actual[-1] - cold[interval_index - 1][-1]
            ) * normalization
            response = (current_actual - cold[interval_index]) * normalization
            for component in range(5):
                for value in response[:, component]:
                    numerator_terms[interval_index][component].append(
                        float(innovation[component]) * float(value)
                    )
                    denominator_terms[interval_index][component].append(
                        float(innovation[component]) ** 2
                    )
                    sample_counts[interval_index, component] += 1
    numerator = np.asarray(
        [
            [math.fsum(numerator_terms[interval][component]) for component in range(5)]
    ...
    denominator = np.asarray(
        [
            [
                math.fsum(denominator_terms[interval][component])
                for component in range(5)
            ]
            for interval in range(6)
        ],
    ...
    gains = np.zeros((6, 5), dtype=np.float64)
    states = [["initial_cold" for _ in range(5)] for _ in range(6)]
    for interval_index in range(1, 6):
        for component in range(5):
            if denominator[interval_index, component] <= floor:
                states[interval_index][component] = "denominator_floor_zero"
                continue
            unconstrained = (
                numerator[interval_index, component]
                / denominator[interval_index, component]
            )
            gains[interval_index, component] = min(
                maximum, max(minimum, unconstrained)
            )
            states[interval_index][component] = (
                "projected_lower"
    ...
    if (
        ...
        raise ValueError("independent R8R37 diagonal gain fit invalid")
    model.update(
        {
...
def _gain_evidence(model: Mapping[str, Any]) -> dict[str, Any]:
    serial = {
        "diagonal_gains": np.asarray(model["diagonal_gains"]).tolist(),
        "gain_numerators": np.asarray(model["gain_numerators"]).tolist(),
        "gain_denominators": np.asarray(model["gain_denominators"]).tolist(),
        "gain_sample_counts": np.asarray(model["gain_sample_counts"]).tolist(),
...
def _model_evidence(model: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "gain_evidence": _gain_evidence(model),
    }
...
def _predict_group(
    ...
    if output.shape != (targets.shape[1],) or not np.isfinite(output).all():
        raise ValueError("independent R8R37 local-constant prediction invalid")
    return output, keys
...
def _predict_base(
    ...
    if not filled.all() or not np.isfinite(output).all():
        raise ValueError("independent R8R37 prediction coverage invalid")
...
def _trajectory_residuals(
    ...
    residual_serial = []
    predictions = []
    gains = np.asarray(model["diagonal_gains"], dtype=np.float64).reshape((6, 5))
    for trajectory in trajectories:
        ...
        for interval_index, row in enumerate(trajectory["intervals"]):
            ...
            adapted = cold + gains[interval_index] * previous_innovation
...
def _outer(
    ...
    for held_pair in pair_ids:
        ...
        model_record = _model_evidence(model)
...
def _schedule(
    ...
    for held_schedule in schedule_ids:
        ...
        if len(held) != 16:
            raise ValueError("independent R8R37 schedule fold cardinality changed")
        groups, evidence, predictions = _trajectory_residuals(model, held, cfg)
        model_record = _model_evidence(model)
        folds.append(
            {
...
def _compute(
    ...
    r8r37._verify_bank(trajectories, bank_evidence, ctx.cfg)
    cardinality = r8r37.r8r34.local_cardinality_audit(trajectories, ctx.cfg)
    if not cardinality["passed"]:
        raise ValueError("independent R8R37 cardinality failed")
    ...
    return r8r37._jsonable(detailed), r8r37._jsonable(artifact)
...
def _gain_map(model: Mapping[str, Any]) -> dict[tuple[str, str], Mapping[str, Any]]:
    output: dict[tuple[str, str], Mapping[str, Any]] = {}
    for family, held_field in (
        ("outer_model_evidence", "held_pair"),
        ("schedule_model_evidence", "held_schedule"),
    ):
        for fold in model[family]:
            key = (family, str(fold[held_field]))
            if key in output:
                raise ValueError("duplicate R8R37 gain-evidence fold")
            output[key] = fold["gain_evidence"]
    return output


def _scaled_gain_difference(
    primary: Mapping[str, Any], independent: Mapping[str, Any]
) -> float:
    left = _gain_map(primary)
    right = _gain_map(independent)
    if set(left) != set(right):
        return math.inf
    maximum = 0.0
    for key in left:
        left_evidence = left[key]
        right_evidence = right[key]
        for field in (
            "gain_sample_counts",
            "gain_projection_states",
            "gain_parameter_count",
            "gain_projection_counts",
        ):
            if left_evidence[field] != right_evidence[field]:
                return math.inf
        for field in (
            "diagonal_gains",
            "gain_numerators",
            ...
            left_array = np.asarray(left_evidence[field], dtype=np.float64)
            right_array = np.asarray(right_evidence[field], dtype=np.float64)
            if left_array.shape != right_array.shape:
                return math.inf
            denominator = np.maximum(
                1.0, np.maximum(np.abs(left_array), np.abs(right_array))
            )
            maximum = max(
                maximum,
                float(np.max(np.abs(left_array - right_array) / denominator)),
            )
    return maximum
...
def audit(args: argparse.Namespace) -> dict[str, Any]:
    ctx = r8r37.load_context(args)
    if ctx.cfg.get("stage") != STAGE or ctx.cfg.get("identity") != IDENTITY:
        raise ValueError("independent R8R37 configuration identity changed")
    ...
    production_authentication = r8r37.authenticate_sources(ctx)
    ...
    independent_detailed, independent_model = _compute(
        ctx,
        {
            "r8r35_final": production_authentication["r8r35_final"],
    ...
    gain_difference = _scaled_gain_difference(primary_model, independent_model)
    ...
    result = {
        ...
        "maximum_scaled_gain_difference": gain_difference,
        ...
        "primary_gain_agreement": gain_difference <= tolerance,
    ...
    result["passed"] = bool(
        ...
        and result["primary_gain_agreement"]
    ...
    if not result["passed"]:
        ...
        raise ValueError("independent R8R37 preflight disagrees with primary")
...
def _parser() -> argparse.ArgumentParser:
    ...
    for name in r8r37.ARGUMENT_NAMES:
        parser.add_argument(
