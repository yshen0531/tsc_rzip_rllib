#!/usr/bin/env python3
"""Independent raw-bank and fixed equal local-affine ensemble audit for R8R41."""
...
STAGE = r8r41.STAGE
IDENTITY = r8r41.IDENTITY
AUDIT_KIND = "fixed_equal_global_ridge_local_affine_cold_ensemble_independent"
...
def _coordinate(row: Mapping[str, Any]) -> np.ndarray:
    ...
    if coordinate.shape != (62,) or not np.isfinite(coordinate).all():
        raise ValueError("independent R8R41 coordinate invalid")
    return coordinate
...
def _fit_local(
    ...
    if not chosen:
        raise ValueError("independent R8R41 local fit selection empty")
    ...
    for interval_index in range(6):
        ...
        for membership, offsets in memberships.items():
            ...
            if (
                ...
                raise ValueError("independent R8R41 local training group invalid")
            groups.append(
                {
...
def _predict_local(
    ...
    if not filled.all() or not np.isfinite(output).all():
        raise ValueError("independent R8R41 local prediction coverage invalid")
...
def _predict(
    ...
    if ensemble.shape != global_prediction.shape or not np.isfinite(ensemble).all():
        raise ValueError("independent R8R41 ensemble prediction invalid")
...
def _schedule(
    ...
    for held_schedule in schedules:
        ...
        if len(held) != 16:
            raise ValueError("independent R8R41 schedule fold cardinality changed")
        residual_groups, residual_digest, predictions = _residuals(model, held, cfg)
        evidence = _model_evidence(model)
        folds.append(
            {
...
def _verify_bank(
    ...
    if (
        ...
        raise ValueError("independent R8R41 bank reproduction changed")
...
def _compute(
    ...
    if not cardinality["passed"]:
        raise ValueError("independent R8R41 cardinality failed")
    ...
    return r8r41._jsonable(detailed), r8r41._jsonable(artifact)
...
def _authenticate_final(stage: Path, contract: Mapping[str, Any], label: str) -> dict[str, Any]:
    ...
    if any(hashes[name] != contract[f"{name}_sha256"] for name in hashes):
        raise ValueError(f"independent R8R41 {label} source hash changed")
    ...
    if (
        ...
        raise ValueError(f"independent R8R41 {label} source state changed")
...
def _authenticate_r8r34(
    ...
    hashes = {name: _sha(path) for name, path in paths.items()}
    if any(hashes[name] != contract[f"{name}_sha256"] for name in hashes):
        raise ValueError("independent R8R41 R8R34 source hash changed")
    summary = _read(paths["primary_summary"])
    ...
    if (
        ...
        raise ValueError("independent R8R41 R8R34 classification changed")
    return {"hashes": hashes, "historical_overall_passed": False, "passed": True}


def audit(args: argparse.Namespace) -> dict[str, Any]:
    ctx = r8r41.load_context(args)
    if ctx.cfg.get("stage") != STAGE or ctx.cfg.get("identity") != IDENTITY:
        raise ValueError("independent R8R41 configuration identity changed")
    ...
    if not result["passed"]:
        ...
        raise ValueError("independent R8R41 preflight disagrees with primary")
...
def _parser() -> argparse.ArgumentParser:
    ...
    for name in r8r41.ARGUMENT_NAMES:
