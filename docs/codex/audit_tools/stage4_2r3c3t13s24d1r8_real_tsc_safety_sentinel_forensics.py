#!/usr/bin/env python3
"""Independent server-side raw forensics for the D1R8 safety sentinel."""
...
def run_forensics(ctx: stage.Context, complete_log: Path) -> dict[str, Any]:
    ...
    for spec in specs:
        ...
        if not result.get("success"):
            ...
            criteria = event.get("criteria") or {}
            applied_events = [
                value.get("r3c3t13s24d1r8_event_detail") or {}
                for value in trace
                if value.get("r3c3t13s24d1r8_event") != "none"
            ]
        ...
        try:
            ...
            events = [
                value["r3c3t13s24d1r8_event_detail"]
                for value in trace
                if value.get("r3c3t13s24d1r8_event") != "none"
            ]
