#!/usr/bin/env python3
"""Independent server raw/snapshot audit for Stage4.2R3c3T13S24D1R14R2."""
...
PASS_ROUTE = "MIXED_BASIS_SENTINEL_PASS_TIME_DISTRIBUTED_ID_DESIGN_REQUIRED"
D1R13_COUNT = 8
D1R13_BYTES = 241_738
...
def audit(
    ...
    for spec in specs:
        ...
        if role == "baseline" and full:
            ...
            direction = int(spec["d1r14r2_direction_index"])
            issue = trace[10].get("r3c3t13s24d1r14r2_event_detail") or {}
            cancel = trace[11].get("r3c3t13s24d1r14r2_event_detail") or {}
            issue_exact = bool(
                ...
                == list(map(float, spec["d1r14r2_requested_coordinate"]))
            )
            cancel_exact = bool(
        ...
        identity = bool(
            ...
            and spec.get("d1r14r2_requested_matrix_digest") == MATRIX_DIGEST
            and str(spec.get("d1r14r2_direction_name", ""))
            == (DIRECTIONS[int(spec["d1r14r2_direction_index"])] if role == "signed_probe" else "")
    ...
    if runtime_error_count > 0 or prefix_failure_count > 0 or inventory["count"] != 72:
        independent_route = "MIXED_BASIS_SENTINEL_RUNTIME_OR_PREFIX_FAIL_STOP"
    elif safety_pass_count != 72:
        independent_route = "MIXED_BASIS_SENTINEL_ACTION_SAFETY_FAIL_REDESIGN_REQUIRED"
    elif not geometry.get("passed"):
        independent_route = "MIXED_BASIS_SENTINEL_RESPONSE_GEOMETRY_FAIL_REDESIGN_REQUIRED"
    else:
