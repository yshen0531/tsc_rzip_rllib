# Stage4.2R3c3T13S24D1R14R8R44 fixed affine-dominant global/local-affine cold-ensemble receding-controller preflight design

Status: prospectively frozen after final R8R41 and the R8R43 design,
implementation, package, transfer, and installed-server validation, but
before any R8R43 primary or independent model fit, prediction, residual,
tube, metric, gate, route, stdout, stderr, state, or result exists. No R8R43
numerical result informed this design.

This is a conditional design only. It may execute if and only if final R8R43
...
Every other final R8R43 route blocks R8R44 before construction or
computation.

## 1. Frozen source and zero-TSC boundary

R8R44 must authenticate the exact final R8R43 primary summary, primary
detailed result, model artifact, independent audit, compact audit, final
report, state, and manifest. Their hashes may be transcribed after R8R43 is
...
R8R44 executes zero Ray, `gotsc`, TSC, controller, plant step, raw, and
...
## 2. Immutable fixed affine-dominant cold ensemble

For every held whole-pair and whole-schedule fold, independently refit only
from complete training trajectories:
...
ensemble prediction                       0.25 * global + 0.75 * local
```

Both weights are exact binary64 and identical for all rows. There is no
innovation, gain, gating, distance switch, expert fallback, feature or ridge
selection, neighbor search, response weighting, clipping, or post-result
adjustment. Pair, history, source, schedule, formal outcome, held target,
future row, and residual remain forbidden predictor inputs.

Carry the final R8R43 componentwise maximum whole-pair/whole-schedule tube
...
measurements. Current or future held targets may test a selected first action
only after selection and may not enter ranking, fallback, or ties.
...
R8R43 source PASS and all integrity, safety, model-binding, and support
gates.
...
```text
R8R43 is not the exact independently reproduced PASS
  FIXED_AFFINE_DOMINANT_COLD_ENSEMBLE_CONTROLLER_PREFLIGHT_BLOCKED_BY_SOURCE

source, package, causality, forbidden-input, runtime, or dual disagreement
  FIXED_AFFINE_DOMINANT_COLD_ENSEMBLE_CONTROLLER_PREFLIGHT_EXECUTION_FAIL_STOP

model binding, tube, support, actuator, safety, or fault-injection failure
  FIXED_AFFINE_DOMINANT_COLD_ENSEMBLE_CONTROLLER_PREFLIGHT_SAFETY_FAIL_NO_TSC
...
all source, integrity, safety, support, authority, and dual gates pass
  FIXED_AFFINE_DOMINANT_COLD_ENSEMBLE_CONTROLLER_PREFLIGHT_PASS_REAL_SENTINEL_DESIGN_REQUIRED
```
...
Before any R8R44 implementation or execution require project-venv
