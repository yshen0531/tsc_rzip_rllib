...
SOURCE_REVISION="${ID2Z10_SOURCE_REVISION:-}"
OUTPUT="${ID2Z10_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z10_five_context_model.json"

fail() { printf 'ID2Z10_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }
test -n "$SOURCE_REVISION" || fail "ID2Z10_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z10_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z10_five_context_model.py \
  --stage-config "$CONFIG" --source-revision "$SOURCE_REVISION" --output "$OUTPUT"
primary_rc=$?
test -f "$OUTPUT/result.json" || exit 2
python scripts/rgeo_zgeo_1ms_id2z10_five_context_model_audit.py \
  --stage-config "$CONFIG" --run-dir "$OUTPUT" --source-revision "$SOURCE_REVISION" \
