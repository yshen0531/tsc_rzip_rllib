...
def _validate_config(cfg: Mapping[str, Any], path: Path) -> None:
    ...
    exact = {
        ...
        "package_revision": "r42r3c3t13s24d1r14r4_time_shifted_sign_split_v1",
    }
    for key, value in exact.items():
        if cfg.get(key) != value:
            raise ValueError(f"D1R14R4 frozen {key} changed")
    for key in ("design_document", "source_d1r13_config"):
        source = (root / str(cfg[key])).resolve()
        if root not in source.parents or not source.is_file():
            raise ValueError(f"D1R14R4 {key} is outside package")
        if _sha256(source) != str(cfg[f"{key}_sha256"]):
            raise ValueError(f"D1R14R4 {key} hash changed")
    ...
    if dict(r3) != expected_r3:
        raise ValueError("D1R14R4 immutable R3 contract changed")
    for key in (
        "config",
        ...
        if root not in source_path.parents or not source_path.is_file():
            raise ValueError(f"D1R14R4 R3 {key} is outside package")
        if _sha256(source_path) != str(r3[f"{key}_sha256"]):
            raise ValueError(f"D1R14R4 R3 {key} hash changed")
    ...
    if dict(r2) != expected_r2:
        raise ValueError("D1R14R4 immutable R2 contract changed")
    ...
    if dict(r1a) != expected_r1a:
        raise ValueError("D1R14R4 immutable R1A contract changed")
    for key in ("config", "implementation"):
        source_path = (root / str(r1a[key])).resolve()
        if root not in source_path.parents or not source_path.is_file():
            raise ValueError(f"D1R14R4 R1A {key} is outside package")
        if _sha256(source_path) != str(r1a[f"{key}_sha256"]):
            raise ValueError(f"D1R14R4 R1A {key} hash changed")
    ...
    if dict(source) != expected_source:
        raise ValueError("D1R14R4 immutable source contract changed")
    ...
    if selected != expected_selected or len(set(selected)) != 8:
        raise ValueError("D1R14R4 selected D1R13 identities changed")
    ...
    expected_controller = {
        ...
        "baseline_count": 8,
        "signed_probe_count": 192,
        "expected_task_count": 200,
        "expected_baseline_post_prefix_action_count": 208,
        "expected_probe_preissue_zero_action_count": 1536,
        "expected_probe_issue_count": 192,
        "expected_probe_cancel_count": 192,
        "expected_probe_zero_after_cancel_action_count": 3072,
    ...
    if (
        ...
        raise ValueError("D1R14R4 controller contract changed")
    ...
    if dict(geometry) != expected_geometry:
        raise ValueError("D1R14R4 response geometry changed")
    ...
    if (
        ...
        raise ValueError("D1R14R4 formal timing changed")
    if int(cfg["parallel"]["n_workers"]) != 96:
        raise ValueError("D1R14R4 Ray capacity changed")
    if cfg["routes"] != {
        "offline_fail": "TIME_SHIFTED_SIGN_SPLIT_SENTINEL_OFFLINE_FAIL_NO_TSC",
        "runtime_fail": "TIME_SHIFTED_SIGN_SPLIT_SENTINEL_RUNTIME_OR_PREFIX_FAIL_STOP",
        "safety_fail": "TIME_SHIFTED_SIGN_SPLIT_SENTINEL_ACTION_SAFETY_FAIL_REDESIGN_REQUIRED",
        "geometry_fail": "TIME_SHIFTED_SIGN_SPLIT_SENTINEL_GEOMETRY_FAIL_REDESIGN_REQUIRED",
        "pass": "TIME_SHIFTED_SIGN_SPLIT_SENTINEL_PASS_CAUSAL_MODEL_FIT_DESIGN_REQUIRED",
    }:
        raise ValueError("D1R14R4 routes changed")
    if (
        ...
        raise ValueError("D1R14R4 scope changed")
    expected_path = root / "configs/stage4_2r3c3t13s24d1r14r4_time_shifted_sign_split_sentinel_370ms.json"
    if path.resolve() != expected_path.resolve():
        raise ValueError("D1R14R4 config path changed")
...
def load_config(
    ...
    **source_kwargs: Path,
    ...
    if source_ctx.paths.run_dir != source_d1r13_run:
        raise ValueError("D1R14R4 source D1R13 run path changed")
    return Context(
        cfg=cfg,
...
def _source_d1r13_results(ctx: Context) -> list[dict[str, Any]]:
    ...
    for experiment_id in map(str, ctx.cfg["selected_source_d1r13_experiment_ids"]):
        ...
        if (
            ...
            raise ValueError(f"D1R14R4 invalid selected D1R13 source: {experiment_id}")
...
def _authenticate_r1a(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("D1R14R4 immutable R1A source changed")
    ...
    if (
        ...
        raise ValueError("D1R14R4 R1A scientific boundary changed")
    return {
...
def _authenticate_r3_r2(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("D1R14R4 R3/R2 source directory identity changed")
    r3_paths = {
    ...
    if any(not path.is_file() for path in r3_paths.values()) or hashes != expected_hashes:
        raise ValueError("D1R14R4 R3 compact source hashes changed")
    ...
    if (
        ...
        raise ValueError("D1R14R4 R3 scientific source boundary changed")
    ...
    if (
        ...
        raise ValueError("D1R14R4 R2 authentic source boundary changed")
    return {
...
def _authenticate_source(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("D1R14R4 D1R13 independent audit changed")
    ...
    if (
        ...
        raise ValueError("D1R14R4 D1R11 training inventory changed")
    ...
    if not snapshots.get("passed") or int(snapshots.get("pass_count", -1)) != 8:
        raise ValueError("D1R14R4 selected snapshot authentication failed")
    return {
        "r1a": r1a,
        "r3_r2": r3_r2,
...
def _package_fingerprint() -> dict[str, Any]:
    ...
    required = {
        ...
        "docs/codex/reports/STAGE4_2R3C3T13S24D1R14R4_REAL_TSC_TIME_SHIFTED_SIGN_SPLIT_SENTINEL_DESIGN.md",
        ...
        "scripts/stage4_2r3c3t13s24d1r14r4_time_shifted_sign_split_sentinel.py",
    ...
    if not required.issubset(hashes):
        raise ValueError(
            "D1R14R4 package import closure is incomplete: "
...
def build_specs(
    ...
    roles = [("baseline", -1, "", 0, -1)] + [
        ("signed_probe", index, name, sign, issue_step)
    ...
    for source_result in source_results:
        ...
        for role, direction_index, direction_name, sign, issue_step in roles:
            ...
            spec.update(
                {
                    "kind": "stage4_2r3c3t13s24d1r14r4_time_shifted_sign_split_sentinel",
                    "stage": STAGE,
                    "campaign_identity": CAMPAIGN_IDENTITY,
                    "controller_revision": CONTROLLER_REVISION,
                    "experiment_id": experiment_id,
                    "phase": "time_shifted_sign_split_safety_identification_sentinel",
                    "category": "time_shifted_safety_identification_only",
                    "environment_variant": f"stage4_2r3c3t13s24d1r14r4_{experiment_id}",
                    ...
                    "d1r14r4_role": role,
                    "d1r14r4_direction_index": direction_index,
                    "d1r14r4_direction_name": direction_name,
                    "d1r14r4_sign": sign,
                    "d1r14r4_requested_coordinate": requested.tolist(),
                    "d1r14r4_requested_matrix_digest": matrix_digest,
                    "d1r14r4_issue_task_step": issue_step,
                    "d1r14r4_cancel_task_step": (
                        issue_step + CANCEL_OFFSET if role == "signed_probe" else -1
                    ),
                    "d1r14r4_zero_after_task_step": (
                        issue_step + ZERO_AFTER_OFFSET if role == "signed_probe" else 10
                    ),
                    "d1r14r4_future_r17_controller_execution_allowed": False,
            ...
            output.append(spec)
    if len(output) != 200 or len({row["experiment_id"] for row in output}) != 200:
        raise ValueError("D1R14R4 spec identity coverage changed")
    counts: dict[str, int] = {}
    horizons = {35: 0, 37: 0}
    for spec in output:
    ...
    if set(counts.values()) != {25} or horizons != {35: 100, 37: 100}:
        raise ValueError("D1R14R4 matrix coverage changed")
    return output
...
def prepare_offline(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage_dir.exists() or ctx.paths.state.exists() or ctx.paths.manifest.exists():
        raise ValueError("D1R14R4 offline requires a fresh run identity")
...
def _controller_source_spec(spec: Mapping[str, Any], schedule: Mapping[str, Any]) -> dict[str, Any]:
    ...
    forbidden = {
        ...
        "d1r14r4_role",
        "d1r14r4_direction_index",
        "d1r14r4_direction_name",
        "d1r14r4_sign",
        "d1r14r4_requested_coordinate",
    }
    prefixed_forbidden = {
        key
        for key in output
        if key.startswith(("d1r13_", "d1r14r4_", "source_d1r13_"))
    }
    forbidden.update(prefixed_forbidden)
    for key in forbidden:
        output.pop(key, None)
    if forbidden.intersection(output) or any(
        key.startswith(("d1r13_", "d1r14r4_", "source_d1r13_")) for key in output
    ):
        raise ValueError("D1R14R4 forbidden experiment label reached controller")
...
def zero_action(step: int) -> np.ndarray:
    if int(step) < PREFIX_END:
        raise ValueError("D1R14R4 zero branch called before task step 10")
...
def _trace_template(step: int, action: np.ndarray) -> dict[str, Any]:
    return {
        ...
        "r3c3t13s24d1r14r4_controller_revision": CONTROLLER_REVISION,
        "r3c3t13s24d1r14r4_delegated_prefix": False,
        "r3c3t13s24d1r14r4_future_r17_executed": False,
        "r3c3t13s24d1r14r4_event": "none",
        "r3c3t13s24d1r14r4_event_detail": {},
        "r3c3t13s24d1r14r4_zero_increment": True,
    }
...
class MixedBasisSignedExcitationController(
        ...
        *,
        ...
        if role == "signed_probe":
            if direction_index not in range(4) or sign not in (-1, 1):
                raise ValueError("D1R14R4 invalid fixed signed coordinate")
            expected = frozen_matrix[:, direction_index] * sign
            if requested.shape != (4,) or not np.array_equal(requested, expected):
                raise ValueError("D1R14R4 requested mixed coordinate changed")
            if (
                ...
                raise ValueError("D1R14R4 per-spec issue schedule changed")
        elif (
            role != "baseline"
            ...
            raise ValueError("D1R14R4 invalid baseline coordinate")
        super().__init__(
            base_worker,
            bundle,
            prepared,
        ...
        self.d1r14r4_role = role
        self.d1r14r4_direction_index = int(direction_index)
        self.d1r14r4_sign = int(sign)
        self.d1r14r4_requested_coordinate = requested
        self.d1r14r4_issue_step = int(issue_step)
        self.d1r14r4_cancel_step = int(cancel_step)
        self.d1r14r4_zero_after_step = int(zero_after_step)
        self.d1r14r4_controller_contract = copy.deepcopy(dict(controller_contract))
    ...
    def _issue(
        ...
        desired_coordinate = self.d1r14r4_requested_coordinate
        ...
        event = {
            ...
            "gate_revision": "d1r14r4_fixed_mixed_action_safety_v1",
            "slot": slot,
            "task_step": self.step,
        ...
        if not event["passed"]:
            raise ValueError(
                "D1R14R4 mixed-basis issue action failed: "
    ...
    def action(self, current_state: Mapping[str, Any]) -> tuple[np.ndarray, dict[str, Any]]:
        if int(current_state["step_index"]) != self.step:
            raise ValueError("D1R14R4 controller/current task-state index mismatch")
        if self.step < PREFIX_END:
            action, trace = super().action(current_state)
            trace.update(
                {
                    "r3c3t13s24d1r14r4_controller_revision": CONTROLLER_REVISION,
                    "r3c3t13s24d1r14r4_delegated_prefix": True,
                    "r3c3t13s24d1r14r4_future_r17_executed": False,
                    "r3c3t13s24d1r14r4_event": "none",
                    "r3c3t13s24d1r14r4_event_detail": {},
                    "r3c3t13s24d1r14r4_zero_increment": False,
                }
            )
        ...
        if (
            self.d1r14r4_role == "signed_probe"
            and self.step == self.d1r14r4_issue_step
        ):
            currents = np.asarray(current_state["currents_a_tsc"], dtype=float)
            action, event = self._issue(
                self.d1r14r4_direction_index, currents, np.zeros(N_COILS, dtype=float)
            )
            trace.update(
                {
                    "action_norm_tsc": action.tolist(),
                    "r3c3t13s24d1r14r4_event": "signed_issue",
                    "r3c3t13s24d1r14r4_event_detail": event,
                    "r3c3t13s24d1r14r4_zero_increment": False,
                }
            )
        elif (
            self.d1r14r4_role == "signed_probe"
            and self.step == self.d1r14r4_cancel_step
        ):
            currents = np.asarray(current_state["currents_a_tsc"], dtype=float)
            ...
            trace.update(
                {
                    "action_norm_tsc": action.tolist(),
                    "r3c3t13s24d1r14r4_event": "stored_center_cancel",
                    "r3c3t13s24d1r14r4_event_detail": event,
                    "r3c3t13s24d1r14r4_zero_increment": False,
                }
            )
...
def _payload(ctx: Context, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            "variant_id": f"stage4_2r3c3t13s24d1r14r4_{experiment_id}",
            "stage4_2r3c3t13s24d1r14r4_restart_snapshot_dir": str(
                spec["restart_snapshot_dir"]
            ),
            "stage4_2r3c3t13s24d1r14r4_snapshot_manifest_digest": str(
                spec["restart_snapshot_manifest_digest"]
            ),
            "stage4_2r3c3t13s24d1r14r4_issue_task_step": int(
                spec["d1r14r4_issue_task_step"]
            ),
            "stage4_2r3c3t13s24d1r14r4_cancel_task_step": int(
                spec["d1r14r4_cancel_task_step"]
            ),
        }
    )
...
class LocalWorker:
    """One fresh authentic TSC process and D1R14R4 controller per rollout."""
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                raise ValueError("D1R14R4 formal horizon changed")
            self.base.env.reset()
            ...
            controller = MixedBasisSignedExcitationController(
                ...
                direction_index=int(spec["d1r14r4_direction_index"]),
                sign=int(spec["d1r14r4_sign"]),
                requested_coordinate=spec["d1r14r4_requested_coordinate"],
            ...
            for step in range(horizon):
                try:
                    ...
                    if (
                        "D1R14R4 mixed-basis issue action failed" in str(exc)
                ...
                if truncated and step + 1 < horizon:
                    ...
                    raise RuntimeError("environment truncated before D1R14R4 horizon")
            calibration_events = [
            ...
            if role == "baseline":
                ...
                role_success = bool(
                    ...
                    and trace[issue_step].get("r3c3t13s24d1r14r4_event")
                    == "signed_issue"
                    and trace[cancel_step].get("r3c3t13s24d1r14r4_event")
            ...
            success = bool(
                ...
                and all(
                    bool(row.get("r3c3t13s24d1r14r4_delegated_prefix"))
                    for row in trace[:PREFIX_END]
                )
                and role_success
            ...
            result.update(
                {
                    "success": success,
                    "completed": True,
                    "failure_reason": "" if success else "incomplete or invalid D1R14R4 rollout",
                    "execution_failure_class": (
                    ...
                    "hidden_history_control_summary": {
                        ...
                        "role": role,
                        "issue_task_step": int(spec["d1r14r4_issue_task_step"]),
                        "cancel_task_step": int(spec["d1r14r4_cancel_task_step"]),
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(
                    failed=failed,
                    reason="stage4_2r3c3t13s24d1r14r4_mixed_basis_signed_excitation",
                )
...
def _ray_actor_class():
    ...
    if _RAY_ACTOR is None:
        ...
        class Stage42R3C3T13S24D1R14R4Actor:
            def __init__(
                self,
                payload,
                library,
        ...
        _RAY_ACTOR = Stage42R3C3T13S24D1R14R4Actor
    return _RAY_ACTOR
...
def _saved_specs(ctx: Context) -> list[dict[str, Any]]:
    ...
    if _digest(saved) != _digest(rebuilt):
        raise ValueError("D1R14R4 frozen specs changed")
    return saved
...
def _assert_run_identity(ctx: Context) -> list[dict[str, Any]]:
    ...
    if (
        ...
        raise ValueError("D1R14R4 prepared run/package/source identity changed")
    return specs
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            worker = LocalWorker(
                ...
                library,
                bundle,
                f"stage42r3c3t13s24d1r14r4_serial_{index:04d}",
                selector,
                lattice,
                calibration,
                dynamic,
            ...
            print(f"[D1R14R4] {index + 1}/{len(pending)}", flush=True)
        ...
        for batch_start in range(0, len(pending), plan.actor_count):
            ...
            for offset, spec in enumerate(batch):
                actor = Actor.remote(
                    ...
                    library,
                    bundle,
                    f"stage42r3c3t13s24d1r14r4_{batch_start + offset:04d}",
                    selector,
                    lattice,
                    calibration,
                    dynamic,
            ...
            try:
                while refs:
                    ...
                    if not ready:
                        print(
                            f"[D1R14R4] waiting {completed}/{len(pending)}",
                            flush=True,
                        )
                        continue
                    for ref in ready:
                        ...
                        completed += 1
                        print(f"[D1R14R4] {completed}/{len(pending)}", flush=True)
            finally:
        ...
        raise ValueError(f"unsupported D1R14R4 backend: {backend}")
    completed = sum(
        _result_complete(
            ctx.paths.raw / f"{spec['experiment_id']}.json.gz",
            spec,
...
def _visible_outputs5(
    ...
    if len(trajectory) < 2:
        raise ValueError("D1R14R4 trajectory is too short for visible velocity")
    rows = []
    ...
    if values.shape != (len(trajectory), 5) or not np.all(np.isfinite(values)):
        raise ValueError("D1R14R4 normalized visible trajectory is invalid")
...
def _response_geometry(
    ...
    *,
    ...
    if issue_steps != (10, 14, 18, 22):
        raise ValueError("D1R14R4 combined issue-time contract changed")
    if source_r2_specs is None or source_r2_results is None:
        ...
        for spec in source_r2_specs:
            ...
            if not d1r14r2._result_complete(path, spec, require_success=True):
                raise ValueError(f"D1R14R4 invalid source R2 raw: {experiment_id}")
            loaded[experiment_id] = _read_gz(path)
        source_r2_results = loaded
    if len(source_r2_specs) != 72 or len(source_r2_results) != 72:
        raise ValueError("D1R14R4 source R2 raw coverage changed")
    ...
    def grouped(
        ...
        for experiment_id, result in bank_results.items():
            ...
            if spec is None:
                raise ValueError("D1R14R4 result/spec identity mismatch")
            ...
            if key in output.setdefault(context_id, {}):
                raise ValueError("D1R14R4 duplicate response-bank member")
    ...
    r4_spec_index, r4_groups = grouped(specs, results, "d1r14r4", -1)
    ...
    for context_id in map(str, ctx.cfg["selected_source_d1r13_experiment_ids"]):
        ...
        if r2_baseline is None or r4_baseline is None:
            raise ValueError(f"D1R14R4 baseline missing for context {context_id}")
        meta = next(
            spec
            for spec in specs
        ...
        for issue_step in issue_steps:
            source_name = "D1R14R2" if issue_step == 10 else "D1R14R4"
            ...
            for sign in (1, -1):
                ...
                for direction_index, direction_name in enumerate(DIRECTIONS):
                    ...
                    if member_values.shape != baseline_values.shape:
                        raise ValueError("D1R14R4 signed response shape mismatch")
                    response = (
    ...
    for pair_id, contexts in sorted(pair_to_contexts.items()):
        if len(contexts) != 2:
            raise ValueError(f"D1R14R4 hidden-history pair coverage changed: {pair_id}")
        for issue_step in issue_steps:
            for sign in (1, -1):
                for direction_index, direction_name in enumerate(DIRECTIONS):
                    ...
                    if left.shape != right.shape:
                        raise ValueError("D1R14R4 matched-history response shape changed")
    ...
    new_issue_rows = [row for row in issue_rows if row["source_stage"] == "D1R14R4"]
    issue_symmetry = {
...
def postprocess(ctx: Context) -> dict[str, Any]:
    ...
    for spec in specs:
        ...
        if role == "baseline" and full_horizon:
            ...
            cancel_step = int(spec["d1r14r4_cancel_task_step"])
            zero_after_step = int(spec["d1r14r4_zero_after_task_step"])
            expected_coordinate = list(map(float, spec["d1r14r4_requested_coordinate"]))
            issue = trace[issue_step].get("r3c3t13s24d1r14r4_event_detail") or {}
            cancel = trace[cancel_step].get("r3c3t13s24d1r14r4_event_detail") or {}
        ...
        rows.append(
            {
                ...
                "role": role,
                "direction_index": int(spec["d1r14r4_direction_index"]),
                "direction_name": str(spec["d1r14r4_direction_name"]),
                "sign": int(spec["d1r14r4_sign"]),
                "issue_task_step": int(spec["d1r14r4_issue_task_step"]),
                "cancel_task_step": int(spec["d1r14r4_cancel_task_step"]),
                "horizon": horizon,
    ...
    if safety_pass_count == 200 and len(valid_results) == 200:
...
def run_real(ctx: Context, *, backend: str, resume: bool) -> dict[str, Any]:
    ...
    if state.get("phase_status") != "offline_ready" or bool(state.get("finished")):
        raise ValueError("D1R14R4 run requires offline_ready state")
    specs = _assert_run_identity(ctx)
...
def execute(
    ...
    if command == "offline":
        if resume:
            raise ValueError("D1R14R4 offline does not support resume")
    ...
    if command == "postprocess":
        ...
        if state.get("phase_status") != "real_complete" or bool(state.get("finished")):
            raise ValueError("D1R14R4 postprocess requires real_complete state")
        return postprocess(ctx)
    raise ValueError(f"unsupported D1R14R4 command: {command}")
