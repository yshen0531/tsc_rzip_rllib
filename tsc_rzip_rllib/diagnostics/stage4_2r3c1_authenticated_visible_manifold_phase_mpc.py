"""Stage4.2R3c1 authenticated-visible-manifold phase MPC development.

R3c1 reuses the exact locked R3b selected snapshots as a development set
after inspecting R3c.  It replaces R3c's ideal-nominal phase reference with
only the authenticated actual R17 closed-loop R/Z/Ip samples at phases
0--20.  Source actions, source coil/wire currents, current-run future values,
and hidden wire state are unavailable to phase selection.  Task/formal time
...
def _scenario_digest(value: Mapping[str, Any]) -> str:
    return "s42r3c1_" + _canonical_digest(dict(value))[:20]
...
def _requested_workers(cfg: Mapping[str, Any]) -> int:
    ...
    requested = int(os.environ.get("STAGE4_2R3C1_WORKERS", fixed))
    if requested != fixed:
        raise ValueError(
            f"Stage4.2R3c1 Ray capacity is frozen at {fixed}, got {requested}"
        )
    return requested
...
def validate_config(cfg: Mapping[str, Any]) -> None:
    if str(cfg.get("stage")) != STAGE:
        raise ValueError("Stage4.2R3c1 config stage mismatch")
    if str(cfg.get("controller_revision")) != CONTROLLER_REVISION:
        raise ValueError("Stage4.2R3c1 controller revision mismatch")
    if str(cfg.get("package_revision")) != PACKAGE_REVISION:
        raise ValueError("Stage4.2R3c1 package revision mismatch")
    if int(cfg["parallel"]["n_workers"]) != 128:
        raise ValueError("Stage4.2R3c1 is fixed to 128 Ray workers")
    ...
    if dict(phase) != expected_phase:
        raise ValueError("Stage4.2R3c1 phase-alignment contract changed")
    matrix = cfg["control_matrix"]
    if list(map(str, matrix["targets"])) != [
        "nominal",
        "RZ_p10_m10",
    ]:
        raise ValueError("Stage4.2R3c1 target matrix changed")
    actuator = [
        (
    ...
    if actuator != [(0, 1.0), (2, 0.9)]:
        raise ValueError("Stage4.2R3c1 actuator matrix changed")
    if (
        ...
        raise ValueError("Stage4.2R3c1 control coverage changed")
    ...
    if (
        ...
        raise ValueError("Stage4.2R3c1 formal timing changed")
    ...
    if (
        ...
        raise ValueError("Stage4.2R3c1 R3b source contract changed")
    if (
        ...
        raise ValueError("Stage4.2R3c1 R3c evidence contract changed")
    expected_ids = [
    ...
    if list(map(str, requirements["required_selected_pair_ids"])) != expected_ids:
        raise ValueError("Stage4.2R3c1 selected pair identities changed")
    if (
        ...
        raise ValueError("Stage4.2R3c1 scientific scope changed")


@dataclass(frozen=True)
class Stage42R3C1Paths:
    run_dir: Path
    ...
    @classmethod
    def from_run_dir(cls, run_dir: Path) -> "Stage42R3C1Paths":
        root = run_dir.expanduser().resolve()
        return cls(
            run_dir=root,
            source_reference=root / "stage4_2r3c1_source_reference",
            variants=root / "stage4_2r3c1_environment_variants",
            control=(
                root
                / "stage4_2r3c1_authenticated_visible_manifold_control"
            ),
            analysis=root / "stage4_2r3c1_analysis",
            state=root / "stage4_2r3c1_state.json",
            manifest=root / "stage4_2r3c1_manifest.json",
        )


@dataclass
class Stage42R3C1Context:
    cfg: dict[str, Any]
    paths: Stage42R3C1Paths
    project_dir: Path
    source_r3b_run: Path
...
def _source_fingerprint(
    ...
    for name, path in files.items():
        if not path.is_file():
            raise FileNotFoundError(
                f"Stage4.2R3c1 source evidence missing: {name}: {path}"
            )
    ...
    if (
        ...
        raise ValueError("Stage4.2R3c1 source R3b scientific state mismatch")
    if (
        ...
        raise ValueError("Stage4.2R3c1 source R3b audit/inventory mismatch")
    rows = [
        {
            "name": name,
    ...
    fingerprint = {
        "schema_version": 1,
        "contract": "r42r3c1_exact_r3b_development_source_v1",
...
def _r3c_evidence_fingerprint(
    ...
    """Authenticate the exact failed R3c evidence used to choose R3c1."""
    ...
    for name, path in files.items():
        if not path.is_file():
            raise FileNotFoundError(
                f"Stage4.2R3c1 R3c evidence missing: {name}: {path}"
            )
    manifest = read_json(files["manifest"])
    state = read_json(files["state"])
    ...
    if (
        ...
        raise ValueError("Stage4.2R3c1 R3c scientific state mismatch")
    if (
        str(inventory.get("digest"))
        ...
        raise ValueError("Stage4.2R3c1 R3c audit/inventory mismatch")
    rows = [
        {
            "name": name,
            "path": str(path.resolve()),
    ...
    fingerprint = {
        "schema_version": 1,
        "contract": "r42r3c1_exact_failed_r3c_evidence_v1",
        "source_run": str(source_run),
        "source_audit": str(source_audit),
        "run_inventory_digest": str(inventory["digest"]),
        "run_inventory_file_count": int(inventory["n_files"]),
...
def load_stage42r3c1_config(
    ...
    *,
    ...
    return Stage42R3C1Context(
        cfg=cfg,
        paths=Stage42R3C1Paths.from_run_dir(run_dir),
...
def _recompute_selected_pairs(
    ...
    if len(raw_paths) != int(
        ...
        raise ValueError("Stage4.2R3c1 source state raw count mismatch")
    state_rows = [
    ...
    if actual_ids != required_ids:
        raise ValueError(
            f"Stage4.2R3c1 raw-recomputed selected pair mismatch: {actual_ids}"
        )
    for pair in selected:
        for member in ("plus_first", "minus_first"):
            ...
            if not _selected_snapshot_state_is_valid(state):
                raise ValueError(
                    "Stage4.2R3c1 selected source snapshot is invalid"
                )
...
def _deployed_package_fingerprint(
    ...
    relative_paths = [
        ...
        "run_stage4_2r3c1_authenticated_visible_manifold_phase_mpc_nohup.sh",
        "run_stage4_2r3c1_self_test.sh",
        "run_stage4_2r3c1_server_postprocess.sh",
        "run_stage4_2r3c1_verify_package.sh",
        "run_stop_stage4_2r3c1_now.sh",
        "scripts/stage4_2r3c1_shell_common.sh",
        "scripts/stage4_2r3c1_authenticated_visible_manifold_phase_mpc.py",
        "scripts/stage4_2r3c1_server_postprocess.py",
    ...
    for relative in relative_paths:
        ...
        if not path.is_file():
            raise FileNotFoundError(
                f"Stage4.2R3c1 package file missing: {relative}"
            )
        rows.append(
            {
                "path": relative,
...
def _source_controller_cases(
    ...
    if set(sources) != required:
        raise ValueError("Stage4.2R3c1 frozen controller source mismatch")
    return sources
...
def _source_raw_files_by_experiment_id(
    ...
    for path in paths:
        ...
        if not experiment_id or experiment_id in output:
            raise ValueError(
                "Stage4.2R3c1 invalid or duplicate R17 source raw identity"
            )
...
def _visible_reference_manifold(
    ...
    """Extract the only R17 future data permitted by R3c1: R/Z/Ip."""

    trajectory = list(source.get("trajectory") or [])
    if len(trajectory) < 21:
        raise ValueError("Stage4.2R3c1 R17 visible manifold is too short")
    values = np.asarray(
        [
            [
    ...
    if values.shape != (21, 3) or not np.all(np.isfinite(values)):
        raise ValueError("Stage4.2R3c1 R17 visible manifold is invalid")
    ...
    if (
        ...
        raise ValueError(
            "Stage4.2R3c1 R17 visible manifold/raw authentication failed"
        )
    manifold = {
        "schema_version": 1,
    ...
    if set(manifold) != allowed:
        raise ValueError("Stage4.2R3c1 visible manifold schema changed")
    return manifold
...
def _visible_reference_manifolds(
    ...
    for key, source in sources.items():
        ...
        if path is None:
            raise FileNotFoundError(
                "Stage4.2R3c1 selected R17 raw source missing: "
                f"{experiment_id}"
            )
        output[key] = _visible_reference_manifold(source, path)
    if set(output) != set(sources):
        raise ValueError(
            "Stage4.2R3c1 visible manifold source coverage mismatch"
        )
    return output
...
def build_control_specs(
            ...
            for target in ctx.cfg["control_matrix"]["targets"]:
                for actuator in ctx.cfg["control_matrix"][
                    ...
                    if r3b._forbidden_future_paths(spec):
                        raise ValueError(
                            "Stage4.2R3c1 source spec contains future data"
                        )
                    identity = {
                        "stage": STAGE,
                        "phase": (
                    ...
                    spec.update(
                        {
                            "kind": (
                                "stage4_2r3c1_authenticated_visible_"
                                "manifold_phase_development_control"
                            ),
                            "stage": STAGE,
                            "controller_revision": CONTROLLER_REVISION,
    ...
    if (
        ...
        raise ValueError("Stage4.2R3c1 control coverage mismatch")
    return specs
...
def _control_payload(
    ...
    *,
    ...
    env_cfg["tsc_workspace_root"] = str(
        Path(
            os.environ.get(
                "STAGE4_2R3C1_TSC_WORKSPACE_ROOT",
    ...
    env_cfg["run_root"] = str(
        Path(
            os.environ.get(
                "STAGE4_2R3C1_TSC_RUN_ROOT", storage["tsc_run_root"]
            )
        )
        .expanduser()
        .resolve()
    ...
    payload["variant_id"] = f"stage4_2r3c1_{experiment_id}"
    payload["start_folder"] = snapshot_dir.name
    payload["slew_scale"] = slew
    payload["stage4_1r4_horizon_steps"] = horizon
    payload["stage4_2r3c1_restart_snapshot_dir"] = str(snapshot_dir)
    payload["stage4_2r3c1_snapshot_manifest_digest"] = str(
...
class AuthenticatedVisibleManifoldPhaseTaskController(r3b.FreshTaskController):
    """R17 MPC aligned from a frozen actual-visible R17 manifold."""
    ...
    def __init__(
        ...
        if (
            ...
            raise ValueError(
                "Stage4.2R3c1 visible reference manifold contract invalid"
            )
...
class LocalAuthenticatedVisibleManifoldPhaseControlWorker:
    def __init__(
        self,
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if horizon != _formal_horizon(float(spec["slew_scale"])):
                raise ValueError("Stage4.2R3c1 formal horizon changed")
            self.base.env.reset()
            ...
            trajectory = [initial]
            controller = AuthenticatedVisibleManifoldPhaseTaskController(
...
def _control_ray_actor_class():
    ...
    if _CONTROL_RAY_ACTOR is None:
        ...
        class Stage42R3C1ControlActor:
            def __init__(
                self, payload, library, bundle, worker_id, selector_cfg
            ):
                self.worker = LocalAuthenticatedVisibleManifoldPhaseControlWorker(
                    payload, library, bundle, worker_id, selector_cfg
                )
        ...
        _CONTROL_RAY_ACTOR = Stage42R3C1ControlActor
    ...
    *,
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            worker = LocalAuthenticatedVisibleManifoldPhaseControlWorker(
                payloads[str(spec["experiment_id"])],
                library,
                bundle,
                f"stage42r3c1_control_serial_{index:03d}",
                selector,
            )
            try:
            ...
            print(
                f"[Stage4.2R3c1 control] {index + 1}/{len(pending)}",
                flush=True,
            )
        ...
        plan = ensure_ray_worker_plan(
            ...
            log_prefix="[Stage4.2R3c1 control]",
        )
        ...
        for index, spec in enumerate(pending):
            actor = Actor.remote(
                ...
                library,
                bundle,
                f"stage42r3c1_control_{index:03d}",
                selector,
            )
            actors.append(actor)
            refs[actor.evaluate.remote(spec)] = spec
        print(
            "[Stage4.2R3c1 control] "
...
def summarize_control(
    ...
    summary = {
        "schema_version": 1,
        "stage": STAGE,
        "phase": "authenticated_visible_manifold_development_control",
        "development_set_only": True,
...
def _prepare_dirs(paths: Stage42R3C1Paths) -> None:
    for path in (
        paths.run_dir,
...
def prepare(
    ...
    *,
    ...
    if len(visible_reference_manifolds) != 4:
        raise ValueError(
            "Stage4.2R3c1 requires four authenticated visible manifolds"
        )
    selected_public = [
        {
            "pair_id": str(pair["pair_id"]),
    ...
    atomic_write_json(
        ctx.paths.run_dir / "stage4_2r3c1_config.resolved.json",
        ctx.cfg,
    )
    atomic_write_json(
...
def run_offline_phase_alignment_audit(
    ...
    for index, key in enumerate(
        ...
        plant = r1.LocalPlantReplayWorker(
            ...
            library,
            bundle,
            f"stage42r3c1_offline_{index:03d}",
            selector,
        )
        try:
            trajectory = list(source.get("trajectory") or [])
            ...
            spec["visible_reference_manifold"] = copy.deepcopy(
                manifolds[key]
            )
            spec["visible_reference_manifold_digest"] = str(
            ...
            hidden_controller = AuthenticatedVisibleManifoldPhaseTaskController(
            ...
            for step in range(horizon):
                ...
                if (
                    ...
                    raise ValueError(
                        "R3c1 offline source trace is not causal"
                    )
            ...
            for pair in selected_pairs:
                for member in ("plus_first", "minus_first"):
                    ...
                    development_controller = AuthenticatedVisibleManifoldPhaseTaskController(
                        plant.base_worker,
                        bundle,
                        spec,
                        initial_development,
...
def analyze(
    ...
    verdict = {
        ...
        "stage": STAGE,
        "verdict": (
            "STAGE4_2R3C1_AUTHENTICATED_VISIBLE_MANIFOLD_PASS"
            if primary_pass
            else "STAGE4_2R3C1_AUTHENTICATED_VISIBLE_MANIFOLD_FAILED"
        ),
        "source_stage4_2r3b_run": str(ctx.source_r3b_run),
        "source_stage4_2r3c_run": str(ctx.source_r3c_run),
        ...
        "next_if_fail": (
            "Preserve R3c1 raw evidence and create a new controller "
    ...
    atomic_write_json(
        ctx.paths.analysis / "stage4_2r3c1_verdict.json", verdict
    )
    atomic_write_json(
        ctx.paths.analysis / "stage4_2r3c1_summary.json", summary
    )
    ...
    state.update(
        {
            ...
            "stop_reason": (
                ""
                if primary_pass
                else "authenticated_visible_manifold_control_failed"
            ),
...
def execute(
    ...
    *,
    ...
    if not bool(offline.get("passed")):
        raise RuntimeError(
            "Stage4.2R3c1 offline phase-alignment gate failed; "
