#!/usr/bin/env python3
"""Stage4.2R3c3T13S9 exact-Card15 lattice transition holdout."""
...
t11 = s1.t11
SCHEMA_VERSION = 1
STAGE = "Stage4.2R3c3T13S9"
RUN_NAME = "stage4_2r3c3t13s9_unified_postqueue_q1_identification"
CAMPAIGN_IDENTITY = "unified_postqueue_q1_identification_v1"
CONTROLLER_REVISION = "unified_postqueue_q1_probe_v42r3c3t13s9_v1"
EXECUTION_PACKAGE_REVISION = "r42r3c3t13s9_unified_postqueue_q1_v1"
PACKAGE_REVISION = "r42r3c3t13s9_unified_postqueue_q1_v1"
...
REPORTING_HOTFIX_PATHS = frozenset({
    ...
    "configs/stage4_2r3c3t13s9_unified_postqueue_q1_identification_370ms.json",
    "run_stage4_2r3c3t13s9_verify_package.sh",
    "scripts/stage4_2r3c3t13s9_server_postprocess.py",
    "tests/test_stage4_2r3c3t13s9_unified_postqueue_q1_identification.py",
    "tsc_rzip_rllib/diagnostics/stage4_2r3c3t13s9_unified_postqueue_q1_identification.py",
})
...
class BlindOpenOrderGuard:
    ...
    def before_open(self, role: str) -> None:
        if role == "development":
            if self.model_sha256 or self.holdout_open_count:
                raise ValueError("development raw opened after T13S9 model freeze")
            self.development_open_count += 1
            return
        if role != "blind_holdout":
            raise ValueError(f"unknown T13S9 raw role: {role}")
        if (
    ...
    def freeze(self, model_sha256: str) -> None:
        if self.development_open_count != self.expected_development_count:
            raise ValueError("cannot freeze T13S9 model before all development raw")
        if self.holdout_open_count:
            raise ValueError("cannot freeze T13S9 model after holdout access")
        if len(model_sha256) != 64:
            raise ValueError("T13S9 model SHA-256 must contain 64 hexadecimal digits")
        int(model_sha256, 16)
...
@dataclass(frozen=True)
class Stage42R3C3T13S9Context:
    cfg: dict[str, Any]
    base_config_path: Path
    base_ctx: t11.Stage42R3C3T11Context
    paths: Stage42R3C3T13S9Paths


def _paths(run_dir: Path) -> Stage42R3C3T13S9Paths:
    root = run_dir.expanduser().resolve()
    control = root / RUN_NAME
    return Stage42R3C3T13S9Paths(
        run_dir=root,
        control=control,
        raw=control / "raw",
        variants=root / "stage4_2r3c3t13s9_environment_variants",
        source_reference=root / "stage4_2r3c3t13s9_source_reference",
        model=root / "stage4_2r3c3t13s9_frozen_model",
        analysis=root / "stage4_2r3c3t13s9_analysis",
        manifest=root / "stage4_2r3c3t13s9_manifest.json",
        state=root / "stage4_2r3c3t13s9_state.json",
    )
...
def _validate_config(cfg: Mapping[str, Any]) -> None:
    if (
        ...
        raise ValueError("T13S9 campaign identity changed")
    expected_contexts = [
        (
            "hard", "development", "p5_q1_a0p900_gap2_settle4",
            "plus_first", "RZ_p10_m10", 2, 0.9,
            "s42r3c1_a7cf656de03a975cccc8",
            "4add3b5354fa2a0660cdd9c3f02212989e1f81f76804a8f9f7eba5a2969548b7",
            False, -0.3596055,
        ),
        (
            "hard", "blind_holdout", "p5_q1_a0p900_gap2_settle4",
            "minus_first", "RZ_p10_m10", 2, 0.9,
            "s42r3c1_7f47bd92b5458cdb0d40",
            "0c4141fe324eb930c9929db60c52190e1f745cf7a4c1cc4ff4badcfbb4ab8cf4",
            False, -0.361785367,
        ),
        (
            "easy", "development", "p9_q1_a0p750_gap2_settle4",
            "plus_first", "nominal", 0, 1.0,
            "s42r3c1_f7ac14b67b3eddb3659d",
            "1edbbc2b7a2d31c7dcdf8e3bbade06e2a55ee91c33bedfa9d13fa0681fa7d910",
            True, 0.103787581,
        ),
        (
            "easy", "blind_holdout", "p9_q1_a0p750_gap2_settle4",
            "minus_first", "nominal", 0, 1.0,
            "s42r3c1_ee404a8e3301e9caacb0",
            "2af7aee6a2fc773ea3c25f4ebb92255300328a3bf4c0ab56bb9ed9aba5be27eb",
            True, 0.0969701,
        ),
    ]
    actual_contexts = [
    ...
    if actual_contexts != expected_contexts:
        raise ValueError("T13S9 evidence split changed")
    probe = cfg["lattice_probe"]
    expected_schedule = {
        "0": {"transport": (2, 3, 3, 4), "braking": (16, 17, 17, 18)},
        "2": {"transport": (0, 1, 1, 2), "braking": (14, 15, 15, 16)},
    }
    actual_schedule = {
        delay: {
            window: tuple(
    ...
    if actual_schedule != expected_schedule:
        raise ValueError("T13S9 causal issue schedule changed")
    if (
        ...
        raise ValueError("T13S9 lattice/actuator contract changed")
    ...
    if (
        ...
        raise ValueError("T13S9 matrix changed")
    ...
    if (
        ...
        raise ValueError("T13S9 formal timing changed")
    if (
        ...
        raise ValueError("T13S9 scope changed")


def load_stage42r3c3t13s9_config(
    config_path: Path,
    *,
    ...
    if (
        ...
        raise ValueError("T13S9 frozen T11 base config mismatch")
    ...
    return Stage42R3C3T13S9Context(
        cfg=cfg,
...
def _selected_pairs(ctx: Stage42R3C3T13S9Context) -> list[dict[str, Any]]:
...
def build_control_specs(
    ...
    if len(source_baselines) != 32 or len(frozen) != 4 or not set(frozen) <= set(source_baselines):
        raise ValueError("T13S9 source context coverage mismatch")
    ...
    for key, row in sorted(frozen.items()):
        ...
        if (
            ...
            raise ValueError("T13S9 source raw/snapshot identity mismatch")
        ...
        for probe_id, window, direction, sign in members:
            ...
            spec.update({
                "kind": "stage4_2r3c3t13s9_unified_postqueue_q1_identification",
                "stage": STAGE,
                "campaign_identity": CAMPAIGN_IDENTITY,
                "controller_revision": CONTROLLER_REVISION,
                ...
                "environment_variant": f"stage4_2r3c3t13s9_{experiment_id}",
                ...
                "r3c3t13s9_offline_role": str(row["offline_role"]),
                "r3c3t13s9_stratum": str(row["stratum"]),
                "r3c3t13s9_source_r3c1_experiment_id": str(row["source_r3c1_experiment_id"]),
    ...
    if len(specs) != expected or len({str(row["experiment_id"]) for row in specs}) != expected:
        raise ValueError("T13S9 control identity coverage mismatch")
    return specs
    ...
    *,
    ...
    for multiplier in map(int, cfg["lambda_multipliers"]):
        ...
        if passed:
            ...
            return row
    raise ValueError(
        "no frozen T13S9 lattice multiplier passed: "
...
def lattice_native_direction(
    ...
    if direction == "mode0_without_coil8":
        ...
        raise ValueError(f"unknown T13S9 lattice direction: {direction}")
    norm = float(np.linalg.norm(vector))
    if not math.isfinite(norm) or norm <= 0.0:
        raise ValueError("T13S9 lattice direction is degenerate")
    return vector / norm
...
_WRAPPER_SPEC_KEYS = frozenset({
    ...
    "r3c3t13s9_schedule_contract",
    "r3c3t13s9_offline_role",
    "r3c3t13s9_stratum",
...
def _controller_spec(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    forbidden = {
        ...
        "r3c3t13s9_offline_role", "r3c3t13s9_stratum",
        "r3c3t13s9_source_r3c1_experiment_id",
    }
    for key in forbidden:
        output.pop(key, None)
    if forbidden.intersection(output):
        raise ValueError("T13S9 forbidden experiment label reached controller")
    return output
    ...
    def __init__(
        ...
        if baseline:
            ...
            valid = bool(
                ...
                and self.first_effect_state == self.issue_step + 1
                and self.cancel_effect_state == self.cancel_step + 1
                and self.cancel_effect_state == self.first_effect_state + 1
            )
        if not valid:
            raise ValueError("T13S9 controller schedule contract invalid")
    ...
    def action(
        ...
        if task_step == self.issue_step:
            ...
            if self._signed_issue_delta_kat is None or self._issue_center_fields is None:
                raise ValueError("T13S9 cancellation has no causal issued displacement")
            ...
            if bool(returned["passed"]):
                ...
                raise ValueError(
                    "T13S9 both causal cancellation candidates failed: "
        ...
        if (
            ...
            raise ValueError("T13S9 selected Card15 action clipped or malformed")
        if event == "issue":
            expected_fields = (
            ...
            if list(selected.card15_fields) != list(expected_fields):
                raise ValueError("T13S9 issued action did not reproduce target fields")
        if event == "cancel" and list(selected.card15_fields) != list(lattice["target_fields"]):
            raise ValueError("T13S9 cancellation did not reproduce target fields")
        forbidden = {
        ...
        trace.update({
            ...
            "r3c3t13s9_probe_issued": event in {"issue", "cancel"},
...
def _control_payload(
    ctx: Stage42R3C3T13S9Context, *, spec: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    payload["variant_id"] = f"stage4_2r3c3t13s9_{experiment_id}"
    payload["stage4_2r3c3t13s9_restart_snapshot_dir"] = str(
        spec["restart_snapshot_dir"]
    )
    payload["stage4_2r3c3t13s9_snapshot_manifest_digest"] = str(
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                raise ValueError("T13S9 formal horizon changed")
            self.base.env.reset()
            ...
            for step in range(horizon):
                ...
                if truncated and step + 1 < horizon:
                    raise RuntimeError("environment truncated before T13S9 horizon")
            baseline = str(spec["r3c3_probe_id"]) == BASELINE_PROBE_ID
            events = [row["r3c3t13s9_lattice_event"] for row in trace]
            ...
            result.update({
                "success": success,
                "completed": True,
                "failure_reason": "" if success else "incomplete or invalid T13S9 rollout",
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(
                    failed=failed,
                    reason="stage4_2r3c3t13s9_unified_postqueue_q1_identification",
                )
...
def _control_ray_actor_class():
    ...
    if _CONTROL_RAY_ACTOR is None:
        ...
        class Stage42R3C3T13S9ControlActor:
            def __init__(
        ...
        _CONTROL_RAY_ACTOR = Stage42R3C3T13S9ControlActor
    return _CONTROL_RAY_ACTOR


def _package_files(ctx: Stage42R3C3T13S9Context) -> list[Path]:
    ...
    required = {
        project / "configs/stage4_2r3c3t13s9_unified_postqueue_q1_identification_370ms.json",
        project / "tsc_rzip_rllib/diagnostics/stage4_2r3c3t13s9_unified_postqueue_q1_identification.py",
        project / "scripts/stage4_2r3c3t13s9_unified_postqueue_q1_identification.py",
        project / "scripts/stage4_2r3c3t13s9_server_postprocess.py",
        project / "scripts/stage4_2r3c3t13s9_shell_common.sh",
        project / "run_stage4_2r3c3t13s9_common.sh",
        project / "run_stage4_2r3c3t13s9_offline.sh",
        project / "run_stage4_2r3c3t13s9_native.sh",
        project / "run_stage4_2r3c3t13s9_nohup.sh",
        project / "run_stage4_2r3c3t13s9_self_test.sh",
        project / "run_stage4_2r3c3t13s9_server_postprocess.sh",
        project / "run_stage4_2r3c3t13s9_verify_package.sh",
        project / "run_stop_stage4_2r3c3t13s9_now.sh",
        project / "docs/codex/reports/STAGE4_2R3C3T13S9_UNIFIED_POSTQUEUE_Q1_DESIGN.md",
    }
    if not required <= set(paths):
        missing = sorted(str(path.relative_to(project)) for path in required - set(paths))
        raise ValueError(f"T13S9 package inventory missing files: {missing}")
    return paths
...
def _deployed_package_fingerprint(
    ...
    return {
        "contract": "r42r3c3t13s9_deployed_package_source_v1",
...
def _prepare_dirs(paths: Stage42R3C3T13S9Paths) -> None:
    for path in (
...
def _resume_compatible(
    ...
    *,
    ...
    if (
        ...
        return
    raise ValueError(
        "T13S9 automatic resume requires exact package identity; changed="
        + repr(changed)
    )
...
def prepare(
    ctx: Stage42R3C3T13S9Context, *, resume: bool
    ...
    if ctx.paths.manifest.is_file():
        if not resume:
            raise FileExistsError("T13S9 run exists; use resume or a fresh run")
        old = t11.t1.r3c3.read_json(ctx.paths.manifest)
        old_resolved_path = (
            ctx.paths.run_dir / "stage4_2r3c3t13s9_config.resolved.json"
        )
        ...
        for key, value in manifest.items():
            ...
            if old.get(key) != value:
                raise ValueError(f"T13S9 resume incompatibility in {key}")
        _resume_compatible(
    ...
    resolved_name = (
        "stage4_2r3c3t13s9_config.resolved.v1h2.json"
        if reporting_hotfix
        else "stage4_2r3c3t13s9_config.resolved.json"
    )
    ...
    *,
    ...
    for spec in specs:
        ...
        worker = t11.t1.r1.LocalPlantReplayWorker(
            ...
            f"stage42r3c3t13s9_offline_{len(context_source):03d}", selector,
        )
        try:
            controller = LatticeTransitionProbeController(
            ...
            for step in range(int(spec["horizon_steps"])):
                ...
                finite = bool(
                    ...
                    and len(trace["r3c3t13s9_actuator_prediction"]["card15_fields"])
                    == N_COILS
                    and all(
                        len(field) == 10
                        for field in trace["r3c3t13s9_actuator_prediction"]["card15_fields"]
                    )
            ...
            message = str(exc)
            issue_prefix = "no frozen T13S9 lattice multiplier passed: "
            cancel_prefix = "T13S9 both causal cancellation candidates failed: "
            ...
            lattice_design_failures.append({
                "experiment_id": str(spec["experiment_id"]),
                "offline_role": str(spec["r3c3t13s9_offline_role"]),
                "stratum": str(spec["r3c3t13s9_stratum"]),
    ...
    summary = {
        ...
        "development_spec_count": sum(
            spec["r3c3t13s9_offline_role"] == "development" for spec in specs
        ),
        "blind_holdout_spec_count": sum(
            spec["r3c3t13s9_offline_role"] == "blind_holdout" for spec in specs
        ),
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            worker = LocalLatticeTransitionProbeWorker(
                ...
                f"stage42r3c3t13s9_serial_{index:03d}", selector,
                ctx.cfg["lattice_probe"],
            )
            try:
            ...
            print(f"[T13S9] {index + 1}/{len(pending)}", flush=True)
        ...
        completed = 0
        print(
            f"[T13S9] maximum_actor_count={plan.actor_count} pending={len(pending)}",
            flush=True,
        )
        for batch_start in range(0, len(pending), plan.actor_count):
            ...
            for offset, spec in enumerate(batch):
                ...
                actor = Actor.remote(
                    payloads[str(spec["experiment_id"])], library, bundle,
                    f"stage42r3c3t13s9_{index:03d}", selector,
                    ctx.cfg["lattice_probe"],
                )
                actors.append(actor)
            ...
            try:
                while refs:
                    ...
                    if not ready:
                        print(f"[T13S9] waiting {completed}/{len(pending)}", flush=True)
                        continue
                    for ref in ready:
                        spec = refs.pop(ref)
                        try:
                        ...
                        if completed % 4 == 0 or not refs:
                            print(f"[T13S9] {completed}/{len(pending)}", flush=True)
            finally:
...
def _effect_response(
    ...
    if cancel != first + 1:
        raise ValueError("T13S9 effect states are not adjacent")
    feature = _feature_arrays(result, dt_s)
    base_feature = _feature_arrays(baseline, dt_s)
    response = feature[first] - base_feature[first]
    current = np.asarray(
    ...
    displacement = current[[first]] - base_current[[first]]
...
def _issue_trace(result: Mapping[str, Any]) -> Mapping[str, Any]:
    rows = [
        ...
        if row.get("r3c3t13s9_lattice_event") == "issue"
    ]
    if len(rows) != 1:
        raise ValueError("T13S9 signed result must contain one issue trace")
    return rows[0]
...
def _cancellation_policy_trace(
    ...
    rows = [
        ...
        if row.get("r3c3t13s9_lattice_event") == "cancel"
    ]
    if baseline:
        return {
            "cancellation_method": "",
    ...
    row = rows[0]
    lattice = row.get("r3c3t13s9_lattice") or {}
    ...
    if bool(returned.get("passed")):
        ...
        expected = ""
        selected = {}
    actuator = row.get("r3c3t13s9_actuator_prediction") or {}
    return {
...
def _actuator_execution(
    ...
    for step, row in enumerate(trace):
        actuator = row["r3c3t13s9_actuator_prediction"]
...
def _expected_context_row(
    ...
    if len(rows) != 1:
        raise ValueError("T13S9 frozen context lookup mismatch")
    return rows[0]
...
def _group_results(
    ...
    for result in results:
        ...
        if name in members:
            raise ValueError("duplicate T13S9 context member")
        members[name] = result
    return grouped
...
def _signed_group_rows(
    ...
    floor_scaled = float(np.linalg.norm(floor_one / scales))
    rows = []
    for context_key, members in sorted(grouped.items()):
        if BASELINE_PROBE_ID not in members:
            raise ValueError("T13S9 context baseline missing")
        ...
        for window in WINDOWS:
            for direction in DIRECTIONS:
                ...
                row: dict[str, Any] = {
                    ...
                    "offline_role": str(baseline["spec"]["r3c3t13s9_offline_role"]),
                    "stratum": str(baseline["spec"]["r3c3t13s9_stratum"]),
                    "probe_window": window,
                ...
                if (
                    ...
                    plus_fields = plus_issue["r3c3t13s9_actuator_prediction"]["card15_fields"]
                    minus_fields = minus_issue["r3c3t13s9_actuator_prediction"]["card15_fields"]
                    plus_center = plus_issue["r3c3t13s9_center_card15_fields"]
                    minus_center = minus_issue["r3c3t13s9_center_card15_fields"]
                    ...
                    odd_scaled_norm = float(
                        np.linalg.norm(odd_output / scales)
                    )
...
def _build_model_artifact(
    ...
    for row in signed_rows:
        if row["offline_role"] != "development":
            raise ValueError("holdout row reached T13S9 model builder")
    ...
    for key, rows in sorted(by_key.items()):
        ...
        if len(rows) != N_DIRECTIONS or [
            ...
            raise ValueError("T13S9 development direction coverage mismatch")
        ...
        entry = {
            ...
            "input_definition": "first_effect_state_full_14_coil_current_A",
            "output_definition": "first_effect_state_R_Z_vR_vZ_Ip_unscaled",
    ...
    if len(entries) != 4:
        raise ValueError("T13S9 model stratum/window coverage mismatch")
    ...
    core = {
        ...
        "stage": STAGE,
        "campaign_identity": CAMPAIGN_IDENTITY,
        "model_contract": "causal_first_effect_lattice_local_map_v1",
        ...
        "source_hashes": {
            "config_sha256": _sha256(
                _project_root() / "configs/stage4_2r3c3t13s9_unified_postqueue_q1_identification_370ms.json"
            ),
...
def _freeze_model(
    ctx: Stage42R3C3T13S9Context, artifact: Mapping[str, Any]
) -> dict[str, Any]:
    path = ctx.paths.model / "stage4_2r3c3t13s9_local_model_tube.json"
    if path.is_file():
        existing = t11.t1.r3c3.read_json(path)
        if existing != dict(artifact):
            raise ValueError("existing frozen T13S9 model differs from development fit")
    else:
        t11.t1.r3c3.atomic_write_json(path, artifact)
    record_path = ctx.paths.model / "stage4_2r3c3t13s9_model_freeze.json"
    record = {
        "schema_version": SCHEMA_VERSION,
        "stage": STAGE,
        "model_path": str(path),
    ...
    if record_path.is_file():
        ...
        if comparable != old_comparable:
            raise ValueError("existing T13S9 model freeze record mismatch")
        record = old
    else:
...
def _evaluate_holdout(
    ...
    scales = np.asarray(ctx.cfg["lattice_probe"]["response_scales"], dtype=float)
    floor = float(np.linalg.norm(
        np.asarray([1e-9, 1e-9, 1e-7, 1e-7, 1e-4])
        / np.asarray(ctx.cfg["lattice_probe"]["response_scales"])
    ))
    models = {
...
def summarize_from_raw(
    ...
    *,
    ...
    development_specs = [
        spec for spec in specs
        if spec["r3c3t13s9_offline_role"] == "development"
    ]
    holdout_specs = [
        spec for spec in specs
        if spec["r3c3t13s9_offline_role"] == "blind_holdout"
    ]
    if len(development_specs) != 34 or len(holdout_specs) != 34:
        raise ValueError("T13S9 development/holdout split mismatch")
    ...
    def open_one(role: str, spec: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not _result_complete(path, spec):
            raise ValueError(f"incomplete or incompatible T13S9 raw: {path}")
        open_events.append({
    ...
    order_path = ctx.paths.model / "stage4_2r3c3t13s9_input_open_order.json"
    ...
    summary = {
        ...
        "stage": STAGE,
        "campaign_identity": CAMPAIGN_IDENTITY,
        "phase": "unified_postqueue_q1_identification",
        "identification_only": True,
        "probe_trajectories_allowed_in_expert_dataset": False,
        "independent_new_history_confirmation": False,
        "independent_long_hold_validated": False,
    ...
    summary["diagnostic_blind_holdout_passed"] = summary[
        "blind_holdout_gate_passed"
    ]
    summary["passed"] = bool(
...
def analyze(
    ctx: Stage42R3C3T13S9Context, summary: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    verdict = {
        ...
        "verdict": (
            ...
            if primary_pass else "UNIFIED_POSTQUEUE_Q1_IDENTIFICATION_FAIL_REDESIGN"
        ),
        ...
        "bc_dagger_or_rl_allowed": False,
        "next_if_pass": (
            "Preregister a zero-new-TSC combined q1/q2 first-effect audit; "
    ...
    t11.t1.r3c3.atomic_write_json(
        ctx.paths.analysis / "stage4_2r3c3t13s9_summary.json", dict(summary)
    )
    t11.t1.r3c3.atomic_write_json(
        ctx.paths.analysis / "stage4_2r3c3t13s9_verdict.json", verdict
    )
    ...
    state.update({
        ...
        "stop_reason": "" if primary_pass else "unified_q1_identification_gate_failed",
        "verdict": verdict,
...
def execute(
    ...
    *,
    ...
    if not bool(offline.get("passed")):
        ...
        raise RuntimeError("T13S9 offline lattice gate failed; no real TSC started")
    if command == "offline":
        state = t11.t1.r3c3.read_json(ctx.paths.state)
        state.update({
            "finished": False,
    ...
    if int(execution["complete_after_run"]) != len(specs):
        raise RuntimeError("T13S9 raw set incomplete; resume safely after runtime repair")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage4.2R3c3T13S9 lattice transition holdout"
    )
    ...
    ctx = load_stage42r3c3t13s9_config(
        args.config,
