"""R8R35 causal last-innovation local-constant schedule preflight."""
...
SCHEMA_VERSION = 1
STAGE = "Stage4.2R3c3T13S24D1R14R8R35"
IDENTITY = "causal_last_innovation_local_constant_schedule_generalizing_preflight_v1"
...
_write = r8r34._write
_sha = r8r34._sha
_digest = r8r34._digest
_jsonable = r8r34._jsonable
...
def validate_config(cfg: Mapping[str, Any], *, project_root: Path) -> None:
    ...
    source_config = (root / str(cfg["source_r8r34_config"])).resolve()
    ...
    expected_routes = {
        "execution_fail": "CAUSAL_LAST_INNOVATION_LOCAL_CONSTANT_PREFLIGHT_EXECUTION_FAIL_STOP",
        "model_fail": "CAUSAL_LAST_INNOVATION_LOCAL_CONSTANT_MODEL_FAIL_NO_TSC",
        "not_useful": "CAUSAL_LAST_INNOVATION_LOCAL_CONSTANT_NOT_USEFUL_NO_TSC",
        "pass": "CAUSAL_LAST_INNOVATION_LOCAL_CONSTANT_PASS_CONTROLLER_PREFLIGHT_DESIGN_REQUIRED",
    }
    invalid = (
        ...
        or _sha(source_config) != cfg.get("source_r8r34_config_sha256")
        ...
        or float(model.get("innovation_gain", -1.0)) != 1.0
    ...
    if invalid:
        raise ValueError("R8R35 frozen config changed")
...
def load_context(args: argparse.Namespace) -> Context:
    ...
    source_args.config = (root / str(cfg["source_r8r34_config"])).resolve()
    source_args.run_dir = args.r8r34_run.expanduser().resolve()
    r8r34_ctx = r8r34.load_context(source_args)
    return Context(
        cfg=cfg,
        ...
        source_ctx=r8r34_ctx.source_ctx,
    )


def authenticate_sources(ctx: Context) -> dict[str, Any]:
    contract = ctx.cfg["source_r8r34"]
    paths = {
        "primary_summary": ctx.r8r34_stage / "analysis/primary_summary.json",
        "primary_detailed": ctx.r8r34_stage / "analysis/primary_detailed.json",
        "model": ctx.r8r34_stage / "model/preflight_model.json",
        "independent_failure": ctx.r8r34_stage / "analysis/independent_failure.json",
        "compact_audit": ctx.r8r34_stage / "analysis/compact_audit.json",
        "final_report": ctx.r8r34_stage / "analysis/final_report.json",
        "stage_state": ctx.r8r34_stage / "stage_state.json",
        "stage_manifest": ctx.r8r34_stage / "stage_manifest.json",
    }
    hashes = {name: _sha(path) for name, path in paths.items()}
    if any(hashes[name] != contract[f"{name}_sha256"] for name in hashes):
        raise ValueError("R8R35 final R8R34 source hash changed")
    ...
    if (
        ...
        raise ValueError("R8R35 final R8R34 source state changed")
    return {
        "r8r34_final": {"hashes": hashes, "passed": True},
        "transitive": r8r34.authenticate_sources(ctx.r8r34_ctx),
        "passed": True,
    }
...
def _verify_bank(
    ...
    if (
        ...
        raise ValueError("R8R35 bank reproduction changed")
...
def fit_model(
    ...
    model = r8r34.fit_model(trajectories, selected, cfg)
    model["model_kind"] = "causal_local_constant_k64_last_innovation"
    return model
...
def predict_group(
    ...
    if output.shape != (targets.shape[1],) or not np.all(np.isfinite(output)):
        raise ValueError("R8R35 local-constant prediction invalid")
    return output, keys
...
def predict_base(
    ...
    *,
    ...
    if not np.all(filled) or not np.all(np.isfinite(output)):
        raise ValueError("R8R35 prediction coverage invalid")
...
def _trajectory_residuals(
    ...
    *,
    ...
    predictions = []
    for trajectory in trajectories:
        correction = np.zeros(5, dtype=float)
        residual_rows = []
        prediction_rows = []
        cold_rows = []
        neighbor_rows = []
...
def outer_model_evaluation(
    ...
    *,
    ...
    for held_pair in pairs:
        ...
        model_record = r8r34.model_evidence(model)
        fold = {
            "held_pair": held_pair,
...
def schedule_jackknife(
    ...
    *,
    ...
    for held_schedule in schedules:
        ...
        if len(held) != 16:
            raise ValueError("R8R35 schedule fold cardinality changed")
        groups, evidence, predictions = _trajectory_residuals(
            model, held, cfg, predict_group_fn=predict_group_fn
        )
        model_record = r8r34.model_evidence(model)
...
def compute_from_bank(
    ...
    *,
    ...
    if not cardinality["passed"]:
        raise ValueError("R8R35 frozen local-neighbor cardinality failed")
...
def run_primary(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage.exists():
        raise ValueError("R8R35 primary requires a fresh stage directory")
...
def _run_finalize_failure(
    ...
    if (
        ...
        raise ValueError("R8R35 independent failure evidence invalid")
    compact = {
        "schema_version": SCHEMA_VERSION,
        "stage": STAGE,
        "identity": IDENTITY,
        "audit_kind": "r8r35_compact_independent_failure_finalization",
        "passed": False,
        "integrity_gate_passed": False,
        "scientific_gate_passed": False,
        "route": ctx.cfg["routes"]["execution_fail"],
    ...
    final["audit_kind"] = "r8r35_final_report"
...
def run_finalize(ctx: Context) -> dict[str, Any]:
    ...
    if any(
        ...
        raise ValueError("R8R35 finalization evidence incomplete or ambiguous")
    summary = _read(paths["primary_summary"])
    ...
    if any(
        ...
        raise ValueError("R8R35 finalization evidence incomplete")
    ...
    if (
        ...
        raise ValueError("R8R35 finalization independent disagreement")
    compact = {
        "schema_version": SCHEMA_VERSION,
        "stage": STAGE,
        "identity": IDENTITY,
        "audit_kind": "r8r35_compact_primary_independent_finalization",
        "passed": True,
        "integrity_gate_passed": True,
        "scientific_gate_passed": bool(summary["scientific_gate_passed"]),
        "route": summary["route"],
...
ARGUMENT_NAMES = (
    ...
    "run-dir",
    "r8r34-run",
) + tuple(name for name in r8r34.ARGUMENT_NAMES if name not in {"config", "run-dir"})
