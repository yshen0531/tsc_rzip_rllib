"""Stage4.2R3a delayed-counterpulse hidden-history validation.

R3a generates real TSC restart states from a common authenticated expert
...
SCHEMA_VERSION = 1
STAGE = "Stage4.2R3a"
CONTROLLER_REVISION = "delayed_counterpulse_hidden_history_initial_state_mpc_v42r3a"
PACKAGE_REVISION = "r42r3a_delayed_counterpulse_hidden_history_v1"
...
def _scenario_digest(payload: Mapping[str, Any]) -> str:
    return "s42r3a_" + _canonical_digest(dict(payload))[:20]
...
def _requested_workers(cfg: Mapping[str, Any]) -> int:
    ...
    requested = int(os.environ.get("STAGE4_2R3A_WORKERS", fixed))
    if requested != fixed:
        raise ValueError(
            f"Stage4.2R3a Ray capacity is frozen at {fixed}, got {requested}"
        )
    return requested
...
def validate_config(cfg: Mapping[str, Any]) -> None:
    if str(cfg.get("stage")) != STAGE:
        raise ValueError("Stage4.2R3a config stage mismatch")
    if str(cfg.get("controller_revision")) != CONTROLLER_REVISION:
        raise ValueError("Stage4.2R3a controller revision mismatch")
    if str(cfg.get("package_revision")) != PACKAGE_REVISION:
        raise ValueError("Stage4.2R3a package revision mismatch")
    if int(cfg["parallel"]["n_workers"]) != 128:
        raise ValueError("Stage4.2R3a is fixed to 128 Ray workers")
    ...
    if (
        ...
        or list(map(int, prefix["prefix_lengths"])) != [4, 8]
        or not bool(prefix["require_exact_authenticated_actions"])
    ):
        raise ValueError("Stage4.2R3a common-prefix source contract changed")
    if list(map(int, generation["nullspace_direction_indices"])) != [1, 2]:
        raise ValueError("Stage4.2R3a nullspace direction grid changed")
    if list(map(float, generation["amplitude_fractions"])) != [0.2, 0.4, 0.8]:
        raise ValueError("Stage4.2R3a amplitude grid changed")
    if list(map(int, generation["gap_steps"])) != [2, 4, 8]:
        raise ValueError("Stage4.2R3a inter-pulse gap grid changed")
    if int(generation["settle_steps"]) != 4:
        raise ValueError("Stage4.2R3a settle contract changed")
    if list(map(str, generation["history_orders"])) != [
        "plus_first",
        "minus_first",
    ]:
        raise ValueError("Stage4.2R3a history order changed")
    if (
        int(generation["expected_pairs"]) != 36
        or int(generation["expected_rollouts"]) != 72
        or int(generation["pulse_steps"]) != 2
        or not math.isclose(
            float(generation["maximum_abs_action"]), 0.8, abs_tol=1e-15
        )
        ...
        raise ValueError("Stage4.2R3a state-generation contract changed")
    gate = cfg["pair_gate"]
    expected_gate = {
    ...
    if (
        ...
        raise ValueError("Stage4.2R3a pair coverage/selection gate changed")
    if list(map(str, gate["selection_order"])) != [
        ...
        raise ValueError("Stage4.2R3a pair selection order changed")
    ...
    for key, expected in expected_initial.items():
        if not math.isclose(float(initial[key]), expected, abs_tol=1e-15):
            raise ValueError(f"Stage4.2R3a initial-state gate changed: {key}")
    if (
        int(initial["minimum_selected_centroids"]) != 2
        or list(map(int, initial["required_common_prefix_lengths"])) != [4, 8]
        or not bool(initial["require_mutual_prefix_group_separation"])
    ):
        raise ValueError("Stage4.2R3a initial-state coverage gate changed")
    matrix = cfg["control_matrix"]
    if list(map(str, matrix["targets"])) != ["nominal", "RZ_p10_m10"]:
        raise ValueError("Stage4.2R3a target matrix changed")
    actuator = [
        (int(row["actual_delay_steps"]), float(row["actual_slew_scale"]))
        for row in matrix["future_actuator_cases"]
    ]
    if actuator != [(0, 1.0), (2, 0.9)]:
        raise ValueError("Stage4.2R3a future actuator matrix changed")
    if (
        ...
        raise ValueError("Stage4.2R3a control rollout coverage changed")
    ...
    if (
        ...
        raise ValueError("Stage4.2R3a formal timing changed")
    if bool(cfg.get("bc_dagger_or_rl_allowed")):
        raise ValueError("Stage4.2R3a may not authorize BC, DAgger, or RL")


@dataclass(frozen=True)
class Stage42R3APaths:
    run_dir: Path
    ...
    @classmethod
    def from_run_dir(cls, run_dir: Path) -> "Stage42R3APaths":
        root = run_dir.expanduser().resolve()
        return cls(
            run_dir=root,
            source_reference=root / "stage4_2r3a_source_reference",
            state_generation=root / "stage4_2r3a_state_generation",
            pair_analysis=root / "stage4_2r3a_pair_analysis",
            variants=root / "stage4_2r3a_environment_variants",
            control=root / "stage4_2r3a_hidden_history_control",
            analysis=root / "stage4_2r3a_analysis",
            state=root / "stage4_2r3a_state.json",
            manifest=root / "stage4_2r3a_manifest.json",
        )


@dataclass
class Stage42R3AContext:
    cfg: dict[str, Any]
    paths: Stage42R3APaths
    project_dir: Path
    source_r2_run: Path
    source_r1_run: Path
    source_r17_run: Path
...
def _source_fingerprint(
    ...
    if len(required) != 30:
        raise ValueError(
            f"Stage4.2R3a source fingerprint expected 30 files, got {len(required)}"
        )
    missing = [path for path in required if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"Stage4.2R3a source file missing: {missing[0]}")
    rows = []
    ...
    return {
        "schema_version": 1,
        "contract": "r42r3a_r2_r1_r17_direct_source_v1",
        "n_files": len(rows),
...
def load_stage42r3a_config(
    ...
    *,
    ...
    if str(manifest.get("stage")) != "Stage4.2R2":
        raise ValueError("Stage4.2R3a source stage mismatch")
    if str(manifest.get("controller_revision")) != EXPECTED_R2_CONTROLLER:
        raise ValueError("Stage4.2R3a source R2 controller mismatch")
    if str(manifest.get("package_revision")) != EXPECTED_R2_PACKAGE:
        raise ValueError("Stage4.2R3a source R2 package mismatch")
    if not bool(state.get("finished")) or not bool(state.get("primary_pass")):
        raise ValueError("Stage4.2R3a requires a finished passing R2 source")
    ...
    if run_dir_override is None:
        root = project_dir / str(cfg.get("output_root", "stage4_2r3a_runs"))
        run_dir = root / f"{cfg['run_name']}_{utc_timestamp()}"
    else:
        run_dir = run_dir_override.expanduser().resolve()
    return Stage42R3AContext(
        cfg=cfg,
        paths=Stage42R3APaths.from_run_dir(run_dir),
...
def _nullspace_directions(modes_tsc: np.ndarray) -> np.ndarray:
    ...
    if modes.shape != (N_COILS, N_MODES) or not np.all(np.isfinite(modes)):
        raise ValueError("Stage4.2R3a frozen mode basis shape/value mismatch")
    ...
    if len(directions) < 3:
        raise ValueError("Stage4.2R3a could not construct three nullspace directions")
    output = np.stack(directions[:3])
    if float(np.max(np.abs(output @ modes))) > 1e-9:
        raise ValueError("Stage4.2R3a generated direction is not orthogonal to modes")
    return output


def _generation_base_payload(ctx: Stage42R3AContext) -> dict[str, Any]:
    path = (
        ctx.source_r1_run
        / "stage4_2r1_restart_variants"
        / "source_slew_1p000_h35.payload.json"
    ...
    env_cfg["tsc_workspace_root"] = str(
        Path(
            os.environ.get(
                "STAGE4_2R3A_TSC_WORKSPACE_ROOT", storage["tsc_workspace_root"]
            )
        )
        .expanduser()
        .resolve()
    ...
    env_cfg["run_root"] = str(
        Path(
            os.environ.get("STAGE4_2R3A_TSC_RUN_ROOT", storage["tsc_run_root"])
        )
        .expanduser()
        .resolve()
    )
    ...
    payload["variant_id"] = "stage4_2r3a_state_generation"
    return payload


def _common_prefix_source(ctx: Stage42R3AContext) -> dict[str, Any]:
    generation = ctx.cfg["state_generation"]
    ...
    if key not in sources:
        raise ValueError(f"Stage4.2R3a common-prefix source missing: {key}")
    source = sources[key]
    ...
    if (
        ...
        raise ValueError("Stage4.2R3a common-prefix action contract invalid")
    payload = {
...
def _deployed_package_fingerprint(ctx: Stage42R3AContext) -> dict[str, Any]:
    relative_paths = [
        "PACKAGE_MANIFEST.json",
        "SHA256SUMS",
        "configs/stage4_2r3a_delayed_counterpulse_hidden_history_initial_state_370ms.json",
        "run_stage4_2r3a_delayed_counterpulse_hidden_history_initial_state_native.sh",
        "run_stage4_2r3a_delayed_counterpulse_hidden_history_initial_state_nohup.sh",
        "run_stage4_2r3a_self_test.sh",
        "run_stage4_2r3a_server_postprocess.sh",
        "run_stage4_2r3a_verify_package.sh",
        "run_stop_stage4_2r3a_now.sh",
        "scripts/stage4_2r3a_delayed_counterpulse_hidden_history_initial_state.py",
        "scripts/stage4_2r3a_server_postprocess.py",
        "scripts/stage4_2r3a_shell_common.sh",
        "tsc_rzip_rllib/diagnostics/stage4_2r3a_delayed_counterpulse_hidden_history_initial_state.py",
    ]
    rows = []
    for relative in relative_paths:
        ...
        if not path.is_file():
            raise FileNotFoundError(
                f"Stage4.2R3a deployed package file missing: {relative}"
            )
        rows.append(
            {
                "path": relative,
    ...
    return {
        "schema_version": 1,
        "contract": "r42r3a_deployed_package_source_v1",
        "n_files": len(rows),
        "digest": _canonical_digest(rows),
        "files": rows,
    }
...
def _state_spec_grid(
                ...
                for gap_steps in map(int, generation["gap_steps"]):
                    ...
                    for order in map(str, generation["history_orders"]):
                        ...
                        if (
                            ...
                            raise ValueError(
                                "Stage4.2R3a generated action sequence is invalid"
                            )
                        identity = {
                            "stage": STAGE,
                        ...
                        specs.append(
                            {
                                "kind": "stage4_2r3a_delayed_counterpulse_state_generation",
                                "controller_revision": CONTROLLER_REVISION,
                                ...
                                "environment_variant": "stage4_2r3a_state_generation",
    ...
    if len(specs) != expected or len({spec["experiment_id"] for spec in specs}) != expected:
        raise ValueError("Stage4.2R3a state-generation specification coverage mismatch")
    return specs
...
def _state_ray_actor_class():
    ...
    if _STATE_RAY_ACTOR is None:
        ...
        _STATE_RAY_ACTOR = Stage42R3AStateActor
    ...
    *,
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            ...
            print(
                f"[Stage4.2R3a state-generation] {index + 1}/{len(pending)}",
                flush=True,
            )
        ...
        plan = ensure_ray_worker_plan(
            ...
            log_prefix="[Stage4.2R3a state-generation]",
        )
        ...
        try:
            while refs:
                ...
                if not ready:
                    print(
                        f"[Stage4.2R3a state-generation] waiting {completed}/{len(pending)}",
                        flush=True,
                    )
                    continue
                for ref in ready:
                    ...
                    if completed % 10 == 0 or not refs:
                        print(
                            f"[Stage4.2R3a state-generation] {completed}/{len(pending)}",
                            flush=True,
                        )
        finally:
...
def _frozen_initial_state(ctx: Stage42R3AContext) -> dict[str, Any]:
    sources = r1._selected_source_cases(ctx.r1_ctx)
    rows = [list(result["trajectory"])[0] for result in sources.values()]
    if len(rows) != 18:
        raise ValueError("Stage4.2R3a frozen source initial-state coverage mismatch")
    reference = rows[0]
    ...
    for row in rows[1:]:
        ...
        if not np.array_equal(vector, reference_vector):
            raise ValueError("Stage4.2R3a R17 cases do not share one frozen initial state")
    return {
        "R": float(reference["R"]),
        "Z": float(reference["Z"]),
...
def analyze_state_generation(
    ctx: Stage42R3AContext, results: Sequence[Mapping[str, Any]]
) -> dict[str, Any]:
...
def _prepare_dirs(paths: Stage42R3APaths) -> None:
    for path in (
        paths.run_dir,
...
def prepare(ctx: Stage42R3AContext, *, resume: bool) -> None:
    ...
    if ctx.paths.manifest.is_file():
        ...
        for key in (
            ...
            if old.get(key) != manifest.get(key):
                raise ValueError(
                    f"Stage4.2R3a resume incompatibility in manifest field {key}"
                )
        if not resume:
            raise FileExistsError(
                "Stage4.2R3a run already exists; use --resume or a fresh run"
            )
    else:
        atomic_write_json(ctx.paths.manifest, manifest)
    atomic_write_json(ctx.paths.run_dir / "stage4_2r3a_config.resolved.json", ctx.cfg)
    atomic_write_json(
        ctx.paths.source_reference / "stage4_2r3a_source_fingerprint.json",
...
def _source_controller_cases(
    ...
    if not required.issubset(sources):
        raise ValueError(f"Stage4.2R3a frozen source cases missing: {required - set(sources)}")
...
def _control_payload(
    ...
    *,
    ...
    payload["env_cfg"] = env_cfg
    payload["train_cfg"] = train_cfg
    payload["variant_id"] = f"stage4_2r3a_{experiment_id}"
    payload["start_folder"] = snapshot_dir.name
    payload["slew_scale"] = float(slew)
    payload["stage4_1r4_horizon_steps"] = horizon
    payload["stage4_2r3a_restart_snapshot_dir"] = str(snapshot_dir)
    payload["stage4_2r3a_snapshot_manifest_digest"] = str(snapshot_digest)
    atomic_write_json(
...
def build_control_specs(
    ctx: Stage42R3AContext, selected_pairs: Sequence[Mapping[str, Any]]
) -> list[dict[str, Any]]:
            ...
            for target in ctx.cfg["control_matrix"]["targets"]:
                for actuator in ctx.cfg["control_matrix"]["future_actuator_cases"]:
                    ...
                    if _forbidden_future_paths(spec):
                        raise ValueError(
                            "Stage4.2R3a frozen controller spec contains future data"
                        )
                    identity = {
                        "stage": STAGE,
                    ...
                    spec.update(
                        {
                            "kind": "stage4_2r3a_authentic_hidden_history_control",
                            "controller_revision": CONTROLLER_REVISION,
                            ...
                            "environment_variant": f"stage4_2r3a_{experiment_id}",
    ...
    if len(specs) != expected or len({spec["experiment_id"] for spec in specs}) != expected:
        raise ValueError("Stage4.2R3a control specification coverage mismatch")
    if expected and not (
        int(ctx.cfg["control_matrix"]["minimum_expected_rollouts"])
        <= expected
        <= int(ctx.cfg["control_matrix"]["maximum_expected_rollouts"])
    ):
        raise ValueError("Stage4.2R3a selected-pair rollout count is outside contract")
    return specs
        ...
        try:
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(
                    failed=failed, reason="stage4_2r3a_hidden_history_control"
                )
...
def _control_ray_actor_class():
    ...
    if _CONTROL_RAY_ACTOR is None:
        ...
        _CONTROL_RAY_ACTOR = Stage42R3AControlActor
    ...
    *,
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            ...
            atomic_write_json_gz(raw_dir / f"{spec['experiment_id']}.json.gz", result)
            print(
                f"[Stage4.2R3a hidden-history-control] {index + 1}/{len(pending)}",
                flush=True,
            )
    elif backend == "ray" and pending:
        import ray
        ...
        plan = ensure_ray_worker_plan(
            ...
            ray_tmpdir=os.environ.get("RAY_TMPDIR")
            or ctx.cfg["parallel"].get("ray_tmpdir"),
            log_prefix="[Stage4.2R3a hidden-history-control]",
        )
        ...
        for index, spec in enumerate(pending):
            ...
            refs[actor.evaluate.remote(spec)] = spec
        print(
            "[Stage4.2R3a hidden-history-control] "
            f"actor_count={plan.actor_count} pending={len(pending)}",
            flush=True,
        )
        completed = 0
        try:
            while refs:
                ...
                if not ready:
                    print(
                        f"[Stage4.2R3a hidden-history-control] waiting {completed}/{len(pending)}",
                        flush=True,
                    )
                    continue
                for ref in ready:
                    ...
                    if completed % 10 == 0 or not refs:
                        print(
                            f"[Stage4.2R3a hidden-history-control] {completed}/{len(pending)}",
                            flush=True,
                        )
        finally:
            s2._close_ray_actors(
...
def _control_row(
    ...
    policy = r1.r13._timing_policy(
        ...
        policy_id=(
            f"r42r3a_{spec['pair_id']}_{spec['history_member']}_"
...
def analyze(
    ...
    if primary_pass:
        verdict_label = (
            "STAGE4_2R3A_AUTHENTIC_HIDDEN_HISTORY_AND_INITIAL_STATE_FINITE_ENVELOPE_PASS"
        )
    elif not state_pass:
        verdict_label = "STAGE4_2R3A_STATE_GENERATION_OR_PAIR_GATE_INCOMPLETE"
    elif control_run:
        verdict_label = "STAGE4_2R3A_AUTHENTIC_HIDDEN_HISTORY_CONTROL_FAILED"
    else:
        verdict_label = "STAGE4_2R3A_CONTROL_NOT_RUN"
    verdict = {
        "schema_version": 1,
        "stage": STAGE,
    ...
    atomic_write_json(ctx.paths.analysis / "stage4_2r3a_verdict.json", verdict)
    atomic_write_json(ctx.paths.analysis / "stage4_2r3a_summary.json", summary)
...
def execute(
    ...
    *,
    ...
    if not bool(offline_summary.get("passed")):
        raise RuntimeError(
            "Stage4.2R3a offline frozen-controller recomputation gate failed; "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage4.2R3a authentic hidden-history and initial-state validation"
    )
