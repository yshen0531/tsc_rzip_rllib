"""Stage4.2R3c3T13S24 authentic sequential transition identification."""
...
CAMPAIGN_IDENTITY = "sequential_amplitude_coded_transition_identification_v1"
CONTROLLER_REVISION = "sequential_amplitude_coded_card15_probe_v42r3c3t13s24_v1"
...
def _validate_config(cfg: Mapping[str, Any], config_path: Path) -> None:
    ...
    exact = {
        ...
        "package_revision": "r42r3c3t13s24_sequential_transition_identification_v1",
    }
    ...
    requested = transition.amplitude_coded_hadamard(schedule)
    ...
    if (
        ...
        or not math.isclose(float(np.linalg.cond(normalized)), 2.82842712474619, abs_tol=1e-12)
    ):
    ...
    routes = cfg["routes"]
    if routes != {
        "offline_fail": "SEQUENTIAL_IDENTIFICATION_PREFLIGHT_FAIL",
        "runtime_fail": "SEQUENTIAL_IDENTIFICATION_RUNTIME_FAIL",
        "training_fail": "SEQUENTIAL_TRANSITION_TRAINING_RECURSIVE_FAIL",
        "calibration_fail": "SEQUENTIAL_TRANSITION_CALIBRATION_FAIL",
        "holdout_fail": "SEQUENTIAL_TRANSITION_HOLDOUT_FAIL_REDESIGN",
        "pass": "SEQUENTIAL_CAUSAL_TRANSITION_HOLDOUT_PASS_MODEL_ONLY",
    }:
    ...
    if config_path.resolve() != (root / "configs/stage4_2r3c3t13s24_sequential_transition_identification_370ms.json").resolve():
        raise ValueError("S24 config path changed")
...
def build_specs(ctx: Context, table: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    requested = transition.amplitude_coded_hadamard(ctx.cfg["schedule_contract"])
    ...
    for context in table:
        ...
        for sequence_index in [-1, *range(24)]:
            ...
            spec.update(
                {
                    "kind": "stage4_2r3c3t13s24_sequential_transition_identification",
                    "stage": STAGE,
                    "campaign_identity": CAMPAIGN_IDENTITY,
                    "controller_revision": CONTROLLER_REVISION,
                    "experiment_id": experiment_id,
                    "phase": "prospective_partitioned_sequential_transition_identification",
                    "category": "identification_only_causal_recursive_transition",
                    "environment_variant": f"stage4_2r3c3t13s24_{experiment_id}",
...
def _payload(ctx: Context, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            "variant_id": f"stage4_2r3c3t13s24_{experiment_id}",
...
def self_test(config_path: Path) -> dict[str, Any]:
    ...
    cfg = _read_json(config_path)
    _validate_config(cfg, config_path)
    matrix = transition.amplitude_coded_hadamard(cfg["schedule_contract"])
