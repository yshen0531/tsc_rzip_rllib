"""R8R43 fixed affine-dominant global/local-affine cold-ensemble preflight."""
...
SCHEMA_VERSION = 1
STAGE = "Stage4.2R3c3T13S24D1R14R8R43"
IDENTITY = "fixed_affine_dominant_global_ridge_local_affine_cold_ensemble_preflight_v1"
...
def validate_config(cfg: Mapping[str, Any], *, project_root: Path) -> None:
    ...
    r41_config = (root / str(cfg["source_r8r41_config"])).resolve()
    bank = cfg["bank_contract"]
    model = cfg["model_contract"]
    gates = cfg["model_gates"]
    scope = cfg["scientific_scope"]
    routes = {
        "execution_fail": "FIXED_AFFINE_DOMINANT_COLD_ENSEMBLE_EXECUTION_FAIL_STOP",
        "model_fail": "FIXED_AFFINE_DOMINANT_COLD_ENSEMBLE_MODEL_FAIL_NO_TSC",
        "pass": "FIXED_AFFINE_DOMINANT_COLD_ENSEMBLE_PASS_CONTROLLER_PREFLIGHT_DESIGN_REQUIRED",
    }
    invalid = (
        ...
        or not r41_config.is_file()
        or _sha(r41_config) != cfg.get("source_r8r41_config_sha256")
        ...
        or float(model.get("global_weight", -1.0)) != 0.25
        or float(model.get("local_weight", -1.0)) != 0.75
    ...
    if invalid:
        raise ValueError("R8R43 frozen config changed")
...
def load_context(args: argparse.Namespace) -> Context:
    ...
    if (
        ...
        raise ValueError("R8R43 R8R31 source context changed")
    return Context(
        cfg=cfg,
        ...
        r8r41_stage=args.r8r41_run.expanduser().resolve() / source_r8r41.RUN_NAME,
        r8r34_ctx=r8r34_ctx,
...
def _authenticate_final_source(
    ...
    if any(hashes[name] != contract[f"{name}_sha256"] for name in hashes):
        raise ValueError(f"R8R43 final {label} source hash changed")
    ...
    if (
        ...
        raise ValueError(f"R8R43 final {label} source state changed")
...
def _authenticate_r8r34_source(
    ...
    hashes = {name: _sha(path) for name, path in paths.items()}
    if any(hashes[name] != contract[f"{name}_sha256"] for name in hashes):
        raise ValueError("R8R43 final R8R34 source hash changed")
    summary = _read(paths["primary_summary"])
    ...
    if (
        ...
        raise ValueError("R8R43 final R8R34 source classification changed")
...
def authenticate_sources(ctx: Context) -> dict[str, Any]:
    ...
    r41 = _authenticate_final_source(
        ctx.r8r41_stage, ctx.cfg["source_r8r41"], label="R8R41"
    )
...
def _verify_bank(
    ...
    if (
        ...
        raise ValueError("R8R43 bank reproduction changed")
...
def _predict_local_affine(
    ...
    if not np.all(filled) or not np.all(np.isfinite(output)):
        raise ValueError("R8R43 local-affine prediction coverage invalid")
...
def predict(
    ...
    if ensemble.shape != global_prediction.shape or not np.all(np.isfinite(ensemble)):
        raise ValueError("R8R43 ensemble prediction invalid")
...
def schedule_jackknife(
    ...
    for held_schedule in schedules:
        ...
        if len(held) != 16:
            raise ValueError("R8R43 schedule fold cardinality changed")
        groups, residual_evidence, predictions = _residuals(model, held, cfg)
        evidence = model_evidence(model)
        folds.append(
            {
...
def compute_from_bank(
    ...
    if not cardinality["passed"]:
        raise ValueError("R8R43 frozen local-neighbor cardinality failed")
...
def run_primary(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage.exists():
        raise ValueError("R8R43 primary requires a fresh stage directory")
...
def run_finalize(ctx: Context) -> dict[str, Any]:
    ...
    if any(not path.is_file() for path in (*paths.values(), ctx.paths.manifest, ctx.paths.state)):
        raise ValueError("R8R43 finalization evidence incomplete")
    if independent_path.is_file() == failure_path.is_file():
        raise ValueError("R8R43 finalization evidence ambiguous")
    summary = _read(paths["primary_summary"])
    ...
    if independent_path.is_file() and not agreement:
        raise ValueError("R8R43 finalization independent disagreement")
    final_route = summary["route"] if agreement else ctx.cfg["routes"]["execution_fail"]
    compact = {
        "schema_version": SCHEMA_VERSION,
        "stage": STAGE,
        "identity": IDENTITY,
        "audit_kind": "r8r43_compact_primary_independent_finalization" if agreement else "r8r43_compact_independent_failure_finalization",
        "passed": agreement,
    ...
    final["audit_kind"] = "r8r43_final_report"
