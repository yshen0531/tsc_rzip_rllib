"""Stage4.2R3b confirmatory hidden-history validation.

R3b generates new real TSC restart states from a common authenticated expert
prefix followed by reversed, time-separated, zero-net full-coil nullspace
pulse histories.  Pair validity is decided only from preregistered checkpoint
gates.  Every selected member is then restarted in a fresh TSC process and
controlled online by the frozen R17 expert.

Full wire current is audit telemetry only.  It is recorded after each action
is chosen and never enters the controller.

The material hidden-current gate was calibrated from the exact frozen R3a
run, but R3b uses a new prefix/amplitude grid and never reuses an R3a snapshot
as a formal result.
"""
...
def _scenario_digest(payload: Mapping[str, Any]) -> str:
    return "s42r3b_" + _canonical_digest(dict(payload))[:20]
...
def _requested_workers(cfg: Mapping[str, Any]) -> int:
    ...
    requested = int(os.environ.get("STAGE4_2R3B_WORKERS", fixed))
    if requested != fixed:
        raise ValueError(
            f"Stage4.2R3b Ray capacity is frozen at {fixed}, got {requested}"
        )
    return requested
...
def validate_config(cfg: Mapping[str, Any]) -> None:
    if str(cfg.get("stage")) != STAGE:
        raise ValueError("Stage4.2R3b config stage mismatch")
    if str(cfg.get("controller_revision")) != CONTROLLER_REVISION:
        raise ValueError("Stage4.2R3b controller revision mismatch")
    if str(cfg.get("package_revision")) != PACKAGE_REVISION:
        raise ValueError("Stage4.2R3b package revision mismatch")
    if int(cfg["parallel"]["n_workers"]) != 128:
        raise ValueError("Stage4.2R3b is fixed to 128 Ray workers")
    generation = cfg["state_generation"]
    prefix = generation["common_prefix_source"]
    if (
        str(prefix["target_id"]) != "nominal"
        or int(prefix["actual_delay_steps"]) != 0
        or not math.isclose(
            float(prefix["actual_slew_scale"]), 1.0, abs_tol=1e-15
        )
        or list(map(int, prefix["prefix_lengths"])) != [5, 9]
        or not bool(prefix["require_exact_authenticated_actions"])
    ):
        raise ValueError("Stage4.2R3b common-prefix source contract changed")
    if list(map(int, generation["nullspace_direction_indices"])) != [1, 2]:
        raise ValueError("Stage4.2R3b nullspace direction grid changed")
    if list(map(float, generation["amplitude_fractions"])) != [0.6, 0.75, 0.9]:
        raise ValueError("Stage4.2R3b amplitude grid changed")
    if list(map(int, generation["gap_steps"])) != [2, 3, 4]:
        raise ValueError("Stage4.2R3b inter-pulse gap grid changed")
    if int(generation["settle_steps"]) != 4:
        raise ValueError("Stage4.2R3b settle contract changed")
    if list(map(str, generation["history_orders"])) != [
        "plus_first",
        "minus_first",
    ]:
        raise ValueError("Stage4.2R3b history order changed")
    if (
        int(generation["expected_pairs"]) != 36
        or int(generation["expected_rollouts"]) != 72
        or int(generation["pulse_steps"]) != 2
        or not math.isclose(
            float(generation["maximum_abs_action"]), 0.9, abs_tol=1e-15
        )
        or not bool(generation["zero_requested_net_nullspace_increment"])
        or not bool(generation["require_authentic_snapshot"])
        or not bool(generation["require_full_wire_current_vector"])
    ):
        raise ValueError("Stage4.2R3b state-generation contract changed")
    gate = cfg["pair_gate"]
    expected_gate = {
        ...
        "wire_max_abs_difference_A_min": 0.25,
        "wire_rms_difference_A_min": 0.1,
        "wire_relative_rms_difference_min": 0.05,
    }
    for key, expected in expected_gate.items():
        if not math.isclose(float(gate[key]), expected, abs_tol=1e-15):
            raise ValueError(f"Stage4.2R3b pair gate changed: {key}")
    if (
        int(gate["wire_count"]) != N_WIRES
        or int(gate["minimum_selected_pairs"]) != 4
        or int(gate["maximum_selected_pairs"]) != 4
        or not bool(gate["require_all_prefix_direction_strata"])
        or not bool(gate["forbid_control_outcome_selection"])
    ):
        raise ValueError("Stage4.2R3b pair coverage/selection gate changed")
    if list(map(str, gate["selection_order"])) != [
        "largest_wire_relative_rms",
        "largest_wire_rms_difference",
        "largest_wire_max_abs_difference",
        "smallest_visible_max_normalized_ratio",
        "lower_amplitude",
        "longer_gap",
        "lexical_pair_id",
    ]:
        raise ValueError("Stage4.2R3b pair selection order changed")
    ...
    for key, expected in expected_initial.items():
        if not math.isclose(float(initial[key]), expected, abs_tol=1e-15):
            raise ValueError(f"Stage4.2R3b initial-state gate changed: {key}")
    if (
        int(initial["minimum_selected_centroids"]) != 4
        or list(map(int, initial["required_common_prefix_lengths"])) != [5, 9]
        or not bool(initial["require_mutual_prefix_group_separation"])
    ):
        raise ValueError("Stage4.2R3b initial-state coverage gate changed")
    matrix = cfg["control_matrix"]
    if list(map(str, matrix["targets"])) != ["nominal", "RZ_p10_m10"]:
        raise ValueError("Stage4.2R3b target matrix changed")
    actuator = [
        (int(row["actual_delay_steps"]), float(row["actual_slew_scale"]))
        for row in matrix["future_actuator_cases"]
    ]
    if actuator != [(0, 1.0), (2, 0.9)]:
        raise ValueError("Stage4.2R3b future actuator matrix changed")
    if (
        list(map(str, matrix["history_members"]))
        != ["plus_first", "minus_first"]
        or int(matrix["expected_rollouts_per_selected_pair"]) != 8
        or int(matrix["minimum_expected_rollouts"]) != 32
        or int(matrix["maximum_expected_rollouts"]) != 32
        ...
        raise ValueError("Stage4.2R3b control rollout coverage changed")
    ...
    if (
        ...
        raise ValueError("Stage4.2R3b formal timing changed")
    if bool(cfg.get("bc_dagger_or_rl_allowed")):
        raise ValueError("Stage4.2R3b may not authorize BC, DAgger, or RL")
    calibration = cfg["source_requirements"]["calibration_stage4_2r3a"]
    expected_calibration = {
        "required_run_name": (
            "stage4_2r3a_delayed_counterpulse_hidden_history_initial_state_"
            "20260730_110128"
        ),
        "required_stage": "Stage4.2R3a",
        ...
        "required_run_inventory_file_count": 739,
        "required_run_inventory_total_bytes": 8535699130,
        ...
        "required_state_raw_count": 72,
        "required_snapshot_valid_count": 72,
        "required_candidate_pair_count": 36,
        "required_visible_match_pair_count": 24,
        "required_original_absolute_gate_pass_count": 0,
        "required_control_raw_count": 0,
        "require_failed_original_primary_gate": True,
    }
    if dict(calibration) != expected_calibration:
        raise ValueError("Stage4.2R3b R3a calibration contract changed")


@dataclass(frozen=True)
class Stage42R3BPaths:
    run_dir: Path
    ...
    @classmethod
    def from_run_dir(cls, run_dir: Path) -> "Stage42R3BPaths":
        root = run_dir.expanduser().resolve()
        return cls(
            run_dir=root,
            source_reference=root / "stage4_2r3b_source_reference",
            state_generation=root / "stage4_2r3b_state_generation",
            pair_analysis=root / "stage4_2r3b_pair_analysis",
            variants=root / "stage4_2r3b_environment_variants",
            control=root / "stage4_2r3b_hidden_history_control",
            analysis=root / "stage4_2r3b_analysis",
            state=root / "stage4_2r3b_state.json",
            manifest=root / "stage4_2r3b_manifest.json",
        )


@dataclass
class Stage42R3BContext:
    cfg: dict[str, Any]
    paths: Stage42R3BPaths
    project_dir: Path
    source_r2_run: Path
    source_r1_run: Path
    source_r17_run: Path
    calibration_r3a_run: Path
    calibration_r3a_audit: Path
    source_r2_manifest: dict[str, Any]
    source_r2_state: dict[str, Any]
    r1_ctx: r1.Stage42R1Context
    source_fingerprint: dict[str, Any]
    calibration_fingerprint: dict[str, Any]
...
def _source_fingerprint(
    ...
    if len(required) != 30:
        raise ValueError(
            f"Stage4.2R3b source fingerprint expected 30 files, got {len(required)}"
        )
    missing = [path for path in required if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"Stage4.2R3b source file missing: {missing[0]}")
    rows = []
    ...
    return {
        "schema_version": 1,
        "contract": "r42r3b_r2_r1_r17_direct_source_v1",
        "n_files": len(rows),
...
def _calibration_fingerprint(
    ...
    """Validate and fingerprint the exact completed R3a calibration evidence."""

    calibration_run = calibration_run.expanduser().resolve()
    contract = cfg["source_requirements"]["calibration_stage4_2r3a"]
    if calibration_run.name != str(contract["required_run_name"]):
        raise ValueError("Stage4.2R3b R3a calibration run identity mismatch")
    calibration_audit = (
        calibration_run.parents[1]
        / "stage4_2r3a_audits"
        / calibration_run.name
    ).resolve()
    paths = {
        "run_manifest": calibration_run / "stage4_2r3a_manifest.json",
        "run_state": calibration_run / "stage4_2r3a_state.json",
        "pair_summary": (
            calibration_run / "stage4_2r3a_pair_analysis" / "summary.json"
        ),
        "run_verdict": (
            calibration_run
        ...
        "server_audit": (
            calibration_audit / "stage4_2r3a_server_audit.json"
        ),
        "run_inventory": (
            calibration_audit / "stage4_2r3a_run_inventory.json"
        ),
        "raw_forensics": (
            calibration_audit / "stage4_2r3a_raw_forensics.json"
        ),
        "snapshot_checks": (
            calibration_audit / "stage4_2r3a_snapshot_checks.json"
        ),
    }
    missing = [path for path in paths.values() if not path.is_file()]
    if missing:
        raise FileNotFoundError(
            f"Stage4.2R3b R3a calibration evidence missing: {missing[0]}"
        )
    expected_hashes = {
        "run_inventory": str(
            contract["required_run_inventory_file_sha256"]
        ),
        "server_audit": str(contract["required_server_audit_sha256"]),
        "raw_forensics": str(contract["required_raw_forensics_sha256"]),
    }
    for name, expected in expected_hashes.items():
        if _sha256_file(paths[name]) != expected:
            raise ValueError(
                f"Stage4.2R3b R3a calibration artifact hash mismatch: {name}"
            )
    manifest = read_json(paths["run_manifest"])
    state = read_json(paths["run_state"])
    pair = read_json(paths["pair_summary"])
    verdict = read_json(paths["run_verdict"])
    audit = read_json(paths["server_audit"])
    inventory = read_json(paths["run_inventory"])
    forensics = read_json(paths["raw_forensics"])
    snapshots = read_json(paths["snapshot_checks"])
    if (
        str(manifest.get("stage")) != str(contract["required_stage"])
        or str(manifest.get("controller_revision"))
        != str(contract["required_controller_revision"])
        or str(manifest.get("package_revision"))
        != str(contract["required_package_revision"])
    ):
        raise ValueError("Stage4.2R3b R3a calibration manifest mismatch")
    if (
        not bool(state.get("finished"))
        or bool(state.get("primary_pass"))
        or str(state.get("phase_status")) != "campaign_complete"
        or str(state.get("stop_reason")) != "insufficient_valid_pairs"
        or not bool(contract["require_failed_original_primary_gate"])
    ):
        raise ValueError("Stage4.2R3b R3a calibration state mismatch")
    if (
        bool(verdict.get("primary_pass"))
        or str(verdict.get("hidden_history_control_status")) != "not_run"
        or bool(verdict.get("matched_visible_different_hidden_history_validated"))
    ):
        raise ValueError("Stage4.2R3b R3a calibration verdict mismatch")
    if (
        int(inventory.get("n_files", -1))
        != int(contract["required_run_inventory_file_count"])
        or int(inventory.get("total_bytes", -1))
        != int(contract["required_run_inventory_total_bytes"])
        or str(inventory.get("digest"))
        != str(contract["required_run_inventory_digest"])
    ):
        raise ValueError("Stage4.2R3b R3a calibration inventory mismatch")
    required_raw = int(contract["required_state_raw_count"])
    required_snapshots = int(contract["required_snapshot_valid_count"])
    required_pairs = int(contract["required_candidate_pair_count"])
    required_visible = int(contract["required_visible_match_pair_count"])
    if (
        int(pair.get("state_rollout_count", -1)) != required_raw
        or int(pair.get("pair_count", -1)) != required_pairs
        or int(pair.get("visible_match_pair_count", -1)) != required_visible
        or int(pair.get("hidden_separation_pair_count", -1))
        != int(contract["required_original_absolute_gate_pass_count"])
        or int(pair.get("selected_pair_count", -1)) != 0
        or int(pair.get("runtime_or_environment_error_count", -1)) != 0
        or int(pair.get("raw_or_snapshot_corruption_count", -1)) != 0
        or not bool(pair.get("complete_state_grid"))
    ):
        raise ValueError("Stage4.2R3b R3a calibration pair summary mismatch")
    if (
        not bool(audit.get("passed"))
        or not bool((audit.get("manifest_compatibility") or {}).get("passed"))
        or int(audit.get("state_raw_actual", -1)) != required_raw
        or int(audit.get("snapshot_valid_count", -1)) != required_snapshots
        or int(audit.get("control_raw_actual", -1))
        != int(contract["required_control_raw_count"])
        or int(audit.get("runtime_or_environment_error_count", -1)) != 0
        or int(audit.get("state_raw_or_snapshot_corruption_count", -1)) != 0
        or str((audit.get("run_inventory") or {}).get("digest"))
        != str(contract["required_run_inventory_digest"])
    ):
        raise ValueError("Stage4.2R3b R3a server audit mismatch")
    original = forensics.get("original_pair_summary") or {}
    if (
        bool(forensics.get("r3a_verdict_changed"))
        or int(forensics.get("raw_file_count", -1)) != required_raw
        or int(forensics.get("raw_success_count", -1)) != required_raw
        or int(forensics.get("snapshot_valid_count", -1)) != required_snapshots
        or int(forensics.get("different_initial_state_pass_count", -1))
        != required_pairs
        or int(original.get("pair_count", -1)) != required_pairs
        or int(original.get("visible_match_count", -1)) != required_visible
        or int(original.get("absolute_hidden_gate_count", -1))
        != int(contract["required_original_absolute_gate_pass_count"])
    ):
        raise ValueError("Stage4.2R3b R3a raw forensics mismatch")
    if (
        not isinstance(snapshots, list)
        or len(snapshots) != required_snapshots
        or not all(bool(row.get("valid")) for row in snapshots)
    ):
        raise ValueError("Stage4.2R3b R3a snapshot audit mismatch")
    rows = [
        {
            "path": name,
            "size_bytes": int(path.stat().st_size),
    ...
    return calibration_audit, {
        "schema_version": 1,
        "contract": "r42r3b_exact_r3a_calibration_evidence_v1",
        ...
        "state_raw_count": required_raw,
        "snapshot_valid_count": required_snapshots,
        "candidate_pair_count": required_pairs,
        "visible_match_pair_count": required_visible,
        "original_absolute_gate_pass_count": int(
            contract["required_original_absolute_gate_pass_count"]
        ),
        "control_raw_count": int(contract["required_control_raw_count"]),
        "n_files": len(rows),
        "digest": _canonical_digest(rows),
        "files": rows,
    }
...
def load_stage42r3b_config(
    config_path: Path,
    *,
    source_stage42r2_run: Path,
    calibration_stage42r3a_run: Path,
    ...
    if str(manifest.get("stage")) != "Stage4.2R2":
        raise ValueError("Stage4.2R3b source stage mismatch")
    if str(manifest.get("controller_revision")) != EXPECTED_R2_CONTROLLER:
        raise ValueError("Stage4.2R3b source R2 controller mismatch")
    if str(manifest.get("package_revision")) != EXPECTED_R2_PACKAGE:
        raise ValueError("Stage4.2R3b source R2 package mismatch")
    if not bool(state.get("finished")) or not bool(state.get("primary_pass")):
        raise ValueError("Stage4.2R3b requires a finished passing R2 source")
    ...
    if run_dir_override is None:
        root = project_dir / str(cfg.get("output_root", "stage4_2r3b_runs"))
        run_dir = root / f"{cfg['run_name']}_{utc_timestamp()}"
    else:
        run_dir = run_dir_override.expanduser().resolve()
    calibration_r3a_run = calibration_stage42r3a_run.expanduser().resolve()
    calibration_r3a_audit, calibration_fingerprint = _calibration_fingerprint(
        calibration_r3a_run, cfg
    )
    return Stage42R3BContext(
        cfg=cfg,
        paths=Stage42R3BPaths.from_run_dir(run_dir),
        project_dir=project_dir,
        source_r2_run=source_r2,
        source_r1_run=source_r1,
        source_r17_run=source_r17,
        calibration_r3a_run=calibration_r3a_run,
        calibration_r3a_audit=calibration_r3a_audit,
...
def _nullspace_directions(modes_tsc: np.ndarray) -> np.ndarray:
    ...
    if modes.shape != (N_COILS, N_MODES) or not np.all(np.isfinite(modes)):
        raise ValueError("Stage4.2R3b frozen mode basis shape/value mismatch")
    ...
    if len(directions) < 3:
        raise ValueError("Stage4.2R3b could not construct three nullspace directions")
    output = np.stack(directions[:3])
    if float(np.max(np.abs(output @ modes))) > 1e-9:
        raise ValueError("Stage4.2R3b generated direction is not orthogonal to modes")
    return output


def _generation_base_payload(ctx: Stage42R3BContext) -> dict[str, Any]:
    path = (
        ctx.source_r1_run
        / "stage4_2r1_restart_variants"
        / "source_slew_1p000_h35.payload.json"
    ...
    env_cfg["tsc_workspace_root"] = str(
        Path(
            os.environ.get(
                "STAGE4_2R3B_TSC_WORKSPACE_ROOT", storage["tsc_workspace_root"]
            )
        )
        .expanduser()
        .resolve()
    ...
    env_cfg["run_root"] = str(
        Path(
            os.environ.get("STAGE4_2R3B_TSC_RUN_ROOT", storage["tsc_run_root"])
        )
        .expanduser()
        .resolve()
    )
    ...
    generation = ctx.cfg["state_generation"]
    maximum_horizon = (
        max(map(int, generation["common_prefix_source"]["prefix_lengths"]))
        + int(generation["pulse_steps"])
        + max(map(int, generation["gap_steps"]))
        + int(generation["settle_steps"])
    )
    train_cfg.setdefault("episode", {})["max_episode_steps"] = maximum_horizon
    atomic_write_json(train_path, train_cfg)
    payload["env_cfg"] = env_cfg
    payload["train_cfg"] = train_cfg
    payload["variant_id"] = "stage4_2r3b_state_generation"
    return payload


def _common_prefix_source(ctx: Stage42R3BContext) -> dict[str, Any]:
    generation = ctx.cfg["state_generation"]
    contract = generation["common_prefix_source"]
    key = (
        str(contract["target_id"]),
        int(contract["actual_delay_steps"]),
        float(contract["actual_slew_scale"]),
    )
    sources = r1._selected_source_cases(ctx.r1_ctx)
    if key not in sources:
        raise ValueError(f"Stage4.2R3b common-prefix source missing: {key}")
    source = sources[key]
    maximum = max(map(int, contract["prefix_lengths"]))
    actions = r1._source_action_sequence(source, horizon=maximum)
    initial_action = r1._source_initial_action(source)
    action_array = np.asarray(actions, dtype=float)
    initial_array = np.asarray(initial_action, dtype=float)
    if (
        action_array.shape != (maximum, N_COILS)
        or initial_array.shape != (N_COILS,)
        or not np.all(np.isfinite(action_array))
        or not np.all(np.isfinite(initial_array))
        or float(np.max(np.abs(action_array))) > 1.0
        or float(np.max(np.abs(initial_array))) > 1.0
    ):
        raise ValueError("Stage4.2R3b common-prefix action contract invalid")
    payload = {
        "source_experiment_id": str(source["experiment_id"]),
        "target_id": key[0],
        "actual_delay_steps": key[1],
        "actual_slew_scale": key[2],
        "maximum_prefix_steps": maximum,
        "source_initial_action": initial_array.tolist(),
        "actions": action_array.tolist(),
    }
    payload["prefix_action_digests"] = {
        str(length): _canonical_digest(
            {
                "source_experiment_id": payload["source_experiment_id"],
                "source_initial_action": payload["source_initial_action"],
                "actions": payload["actions"][:length],
            }
        )
        for length in map(int, contract["prefix_lengths"])
    }
    payload["action_digest"] = _canonical_digest(
        {
            "source_experiment_id": payload["source_experiment_id"],
            "source_initial_action": payload["source_initial_action"],
...
def _deployed_package_fingerprint(ctx: Stage42R3BContext) -> dict[str, Any]:
    relative_paths = [
        ...
        "run_stage4_2r3b_self_test.sh",
        "run_stage4_2r3b_server_postprocess.sh",
        "run_stage4_2r3b_verify_package.sh",
        "run_stop_stage4_2r3b_now.sh",
    ...
    for relative in relative_paths:
        ...
        if not path.is_file():
            raise FileNotFoundError(
                f"Stage4.2R3b deployed package file missing: {relative}"
            )
        rows.append(
            {
                "path": relative,
    ...
    return {
        "schema_version": 1,
        "contract": "r42r3b_deployed_package_source_v1",
        "n_files": len(rows),
        "digest": _canonical_digest(rows),
        "files": rows,
    }
...
def _state_spec_grid(
    ...
    source_actions = np.asarray(common_prefix["actions"], dtype=float)
    source_initial_action = np.asarray(
        common_prefix["source_initial_action"], dtype=float
    )
    settle_steps = int(generation["settle_steps"])
    for prefix_steps in map(
        int, generation["common_prefix_source"]["prefix_lengths"]
    ):
        prefix_actions = source_actions[:prefix_steps].tolist()
        for direction_index in map(
            int, generation["nullspace_direction_indices"]
        ):
            direction = np.asarray(
                directions[direction_index], dtype=float
            ).reshape(N_COILS)
            for amplitude in map(float, generation["amplitude_fractions"]):
                for gap_steps in map(int, generation["gap_steps"]):
                    pair_id = (
                        f"p{prefix_steps}_q{direction_index}_a{amplitude:.3f}"
                        f"_gap{gap_steps}_settle{settle_steps}"
                    ).replace(".", "p")
                    horizon = (
                        prefix_steps
                        + int(generation["pulse_steps"])
                    ...
                    for order in map(str, generation["history_orders"]):
                        ...
                        actions = copy.deepcopy(prefix_actions)
                        actions.append((sign * amplitude * direction).tolist())
                        actions.extend(
                            [[0.0] * N_COILS for _ in range(gap_steps)]
                        )
                        actions.append((-sign * amplitude * direction).tolist())
                        actions.extend(
                            [[0.0] * N_COILS for _ in range(settle_steps)]
                        )
                        action_array = np.asarray(actions, dtype=float)
                        if (
                            action_array.shape != (horizon, N_COILS)
                            or float(np.max(np.abs(action_array))) > 1.0
                        ):
                            raise ValueError(
                                "Stage4.2R3b generated action sequence is invalid"
                            )
                        identity = {
                            "stage": STAGE,
                            ...
                            "common_prefix_action_digest": common_prefix[
                        ...
                        specs.append(
                            {
                                "kind": "stage4_2r3b_confirmatory_state_generation",
                                "controller_revision": CONTROLLER_REVISION,
                                ...
                                "environment_variant": "stage4_2r3b_state_generation",
                                "horizon_steps": horizon,
                                "checkpoint_step": horizon,
                                "source_initial_action": source_initial_action.tolist(),
                                ...
                                "gap_steps": gap_steps,
                                "settle_steps": settle_steps,
                                "zero_requested_net_nullspace_increment": True,
    ...
    if len(specs) != expected or len({spec["experiment_id"] for spec in specs}) != expected:
        raise ValueError("Stage4.2R3b state-generation specification coverage mismatch")
    return specs
...
class LocalStateGenerationWorker:
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        result["state_generation_summary"] = {
            ...
            "common_prefix_steps": int(spec["common_prefix_steps"]),
...
def _state_ray_actor_class():
    ...
    if _STATE_RAY_ACTOR is None:
        ...
        _STATE_RAY_ACTOR = Stage42R3BStateActor
    ...
    *,
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            ...
            print(
                f"[Stage4.2R3b state-generation] {index + 1}/{len(pending)}",
                flush=True,
            )
        ...
        plan = ensure_ray_worker_plan(
            ...
            log_prefix="[Stage4.2R3b state-generation]",
        )
        ...
        try:
            while refs:
                ...
                if not ready:
                    print(
                        f"[Stage4.2R3b state-generation] waiting {completed}/{len(pending)}",
                        flush=True,
                    )
                    continue
                for ref in ready:
                    ...
                    if completed % 10 == 0 or not refs:
                        print(
                            f"[Stage4.2R3b state-generation] {completed}/{len(pending)}",
                            flush=True,
                        )
        finally:
...
def _state_row(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "history_order": str(spec["history_order"]),
        "common_prefix_steps": int(spec["common_prefix_steps"]),
        "common_prefix_source_experiment_id": str(
            spec["common_prefix_source_experiment_id"]
        ),
        "common_prefix_action_digest": str(spec["common_prefix_action_digest"]),
...
def _pair_metrics(
    ...
    hidden_pass = bool(
        ...
        and metrics["wire_rms_difference_A"]
        >= float(gate["wire_rms_difference_A_min"])
        and metrics["wire_relative_rms_difference"]
        >= float(gate["wire_relative_rms_difference_min"])
    )
    return {
        "pair_id": str(left["pair_id"]),
        "common_prefix_steps": int(left["common_prefix_steps"]),
        "common_prefix_source_experiment_id": str(
            left["common_prefix_source_experiment_id"]
        ),
        "common_prefix_action_digest": str(left["common_prefix_action_digest"]),
        "nullspace_direction_index": int(left["nullspace_direction_index"]),
        "amplitude_fraction": float(left["amplitude_fraction"]),
        "gap_steps": int(left["gap_steps"]),
...
def _frozen_initial_state(ctx: Stage42R3BContext) -> dict[str, Any]:
    sources = r1._selected_source_cases(ctx.r1_ctx)
    rows = [list(result["trajectory"])[0] for result in sources.values()]
    if len(rows) != 18:
        raise ValueError("Stage4.2R3b frozen source initial-state coverage mismatch")
    reference = rows[0]
    ...
    for row in rows[1:]:
        ...
        if not np.array_equal(vector, reference_vector):
            raise ValueError("Stage4.2R3b R17 cases do not share one frozen initial state")
    return {
        "R": float(reference["R"]),
        "Z": float(reference["Z"]),
...
def _different_initial_metrics(
    ...
    metrics = {
        ...
        "centroid_coil_currents_a": centroid_coil.tolist(),
...
def _prefix_group_separation(
    selected: Sequence[Mapping[str, Any]],
    required_prefixes: Sequence[int],
    gate: Mapping[str, Any],
) -> dict[str, Any]:
    groups: dict[int, list[Mapping[str, Any]]] = {
        int(prefix): [] for prefix in required_prefixes
    }
    for row in selected:
        prefix = int(row["common_prefix_steps"])
        if prefix in groups:
            groups[prefix].append(row)
    covered = sorted(prefix for prefix, rows in groups.items() if rows)
    output: dict[str, Any] = {
        "required_common_prefix_lengths": list(map(int, required_prefixes)),
        "selected_common_prefix_lengths": covered,
        "common_prefix_coverage_pass": covered
        == sorted(map(int, required_prefixes)),
        "mutual_prefix_group_separation_pass": False,
    }
    if len(groups) != 2 or not all(groups.values()):
        return output
    centroids: dict[int, dict[str, Any]] = {}
    for prefix, rows in groups.items():
        centroids[prefix] = {
            "R": float(np.mean([float(row["centroid_R"]) for row in rows])),
            "Z": float(np.mean([float(row["centroid_Z"]) for row in rows])),
            "Ip": float(np.mean([float(row["centroid_Ip"]) for row in rows])),
            "coil_currents_a": np.mean(
                np.asarray(
                    [row["centroid_coil_currents_a"] for row in rows],
                    dtype=float,
                ),
                axis=0,
            ).tolist(),
            "selected_pair_ids": [str(row["pair_id"]) for row in rows],
        }
    left_prefix, right_prefix = sorted(centroids)
    left = centroids[left_prefix]
    right = centroids[right_prefix]
    metrics = {
        "R_abs_difference_m": abs(float(left["R"]) - float(right["R"])),
        "Z_abs_difference_m": abs(float(left["Z"]) - float(right["Z"])),
        "Ip_abs_difference_A": abs(float(left["Ip"]) - float(right["Ip"])),
        "coil_rms_difference_A": _rms(
            np.asarray(left["coil_currents_a"], dtype=float)
            - np.asarray(right["coil_currents_a"], dtype=float)
        ),
    }
    passed = bool(
        metrics["R_abs_difference_m"]
        >= float(gate["R_centroid_difference_m_min"])
        or metrics["Z_abs_difference_m"]
        >= float(gate["Z_centroid_difference_m_min"])
        or metrics["Ip_abs_difference_A"]
        >= float(gate["Ip_centroid_difference_A_min"])
        or metrics["coil_rms_difference_A"]
        >= float(gate["coil_centroid_rms_difference_A_min"])
    )
    output.update(
        {
            "prefix_group_centroids": centroids,
            "prefix_group_separation_metrics": metrics,
            "mutual_prefix_group_separation_pass": passed,
        }
    )
    return output
...
def analyze_state_generation(
    ctx: Stage42R3BContext, results: Sequence[Mapping[str, Any]]
) -> dict[str, Any]:
    ...
    for prefix_steps in map(
        int,
        ctx.cfg["state_generation"]["common_prefix_source"]["prefix_lengths"],
    ):
        for direction in map(
            ...
            candidates = [
                row
                for row in accepted
                if int(row["common_prefix_steps"]) == prefix_steps
    ...
    required_prefixes = list(
        map(int, initial_gate["required_common_prefix_lengths"])
    )
    prefix_separation = _prefix_group_separation(
        selected, required_prefixes, initial_gate
    )
    ...
    expected_strata = {
        (int(prefix), int(direction))
        for prefix in ctx.cfg["state_generation"]["common_prefix_source"][
            "prefix_lengths"
        ]
        for direction in ctx.cfg["state_generation"][
            "nullspace_direction_indices"
        ]
    }
    selected_strata = {
        (
            int(row["common_prefix_steps"]),
            int(row["nullspace_direction_index"]),
        )
        for row in selected
    }
    strata_coverage_pass = bool(selected_strata == expected_strata)
    ...
    passed = bool(
        ...
        and strata_coverage_pass
        and bool(prefix_separation["common_prefix_coverage_pass"])
        and bool(prefix_separation["mutual_prefix_group_separation_pass"])
    )
    state_public = [
        {
            key: value
    ...
    summary = {
        ...
        "required_prefix_direction_strata": [
            {
                "common_prefix_steps": prefix,
                "nullspace_direction_index": direction,
            }
            for prefix, direction in sorted(expected_strata)
        ],
        "selected_prefix_direction_strata": [
            {
                "common_prefix_steps": prefix,
                "nullspace_direction_index": direction,
            }
            for prefix, direction in sorted(selected_strata)
        ],
        "prefix_direction_strata_coverage_pass": strata_coverage_pass,
        **prefix_separation,
        ...
        "stop_reason": (
            ...
            if different_count < minimum_initial
            else "missing_prefix_direction_stratum"
            if not strata_coverage_pass
            else "missing_common_prefix_coverage"
            if not bool(prefix_separation["common_prefix_coverage_pass"])
            else "insufficient_mutual_prefix_group_separation"
        ),
    }
...
def _prepare_dirs(paths: Stage42R3BPaths) -> None:
    for path in (
        paths.run_dir,
...
def prepare(ctx: Stage42R3BContext, *, resume: bool) -> None:
    _prepare_dirs(ctx.paths)
    payload = _generation_base_payload(ctx)
    directions = _nullspace_directions(np.asarray(payload["modes_tsc"], dtype=float))
    common_prefix = _common_prefix_source(ctx)
    specs = _state_spec_grid(directions, ctx.cfg, common_prefix)
    ...
    manifest = {
        ...
        "calibration_stage4_2r3a_audit": str(ctx.calibration_r3a_audit),
        "source_fingerprint": ctx.source_fingerprint,
        "calibration_fingerprint": ctx.calibration_fingerprint,
        "deployed_package_fingerprint": package_fingerprint,
        "common_prefix_source": common_prefix,
    ...
    if ctx.paths.manifest.is_file():
        ...
        for key in (
            ...
            "calibration_stage4_2r3a_audit",
            "config_digest",
            "state_generation_spec_digest",
            "source_fingerprint",
            "calibration_fingerprint",
            "deployed_package_fingerprint",
            "common_prefix_source",
        ):
            if old.get(key) != manifest.get(key):
                raise ValueError(
                    f"Stage4.2R3b resume incompatibility in manifest field {key}"
                )
        if not resume:
            raise FileExistsError(
                "Stage4.2R3b run already exists; use --resume or a fresh run"
            )
    else:
        atomic_write_json(ctx.paths.manifest, manifest)
    atomic_write_json(ctx.paths.run_dir / "stage4_2r3b_config.resolved.json", ctx.cfg)
    atomic_write_json(
        ctx.paths.source_reference / "stage4_2r3b_source_fingerprint.json",
        ctx.source_fingerprint,
    )
    atomic_write_json(
        ctx.paths.source_reference / "stage4_2r3a_calibration_fingerprint.json",
    ...
    atomic_write_json(
        ctx.paths.source_reference / "common_prefix_source.json",
        common_prefix,
    )
    atomic_write_json(
...
def _source_controller_cases(
    ...
    if not required.issubset(sources):
        raise ValueError(f"Stage4.2R3b frozen source cases missing: {required - set(sources)}")
...
def _control_payload(
    ...
    *,
    ...
    payload["env_cfg"] = env_cfg
    payload["train_cfg"] = train_cfg
    payload["variant_id"] = f"stage4_2r3b_{experiment_id}"
    payload["start_folder"] = snapshot_dir.name
    payload["slew_scale"] = float(slew)
    payload["stage4_1r4_horizon_steps"] = horizon
    payload["stage4_2r3b_restart_snapshot_dir"] = str(snapshot_dir)
    payload["stage4_2r3b_snapshot_manifest_digest"] = str(snapshot_digest)
    atomic_write_json(
...
def build_control_specs(
    ctx: Stage42R3BContext, selected_pairs: Sequence[Mapping[str, Any]]
) -> list[dict[str, Any]]:
            ...
            for target in ctx.cfg["control_matrix"]["targets"]:
                for actuator in ctx.cfg["control_matrix"]["future_actuator_cases"]:
                    ...
                    if _forbidden_future_paths(spec):
                        raise ValueError(
                            "Stage4.2R3b frozen controller spec contains future data"
                        )
                    identity = {
                        "stage": STAGE,
                    ...
                    spec.update(
                        {
                            "kind": "stage4_2r3b_authentic_hidden_history_control",
                            "controller_revision": CONTROLLER_REVISION,
                            ...
                            "environment_variant": f"stage4_2r3b_{experiment_id}",
    ...
    if len(specs) != expected or len({spec["experiment_id"] for spec in specs}) != expected:
        raise ValueError("Stage4.2R3b control specification coverage mismatch")
    if expected and not (
        int(ctx.cfg["control_matrix"]["minimum_expected_rollouts"])
        <= expected
        <= int(ctx.cfg["control_matrix"]["maximum_expected_rollouts"])
    ):
        raise ValueError("Stage4.2R3b selected-pair rollout count is outside contract")
    return specs
        ...
        try:
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(
                    failed=failed, reason="stage4_2r3b_hidden_history_control"
                )
...
def _control_ray_actor_class():
    ...
    if _CONTROL_RAY_ACTOR is None:
        ...
        _CONTROL_RAY_ACTOR = Stage42R3BControlActor
    ...
    *,
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            ...
            atomic_write_json_gz(raw_dir / f"{spec['experiment_id']}.json.gz", result)
            print(
                f"[Stage4.2R3b hidden-history-control] {index + 1}/{len(pending)}",
                flush=True,
            )
    elif backend == "ray" and pending:
        import ray
        ...
        plan = ensure_ray_worker_plan(
            ...
            ray_tmpdir=os.environ.get("RAY_TMPDIR")
            or ctx.cfg["parallel"].get("ray_tmpdir"),
            log_prefix="[Stage4.2R3b hidden-history-control]",
        )
        ...
        for index, spec in enumerate(pending):
            ...
            refs[actor.evaluate.remote(spec)] = spec
        print(
            "[Stage4.2R3b hidden-history-control] "
            f"actor_count={plan.actor_count} pending={len(pending)}",
            flush=True,
        )
        completed = 0
        try:
            while refs:
                ...
                if not ready:
                    print(
                        f"[Stage4.2R3b hidden-history-control] waiting {completed}/{len(pending)}",
                        flush=True,
                    )
                    continue
                for ref in ready:
                    ...
                    if completed % 10 == 0 or not refs:
                        print(
                            f"[Stage4.2R3b hidden-history-control] {completed}/{len(pending)}",
                            flush=True,
                        )
        finally:
            s2._close_ray_actors(
...
def _control_row(
    ...
    policy = r1.r13._timing_policy(
        ...
        policy_id=(
            f"r42r3b_{spec['pair_id']}_{spec['history_member']}_"
...
def _control_pair_divergence(
    plus_result: Mapping[str, Any] | None,
    minus_result: Mapping[str, Any] | None,
) -> dict[str, Any]:
    """Report history-member action/visible divergence without gating on it."""

    empty = {
        "same_trajectory_length": False,
        "maximum_action_abs_difference": None,
        "maximum_R_abs_difference_m": None,
        "maximum_Z_abs_difference_m": None,
        "maximum_Ip_abs_difference_A": None,
        "maximum_coil_abs_difference_A": None,
        "maximum_wire_abs_difference_A": None,
    }
    if not plus_result or not minus_result:
        return empty
    plus = list(plus_result.get("trajectory") or [])
    minus = list(minus_result.get("trajectory") or [])
    if not plus or len(plus) != len(minus):
        return empty
    try:
        plus_action = np.asarray(
            [row["action_norm_tsc"] for row in plus[1:]], dtype=float
        )
        minus_action = np.asarray(
            [row["action_norm_tsc"] for row in minus[1:]], dtype=float
        )
        plus_rzi = np.asarray(
            [[row["R"], row["Z"], row["Ip"]] for row in plus], dtype=float
        )
        minus_rzi = np.asarray(
            [[row["R"], row["Z"], row["Ip"]] for row in minus], dtype=float
        )
        plus_coils = np.asarray(
            [row["currents_a_tsc"] for row in plus], dtype=float
        )
        minus_coils = np.asarray(
            [row["currents_a_tsc"] for row in minus], dtype=float
        )
        plus_wire = np.asarray(
            [row["wire_currents_a"] for row in plus], dtype=float
        )
        minus_wire = np.asarray(
            [row["wire_currents_a"] for row in minus], dtype=float
        )
    except (KeyError, TypeError, ValueError):
        return empty
    if (
        plus_action.shape != minus_action.shape
        or plus_action.ndim != 2
        or plus_action.shape[1] != N_COILS
        or plus_rzi.shape != minus_rzi.shape
        or plus_rzi.shape != (len(plus), 3)
        or plus_coils.shape != minus_coils.shape
        or plus_coils.shape != (len(plus), N_COILS)
        or plus_wire.shape != minus_wire.shape
        or plus_wire.shape != (len(plus), N_WIRES)
        or not all(
        ...
        return empty
    action_diff = plus_action - minus_action
    rzi_diff = plus_rzi - minus_rzi
    return {
        "same_trajectory_length": True,
        "maximum_action_abs_difference": float(
            np.max(np.abs(action_diff))
        ),
        "maximum_R_abs_difference_m": float(
            np.max(np.abs(rzi_diff[:, 0]))
        ),
        "maximum_Z_abs_difference_m": float(
            np.max(np.abs(rzi_diff[:, 1]))
        ),
        "maximum_Ip_abs_difference_A": float(
            np.max(np.abs(rzi_diff[:, 2]))
        ),
        "maximum_coil_abs_difference_A": float(
            np.max(np.abs(plus_coils - minus_coils))
        ),
        "maximum_wire_abs_difference_A": float(
            np.max(np.abs(plus_wire - minus_wire))
        ),
    }
...
def summarize_control(
    ...
    for key in sorted(group_map):
        ...
        member_result = {
            str(row["history_member"]): result_by_id.get(
                str(row["experiment_id"])
            )
            for row in members
        }
        divergence = _control_pair_divergence(
            member_result.get("plus_first"),
            member_result.get("minus_first"),
        )
        pair_rows.append(
            {
                "pair_id": key[0],
...
def analyze(
    ...
    if primary_pass:
        verdict_label = (
            "STAGE4_2R3B_AUTHENTIC_HIDDEN_HISTORY_AND_INITIAL_STATE_FINITE_ENVELOPE_PASS"
        )
    elif not state_pass:
        verdict_label = "STAGE4_2R3B_STATE_GENERATION_OR_PAIR_GATE_INCOMPLETE"
    elif control_run:
        verdict_label = "STAGE4_2R3B_AUTHENTIC_HIDDEN_HISTORY_CONTROL_FAILED"
    else:
        verdict_label = "STAGE4_2R3B_CONTROL_NOT_RUN"
    verdict = {
        "schema_version": 1,
        "stage": STAGE,
    ...
    atomic_write_json(ctx.paths.analysis / "stage4_2r3b_verdict.json", verdict)
    atomic_write_json(ctx.paths.analysis / "stage4_2r3b_summary.json", summary)
...
def execute(
    ...
    *,
    ...
    if not bool(offline_summary.get("passed")):
        raise RuntimeError(
            "Stage4.2R3b offline frozen-controller recomputation gate failed; "
    ...
    directions = _nullspace_directions(np.asarray(payload["modes_tsc"], dtype=float))
    common_prefix = _common_prefix_source(ctx)
    state_specs = _state_spec_grid(directions, ctx.cfg, common_prefix)
...
def self_test() -> dict[str, Any]:
    ...
    base = {
        ...
        "common_prefix_source_experiment_id": "synthetic-prefix",
        "common_prefix_action_digest": "synthetic-prefix-digest",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage4.2R3b authentic hidden-history and initial-state validation"
    )
    parser.add_argument("--config", type=Path)
    parser.add_argument("--source-stage4-2r2-run", type=Path)
    parser.add_argument("--calibration-stage4-2r3a-run", type=Path)
    ...
    missing = [
        ...
        for name, value in (
            ...
            (
                "--calibration-stage4-2r3a-run",
                args.calibration_stage4_2r3a_run,
            ),
