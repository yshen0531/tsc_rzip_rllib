#!/usr/bin/env python3
"""Stage4.2R3c3T13S4 exact-Card15 lattice transition holdout."""
...
N_MODES = 3
N_COILS = 14
EXPECTED_CURRENT_COMPONENTS = 26_208
...
class BlindOpenOrderGuard:
    ...
    expected_development_count: int = 26
    ...
    def before_open(self, role: str) -> None:
        if role == "development":
            if self.model_sha256 or self.holdout_open_count:
                raise ValueError("development raw opened after T13S4 model freeze")
            self.development_open_count += 1
            return
        if role != "blind_holdout":
            raise ValueError(f"unknown T13S4 raw role: {role}")
        if (
    ...
    def freeze(self, model_sha256: str) -> None:
        if self.development_open_count != self.expected_development_count:
            raise ValueError("cannot freeze T13S4 model before all development raw")
        if self.holdout_open_count:
            raise ValueError("cannot freeze T13S4 model after holdout access")
        if len(model_sha256) != 64:
            raise ValueError("T13S4 model SHA-256 must contain 64 hexadecimal digits")
        int(model_sha256, 16)
...
@dataclass(frozen=True)
class Stage42R3C3T13S4Context:
    cfg: dict[str, Any]
    base_config_path: Path
    base_ctx: t11.Stage42R3C3T11Context
    paths: Stage42R3C3T13S4Paths


def _paths(run_dir: Path) -> Stage42R3C3T13S4Paths:
    root = run_dir.expanduser().resolve()
    control = root / RUN_NAME
    return Stage42R3C3T13S4Paths(
        run_dir=root,
        control=control,
        raw=control / "raw",
        variants=root / "stage4_2r3c3t13s4_environment_variants",
        source_reference=root / "stage4_2r3c3t13s4_source_reference",
        model=root / "stage4_2r3c3t13s4_frozen_model",
        analysis=root / "stage4_2r3c3t13s4_analysis",
        manifest=root / "stage4_2r3c3t13s4_manifest.json",
        state=root / "stage4_2r3c3t13s4_state.json",
    )
...
def _validate_config(cfg: Mapping[str, Any]) -> None:
    if (
        ...
        raise ValueError("T13S4 campaign identity changed")
    expected_contexts = [
        (
    ...
    if actual_contexts != expected_contexts:
        raise ValueError("T13S4 evidence split changed")
    ...
    if actual_schedule != expected_schedule:
        raise ValueError("T13S4 causal issue schedule changed")
    if (
        tuple(map(int, probe["probe_modes"])) != MODES
        ...
        or int(probe["minimum_significant_grid_steps"]) != 4
        ...
        raise ValueError("T13S4 lattice/actuator contract changed")
    ...
    if (
        ...
        or int(matrix["expected_signed_groups"]) != 24
        ...
        raise ValueError("T13S4 matrix changed")
    ...
    if (
        ...
        raise ValueError("T13S4 formal timing changed")
    if (
        ...
        raise ValueError("T13S4 scope changed")


def load_stage42r3c3t13s4_config(
    config_path: Path,
    *,
    ...
    if (
        ...
        raise ValueError("T13S4 frozen T11 base config mismatch")
    ...
    return Stage42R3C3T13S4Context(
        cfg=cfg,
...
def _selected_pairs(ctx: Stage42R3C3T13S4Context) -> list[dict[str, Any]]:
...
def build_control_specs(
    ...
    if len(source_baselines) != 32 or len(frozen) != 4 or not set(frozen) <= set(source_baselines):
        raise ValueError("T13S4 source context coverage mismatch")
    ...
    for key, row in sorted(frozen.items()):
        ...
        if (
            ...
            raise ValueError("T13S4 source raw/snapshot identity mismatch")
        delay = int(template["action_delay_steps"])
        horizon = _formal_horizon(float(template["slew_scale"]))
        members = [(BASELINE_PROBE_ID, "baseline", -1, 0)]
        members.extend(
            (f"lattice_{window}_mode{mode}", window, mode, sign)
            for window in WINDOWS for mode in MODES for sign in SIGNS
        )
        for probe_id, window, mode, sign in members:
            ...
            spec.update({
                "kind": "stage4_2r3c3t13s4_lattice_transition_holdout",
                "stage": STAGE,
                "campaign_identity": CAMPAIGN_IDENTITY,
                "controller_revision": CONTROLLER_REVISION,
                ...
                "environment_variant": f"stage4_2r3c3t13s4_{experiment_id}",
                ...
                "r3c3t13s4_schedule_contract": "dynamic_card15_lattice_adjacent_exact_inverse_v1",
                "r3c3t13s4_offline_role": str(row["offline_role"]),
                "r3c3t13s4_stratum": str(row["stratum"]),
                "r3c3t13s4_source_r3c1_experiment_id": str(row["source_r3c1_experiment_id"]),
    ...
    if len(specs) != expected or len({str(row["experiment_id"]) for row in specs}) != expected:
        raise ValueError("T13S4 control identity coverage mismatch")
    return specs
...
_WRAPPER_SPEC_KEYS = frozenset({
    ...
    "r3c3t13s4_schedule_contract",
    "r3c3t13s4_offline_role",
    "r3c3t13s4_stratum",
    "r3c3t13s4_source_r3c1_experiment_id",
})
...
def _controller_spec(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    forbidden = {
        ...
        "r3c3t13s4_offline_role", "r3c3t13s4_stratum",
        "r3c3t13s4_source_r3c1_experiment_id",
    }
    for key in forbidden:
        output.pop(key, None)
    if forbidden.intersection(output):
        raise ValueError("T13S4 forbidden experiment label reached controller")
    return output
...
class LatticeTransitionProbeController(
    ...
    def __init__(
        ...
        self.probe_mode = int(wrapper["r3c3_probe_mode"])
        ...
        if baseline:
            ...
            valid = bool(
                self.probe_id == f"lattice_{self.probe_window}_mode{self.probe_mode}"
        ...
        if not valid:
            raise ValueError("T13S4 controller schedule contract invalid")
    ...
    def action(
        ...
        if task_step == self.issue_step:
            event = "issue"
            mode_vector = np.asarray(self.base.modes_tsc, dtype=float).reshape(
                N_COILS, N_MODES
            )[:, self.probe_mode]
            ...
            if self._signed_issue_delta_kat is None:
                raise ValueError("T13S4 cancellation has no causal issued displacement")
        ...
        if (
            ...
            raise ValueError("T13S4 selected Card15 action clipped or malformed")
        if event == "issue":
            expected_fields = (
            ...
            if list(selected.card15_fields) != list(expected_fields):
                raise ValueError("T13S4 issued action did not reproduce target fields")
        if event == "cancel" and list(selected.card15_fields) != list(lattice["target_fields"]):
            raise ValueError("T13S4 inverse action did not reproduce target fields")
        forbidden = {
        ...
        trace.update({
            ...
            "r3c3t13s4_identification_only": True,
            ...
            "r3c3t13s4_lattice_event": event,
            "r3c3t13s4_probe_issued": event in {"issue", "cancel"},
            "r3c3t13s4_baseline_action_norm_tsc": baseline_action.tolist(),
            "r3c3t13s4_center_card15_fields": list(center.card15_fields),
            "r3c3t13s4_lattice": lattice,
            "r3c3t13s4_signed_issue_delta_kAt_tsc": (
                [0.0] * N_COILS
                if self._signed_issue_delta_kat is None
                else list(self._signed_issue_delta_kat)
            ),
            "r3c3t13s4_requested_net_kAt_tsc": [0.0] * N_COILS,
            "action_norm_tsc": selected_action.tolist(),
            "r3c3t13s4_actuator_prediction": selected.to_dict(),
...
def _control_payload(
    ctx: Stage42R3C3T13S4Context, *, spec: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    payload["variant_id"] = f"stage4_2r3c3t13s4_{experiment_id}"
    payload["stage4_2r3c3t13s4_restart_snapshot_dir"] = str(
        spec["restart_snapshot_dir"]
    )
    payload["stage4_2r3c3t13s4_snapshot_manifest_digest"] = str(
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                raise ValueError("T13S4 formal horizon changed")
            self.base.env.reset()
            ...
            for step in range(horizon):
                ...
                if truncated and step + 1 < horizon:
                    raise RuntimeError("environment truncated before T13S4 horizon")
            baseline = str(spec["r3c3_probe_id"]) == BASELINE_PROBE_ID
            events = [row["r3c3t13s4_lattice_event"] for row in trace]
            ...
            result.update({
                "success": success,
                "completed": True,
                "failure_reason": "" if success else "incomplete or invalid T13S4 rollout",
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(
                    failed=failed,
                    reason="stage4_2r3c3t13s4_lattice_transition_holdout",
                )
...
def _control_ray_actor_class():
    ...
    if _CONTROL_RAY_ACTOR is None:
        ...
        class Stage42R3C3T13S4ControlActor:
            def __init__(
        ...
        _CONTROL_RAY_ACTOR = Stage42R3C3T13S4ControlActor
    return _CONTROL_RAY_ACTOR


def _package_files(ctx: Stage42R3C3T13S4Context) -> list[Path]:
    ...
    required = {
        project / "configs/stage4_2r3c3t13s4_lattice_transition_holdout_370ms.json",
        project / "tsc_rzip_rllib/diagnostics/stage4_2r3c3t13s4_lattice_transition_holdout.py",
        project / "scripts/stage4_2r3c3t13s4_lattice_transition_holdout.py",
        project / "scripts/stage4_2r3c3t13s4_server_postprocess.py",
        project / "scripts/stage4_2r3c3t13s4_shell_common.sh",
        project / "run_stage4_2r3c3t13s4_common.sh",
        project / "run_stage4_2r3c3t13s4_offline.sh",
        project / "run_stage4_2r3c3t13s4_native.sh",
        project / "run_stage4_2r3c3t13s4_nohup.sh",
        project / "run_stage4_2r3c3t13s4_self_test.sh",
        project / "run_stage4_2r3c3t13s4_server_postprocess.sh",
        project / "run_stage4_2r3c3t13s4_verify_package.sh",
        project / "run_stop_stage4_2r3c3t13s4_now.sh",
    }
    if not required <= set(paths):
        missing = sorted(str(path.relative_to(project)) for path in required - set(paths))
        raise ValueError(f"T13S4 package inventory missing files: {missing}")
    return paths
...
def _deployed_package_fingerprint(
    ...
    return {
        "contract": "r42r3c3t13s4_deployed_package_source_v1",
...
def _prepare_dirs(paths: Stage42R3C3T13S4Paths) -> None:
    for path in (
...
def _resume_compatible(old: Mapping[str, Any], new: Mapping[str, Any]) -> None:
    ...
    raise ValueError(
        "T13S4 automatic resume requires exact package identity; changed="
        + repr(changed)
    )


def prepare(
    ctx: Stage42R3C3T13S4Context, *, resume: bool
    ...
    if ctx.paths.manifest.is_file():
        if not resume:
            raise FileExistsError("T13S4 run exists; use resume or a fresh run")
        old = t11.t1.r3c3.read_json(ctx.paths.manifest)
        for key, value in manifest.items():
            if key == "deployed_package_fingerprint":
                continue
            if old.get(key) != value:
                raise ValueError(f"T13S4 resume incompatibility in {key}")
        _resume_compatible(
    ...
    t11.t1.r3c3.atomic_write_json(
        ctx.paths.run_dir / "stage4_2r3c3t13s4_config.resolved.json", ctx.cfg
    )
    t11.t1.r3c3.atomic_write_json(
    ...
    *,
    ...
    for spec in specs:
        ...
        worker = t11.t1.r1.LocalPlantReplayWorker(
            ...
            f"stage42r3c3t13s4_offline_{len(context_source):03d}", selector,
        )
        try:
            controller = LatticeTransitionProbeController(
            ...
            for step in range(int(spec["horizon_steps"])):
                ...
                finite = bool(
                    ...
                    and len(trace["r3c3t13s4_actuator_prediction"]["card15_fields"])
                    == N_COILS
                    and all(
                        len(field) == 10
                        for field in trace["r3c3t13s4_actuator_prediction"]["card15_fields"]
                    )
                ...
                if trace["r3c3t13s4_lattice_event"] != "none":
                    events.append(trace["r3c3t13s4_lattice_event"])
    ...
    summary = {
        ...
        "development_spec_count": sum(
            spec["r3c3t13s4_offline_role"] == "development" for spec in specs
        ),
        "blind_holdout_spec_count": sum(
            spec["r3c3t13s4_offline_role"] == "blind_holdout" for spec in specs
        ),
    ...
    summary["passed"] = bool(
        ...
        and summary["development_spec_count"] == 26
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            worker = LocalLatticeTransitionProbeWorker(
                ...
                f"stage42r3c3t13s4_serial_{index:03d}", selector,
                ctx.cfg["lattice_probe"],
            )
            try:
            ...
            print(f"[T13S4] {index + 1}/{len(pending)}", flush=True)
        ...
        completed = 0
        print(
            f"[T13S4] maximum_actor_count={plan.actor_count} pending={len(pending)}",
            flush=True,
        )
        for batch_start in range(0, len(pending), plan.actor_count):
            ...
            for offset, spec in enumerate(batch):
                ...
                actor = Actor.remote(
                    payloads[str(spec["experiment_id"])], library, bundle,
                    f"stage42r3c3t13s4_{index:03d}", selector,
                    ctx.cfg["lattice_probe"],
                )
                actors.append(actor)
            ...
            try:
                while refs:
                    ...
                    if not ready:
                        print(f"[T13S4] waiting {completed}/{len(pending)}", flush=True)
                        continue
                    for ref in ready:
                        spec = refs.pop(ref)
                        try:
                        ...
                        if completed % 4 == 0 or not refs:
                            print(f"[T13S4] {completed}/{len(pending)}", flush=True)
            finally:
...
def _effect_response(
    ...
    if cancel != first + 1:
        raise ValueError("T13S4 effect states are not adjacent")
    ...
    return {
        ...
        "input_modal_current_A": modal.reshape(-1),
        "off_mode_current_A": (
...
def _issue_trace(result: Mapping[str, Any]) -> Mapping[str, Any]:
    rows = [
        ...
        if row.get("r3c3t13s4_lattice_event") == "issue"
    ]
    if len(rows) != 1:
        raise ValueError("T13S4 signed result must contain one issue trace")
    return rows[0]
...
def _actuator_execution(
    ...
    for step, row in enumerate(trace):
        actuator = row["r3c3t13s4_actuator_prediction"]
...
def _expected_context_row(
    ...
    if len(rows) != 1:
        raise ValueError("T13S4 frozen context lookup mismatch")
    return rows[0]
...
def _execution_rows(
    ...
    for result in results:
        ...
        expected_events = [] if baseline else ["issue", "cancel"]
        actual_events = [
            row.get("r3c3t13s4_lattice_event") for row in trace
            if row.get("r3c3t13s4_lattice_event") != "none"
        ]
        trace_pass = bool(
...
def _group_results(
    ...
    for result in results:
        ...
        name = (
            ...
            else f"{spec['r3c3_probe_window']}:{spec['r3c3_probe_mode']}:{spec['r3c3_probe_sign']}"
        )
        members = grouped.setdefault(_context_key(spec), {})
        if name in members:
            raise ValueError("duplicate T13S4 context member")
        members[name] = result
    return grouped
...
def _signed_group_rows(
    ...
    for context_key, members in sorted(grouped.items()):
        if BASELINE_PROBE_ID not in members:
            raise ValueError("T13S4 context baseline missing")
        ...
        for window in WINDOWS:
            for mode in MODES:
                signed = {
                    sign: members.get(f"{window}:{mode}:{sign}") for sign in SIGNS
                }
                row: dict[str, Any] = {
                    ...
                    "offline_role": str(baseline["spec"]["r3c3t13s4_offline_role"]),
                    "stratum": str(baseline["spec"]["r3c3t13s4_stratum"]),
                    "probe_window": window,
                    "probe_mode": mode,
                ...
                if (
                    ...
                    plus_fields = plus_issue["r3c3t13s4_actuator_prediction"]["card15_fields"]
                    minus_fields = minus_issue["r3c3t13s4_actuator_prediction"]["card15_fields"]
                    plus_center = plus_issue["r3c3t13s4_center_card15_fields"]
                    minus_center = minus_issue["r3c3t13s4_center_card15_fields"]
                    ...
                    odd_input = (
                        response[1]["input_modal_current_A"]
                        - response[-1]["input_modal_current_A"]
                    ) / 2.0
                    odd_output = (
                    ...
                    row.update({
                        ...
                        "odd_input_modal_current_A": odd_input.tolist(),
                        "odd_response_unscaled": odd_output.tolist(),
                        "signed_inputs_modal_current_A": {
                            str(sign): response[sign]["input_modal_current_A"].tolist()
                            for sign in SIGNS
                        },
...
def _build_model_artifact(
    ...
    for row in signed_rows:
        if row["offline_role"] != "development":
            raise ValueError("holdout row reached T13S4 model builder")
        by_key.setdefault((str(row["stratum"]), str(row["probe_window"])), []).append(row)
    for key, rows in sorted(by_key.items()):
        rows = sorted(rows, key=lambda row: int(row["probe_mode"]))
        if len(rows) != 3 or [int(row["probe_mode"]) for row in rows] != [0, 1, 2]:
            raise ValueError("T13S4 development mode coverage mismatch")
        x = np.asarray([row["odd_input_modal_current_A"] for row in rows], dtype=float)
        ...
        for row in rows:
            for sign in SIGNS:
                xi = np.asarray(row["signed_inputs_modal_current_A"][str(sign)], dtype=float)
        ...
        entry = {
            ...
            "input_definition": "two_effect_states_x_three_measured_modal_current_A",
    ...
    if len(entries) != 4:
        raise ValueError("T13S4 model stratum/window coverage mismatch")
    ...
    core = {
        ...
        "source_hashes": {
            "config_sha256": _sha256(
                _project_root() / "configs/stage4_2r3c3t13s4_lattice_transition_holdout_370ms.json"
            ),
...
def _freeze_model(
    ctx: Stage42R3C3T13S4Context, artifact: Mapping[str, Any]
) -> dict[str, Any]:
    path = ctx.paths.model / "stage4_2r3c3t13s4_local_model_tube.json"
    if path.is_file():
        existing = t11.t1.r3c3.read_json(path)
        if existing != dict(artifact):
            raise ValueError("existing frozen T13S4 model differs from development fit")
    else:
        t11.t1.r3c3.atomic_write_json(path, artifact)
    record_path = ctx.paths.model / "stage4_2r3c3t13s4_model_freeze.json"
    record = {
        "schema_version": SCHEMA_VERSION,
        "stage": STAGE,
        "model_path": str(path),
    ...
    if record_path.is_file():
        ...
        if comparable != old_comparable:
            raise ValueError("existing T13S4 model freeze record mismatch")
        record = old
    else:
    ...
    for group in holdout_signed_rows:
        ...
        for sign in SIGNS:
            ...
            model_input = np.asarray(
                group["signed_inputs_modal_current_A"][str(sign)], dtype=float
            )
...
def summarize_from_raw(
    ...
    *,
    ...
    development_specs = [
        spec for spec in specs
        if spec["r3c3t13s4_offline_role"] == "development"
    ]
    holdout_specs = [
        spec for spec in specs
        if spec["r3c3t13s4_offline_role"] == "blind_holdout"
    ]
    if len(development_specs) != len(holdout_specs) != 26:
        raise ValueError("unreachable T13S4 split")
    if len(development_specs) != 26 or len(holdout_specs) != 26:
        raise ValueError("T13S4 development/holdout split mismatch")
    open_events: list[dict[str, Any]] = []
    guard = BlindOpenOrderGuard(expected_development_count=26)
    ...
    def open_one(role: str, spec: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not _result_complete(path, spec):
            raise ValueError(f"incomplete or incompatible T13S4 raw: {path}")
        open_events.append({
    ...
    pre_holdout_order = {
        ...
        "passed": len(open_events) == 26,
    }
    order_path = ctx.paths.model / "stage4_2r3c3t13s4_input_open_order.json"
    ...
    order = {
        **pre_holdout_order,
        ...
        "development_first_exact": all(
            row["offline_role"] == "development" for row in open_events[:26]
        ) and all(
            row["offline_role"] == "blind_holdout" for row in open_events[26:]
        ),
        "holdout_first_open_ordinal": 27,
    ...
    summary["execution_gate_passed"] = bool(
        len(execution_rows) == summary["execution_pass_count"] == 52
    ...
    summary["actuator_signed_group_gate_passed"] = bool(
        len(all_signed) == 24
        and summary["target_field_central_symmetry_exact_count"] == 24
        and summary["observed_current_signal_pass_count"] == 24
        and summary["observed_current_symmetry_pass_count"] == 24
    )
    summary["pre_effect_causality_gate_passed"] = bool(
        len(all_signed) == summary["pre_effect_causality_pass_count"] == 24
    )
    summary["development_model_gate_passed"] = bool(
        len(development_group_rows) == summary["development_signal_pass_count"] == 12
...
def analyze(
    ctx: Stage42R3C3T13S4Context, summary: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    t11.t1.r3c3.atomic_write_json(
        ctx.paths.analysis / "stage4_2r3c3t13s4_summary.json", dict(summary)
    )
    t11.t1.r3c3.atomic_write_json(
        ctx.paths.analysis / "stage4_2r3c3t13s4_verdict.json", verdict
    )
...
def execute(
    ...
    *,
    ...
    if int(execution["complete_after_run"]) != len(specs):
        raise RuntimeError("T13S4 raw set incomplete; resume safely after runtime repair")
...
def self_test() -> dict[str, Any]:
    ...
    mode = np.ones(N_COILS, dtype=float) / math.sqrt(N_COILS)
    plan = choose_lattice_displacement(
    ...
    passed = bool(
        plan["passed"] and inverse["passed"]
        and plan["target_field_central_symmetry_exact"]
        and plan["minimum_significant_grid_steps_actual"] >= 4
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage4.2R3c3T13S4 lattice transition holdout"
    )
    ...
    ctx = load_stage42r3c3t13s4_config(
        args.config,
