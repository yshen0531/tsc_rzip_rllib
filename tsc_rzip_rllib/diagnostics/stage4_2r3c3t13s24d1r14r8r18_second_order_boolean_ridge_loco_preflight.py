#!/usr/bin/env python3
"""Run the frozen zero-TSC R8R18 second-order Boolean ridge LOCO audit."""
...
def penalty_matrix(cfg: Mapping[str, Any]) -> np.ndarray:
    ...
    if diagonal.shape != (11,) or not np.all(np.isfinite(diagonal)):
        raise ValueError("R8R18 ridge penalty changed")
...
def validate_config(cfg: Mapping[str, Any], *, project_root: Path) -> None:
    ...
    if (
        ...
        or tuple(model["feature_order"]) != ("intercept","q10","q14","q18","q22","q10_q14","q10_q18","q10_q22","q14_q18","q14_q22","q18_q22")
        or float(model["pair_ridge_lambda"]) != 10.0
        or tuple(map(float, model["penalty_diagonal"])) != (0,0,0,0,0,10,10,10,10,10,10)
        ...
        or int(model["measured_rank"]) != 9 or int(model["cube_rank"]) != 11
        or (float(model["maximum_cube_condition"]), int(model["minimum_loco_affine_rank"]), int(model["minimum_loco_augmented_rank"])) != (1.01,5,11)
        or (float(model["maximum_loco_normal_condition"]), float(model["maximum_loco_weight_l2"]), float(model["maximum_loco_weight_absolute"])) != (32.0,1.30,1.01)
        or (float(model["maximum_full_normal_condition"]), float(model["maximum_missing_weight_l2"]), float(model["maximum_missing_weight_absolute"])) != (11.10,1.30,0.67)
        ...
        or any(bool(scope[key]) for key in ("real_tsc_executed","controller_executed","physical_authority_validated","real_mpc_executed","gate_a_qualified","all_source_trajectories_allowed_in_expert_dataset","expert_data_allowed","bc_dagger_or_rl_allowed","global_plant_reachability_claimed","r8r16_or_r8r17_predictions_used_as_model_input"))
        or int(scope["new_raw_count"]) != 0
    ):
        raise ValueError("R8R18 frozen design changed")
    stability = _stability(cfg, measured, missing)
    if (
        stability["measured_rank"] != 9 or stability["cube_rank"] != 11
        or stability["cube_condition"] > float(model["maximum_cube_condition"])
        or any(row["affine_rank"] < 5 or row["augmented_rank"] < 11 for row in stability["fold_rows"])
        ...
        raise ValueError("R8R18 frozen matrix stability changed")
...
def _authenticate_r8r16(ctx: Context) -> dict[str, Any]:
    ...
    if any(hashes[name] != str(expected[f"{name}_sha256"]) for name in hashes):
        raise ValueError("R8R18 source R8R16 hash changed")
...
def _authenticate_r8r17(ctx: Context) -> dict[str, Any]:
    ...
    hashes = {name: _sha(path) for name, path in paths.items()}
    if any(hashes[name] != str(expected[f"{name}_sha256"]) for name in hashes):
        raise ValueError("R8R18 source R8R17 hash changed")
    summary = _read(paths["primary_summary"])
    independent = _read(paths["independent"])
    final = _read(paths["final_report"])
    state = _read(paths["stage_state"])
...
def _authenticate_source(ctx: Context) -> dict[str, Any]:
    ...
    for name, digest in hashes.items():
        if digest != str(expected[f"{name}_sha256"]):
            raise ValueError(f"R8R18 source R8R15 {name} changed")
    ...
    for phase in ("safety", "qualification"):
        ...
        if inventories[phase] != {
            ...
            raise ValueError(f"R8R18 source R8R15 {phase} inventory changed")
    if (
        ...
        raise ValueError("R8R18 source R8R15 outcome changed")
...
def _collect(ctx: Context) -> tuple[list[dict[str, Any]], dict[tuple[str, str], dict[str, Any]]]:
    ...
    if len(groups) != 16 or any(set(group["candidates"]) != expected_codes for group in groups.values()):
        raise ValueError("R8R18 measured context/code coverage changed")
...
def _response(result: Mapping[str, Any], baseline: Mapping[str, Any], first: int) -> np.ndarray:
    ...
    if len(trajectory) != len(center) or len(trajectory) <= first:
        raise ValueError("R8R18 source trajectory horizon changed")
    ...
    if not np.all(np.isfinite(values)) or not np.all(np.isfinite(base)):
        raise ValueError("R8R18 source trajectory is non-finite")
...
def fit_svd(
    ...
    *,
    ...
    if y.ndim != 3 or y.shape[0] != len(codes) or not np.all(np.isfinite(y)):
        raise ValueError("R8R18 response fit shape changed")
    root_penalty = np.sqrt(penalty)
    augmented_x = np.vstack((x, root_penalty))
    augmented_y = np.concatenate((y.reshape(len(codes), -1), np.zeros((11, y.shape[1] * y.shape[2]))), axis=0)
    ...
    return coefficients.reshape(11, y.shape[1], y.shape[2])
...
def predict(coefficients: np.ndarray, code: str) -> np.ndarray:
    ...
    if output.ndim != 2 or output.shape[1] != 3 or not np.all(np.isfinite(output)):
        raise ValueError("R8R18 predicted response is invalid")
    return output
...
def _predicted_result(baseline: Mapping[str, Any], response: np.ndarray, first: int) -> dict[str, Any]:
    ...
    if response.shape != (len(trajectory) - first, 3):
        raise ValueError("R8R18 predicted horizon changed")
...
def run_primary(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage.exists() and any(ctx.paths.stage.iterdir()):
        raise ValueError("R8R18 output directory is not empty")
...
def postprocess(ctx: Context) -> dict[str, Any]:
    ...
    if independent.get("passed") is not True or independent.get("primary_agreement") is not True:
        raise ValueError("R8R18 independent agreement failed")
    final = {
    ...
    state.update({
        ...
        "stop_reason": (
            ...
            else "second_order_boolean_ridge_model_gate_failed" if not primary["model_gate_passed"]
