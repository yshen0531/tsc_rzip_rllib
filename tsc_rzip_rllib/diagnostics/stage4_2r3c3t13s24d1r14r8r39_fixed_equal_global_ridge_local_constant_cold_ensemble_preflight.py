"""R8R39 fixed equal global-ridge/local-constant cold-ensemble preflight."""
...
SCHEMA_VERSION = 1
STAGE = "Stage4.2R3c3T13S24D1R14R8R39"
IDENTITY = "fixed_equal_global_ridge_local_constant_cold_ensemble_preflight_v1"
...
_write = r8r37._write
_sha = r8r37._sha
_digest = r8r37._digest
_jsonable = r8r37._jsonable
...
def validate_config(cfg: Mapping[str, Any], *, project_root: Path) -> None:
    ...
    r37_config = (root / str(cfg["source_r8r37_config"])).resolve()
    bank = cfg["bank_contract"]
    model = cfg["model_contract"]
    gates = cfg["model_gates"]
    scope = cfg["scientific_scope"]
    routes = {
        "execution_fail": "FIXED_EQUAL_GLOBAL_LOCAL_COLD_ENSEMBLE_PREFLIGHT_EXECUTION_FAIL_STOP",
        "model_fail": "FIXED_EQUAL_GLOBAL_LOCAL_COLD_ENSEMBLE_MODEL_FAIL_NO_TSC",
        "pass": "FIXED_EQUAL_GLOBAL_LOCAL_COLD_ENSEMBLE_PASS_CONTROLLER_PREFLIGHT_DESIGN_REQUIRED",
    }
    invalid = (
        ...
        or not r37_config.is_file()
        or _sha(r37_config) != cfg.get("source_r8r37_config_sha256")
        ...
        or int(model.get("local_slope_dimension", -1)) != 0
    ...
    if invalid:
        raise ValueError("R8R39 frozen config changed")
...
def load_context(args: argparse.Namespace) -> Context:
    ...
    source_args.config = (root / str(cfg["source_r8r37_config"])).resolve()
    source_args.run_dir = args.r8r37_run.expanduser().resolve()
    r8r37_ctx = r8r37.load_context(source_args)
    source_ctx = r8r37_ctx.source_ctx
    if (
        ...
        raise ValueError("R8R39 R8R31 source context changed")
    return Context(
        cfg=cfg,
        config_path=config_path,
        paths=_paths(args.run_dir),
        r8r37_ctx=r8r37_ctx,
        r8r37_stage=r8r37_ctx.paths.stage,
...
def _authenticate_final_source(
    ...
    if any(hashes[name] != contract[f"{name}_sha256"] for name in hashes):
        raise ValueError(f"R8R39 final {label} source hash changed")
    ...
    if (
        ...
        raise ValueError(f"R8R39 final {label} source state changed")
...
def authenticate_sources(ctx: Context) -> dict[str, Any]:
    ...
    r37 = _authenticate_final_source(
        ctx.r8r37_stage, ctx.cfg["source_r8r37"], label="R8R37"
    )
    transitive = r8r37.authenticate_sources(ctx.r8r37_ctx)
    return {"r8r31_final": r31, "r8r37_final": r37, "transitive": transitive, "passed": True}
...
def _verify_bank(
    ...
    if (
        ...
        raise ValueError("R8R39 bank reproduction changed")
...
def fit_model(
    ...
    local_model = r8r35.fit_model(trajectories, selected, _local_cfg(cfg))
    local_model["model_kind"] = "causal_local_constant_k64_cold"
    return {
        "model_kind": "fixed_equal_global_ridge_local_constant_cold_ensemble",
...
def predict(
    ...
    local_prediction, neighbors = r8r35.predict_base(
        model["local_model"], row, _local_cfg(cfg)
    )
    ...
    if ensemble.shape != global_prediction.shape or not np.all(np.isfinite(ensemble)):
        raise ValueError("R8R39 ensemble prediction invalid")
...
def schedule_jackknife(
    ...
    for held_schedule in schedules:
        ...
        if len(held) != 16:
            raise ValueError("R8R39 schedule fold cardinality changed")
        groups, residual_evidence, predictions = _residuals(model, held, cfg)
        evidence = model_evidence(model)
        folds.append(
            {
...
def compute_from_bank(
    ...
    if not cardinality["passed"]:
        raise ValueError("R8R39 frozen local-neighbor cardinality failed")
...
def run_primary(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage.exists():
        raise ValueError("R8R39 primary requires a fresh stage directory")
...
def run_finalize(ctx: Context) -> dict[str, Any]:
    ...
    if any(not path.is_file() for path in (*paths.values(), ctx.paths.manifest, ctx.paths.state)):
        raise ValueError("R8R39 finalization evidence incomplete")
    if independent_path.is_file() == failure_path.is_file():
        raise ValueError("R8R39 finalization evidence ambiguous")
    summary = _read(paths["primary_summary"])
    ...
    if independent_path.is_file() and not agreement:
        raise ValueError("R8R39 finalization independent disagreement")
    final_route = summary["route"] if agreement else ctx.cfg["routes"]["execution_fail"]
    compact = {
        "schema_version": SCHEMA_VERSION,
        "stage": STAGE,
        "identity": IDENTITY,
        "audit_kind": "r8r39_compact_primary_independent_finalization" if agreement else "r8r39_compact_independent_failure_finalization",
        "passed": agreement,
    ...
    final["audit_kind"] = "r8r39_final_report"
...
ARGUMENT_NAMES = (
    ...
    "run-dir",
    "r8r37-run",
) + tuple(name for name in r8r37.ARGUMENT_NAMES if name not in {"config", "run-dir"})
