#!/usr/bin/env python3
"""Run the frozen zero-TSC R8R19 saturated Boolean kernel LOCO audit."""
...
SCHEMA_VERSION = 1
STAGE = "Stage4.2R3c3T13S24D1R14R8R19"
IDENTITY = "saturated_boolean_kernel_loco_preflight_v1"
RUN_NAME = "stage4_2r3c3t13s24d1r14r8r19_saturated_boolean_kernel_loco_preflight"
COMPONENTS = ("R", "Z", "Ip")
...
def code_row(code: str) -> np.ndarray:
    if len(code) != 4 or any(symbol not in "UV" for symbol in code):
        raise ValueError(f"invalid R8R19 code: {code}")
    q = np.asarray([1.0 if symbol == "U" else -1.0 for symbol in code])
    pairs = [q[i] * q[j] for i in range(4) for j in range(i + 1, 4)]
    triples = [q[i] * q[j] * q[k] for i in range(4) for j in range(i + 1, 4) for k in range(j + 1, 4)]
    return np.asarray([1.0, *q, *pairs, *triples, float(np.prod(q))])
...
def penalty_matrix(cfg: Mapping[str, Any]) -> np.ndarray:
    diagonal = np.asarray(cfg["model_contract"]["penalty_diagonal"], dtype=float)
    if diagonal.shape != (16,) or not np.all(np.isfinite(diagonal)):
        raise ValueError("R8R19 ridge penalty changed")
...
def _stability(cfg: Mapping[str, Any], measured: Sequence[str], missing: Sequence[str]) -> dict[str, Any]:
    p = penalty_matrix(cfg)
    x = design_matrix(measured)
    fold_rows = []
    for held_index, held_code in enumerate(measured):
        train = np.delete(x, held_index, axis=0)
        normal = train.T @ train + p
        weights = code_row(held_code) @ np.linalg.inv(normal) @ train.T
        augmented = np.vstack((train, np.sqrt(p)))
        fold_rows.append({
            "held_code": held_code,
    ...
    missing_rows = []
    for code in missing:
        weights = code_row(code) @ np.linalg.inv(normal) @ x.T
        missing_rows.append({
            "code": code,
            "weight_l2": float(np.linalg.norm(weights)),
            "weight_absolute": float(np.max(np.abs(weights))),
        })
    cube = design_matrix(_cube_codes())
    return {
        "measured_rank": int(np.linalg.matrix_rank(x)),
...
def validate_config(cfg: Mapping[str, Any], *, project_root: Path) -> None:
    ...
    measured = tuple(cfg["code_contract"]["measured_codes"])
    ...
    if (
        ...
        or cfg.get("stage") != STAGE or cfg.get("identity") != IDENTITY or cfg.get("run_name") != RUN_NAME
        or cfg.get("package_revision") != "r42r3c3t13s24d1r14r8r19_saturated_boolean_kernel_loco_v1"
        or not design.is_file() or _sha(design) != str(cfg["design_document_sha256"])
        or not source_config.is_file() or _sha(source_config) != str(cfg["source_r8r15_config_sha256"])
        or measured != (*development, *calibration)
        or development != ("UUUU", "VVVV", "UVVV", "UUVV", "UUUV", "VUUU")
        or calibration != ("VVUU", "VVVU", "UVUV", "VUVU")
        or missing != ("UVUU", "UUVU", "UVVU", "VUUV", "VVUV", "VUVV")
        or set(measured) | set(missing) != set(_cube_codes()) or set(measured) & set(missing)
        or int(cfg["code_contract"]["loco_fold_count"]) != 10
        or int(cfg["code_contract"]["loco_trajectory_count"]) != 160
        or (int(cfg["code_contract"]["measured_code_count"]), int(cfg["code_contract"]["missing_code_count"]), int(cfg["code_contract"]["context_count"]), int(cfg["code_contract"]["calibration_trajectory_count"]), int(cfg["code_contract"]["missing_prediction_count"])) != (10,6,16,160,96)
        or model["family"] != "per_context_saturated_boolean_walsh_kernel_ridge"
        or tuple(model["feature_order"]) != ("intercept","q10","q14","q18","q22","q10_q14","q10_q18","q10_q22","q14_q18","q14_q22","q18_q22","q10_q14_q18","q10_q14_q22","q10_q18_q22","q14_q18_q22","q10_q14_q18_q22")
        or float(model["nonlinear_ridge_lambda"]) != 10.0
        or tuple(map(float, model["penalty_diagonal"])) != (0,0,0,0,0,10,10,10,10,10,10,10,10,10,10,10)
        or model["primary_solver"] != "augmented_svd" or model["independent_solver"] != "normal_equation_direct_solve"
        or float(model["svd_rcond"]) != 1e-12 or int(model["first_state_index"]) != 10
        or tuple(model["response_components"]) != COMPONENTS or not bool(model["response_relative_to_matched_baseline"])
        or int(model["measured_rank"]) != 10 or int(model["cube_rank"]) != 16
        or (float(model["maximum_cube_condition"]), int(model["minimum_loco_affine_rank"]), int(model["minimum_loco_augmented_rank"])) != (1.01,5,16)
        or (float(model["maximum_loco_normal_condition"]), float(model["maximum_loco_weight_l2"]), float(model["maximum_loco_weight_absolute"])) != (64.0,1.25,1.01)
        or (float(model["maximum_full_normal_condition"]), float(model["maximum_missing_weight_l2"]), float(model["maximum_missing_weight_absolute"])) != (28.0,1.30,0.61)
        or bool(model["per_context_hyperparameters_allowed"]) or bool(model["outcome_selected_lambda_allowed"])
        ...
        or (int(gates["required_trajectory_pass_count"]), int(gates["required_formal_classification_count"])) != (160,160)
        or float(gates["maximum_minimum_margin_absolute_error"]) != 0.05
        or float(gates["tube_multiplier"]) != 2.0 or tuple(map(float,gates["tube_caps"])) != (0.01,0.01,3000.0)
        ...
        or not bool(authority["require_oracle_strictly_improves_baseline"]) or not bool(authority["robust_margin_strictly_positive"])
        ...
        or routes != {
            "source_fail": "SATURATED_BOOLEAN_KERNEL_SOURCE_OR_INTEGRITY_FAIL_NO_TSC",
            "model_fail": "SATURATED_BOOLEAN_KERNEL_MODEL_INADEQUATE_DIRECT_CUBE_COMPLETION_REDESIGN_REQUIRED",
            "authority_fail": "SATURATED_BOOLEAN_KERNEL_AUTHORITY_INSUFFICIENT_CONTINUOUS_MULTIDIRECTION_REDESIGN_REQUIRED",
            "pass": "SATURATED_BOOLEAN_KERNEL_COMPLETION_PASS_FRESH_MISSING_SEQUENCE_SENTINEL_REQUIRED",
        }
        or any(bool(scope[key]) for key in ("real_tsc_executed","controller_executed","physical_authority_validated","real_mpc_executed","gate_a_qualified","all_source_trajectories_allowed_in_expert_dataset","expert_data_allowed","bc_dagger_or_rl_allowed","global_plant_reachability_claimed","prior_stage_predictions_used_as_model_input"))
        or int(scope["new_raw_count"]) != 0
    ):
        raise ValueError("R8R19 frozen design changed")
    stability = _stability(cfg, measured, missing)
    if (
        stability["measured_rank"] != 10 or stability["cube_rank"] != 16
        or stability["cube_condition"] > float(model["maximum_cube_condition"])
        or any(row["raw_rank"] != 9 or row["affine_rank"] < 5 or row["augmented_rank"] < 16 for row in stability["fold_rows"])
        or stability["maximum_loco_normal_condition"] > float(model["maximum_loco_normal_condition"])
        or stability["maximum_loco_weight_l2"] > float(model["maximum_loco_weight_l2"])
        or stability["maximum_loco_weight_absolute"] > float(model["maximum_loco_weight_absolute"])
        or stability["full_normal_condition"] > float(model["maximum_full_normal_condition"])
        or stability["maximum_missing_weight_l2"] > float(model["maximum_missing_weight_l2"])
        or stability["maximum_missing_weight_absolute"] > float(model["maximum_missing_weight_absolute"])
    ):
        raise ValueError("R8R19 frozen matrix stability changed")
...
def load_context(args: argparse.Namespace) -> Context:
    ...
    return Context(
        ...
        r8r17_run=args.r8r17_run.expanduser().resolve(),
        r8r18_run=args.r8r18_run.expanduser().resolve(),
    )
...
def _authenticate_r8r16(ctx: Context) -> dict[str, Any]:
    ...
    if any(hashes[name] != str(expected[f"{name}_sha256"]) for name in hashes):
        raise ValueError("R8R19 source R8R16 hash changed")
...
def _authenticate_r8r17(ctx: Context) -> dict[str, Any]:
    expected = ctx.cfg["source_r8r17"]
    stage = ctx.r8r17_run / str(expected["stage_directory"])
    paths = {
        "primary_detailed": stage / "analysis/primary_detailed.json",
        "primary_summary": stage / "analysis/primary_summary.json",
        "independent": stage / "analysis/independent.json",
    ...
    hashes = {name: _sha(path) for name, path in paths.items()}
    if any(hashes[name] != str(expected[f"{name}_sha256"]) for name in hashes):
        raise ValueError("R8R19 source R8R17 hash changed")
    summary = _read(paths["primary_summary"])
    independent = _read(paths["independent"])
    final = _read(paths["final_report"])
    state = _read(paths["stage_state"])
    passed = bool(
        ...
        and summary.get("calibration_trajectory_count") == int(expected["calibration_trajectory_count"])
        and summary.get("calibration_pass_count") == int(expected["calibration_pass_count"])
        and summary.get("real_tsc_executed") is False and summary.get("new_raw_count") == 0
        and independent.get("passed") is True and independent.get("primary_agreement") is True
        and final.get("route") == expected["required_route"]
        and state.get("finished") is True and state.get("verdict", {}).get("route") == expected["required_route"]
    )
    result = {"stage": str(stage), "hashes": hashes, "route": final["route"], "passed": passed}
    _write(ctx.paths.source_reference / "r8r17_authentication.json", result)
    return result


def _authenticate_r8r18(ctx: Context) -> dict[str, Any]:
    expected = ctx.cfg["source_r8r18"]
    stage = ctx.r8r18_run / str(expected["stage_directory"])
    paths = {
        "primary_detailed": stage / "analysis/primary_detailed.json",
        "primary_summary": stage / "analysis/primary_summary.json",
        "independent": stage / "analysis/independent.json",
        ...
        "r8r15_authentication": stage / "source_reference/r8r15_authentication.json",
        "r8r16_authentication": stage / "source_reference/r8r16_authentication.json",
        "r8r17_authentication": stage / "source_reference/r8r17_authentication.json",
        "stage_manifest": stage / "stage_manifest.json",
        "stage_state": stage / "stage_state.json",
    ...
    hashes = {name: _sha(path) for name, path in paths.items()}
    if any(hashes[name] != str(expected[f"{name}_sha256"]) for name in hashes):
        raise ValueError("R8R19 source R8R18 hash changed")
    summary = _read(paths["primary_summary"])
    independent = _read(paths["independent"])
    final = _read(paths["final_report"])
    state = _read(paths["stage_state"])
    ...
    passed = bool(
        ...
        and summary.get("calibration_trajectory_count") == int(expected["calibration_trajectory_count"])
        and summary.get("calibration_pass_count") == int(expected["calibration_pass_count"])
        and summary.get("calibration_formal_classification_count") == int(expected["calibration_formal_classification_count"])
        and summary.get("real_tsc_executed") is False and summary.get("new_raw_count") == 0
        and independent.get("passed") is True and independent.get("primary_agreement") is True
        and final.get("route") == expected["required_route"]
        and state.get("finished") is True and state.get("verdict", {}).get("route") == expected["required_route"]
        and evidence.get("route") == expected["required_route"] and evidence.get("failed_loco_row_count") == 3
    )
    result = {"stage": str(stage), "hashes": hashes, "route": final["route"], "passed": passed}
    _write(ctx.paths.source_reference / "r8r18_authentication.json", result)
    return result
...
def _authenticate_source(ctx: Context) -> dict[str, Any]:
    ...
    for name, digest in hashes.items():
        if digest != str(expected[f"{name}_sha256"]):
            raise ValueError(f"R8R19 source R8R15 {name} changed")
    ...
    for phase in ("safety", "qualification"):
        ...
        if inventories[phase] != {
            ...
            raise ValueError(f"R8R19 source R8R15 {phase} inventory changed")
    if (
        ...
        raise ValueError("R8R19 source R8R15 outcome changed")
...
def _collect(ctx: Context) -> tuple[list[dict[str, Any]], dict[tuple[str, str], dict[str, Any]]]:
    ...
    if len(groups) != 16 or any(set(group["candidates"]) != expected_codes for group in groups.values()):
        raise ValueError("R8R19 measured context/code coverage changed")
...
def _response(result: Mapping[str, Any], baseline: Mapping[str, Any], first: int) -> np.ndarray:
    ...
    if len(trajectory) != len(center) or len(trajectory) <= first:
        raise ValueError("R8R19 source trajectory horizon changed")
    ...
    if not np.all(np.isfinite(values)) or not np.all(np.isfinite(base)):
        raise ValueError("R8R19 source trajectory is non-finite")
...
def fit_svd(
    ...
    *,
    ...
    if y.ndim != 3 or y.shape[0] != len(codes) or not np.all(np.isfinite(y)):
        raise ValueError("R8R19 response fit shape changed")
    root_penalty = np.sqrt(penalty)
    augmented_x = np.vstack((x, root_penalty))
    augmented_y = np.concatenate((y.reshape(len(codes), -1), np.zeros((16, y.shape[1] * y.shape[2]))), axis=0)
    u, singular, vt = np.linalg.svd(augmented_x, full_matrices=False)
    cutoff = rcond * singular[0]
    inverse = np.asarray([1.0 / value if value > cutoff else 0.0 for value in singular])
    coefficients = (vt.T * inverse) @ u.T @ augmented_y
    return coefficients.reshape(16, y.shape[1], y.shape[2])
...
def predict(coefficients: np.ndarray, code: str) -> np.ndarray:
    ...
    if output.ndim != 2 or output.shape[1] != 3 or not np.all(np.isfinite(output)):
        raise ValueError("R8R19 predicted response is invalid")
    return output
...
def _predicted_result(baseline: Mapping[str, Any], response: np.ndarray, first: int) -> dict[str, Any]:
    ...
    if response.shape != (len(trajectory) - first, 3):
        raise ValueError("R8R19 predicted horizon changed")
...
def compute(ctx: Context) -> dict[str, Any]:
    ...
    r8r17_source = _authenticate_r8r17(ctx)
    r8r18_source = _authenticate_r8r18(ctx)
    ...
    stability = _stability(ctx.cfg, measured, missing)
    ...
    for key in sorted(groups):
        ...
        context_models[key] = {"responses": responses}
        evaluator = evaluator_by_key[key]
        for held_index, held_code in enumerate(measured):
            train_codes = tuple(code for index, code in enumerate(measured) if index != held_index)
            coefficients = fit_svd(train_codes, [responses[code] for code in train_codes], penalty=penalty, rcond=rcond)
            predicted = predict(coefficients, held_code)
            residual = predicted - responses[held_code]
            absolute = np.abs(residual)
            component_max = np.max(absolute, axis=0)
            scaled_max = float(np.max(absolute / scales))
            actual_formal = _formal(evaluator, group["candidates"][held_code])
            ...
            calibration_rows.append({
                "pair_id": key[0], "history_member": key[1], "held_code": held_code,
    ...
    source_passed = bool(source["passed"] and r8r16_source["passed"] and r8r17_source["passed"] and r8r18_source["passed"])
    model_gate = bool(
        source_passed
    ...
    baseline_pass_count = robust_repairs = robust_oracle_pass = 0
    if model_gate:
        for key in sorted(groups):
            group = groups[key]
            responses = context_models[key]["responses"]
            coefficients = fit_svd(measured, [responses[code] for code in measured], penalty=penalty, rcond=rcond)
            evaluator = evaluator_by_key[key]
            ...
            for code in missing:
                ...
                robust_lower = float(formal["formal_minimum_signed_margin"]) - margin_buffer
                eligible = bool(formal["formal_contract_pass"] and robust_lower > 0.0)
                row = {"pair_id": key[0], "history_member": key[1], "sequence_code": code, "predicted_formal": formal, "robust_lower_margin": robust_lower, "robust_eligible": eligible}
                missing_rows.append(row)
                candidates.append(row)
            eligible_rows = [row for row in candidates if row["robust_eligible"]]
            best = max(eligible_rows, key=lambda row: (row["robust_lower_margin"], -missing.index(row["sequence_code"]))) if eligible_rows else None
            repaired = bool(not baseline_formal["formal_contract_pass"] and best is not None)
            robust_repairs += int(repaired)
            robust_oracle_pass += int(baseline_formal["formal_contract_pass"] or best is not None)
            context_rows.append({"pair_id": key[0], "history_member": key[1], "baseline_formal": baseline_formal, "selected_code": None if best is None else best["sequence_code"], "repaired": repaired})
    ...
    route = (
        ctx.cfg["routes"]["source_fail"] if not source_passed
        else ctx.cfg["routes"]["model_fail"] if not model_gate
        else ctx.cfg["routes"]["pass"] if authority_gate
        else ctx.cfg["routes"]["authority_fail"]
    )
    return {
        "schema_version": SCHEMA_VERSION, "stage": STAGE, "identity": IDENTITY,
        "source_authenticated": source_passed, "geometry": geometry,
        "measured_codes": list(measured), "missing_codes": list(missing),
        "calibration_trajectory_count": len(calibration_rows), "calibration_pass_count": calibration_pass_count,
        ...
        "formal_margin_buffer": margin_buffer, "tube_passed": tube_passed,
        "model_gate_passed": model_gate, "missing_prediction_count": len(missing_rows),
        "baseline_formal_pass_count": baseline_pass_count,
        "failed_baseline_count": len(context_rows) - baseline_pass_count if model_gate else 0,
        ...
        "stability_fold_rows": stability["fold_rows"], "stability_missing_rows": stability["missing_rows"],
        "calibration_rows": calibration_rows, "tube_rows": tube_rows,
        "missing_rows": missing_rows, "context_rows": context_rows,
        "route": route, "passed": authority_gate,
...
def run_primary(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage.exists() and any(ctx.paths.stage.iterdir()):
        raise ValueError("R8R19 output directory is not empty")
    ...
    manifest = {
        ...
        "source_r8r17_run": str(ctx.r8r17_run),
        "source_r8r18_run": str(ctx.r8r18_run),
...
def postprocess(ctx: Context) -> dict[str, Any]:
    ...
    if independent.get("passed") is not True or independent.get("primary_agreement") is not True:
        raise ValueError("R8R19 independent agreement failed")
    final = {
    ...
    state.update({
        ...
        "stop_reason": (
            "" if primary["passed"]
            else "source_authentication_failed" if not primary["source_authenticated"]
            else "saturated_boolean_kernel_model_gate_failed" if not primary["model_gate_passed"]
            else "robust_predicted_authority_gate_failed"
        ),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--r8r18-run", type=Path, required=True)
