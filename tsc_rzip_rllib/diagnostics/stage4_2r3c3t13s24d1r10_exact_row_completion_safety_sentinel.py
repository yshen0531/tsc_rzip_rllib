"""Stage4.2R3c3T13S24D1R10 authentic real-TSC safety sentinel."""
...
RUN_NAME = "stage4_2r3c3t13s24d1r10_exact_row_completion_safety_sentinel"
CAMPAIGN_IDENTITY = "exact_row_completion_safety_sentinel_v1"
CONTROLLER_REVISION = (
    "sequential_exact_row_completion_card15_probe_v42r3c3t13s24d1r10_v1"
)
PACKAGE_REVISION = "r42r3c3t13s24d1r10_exact_row_completion_safety_sentinel_v1"
N_COILS = 14
...
class Context:
    ...
    config_path: Path
    base_s24_ctx: Any
    source_d1r9_output: Path
    source_d1r9_log: Path
    paths: Paths
...
def _package_path(cfg: Mapping[str, Any], key: str) -> Path:
    ...
    if path != root and root not in path.parents:
        raise ValueError(f"D1R10 {key} is outside package")
    return path
...
def _validate_config(cfg: Mapping[str, Any], config_path: Path) -> None:
    exact = {
        ...
        "selection_status": (
            "frozen_after_two_d1r9_passes_before_d1r10_implementation_or_tsc"
        ),
    }
    for key, expected in exact.items():
        if cfg.get(key) != expected:
            raise ValueError(f"D1R10 frozen {key} changed")
    for key in ("design_document", "base_s24_config", "base_s24_implementation"):
        ...
        if not path.is_file() or _sha256(path) != str(cfg[f"{key}_sha256"]):
            raise ValueError(f"D1R10 frozen {key} hash changed")
    expected_config = (
        _project_root()
        / "configs/stage4_2r3c3t13s24d1r10_exact_row_completion_safety_sentinel_370ms.json"
    ).resolve()
    if config_path.resolve() != expected_config:
        raise ValueError("D1R10 config path changed")
    source = cfg["source_contract"]
    if (
        source["d1r9_output_name_v1"]
        != "stage4_2r3c3t13s24d1r9_central_row_replacement_preflight_20260803_a4547d5_v1"
        or source["d1r9_output_name_v2"]
        != "stage4_2r3c3t13s24d1r9_central_row_replacement_preflight_20260803_a4547d5_v2"
        or source["required_d1r9_route"]
        ...
        or tuple(
            int(source[key])
            for key in (
                "required_combination_count",
                "required_eligible_combination_count",
        ...
        != (12870, 495, 7680, 3840, 3840, 1280, 40, 160, 40, 126)
        or list(map(int, source["required_candidate_row_indices"]))
        != [3, 7, 11, 15, 20, 21, 22]
        or not bool(source["byte_identical_d1r9_outputs_required"])
        or any(
            len(str(value)) != 64
        ...
        raise ValueError("D1R10 immutable D1R9 source contract changed")
    ...
    if (
        ...
        != (126, 18, 9, 18)
        or specs["expected_horizon_counts"] != {"35": 56, "37": 70}
        ...
        raise ValueError("D1R10 spec contract changed")
    ...
    if (
        ...
        raise ValueError("D1R10 action schedule or gates changed")
    ...
    if (
        ...
        raise ValueError("D1R10 formal timing changed")
    ...
    ) != (126, 126, 504, 504, 126, 126, 126, 126, 126, 0):
        raise ValueError("D1R10 execution gate changed")
    if cfg["routes"] != {
        "offline_fail": "EXACT_ROW_COMPLETION_SENTINEL_PREFLIGHT_FAIL_NO_TSC",
        "runtime_fail": "EXACT_ROW_COMPLETION_SENTINEL_RUNTIME_FAIL_STOP",
        "action_fail": "EXACT_ROW_COMPLETION_SENTINEL_ACTION_MARGIN_FAIL_REDESIGN_REQUIRED",
        "pass": "EXACT_ROW_COMPLETION_SENTINEL_PASS_FULL_REPLACEMENT_IDENTIFICATION_DESIGN_REQUIRED",
    }:
        raise ValueError("D1R10 routes changed")
    if int(cfg["parallel"]["n_workers"]) != 96:
        raise ValueError("D1R10 Ray capacity changed")
    ...
    if (
        ...
        raise ValueError("D1R10 scientific scope changed")
...
def load_config(
    ...
    **source_kwargs: Path,
    ...
    return Context(
        ...
        source_d1r9_output=source_d1r9_output.expanduser().resolve(),
        source_d1r9_log=source_d1r9_log.expanduser().resolve(),
        paths=_paths(run_dir),
    )


def _authenticate_d1r9(ctx: Context) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """Authenticate both accepted D1R9 outputs and return exact D1R10 specs."""
    source = ctx.cfg["source_contract"]
    root_v1 = ctx.source_d1r9_output
    root_v2 = root_v1.parent / str(source["d1r9_output_name_v2"])
    if root_v1.name != source["d1r9_output_name_v1"] or not root_v2.is_dir():
        raise ValueError("D1R10 exact D1R9 source directories changed")

    package_sources = {
        "config": (
            _project_root()
            / "configs/stage4_2r3c3t13s24d1r9_central_row_replacement_preflight_v1.json",
            "d1r9_config_sha256",
        ),
        "primary_implementation": (
            _project_root()
            / "docs/codex/audit_tools/stage4_2r3c3t13s24d1r9_central_row_replacement_preflight.py",
            "d1r9_primary_implementation_sha256",
        ),
        "independent_implementation": (
            _project_root()
            / "docs/codex/audit_tools/stage4_2r3c3t13s24d1r9_independent_forensics.py",
            "d1r9_independent_implementation_sha256",
        ),
        "design": (
            _project_root()
            / "docs/codex/reports/STAGE4_2R3C3T13S24D1R9_CENTRAL_ROW_REPLACEMENT_PREFLIGHT_DESIGN.md",
            "d1r9_design_sha256",
        ),
        "forensic_report": (
            _project_root()
            / "docs/codex/reports/STAGE4_2R3C3T13S24D1R9_FORENSIC_REPORT.md",
            "d1r9_forensic_report_sha256",
        ),
    }
    package_hashes: dict[str, str] = {}
    for name, (path, key) in package_sources.items():
        if not path.is_file() or _sha256(path) != str(source[key]):
            raise ValueError(f"D1R10 immutable D1R9 {name} changed")
        package_hashes[name] = _sha256(path)

    file_names = {
        "candidate_specs": (
            "stage4_2r3c3t13s24d1r9_d1r10_candidate_specs_v1.json",
            "d1r9_candidate_specs_sha256",
        ),
        "detailed": (
            "stage4_2r3c3t13s24d1r9_central_row_replacement_preflight_v1.json",
            "d1r9_detailed_sha256",
        ),
        "summary": (
        ...
        "manifest": (
            ...
            "d1r9_manifest_sha256",
        ),
        "independent": (
            "stage4_2r3c3t13s24d1r9_independent_forensics_v1.json",
            "d1r9_independent_output_sha256",
        ),
    }
    output_hashes: dict[str, str] = {}
    for name, (rel, key) in file_names.items():
        path_v1 = root_v1 / rel
        path_v2 = root_v2 / rel
        if (
            not path_v1.is_file()
            or not path_v2.is_file()
            or path_v1.read_bytes() != path_v2.read_bytes()
            or _sha256(path_v1) != str(source[key])
        ):
            raise ValueError(f"D1R10 immutable byte-identical D1R9 {name} changed")
        output_hashes[name] = _sha256(path_v1)
    inventory_v1 = sorted(
        path.relative_to(root_v1).as_posix()
        for path in root_v1.rglob("*")
        if path.is_file()
    )
    inventory_v2 = sorted(
        path.relative_to(root_v2).as_posix()
        for path in root_v2.rglob("*")
        if path.is_file()
    )
    if inventory_v1 != inventory_v2 or inventory_v1 != sorted(
        rel for rel, _ in file_names.values()
    ):
        raise ValueError("D1R10 D1R9 byte-identical output inventory changed")

    acceptance_path = root_v1.parent / "d1r9_server_acceptance_a4547d5.json"
    if (
        not acceptance_path.is_file()
        or _sha256(acceptance_path) != source["d1r9_server_acceptance_sha256"]
        or not ctx.source_d1r9_log.is_file()
        or _sha256(ctx.source_d1r9_log) != source["d1r9_official_log_sha256"]
    ):
        raise ValueError("D1R10 D1R9 server acceptance or complete log changed")

    candidate_path = root_v1 / file_names["candidate_specs"][0]
    if candidate_path.stat().st_size != int(
        source["d1r9_candidate_specs_size_bytes"]
    ):
        raise ValueError("D1R10 D1R9 candidate size changed")
    candidate = _read_json(candidate_path)
    detailed = _read_json(root_v1 / file_names["detailed"][0])
    summary = _read_json(root_v1 / file_names["summary"][0])
    manifest = _read_json(root_v1 / file_names["manifest"][0])
    independent = _read_json(root_v1 / file_names["independent"][0])
    acceptance = _read_json(acceptance_path)
    selection = detailed.get("selection") or {}
    static = detailed.get("static_replay") or {}
    static_counts = static.get("counts") or {}
    if not (
        detailed.get("passed") is True
        and summary.get("passed") is True
        and manifest.get("passed") is True
        and independent.get("passed") is True
        and acceptance.get("passed") is True
        and independent.get("primary_output_exact_match") is True
        and acceptance.get("byte_identical_runs") is True
        and all(
            value.get("route") == source["required_d1r9_route"]
            for value in (detailed, summary, manifest, independent, acceptance)
        )
        and selection.get("requested_matrix_digest")
        == source["requested_matrix_digest"]
        and candidate.get("requested_matrix_digest")
        == source["requested_matrix_digest"]
        and independent.get("requested_matrix_digest")
        == source["requested_matrix_digest"]
        and int(selection.get("combination_count", -1))
        == int(source["required_combination_count"])
        and int(selection.get("eligible_combination_count", -1))
        == int(source["required_eligible_combination_count"])
        and int(static_counts.get("finite", -1))
        == int(source["required_finite_constructions"])
        and int(static_counts.get("issue", -1))
        == int(source["required_issue_gate_pass"])
        and int(static_counts.get("cancel", -1))
        == int(source["required_cancellation_gate_pass"])
        and int(static_counts.get("central", -1))
        == int(source["required_central_gate_pass"])
        and int(static_counts.get("global", -1))
        == int(source["required_global_gate_pass"])
        and int(static_counts.get("slot", -1))
        == int(source["required_slot_gate_pass"])
        and int(static_counts.get("late", -1))
        == int(source["required_late_gate_pass"])
        and candidate.get("candidate_row_indices")
        ...
        raise ValueError("D1R10 D1R9 scientific source contract changed")

    specs = [copy.deepcopy(dict(row)) for row in candidate.get("spec_rows") or []]
    ...
    if (
        ...
        or _digest(specs) != source["d1r9_candidate_spec_digest"]
        ...
        or any(
            ...
            or row.get("s24_schedule_digest")
            != source["requested_matrix_digest"]
            ...
            or bool(row.get("probe_trajectory_allowed_in_expert_dataset", True))
            for row in specs
        )
    ):
        raise ValueError("D1R10 candidate spec inventory changed")
    return (
        {
            "source_dir_v1": str(root_v1),
            "source_dir_v2": str(root_v2),
            "package_source_hashes": package_hashes,
            "output_hashes": output_hashes,
            "server_acceptance_sha256": _sha256(acceptance_path),
            "official_log_sha256": _sha256(ctx.source_d1r9_log),
            "source_route": str(summary["route"]),
            "source_spec_count": len(specs),
            "source_spec_digest": _digest(specs),
...
def _selected_snapshot_audit(
    ...
    """Re-aggregate the inherited source audit for D1R10's selected subset."""
...
def prepare_offline(ctx: Context) -> dict[str, Any]:
    if ctx.paths.run_dir.exists() and any(ctx.paths.run_dir.iterdir()):
        raise ValueError("D1R10 offline requires a fresh empty run identity")
    auth, specs = _authenticate_d1r9(ctx)
    ...
    _write_json(ctx.paths.source_reference / "d1r9_authentication.json", auth)
    ...
    output = {
        ...
        "exact_d1r9_spec_reuse_passed": True,
        "spec_count": len(specs),
        "context_count": len(contexts),
        "snapshot_pass_count": int(snapshots["pass_count"]),
        "new_raw_count": 0,
...
def _saved_specs(ctx: Context) -> list[dict[str, Any]]:
    ...
    _, rebuilt = _authenticate_d1r9(ctx)
    if saved != rebuilt:
        raise ValueError("D1R10 frozen D1R9 specs changed")
    manifest = _read_json(ctx.paths.manifest)
    if _digest(saved) != manifest.get("spec_digest"):
        raise ValueError("D1R10 frozen D1R9 spec digest changed")
    return saved
...
def _payload(ctx: Context, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            "variant_id": f"stage4_2r3c3t13s24d1r10_{experiment_id}",
            "stage4_2r3c3t13s24d1r10_restart_snapshot_dir": str(
                spec["restart_snapshot_dir"]
            ),
            "stage4_2r3c3t13s24d1r10_snapshot_manifest_digest": str(
                spec["restart_snapshot_manifest_digest"]
            ),
            "stage4_2r3c3t13s24d1r10_selection_label_available_to_controller": False,
        }
    )
...
class ExactRowCompletionSafetySentinelController(
    ...
    def _cancel(
        ...
        if self._active_issue is None or int(self._active_issue["slot"]) != slot:
            raise ValueError("D1R10 cancellation has no matching causal issue")
        ...
        if not event["passed"]:
            ...
            raise ValueError(
                "D1R10 sequential cancel action failed: "
    ...
    def action(
        ...
        trace.update(
            {
                "r3c3t13s24d1r10_identification_only": True,
                "r3c3t13s24d1r10_safety_sentinel_only": True,
                "r3c3t13s24d1r10_controller_revision": CONTROLLER_REVISION,
                "r3c3t13s24d1r10_event": trace.get("r3c3t13s24_event", "none"),
                "r3c3t13s24d1r10_event_detail": copy.deepcopy(event),
                "r3c3t13s24d1r10_source_selection_label_used": False,
                "r3c3t13s24d1r10_pair_history_partition_label_used": False,
                "r3c3t13s24d1r10_source_outcome_used": False,
                "r3c3t13s24d1r10_future_measurement_used": False,
                "r3c3t13s24d1r10_future_executed_action_used": False,
                "r3c3t13s24d1r10_hidden_wire_current_used": False,
                "r3c3t13s24d1r10_schedule_available_to_underlying_controller": False,
            }
        )
        return action, trace
...
class LocalWorker:
    """One fresh authentic TSC process and D1R10 controller per rollout."""
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        controller: ExactRowCompletionSafetySentinelController | None = None
        result: dict[str, Any] = {
        ...
        try:
            ...
            if (
                ...
                raise ValueError("D1R10 formal horizon changed")
            self.base.env.reset()
            ...
            controller = ExactRowCompletionSafetySentinelController(
                self.base,
                self.bundle,
            ...
            for step in range(horizon):
                ...
                if truncated and step + 1 < horizon:
                    raise RuntimeError(
                        "environment truncated before D1R10 formal horizon"
                    )
            calibration_events = [
            ...
            sequence_events = [
                row["r3c3t13s24d1r10_event"]
                for row in trace
                if row["r3c3t13s24d1r10_event"] != "none"
            ]
            expected_sequence = [
                "sequential_issue",
                "sequential_cancel",
            ...
            forbidden_keys = (
                ...
                "r3c3t13s24d1r10_source_selection_label_used",
                "r3c3t13s24d1r10_pair_history_partition_label_used",
                "r3c3t13s24d1r10_source_outcome_used",
                "r3c3t13s24d1r10_future_measurement_used",
                "r3c3t13s24d1r10_future_executed_action_used",
                "r3c3t13s24d1r10_hidden_wire_current_used",
                "r3c3t13s24d1r10_schedule_available_to_underlying_controller",
            )
            forbidden = sum(
                any(bool(row.get(key)) for key in forbidden_keys) for row in trace
            )
            event_details = [
                row["r3c3t13s24d1r10_event_detail"]
                for row in trace
                if row["r3c3t13s24d1r10_event"] != "none"
            ]
            cancels = [
            ...
            result.update(
                {
                    ...
                    "failure_reason": "" if success else "invalid D1R10 full rollout",
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(
                    failed=failed,
                    reason=(
                        "stage4_2r3c3t13s24d1r10_geometry_restored_"
...
def _ray_actor_class():
    ...
    if _RAY_ACTOR is None:
        ...
        class Stage42R3C3T13S24D1R10Actor:
            def __init__(
                self,
                payload,
                library,
        ...
        _RAY_ACTOR = Stage42R3C3T13S24D1R10Actor
    return _RAY_ACTOR
    ...
    *,
    ...
    for spec in specs:
        ...
        if path.exists() and not _result_covered(path, spec):
            raise ValueError(
                "D1R10 existing raw is corrupt or incompatible; refusing overwrite"
            )
    pending = [
        spec
        for spec in specs
    ...
    if not resume and any(ctx.paths.raw.glob("*.json.gz")):
        raise ValueError("D1R10 fresh rollout requires empty raw directory")
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            worker = LocalWorker(
                ...
                library,
                bundle,
                f"stage42r3c3t13s24d1r10_serial_{index:04d}",
                selector,
                lattice,
                calibration,
                dynamic,
            ...
            print(f"[T13S24D1R10] {index + 1}/{len(pending)}", flush=True)
        ...
        for batch_start in range(0, len(pending), plan.actor_count):
            ...
            for offset, spec in enumerate(batch):
                actor = Actor.remote(
                    ...
                    library,
                    bundle,
                    f"stage42r3c3t13s24d1r10_{batch_start + offset:04d}",
                    selector,
                    lattice,
                    calibration,
                    dynamic,
            ...
            try:
                while refs:
                    ...
                    if not ready:
                        print(
                            f"[T13S24D1R10] waiting {completed}/{len(pending)}",
                            flush=True,
                        )
                        continue
                    for ref in ready:
                        ...
                        completed += 1
                        print(
                            f"[T13S24D1R10] {completed}/{len(pending)}", flush=True
                        )
            finally:
        ...
        raise ValueError(f"unsupported D1R10 backend: {backend}")
    covered = sum(
...
def _execution_audit(
    ...
    for spec in specs:
        ...
        if not result.get("success"):
            ...
            applied_events = [
                row.get("r3c3t13s24d1r10_event_detail") or {}
                for row in trace
                if row.get("r3c3t13s24d1r10_event") != "none"
            ]
...
def run_rollout(ctx: Context, *, backend: str, resume: bool) -> dict[str, Any]:
    ...
    if state.get("phase_status") != "offline_ready" or state.get("finished"):
        raise ValueError("D1R10 rollout requires open offline-ready state")
...
def run_postprocess(ctx: Context) -> dict[str, Any]:
    if not ctx.paths.state.is_file() or not ctx.paths.final.is_file():
        raise ValueError("D1R10 postprocess requires completed sentinel outputs")
    specs = _saved_specs(ctx)
    audit = _execution_audit(ctx, specs)
    inventory = _raw_inventory(ctx.paths)
    output = {
...
def execute(
    ...
    if command == "offline":
        if resume:
            raise ValueError("D1R10 offline cannot resume")
...
def self_test(config_path: Path) -> dict[str, Any]:
    ...
    specs = cfg["spec_contract"]
    exact_contract = bool(
        int(specs["expected_specs"]) == 126
        and specs["expected_horizon_counts"] == {"35": 56, "37": 70}
    ...
    return {
        "schema_version": 1,
        "stage": STAGE,
        "exact_d1r9_spec_contract": exact_contract,
