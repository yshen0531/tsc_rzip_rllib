#!/usr/bin/env python3
"""Execute the frozen R8R12 causal cumulative direction-2 staircase sentinel."""
...
class Paths:
    ...
    def phase_raw(self, phase: str) -> Path:
        if phase not in PHASES:
            raise ValueError(f"invalid R8R12 phase: {phase}")
...
def validate_config(cfg: Mapping[str, Any], *, project_root: Path) -> None:
    ...
    if (
        ...
        or int(schedule["direction_index"]) != 2
        or int(schedule["sign"]) != 1
        ...
        raise ValueError("R8R12 frozen design changed")
...
def load_context(args: argparse.Namespace) -> Context:
    ...
    return Context(
        ...
        r8r11_run=args.r8r11_run.expanduser().resolve(),
...
def _r8r11_stage(ctx: Context) -> Path:
    return ctx.r8r11_run / str(ctx.cfg["source_r8r11"]["stage_directory"])
...
def _source_baselines(
    ...
    if len(specs) != 16 or {
        ...
        raise ValueError("R8R12 source baseline coverage changed")
    results = {
...
def _authenticate_r8r11(ctx: Context) -> dict[str, Any]:
    stage = _r8r11_stage(ctx)
    expected = ctx.cfg["source_r8r11"]
    paths = {
    ...
    for name, key in (
        ...
        if hashes[name] != str(expected[key]):
            raise ValueError(f"R8R12 R8R11 source {name} changed")
    ...
    for phase in PHASES:
        ...
        if (
            ...
            raise ValueError(f"R8R12 R8R11 {phase} raw inventory changed")
    if (
        ...
        raise ValueError("R8R12 R8R11 final route changed")
    return {
        "stage": str(stage),
        "hashes": hashes,
...
def build_specs(ctx: Context) -> list[dict[str, Any]]:
    ...
    requested = np.asarray(schedule["canonical_matrix_columns"], dtype=float)[:, 2]
    output = []
        ...
        spec.update(
            {
                ...
                "r8r12_matrix_digest": str(schedule["canonical_matrix_float64_le_c_sha256"]),
                "r8r12_decision_task_steps": list(map(int, schedule["decision_task_steps"])),
    ...
    if counts != {"safety": 4, "qualification": 12} or len(output) != 16:
        raise ValueError("R8R12 frozen specification matrix changed")
    return output
...
def _payload(ctx: Context, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            "variant_id": f"stage4_2r3c3t13s24d1r14r8r12_{experiment_id}",
            "stage4_2r3c3t13s24d1r14r8r12_decision_task_steps": list(
                map(int, spec["r8r12_decision_task_steps"])
            ),
            "stage4_2r3c3t13s24d1r14r8r12_pair_history_label_available_to_controller": False,
        }
    )
...
def prepare_offline(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage.exists():
        raise ValueError("R8R12 offline requires a fresh run identity")
    source_r8r7 = r8r9._authenticate_r8r7(_source_stage(ctx), ctx.cfg)
    source_r8r11 = _authenticate_r8r11(ctx)
    lineage = {
    ...
    if not preflight["passed"]:
        raise ValueError("R8R12 frozen cumulative construction preflight failed")
    ...
    _write(ctx.paths.source_reference / "r8r11_authentication.json", source_r8r11)
    ...
    manifest = {
        ...
        "source_r8r11_run": str(ctx.r8r11_run),
        "source_r8r7_authenticated": True,
        "source_r8r11_authenticated": True,
...
def _saved_specs(ctx: Context) -> list[dict[str, Any]]:
    ...
    if (
        ...
        raise ValueError("R8R12 saved identity or specifications changed")
    return specs


def _phase_specs(ctx: Context, phase: str) -> list[dict[str, Any]]:
    rows = [row for row in _saved_specs(ctx) if row["partition"] == phase]
    if len(rows) != {"safety": 4, "qualification": 12}[phase]:
        raise ValueError("R8R12 saved phase coverage changed")
    return rows
...
class CausalCumulativeDirection2StaircaseController(
    ...
    def __init__(
        ...
        *,
        ...
        requested = np.asarray(spec["r8r12_requested_coordinate"], dtype=float)
        super().__init__(
            base,
            bundle,
            source_spec,
        ...
        self.r8r12_issue_count = 0
        self.r8r12_refresh_count = 0

    def _refresh(self, currents: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        if self._active_issue is None:
            raise ValueError("R8R12 refresh has no active exact target")
        event = _refresh_event(
            ...
            contract=self.r8r12_contract,
        )
        if not event["passed"]:
            raise ValueError(
                "R8R12 exact target refresh action failed: "
                + json.dumps(event, sort_keys=True)
            )
        self.r8r12_refresh_count += 1
    ...
    def action(self, current_state: Mapping[str, Any]) -> tuple[np.ndarray, dict[str, Any]]:
        if int(current_state["step_index"]) != self.step:
            raise ValueError("R8R12 controller/current task-state index mismatch")
        if self.step < PREFIX_END:
            action, trace = super().action(current_state)
            trace.update(
                {
                    "r3c3t13s24d1r14r8r12_controller_revision": CONTROLLER_REVISION,
                    "r3c3t13s24d1r14r8r12_delegated_prefix": True,
                    "r3c3t13s24d1r14r8r12_event": "none",
                    "r3c3t13s24d1r14r8r12_event_detail": {},
                    "r3c3t13s24d1r14r8r12_forbidden_input_used": False,
                }
            )
        ...
        if self.step in self.r8r12_decisions:
            action, event = self._issue(2, currents, np.zeros(N_COILS))
            event_name = "staircase_issue"
            self.r8r12_issue_count += 1
        else:
        ...
        trace.update(
            {
                ...
                "r3c3t13s24d1r14r8r12_controller_revision": CONTROLLER_REVISION,
                "r3c3t13s24d1r14r8r12_delegated_prefix": False,
                "r3c3t13s24d1r14r8r12_event": event_name,
                "r3c3t13s24d1r14r8r12_event_detail": copy.deepcopy(event),
                "r3c3t13s24d1r14r8r12_forbidden_input_used": False,
                "r3c3t13s24d1r14r8r12_pair_or_history_label_used": False,
                "r3c3t13s24d1r14r8r12_future_measurement_used": False,
                "r3c3t13s24d1r14r8r12_future_action_used": False,
                "r3c3t13s24d1r14r8r12_source_result_used": False,
                "r3c3t13s24d1r14r8r12_hidden_wire_used": False,
            }
        )
        return np.asarray(action, dtype=float), trace
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if horizon != int(spec["formal_horizon_steps"]) or horizon != int(
                ...
                raise ValueError("R8R12 formal horizon changed")
            self.base.env.reset()
            ...
            controller = CausalCumulativeDirection2StaircaseController(
                self.base,
                self.bundle,
                spec,
                trajectory[0],
            ...
            for step in range(horizon):
                ...
                if not finite or bool(current.get("abnormal")):
                    ...
                    raise ValueError("R8R12 pre-action visible state safety gate failed")
                try:
                ...
                if truncated and step + 1 < horizon:
                    ...
                    raise RuntimeError("environment truncated before R8R12 horizon")
            calibration_events = [
            ...
            events = [
                row.get("r3c3t13s24d1r14r8r12_event")
                for row in trace[PREFIX_END:]
            ]
            success = bool(
                ...
                and all(
                    bool(row.get("r3c3t13s24d1r14r8r12_delegated_prefix"))
                    for row in trace[:PREFIX_END]
                )
                and all(
                    not bool(row.get("r3c3t13s24d1r14r8r12_delegated_prefix"))
                    for row in trace[PREFIX_END:]
                )
                ...
                and all(
                    bool(
                        (row.get("r3c3t13s24d1r14r8r12_event_detail") or {}).get(
                            "passed"
                        )
                    )
                    for row in trace[PREFIX_END:]
                )
                and controller.r8r12_issue_count == 4
                and controller.r8r12_refresh_count == horizon - PREFIX_END - 4
            ...
            result.update(
                {
                    ...
                    "failure_reason": ""
                    if success
                    else "incomplete or invalid R8R12 rollout",
                    "execution_failure_class": ""
                    if success
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(
                    failed=failed, reason="stage4_2r3c3t13s24d1r14r8r12"
                )
    ...
    *,
    ...
    def args_for(spec: Mapping[str, Any], index: int) -> tuple[Any, ...]:
        return (
            ...
            library,
            bundle,
            f"stage42r8r12_{phase}_{index:03d}",
            selector,
            lattice,
            calibration,
            dynamic,
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            ...
            print(f"[R8R12 {phase}] {index + 1}/{len(pending)}", flush=True)
        ...
        for start in range(0, len(pending), plan.actor_count):
            ...
            try:
                while refs:
                    ...
                    if not ready:
                        print(
                            f"[R8R12 {phase}] waiting {completed}/{len(pending)}",
                            flush=True,
                        )
                        continue
                    ref = ready[0]
                    ...
                    completed += 1
                    print(f"[R8R12 {phase}] {completed}/{len(pending)}", flush=True)
            finally:
        ...
        raise ValueError(f"unsupported R8R12 backend: {backend}")
    successful = sum(
...
def authorize_safety(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R12 offline independent gate failed")
    _set_state(
        ctx,
...
def run_phase(
    ...
    if state.get("phase_status") != required:
        raise ValueError(f"R8R12 {phase} is not authorized")
    if phase == "safety" and any(ctx.paths.phase_raw("qualification").glob("*.json.gz")):
        raise ValueError("R8R12 qualification raw opened before safety authorization")
    specs = _phase_specs(ctx, phase)
...
def authorize_qualification(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R12 safety independent gate failed")
    _set_state(
        ctx,
...
def compute_formal_authority(ctx: Context) -> dict[str, Any]:
    ...
    evaluators, _ = r8r7.r8.d1r11._formal_callback(
        ctx.source_ctx.r8_ctx.d1r11_ctx, [*baseline_specs, *specs]
    )
    rows = []
    ...
    for key, group in sorted(groups.items()):
        ...
        candidate = [row for row in group if row["kind"] == "candidate"]
        if len(baseline) != 1 or len(candidate) != 1:
            raise ValueError("R8R12 formal context coverage changed")
        base, cand = baseline[0], candidate[0]
        oracle = max(
            (base, cand),
            key=lambda row: (
        ...
        contexts.append(
            {
                ...
                "baseline": base,
                "candidate": cand,
                "candidate_minimum_margin_gain": float(
                    cand["formal_minimum_signed_margin"]
                )
                - float(base["formal_minimum_signed_margin"]),
                "failed_baseline_repaired": bool(
                    not base["formal_contract_pass"] and cand["formal_contract_pass"]
                ),
    ...
    gains = [
        float(row["candidate_minimum_margin_gain"])
        for row in contexts
    ...
    return {
        ...
        "candidate_formal_pass_count": sum(
            bool(row["candidate"]["formal_contract_pass"]) for row in contexts
        ),
        "repaired_failed_baseline_count": repairs,
        "held_oracle_formal_pass_count": oracle_pass,
        "failed_baseline_candidate_margin_gain": {
            "minimum": min(gains),
...
def finalize_primary(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R12 raw independent gates incomplete")
    ...
    summary = {
        ...
        "candidate_formal_pass_count": formal["candidate_formal_pass_count"],
        ...
        "failed_baseline_candidate_margin_gain": formal[
            "failed_baseline_candidate_margin_gain"
        ],
        "new_raw_count": 16,
        "real_tsc_executed": True,
...
def postprocess(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R12 final independent agreement failed")
...
def execute(
    ...
    raise ValueError(f"unsupported R8R12 command: {command}")
