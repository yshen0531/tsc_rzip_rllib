#!/usr/bin/env python3
"""Execute the frozen R8R51 q0-to-transport causal bridge sentinel."""
...
SCHEMA_VERSION = 1
STAGE = "Stage4.2R3c3T13S24D1R14R8R51"
IDENTITY = "reduced_q0_transport_bridge_identification_sentinel_v1"
RUN_NAME = "stage4_2r3c3t13s24d1r14r8r51_reduced_q0_transport_bridge_identification_sentinel"
CONTROLLER_REVISION = "reduced_q0_transport_bridge_v42r3c3t13s24d1r14r8r51_v1"
...
def validate_config(cfg: Mapping[str, Any], *, project_root: Path) -> None:
    ...
    expected_routes = {
        "source_blocked": "REDUCED_Q0_TRANSPORT_BRIDGE_SENTINEL_BLOCKED_BY_SOURCE",
        "offline_fail": "REDUCED_Q0_TRANSPORT_BRIDGE_SENTINEL_OFFLINE_FAIL_NO_REAL_TSC",
        "execution_fail": "REDUCED_Q0_TRANSPORT_BRIDGE_SENTINEL_EXECUTION_FAIL_STOP",
        "safety_fail": "REDUCED_Q0_TRANSPORT_BRIDGE_SENTINEL_SAFETY_FAIL_REDESIGN",
        "pass": "REDUCED_Q0_TRANSPORT_BRIDGE_IDENTIFICATION_COMPLETE_MODEL_PREFLIGHT_REQUIRED",
    }
    invalid = (
        ...
        != "r42r3c3t13s24d1r14r8r51_reduced_q0_transport_bridge_v1"
    ...
    if invalid:
        raise ValueError("R8R51 frozen design changed")
...
def _source_baselines(ctx: Context) -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    if len(specs) != 16 or len(results) != 16:
        raise SourceBlockedError("R8R51 source baseline coverage changed")
...
def authenticate_sources(ctx: Context) -> dict[str, Any]:
    ...
    if any(not path.is_file() for path in paths.values()):
        raise SourceBlockedError("R8R51 final R8R48 evidence incomplete")
    hashes = {name: _sha(path) for name, path in paths.items()}
    if any(hashes[name] != contract[f"{name}_sha256"] for name in hashes):
        raise SourceBlockedError("R8R51 final R8R48 hash changed")
    ...
    if (
        ...
        raise SourceBlockedError("R8R51 final R8R48 route changed")
    ...
    if any(not path.is_file() for path in r8r49_paths.values()):
        raise SourceBlockedError("R8R51 final R8R49 evidence incomplete")
    ...
    if any(r8r49_hashes[name] != r8r49_contract[f"{name}_sha256"] for name in r8r49_hashes):
        raise SourceBlockedError("R8R51 final R8R49 hash changed")
    ...
    if (
        ...
        raise SourceBlockedError("R8R51 final R8R49 route changed")
    ...
    if (
        ...
        raise SourceBlockedError("R8R51 R8R31 candidate matrix changed")
...
def build_specs(ctx: Context) -> list[dict[str, Any]]:
    ...
    for context_index, source in enumerate(baselines):
        for candidate in candidates:
            ...
            spec.update(
                {
                    "kind": "stage4_2r3c3t13s24d1r14r8r51_q0_transport_bridge",
                    "stage": STAGE,
                    "campaign_identity": IDENTITY,
                    "controller_revision": CONTROLLER_REVISION,
                    "experiment_id": f"r8r51_c{context_index:02d}_k{int(candidate['index']):02d}_{candidate['id']}",
                    "source_r8r7_baseline_experiment_id": str(source["experiment_id"]),
                    "r8r51_context_index": context_index,
                    "r8r51_candidate_index": int(candidate["index"]),
                    "r8r51_candidate_id": str(candidate["id"]),
                    "r8r51_candidate_q": q.tolist(),
                    "r8r51_requested_coordinate": requested.tolist(),
                    "r8r51_q0_issue_task_step": Q0_STEP,
                    "r8r51_candidate_issue_task_step": ISSUE_STEP,
                    "r8r51_first_effect_state_step": FIRST_EFFECT_STATE,
                    "r8r51_second_effect_state_step": SECOND_EFFECT_STATE,
                    "r8r51_return_task_step": RETURN_STEP,
                    "r8r51_allowed_in_expert_dataset": False,
            ...
            rows.append(spec)
    coverage = {
        (str(row["pair_id"]), str(row["history_member"]), int(row["r8r51_candidate_index"]))
        for row in rows
    }
    if len(rows) != 208 or len(coverage) != 208 or len({row["experiment_id"] for row in rows}) != 208:
        raise ValueError("R8R51 frozen specification matrix changed")
    return rows
...
def _payload(ctx: Context, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            "variant_id": f"stage4_2r3c3t13s24d1r14r8r51_{experiment_id}",
            "stage4_2r3c3t13s24d1r14r8r51_candidate_id": str(spec["r8r51_candidate_id"]),
            "stage4_2r3c3t13s24d1r14r8r51_pair_history_label_available_to_controller": False,
        }
    )
...
def _offline_construction(ctx: Context, specs: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for spec in specs:
        ...
        issue = r8r22._construct_coordinate_issue(
            ...
            requested_coordinate=spec["r8r51_requested_coordinate"],
            candidate_id=str(spec["r8r51_candidate_id"]),
            actuator=actuator,
        ...
        criteria = {
            ...
            "candidate_identity": issue.get("candidate_id") == spec["r8r51_candidate_id"],
            "candidate_coordinate": bool(np.array_equal(
                np.asarray(issue.get("requested_coordinate"), dtype=float),
                np.asarray(spec["r8r51_requested_coordinate"], dtype=float),
            )),
        }
        rows.append(
            {
                ...
                "candidate_index": spec["r8r51_candidate_index"],
                "candidate_id": spec["r8r51_candidate_id"],
...
def prepare_offline(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage.exists():
        raise ValueError("R8R51 offline requires a fresh run identity")
...
def _saved_specs(ctx: Context) -> list[dict[str, Any]]:
    ...
    if len(specs) != 208 or _digest(specs) != manifest.get("spec_digest") or _sha(ctx.config_path) != manifest.get("config_sha256"):
        raise ValueError("R8R51 saved specification identity changed")
    return specs
...
def authorize_real(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R51 offline independent authorization gate failed")
    _set_state(
        ctx,
...
class BridgeController(r6.d1r11.SequentialAmplitudeCodedProbeController):
    ...
    def __init__(self, base: Any, bundle: Mapping[str, Any], source_spec: Mapping[str, Any], initial_state: Mapping[str, Any], lattice_cfg: Mapping[str, Any], calibration_cfg: Mapping[str, Any], dynamic_cfg: Mapping[str, Any], schedule_cfg: Mapping[str, Any], controller_cfg: Mapping[str, Any], *, spec: Mapping[str, Any]):
        ...
        if matrix.shape != (4, 4):
            raise ValueError("R8R51 canonical matrix shape changed")
        self.r8r51_candidate_id = str(spec["r8r51_candidate_id"])
        self.r8r51_requested_coordinate = np.asarray(spec["r8r51_requested_coordinate"], dtype=float)
        self.r8r51_contract = copy.deepcopy(dict(controller_cfg))
        self.r8r51_center_fields: list[str] | None = None
        self.r8r51_center_current: list[float] | None = None
        self.r8r51_issue_event: dict[str, Any] | None = None
    ...
    def _q0(self, currents: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        event["passed"] = bool(all(event["criteria"].values()))
        if not event["passed"]:
            raise ValueError("R8R51 q0 action safety gate failed: " + json.dumps(event, sort_keys=True))
        self.r8r51_center_fields = list(center.card15_fields)
        self.r8r51_center_current = list(map(float, event["nominal_readback_current_a_tsc"]))
    ...
    def _issue_candidate(self, currents: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        event = r8r22._construct_coordinate_issue(
            ...
            fixed_basis_delta_field_kat_tsc=field_basis.T,
            requested_coordinate=self.r8r51_requested_coordinate,
            candidate_id=self.r8r51_candidate_id, actuator=self.actuator,
            controller_cfg=self.r8r51_contract, lattice_cfg=self.lattice_cfg,
        )
        event["center_nominal_readback_current_a_tsc"] = list(self.r8r51_center_current or [])
        if not event["passed"]:
            raise ValueError("R8R51 candidate action safety gate failed: " + json.dumps(event, sort_keys=True))
        self.r8r51_issue_event = copy.deepcopy(event)
        self._active_issue = copy.deepcopy(event)
        return np.asarray(event["action_norm_tsc"], dtype=float), event

    def _return(self, currents: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        if self.r8r51_center_fields is None or self.r8r51_issue_event is None:
            raise ValueError("R8R51 return has no causal stored center/issue")
        field_basis, _ = self._basis_current()
        event = _return_event(
            task_step=self.step, currents=currents,
            center_fields=self.r8r51_center_fields, issue_event=self.r8r51_issue_event,
            field_basis=field_basis, actuator=self.actuator, lattice=self.lattice_cfg,
            contract=self.r8r51_contract,
        )
        if not event["passed"]:
            raise ValueError("R8R51 return action safety gate failed: " + json.dumps(event, sort_keys=True))
        self._active_issue = None
        return np.asarray(event["action_norm_tsc"], dtype=float), event

    def _center_refresh(self, currents: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        if self.r8r51_center_fields is None:
            raise ValueError("R8R51 center refresh has no stored center")
        event = r8r22._refresh_event(
            task_step=self.step, currents=currents, target_fields=self.r8r51_center_fields,
            actuator=self.actuator, turns_tsc=self.turns_tsc,
            max_delta_a=float(self.base.max_delta_a), minimum_current=self.base.min_current,
            maximum_current=self.base.max_current, lattice_cfg=self.lattice_cfg,
            contract=self.r8r51_contract,
        ...
        if not event["passed"]:
            raise ValueError("R8R51 refresh action safety gate failed: " + json.dumps(event, sort_keys=True))
        return np.asarray(event["action_norm_tsc"], dtype=float), event

    def action(self, current_state: Mapping[str, Any]) -> tuple[np.ndarray, dict[str, Any]]:
        if int(current_state["step_index"]) != self.step:
            raise ValueError("R8R51 controller/current state index mismatch")
        if self.step < PREFIX_END:
            ...
            if self.step == Q0_STEP:
                ...
                event = _observe_event(
                    task_step=self.step, currents=currents,
                    target_fields=self.r8r51_center_fields or [], actuator=self.actuator,
                    contract=self.r8r51_contract, event="q0_observe_hold",
                )
                ...
                if self.r8r51_issue_event is None:
                    raise ValueError("R8R51 candidate observation has no issue")
                event = _observe_event(
                    task_step=self.step, currents=currents,
        ...
        trace.update(
            {
                ...
                "r3c3t13s24d1r14r8r51_controller_revision": CONTROLLER_REVISION,
                "r3c3t13s24d1r14r8r51_delegated_prefix": delegated,
                "r3c3t13s24d1r14r8r51_event": event_name,
                "r3c3t13s24d1r14r8r51_event_detail": copy.deepcopy(event),
                "r3c3t13s24d1r14r8r51_forbidden_input_used": False,
                "r3c3t13s24d1r14r8r51_pair_or_history_label_used": False,
                "r3c3t13s24d1r14r8r51_future_measurement_used": False,
                "r3c3t13s24d1r14r8r51_future_action_used": False,
                "r3c3t13s24d1r14r8r51_source_result_used": False,
                "r3c3t13s24d1r14r8r51_hidden_wire_used": False,
            }
        )
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if horizon != int(spec["formal_horizon_steps"]) or horizon != int(self.base.env.max_episode_steps) or horizon not in (35, 37):
                raise ValueError("R8R51 formal horizon changed")
            self.base.env.reset()
            ...
            for step in range(horizon):
                ...
                if not finite or bool(current.get("abnormal")):
                    ...
                    raise ValueError("R8R51 pre-action visible state safety gate failed")
                try:
                ...
                if truncated and step + 1 < horizon:
                    ...
                    raise RuntimeError("environment truncated before R8R51 horizon")
            names = [row.get("r3c3t13s24d1r14r8r51_event") for row in trace[PREFIX_END:]]
            event_pass = all(bool((row.get("r3c3t13s24d1r14r8r51_event_detail") or {}).get("passed")) for row in trace[PREFIX_END:])
            first_effect = bool(
            ...
            success = bool(
                ...
                and all(bool(row.get("r3c3t13s24d1r14r8r51_delegated_prefix")) for row in trace[:PREFIX_END])
            ...
            result.update(
                {
                    ...
                    "failure_reason": "" if success else "incomplete or invalid R8R51 rollout",
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(failed=failed, reason="stage4_2r3c3t13s24d1r14r8r51")
...
def evaluate_specs(ctx: Context, specs: Sequence[dict[str, Any]], *, backend: str, resume: bool) -> dict[str, Any]:
    ...
    def args_for(spec: Mapping[str, Any], index: int) -> tuple[Any, ...]:
        return (payloads[str(spec["experiment_id"])], library, bundle, f"stage42r8r51_{index:03d}", selector, lattice, calibration, dynamic, schedule_cfg, controller_cfg)
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            ...
            print(f"[R8R51] {index + 1}/{len(pending)}", flush=True)
        ...
        for start in range(0, len(pending), plan.actor_count):
            ...
            try:
                while refs:
                    ...
                    if not ready:
                        print(f"[R8R51] waiting {completed}/{len(pending)}", flush=True)
                        continue
                    ref = ready[0]
                    spec = refs.pop(ref)
                    try:
                    ...
                    r8r7.r8._write_gz(ctx.paths.raw / f"{spec['experiment_id']}.json.gz", result)
                    completed += 1
                    print(f"[R8R51] {completed}/{len(pending)}", flush=True)
            finally:
        ...
        raise ValueError(f"unsupported R8R51 backend: {backend}")
...
FORBIDDEN_KEYS = (
    "r3c3t13s24d1r14r8r51_forbidden_input_used",
    "r3c3t13s24d1r14r8r51_pair_or_history_label_used",
    "r3c3t13s24d1r14r8r51_future_measurement_used",
    "r3c3t13s24d1r14r8r51_future_action_used",
    "r3c3t13s24d1r14r8r51_source_result_used",
    "r3c3t13s24d1r14r8r51_hidden_wire_used",
)
...
def audit_raw(ctx: Context, *, write: bool = True) -> dict[str, Any]:
    ...
    for spec in specs:
        ...
        events = [trace[step].get("r3c3t13s24d1r14r8r51_event_detail") or {} for step in range(PREFIX_END, horizon)] if full else []
        names = [trace[step].get("r3c3t13s24d1r14r8r51_event") for step in range(PREFIX_END, horizon)] if full else []
        ...
        expected_coordinate = matrix @ np.asarray(spec["r8r51_candidate_q"], dtype=float)
        candidate_exact = bool(
            issue.get("candidate_id") == spec["r8r51_candidate_id"]
            and np.array_equal(np.asarray(issue.get("requested_coordinate") or []), expected_coordinate)
            and np.array_equal(expected_coordinate, np.asarray(spec["r8r51_requested_coordinate"], dtype=float))
        )
        q0_exact = bool(
...
def run_real(ctx: Context, *, backend: str, resume: bool) -> dict[str, Any]:
    ...
    if state.get("phase_status") != "real_authorized":
        raise ValueError("R8R51 real execution is not authorized")
    specs = _saved_specs(ctx)
...
def finalize_primary(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R51 raw independent gate incomplete")
...
def postprocess(ctx: Context) -> dict[str, Any]:
    ...
    if state.get("phase_status") != "final_primary_ready" or independent.get("passed") is not True or independent.get("primary_agreement") is not True:
        raise ValueError("R8R51 final independent agreement failed")
    compact = {
...
def execute(ctx: Context, *, command: str, backend: str, resume: bool) -> dict[str, Any]:
    ...
    raise ValueError(f"unsupported R8R51 command: {command}")
...
def _parser() -> argparse.ArgumentParser:
    ...
    for name in (
        "r8r49-run", "r8r48-run", "r8r46-run", "r8r44-run", "r8r43-run", "r8r41-run",
