#!/usr/bin/env python3
"""Execute the frozen R8R15 binary temporal-switching authority sentinel."""
...
SCHEMA_VERSION = 1
STAGE = "Stage4.2R3c3T13S24D1R14R8R15"
IDENTITY = "binary_temporal_switching_staircase_authority_sentinel_v1"
RUN_NAME = "stage4_2r3c3t13s24d1r14r8r15_binary_temporal_switching_staircase_authority_sentinel"
CONTROLLER_REVISION = "binary_temporal_switching_staircase_v42r3c3t13s24d1r14r8r15_v1"
N_COILS = 14
PREFIX_END = 10
...
class Paths:
    ...
    def phase_raw(self, phase: str) -> Path:
        if phase not in PHASES:
            raise ValueError(f"invalid R8R15 phase: {phase}")
...
def validate_config(cfg: Mapping[str, Any], *, project_root: Path) -> None:
    ...
    if (
        ...
        or tuple(map(str, schedule["reused_sequences"])) != ("UUUU", "VVVV")
        ...
        or int(schedule["final_atlas_spec_count"]) != 176
        or (
        ...
        raise ValueError("R8R15 frozen design changed")
...
def _source_baselines(
    ...
    if len(specs) != 16 or {
        ...
        raise ValueError("R8R15 source baseline coverage changed")
    results = {
...
def _authenticate_r8r12(ctx: Context) -> dict[str, Any]:
    ...
    for name, key in (
        ...
        if hashes[name] != str(expected[key]):
            raise ValueError(f"R8R15 R8R12 source {name} changed")
    ...
    for phase in PHASES:
        ...
        if (
            ...
            raise ValueError(f"R8R15 R8R12 {phase} raw inventory changed")
    if (
        ...
        raise ValueError("R8R15 R8R12 final route changed")
    return {
        "stage": str(stage),
        "hashes": hashes,
...
def _authenticate_r8r14(ctx: Context) -> dict[str, Any]:
    ...
    for name, key in (
        ...
        if hashes[name] != str(expected[key]):
            raise ValueError(f"R8R15 R8R14 provenance {name} changed")
    if _sha(stage / "specs/all_specs.json") != str(expected["all_specs_sha256"]):
        raise ValueError("R8R15 R8R14 source specifications changed")
    inventories = {
        phase: r8r7.r8._inventory(stage / "raw" / phase) for phase in PHASES
    }
    for phase in PHASES:
        ...
        if (
            ...
            raise ValueError(f"R8R15 R8R14 {phase} raw inventory changed")
    ...
    if (
        ...
        raise ValueError("R8R15 R8R14 final provenance changed")
    return {
        "stage": str(stage),
        "hashes": hashes,
        "inventories": inventories,
...
def _source_r8r12_candidates(
    ...
    if (
        ...
        raise ValueError("R8R15 source R8R12 candidate matrix changed")
    results = {
        str(row["experiment_id"]): r8r7.r8._read_gz(
...
def _source_r8r14_candidates(
    ...
    if (
        ...
        raise ValueError("R8R15 source R8R14 VVVV candidate matrix changed")
    results = {
        str(row["experiment_id"]): r8r7.r8._read_gz(
            stage / "raw" / str(row["partition"]) / f"{row['experiment_id']}.json.gz"
        )
...
def build_specs(ctx: Context) -> list[dict[str, Any]]:
    ...
    for index, source in enumerate(baselines):
        ...
        for candidate_index, code in enumerate(schedule["new_sequences"]):
            ...
            spec.update(
                {
                    "kind": "stage4_2r3c3t13s24d1r14r8r15_binary_temporal_switching_staircase",
                    "stage": STAGE,
                    "campaign_identity": IDENTITY,
                    "controller_revision": CONTROLLER_REVISION,
                    "partition": phase,
                    "experiment_id": f"r8r15_{phase}_{index:02d}_s{candidate_index}_{str(code).lower()}",
                    "source_r8r7_baseline_experiment_id": str(source["experiment_id"]),
                    "r8r15_role": "binary_temporal_switching_staircase",
                    "r8r15_candidate_index": candidate_index,
                    "r8r15_sequence_code": str(code),
                    "r8r15_sequence": sequence,
                    "r8r15_canonical_scale": 1.0,
                    "r8r15_matrix_digest": str(schedule["canonical_matrix_float64_le_c_sha256"]),
                    "r8r15_decision_task_steps": list(map(int, schedule["decision_task_steps"])),
                    "r8r15_allowed_in_expert_dataset": False,
    ...
    if counts != {"safety": 32, "qualification": 96} or len(output) != 128:
        raise ValueError("R8R15 frozen specification matrix changed")
    return output
...
def _payload(ctx: Context, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload.update(
        {
            "variant_id": f"stage4_2r3c3t13s24d1r14r8r15_{experiment_id}",
            "stage4_2r3c3t13s24d1r14r8r15_decision_task_steps": list(
                map(int, spec["r8r15_decision_task_steps"])
            ),
            "stage4_2r3c3t13s24d1r14r8r15_pair_history_label_available_to_controller": False,
        }
    )
...
def _offline_construction(
    ...
    for spec in specs:
        ...
        decisions = tuple(map(int, spec["r8r15_decision_task_steps"]))
        for level, decision in enumerate(decisions, start=1):
            symbol = spec["r8r15_sequence"][level - 1]
            ...
            candidate = mpc.Candidate(
                ...
                direction,
                sign,
                float(spec["r8r15_canonical_scale"]),
            )
    ...
    return {
        ...
        "passed": bool(
            len(rows) == 128
            and sum(len(row["levels"]) for row in rows) == 512
            and sum(
                len(level["refreshes"]) for row in rows for level in row["levels"]
            ) == 2816
...
def prepare_offline(ctx: Context) -> dict[str, Any]:
    if ctx.paths.stage.exists():
        raise ValueError("R8R15 offline requires a fresh run identity")
    ...
    if not preflight["passed"]:
        raise ValueError("R8R15 frozen binary temporal construction preflight failed")
    ...
    _write(ctx.paths.analysis / "offline_binary_temporal_preflight.json", preflight)
    ...
    report = {
        ...
        "lineage_authenticated": True,
        "spec_count": 128,
        "safety_spec_count": 32,
        "qualification_spec_count": 96,
...
def _saved_specs(ctx: Context) -> list[dict[str, Any]]:
    ...
    if (
        ...
        raise ValueError("R8R15 saved identity or specifications changed")
    return specs


def _phase_specs(ctx: Context, phase: str) -> list[dict[str, Any]]:
    rows = [row for row in _saved_specs(ctx) if row["partition"] == phase]
    if len(rows) != {"safety": 32, "qualification": 96}[phase]:
        raise ValueError("R8R15 saved phase coverage changed")
    return rows
...
class BinaryTemporalSwitchingStaircaseController(
    ...
    def __init__(
        ...
        *,
        ...
        sequence = tuple(copy.deepcopy(spec["r8r15_sequence"]))
        ...
        if (
            ...
            raise ValueError("R8R15 frozen binary sequence changed")
        first = sequence[0]
        ...
        self.r8r15_decisions = tuple(map(int, spec["r8r15_decision_task_steps"]))
        self.r8r15_sequence_code = code
        self.r8r15_sequence = sequence
        self.r8r15_contract = copy.deepcopy(dict(controller_cfg))
        self.r8r15_issue_count = 0
        self.r8r15_refresh_count = 0

    def _issue_symbol(
        self, level_index: int, currents: np.ndarray
    ) -> tuple[np.ndarray, dict[str, Any]]:
        symbol = self.r8r15_sequence[level_index]
        direction, sign = int(symbol["direction_index"]), int(symbol["sign"])
        ...
        event.update(
            {
                "sequence_code": self.r8r15_sequence_code,
    ...
    def _refresh(self, currents: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        if self._active_issue is None:
            raise ValueError("R8R15 refresh has no active exact target")
        event = _refresh_event(
            ...
            contract=self.r8r15_contract,
        )
        if not event["passed"]:
            raise ValueError(
                "R8R15 exact target refresh action failed: "
                + json.dumps(event, sort_keys=True)
            )
        self.r8r15_refresh_count += 1
    ...
    def action(self, current_state: Mapping[str, Any]) -> tuple[np.ndarray, dict[str, Any]]:
        if int(current_state["step_index"]) != self.step:
            raise ValueError("R8R15 controller/current task-state index mismatch")
        if self.step < PREFIX_END:
            action, trace = super().action(current_state)
            trace.update(
                {
                    "r3c3t13s24d1r14r8r15_controller_revision": CONTROLLER_REVISION,
                    "r3c3t13s24d1r14r8r15_delegated_prefix": True,
                    "r3c3t13s24d1r14r8r15_event": "none",
                    "r3c3t13s24d1r14r8r15_event_detail": {},
                    "r3c3t13s24d1r14r8r15_forbidden_input_used": False,
                }
            )
        ...
        if self.step in self.r8r15_decisions:
            level_index = self.r8r15_decisions.index(self.step)
            action, event = self._issue_symbol(level_index, currents)
            event_name = "staircase_issue"
            self.r8r15_issue_count += 1
        else:
        ...
        trace.update(
            {
                ...
                "r3c3t13s24d1r14r8r15_controller_revision": CONTROLLER_REVISION,
                "r3c3t13s24d1r14r8r15_delegated_prefix": False,
                "r3c3t13s24d1r14r8r15_event": event_name,
                "r3c3t13s24d1r14r8r15_event_detail": copy.deepcopy(event),
                "r3c3t13s24d1r14r8r15_forbidden_input_used": False,
                "r3c3t13s24d1r14r8r15_pair_or_history_label_used": False,
                "r3c3t13s24d1r14r8r15_future_measurement_used": False,
                "r3c3t13s24d1r14r8r15_future_action_used": False,
                "r3c3t13s24d1r14r8r15_source_result_used": False,
                "r3c3t13s24d1r14r8r15_hidden_wire_used": False,
            }
        )
        return np.asarray(action, dtype=float), trace
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if horizon != int(spec["formal_horizon_steps"]) or horizon != int(
                ...
                raise ValueError("R8R15 formal horizon changed")
            self.base.env.reset()
            ...
            controller = BinaryTemporalSwitchingStaircaseController(
                self.base,
                self.bundle,
                spec,
                trajectory[0],
            ...
            for step in range(horizon):
                ...
                if not finite or bool(current.get("abnormal")):
                    ...
                    raise ValueError("R8R15 pre-action visible state safety gate failed")
                try:
                ...
                if truncated and step + 1 < horizon:
                    ...
                    raise RuntimeError("environment truncated before R8R15 horizon")
            calibration_events = [
            ...
            events = [
                row.get("r3c3t13s24d1r14r8r15_event")
                for row in trace[PREFIX_END:]
            ]
            success = bool(
                ...
                and all(
                    bool(row.get("r3c3t13s24d1r14r8r15_delegated_prefix"))
                    for row in trace[:PREFIX_END]
                )
                and all(
                    not bool(row.get("r3c3t13s24d1r14r8r15_delegated_prefix"))
                    for row in trace[PREFIX_END:]
                )
                ...
                and all(
                    bool(
                        (row.get("r3c3t13s24d1r14r8r15_event_detail") or {}).get(
                            "passed"
                        )
                    )
                    for row in trace[PREFIX_END:]
                )
                and controller.r8r15_issue_count == 4
                and controller.r8r15_refresh_count == horizon - PREFIX_END - 4
            ...
            result.update(
                {
                    ...
                    "failure_reason": ""
                    if success
                    else "incomplete or invalid R8R15 rollout",
                    "execution_failure_class": ""
                    if success
            ...
            if runner is not None:
                runner.cleanup_episode_workspace(
                    failed=failed, reason="stage4_2r3c3t13s24d1r14r8r15"
                )
    ...
    *,
    ...
    def args_for(spec: Mapping[str, Any], index: int) -> tuple[Any, ...]:
        return (
            ...
            library,
            bundle,
            f"stage42r8r15_{phase}_{index:03d}",
            selector,
            lattice,
            calibration,
            dynamic,
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            ...
            print(f"[R8R15 {phase}] {index + 1}/{len(pending)}", flush=True)
        ...
        for start in range(0, len(pending), plan.actor_count):
            ...
            try:
                while refs:
                    ...
                    if not ready:
                        print(
                            f"[R8R15 {phase}] waiting {completed}/{len(pending)}",
                            flush=True,
                        )
                        continue
                    ref = ready[0]
                    ...
                    completed += 1
                    print(f"[R8R15 {phase}] {completed}/{len(pending)}", flush=True)
            finally:
        ...
        raise ValueError(f"unsupported R8R15 backend: {backend}")
    successful = sum(
...
def audit_raw_phase(ctx: Context, phase: str) -> dict[str, Any]:
    ...
    for spec in specs:
        ...
        events = [
            trace[step].get("r3c3t13s24d1r14r8r15_event_detail") or {}
            for step in range(PREFIX_END, horizon)
        ] if full else []
        event_names = [
            trace[step].get("r3c3t13s24d1r14r8r15_event")
            for step in range(PREFIX_END, horizon)
        ] if full else []
        event_exact = bool(
            full
        ...
        forbidden = sum(
            any(
                bool(row.get(key))
                for key in (
                    "r3c3t13s24d1r14r8r15_forbidden_input_used",
                    "r3c3t13s24d1r14r8r15_pair_or_history_label_used",
                    "r3c3t13s24d1r14r8r15_future_measurement_used",
                    "r3c3t13s24d1r14r8r15_future_action_used",
                    "r3c3t13s24d1r14r8r15_source_result_used",
                    "r3c3t13s24d1r14r8r15_hidden_wire_used",
                )
            )
            for row in trace
        )
        ...
        sequence_exact = bool(
            len(issue_events) == 4
            and all(
                event.get("sequence_code") == spec["r8r15_sequence_code"]
                ...
                for level, (event, symbol) in enumerate(
                    zip(issue_events, spec["r8r15_sequence"])
                )
            )
        )
        issue_increment = max(
    ...
    expected = {"safety": 32, "qualification": 96}[phase]
    report = {
        "schema_version": 1,
        "stage": STAGE,
        "phase": phase,
...
def authorize_safety(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R15 offline independent gate failed")
    _set_state(
        ctx,
...
def run_phase(
    ...
    if state.get("phase_status") != required:
        raise ValueError(f"R8R15 {phase} is not authorized")
    if phase == "safety" and any(ctx.paths.phase_raw("qualification").glob("*.json.gz")):
        raise ValueError("R8R15 qualification raw opened before safety authorization")
    specs = _phase_specs(ctx, phase)
...
def repair_safety_report(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R15 immutable safety reporting repair precondition failed")
    raw = [
        r8r7.r8._read_gz(
            ctx.paths.phase_raw("safety") / f"{spec['experiment_id']}.json.gz"
        )
    ...
    if any(result.get("success") for result in raw):
        raise ValueError("R8R15 safety reporting repair is only for structured failures")
...
def authorize_qualification(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R15 safety independent gate failed")
    _set_state(
        ctx,
...
def compute_formal_authority(ctx: Context) -> dict[str, Any]:
        ...
        rows.append(
            _formal_row(
                ...
                result,
                {
                    ...
                    "pair_id": spec["pair_id"],
                    "history_member": spec["history_member"],
                    "sequence_code": str(spec["r8r15_sequence_code"]),
                    "candidate_index": int(spec["r8r15_candidate_index"]),
                },
            )
        )
    ...
    for key, group in sorted(groups.items()):
        ...
        if len(baseline) != 1 or len(candidates) != 10 or members != expected_codes:
            raise ValueError("R8R15 formal context coverage changed")
        base = baseline[0]
        oracle = max(
            [base, *candidates],
            key=lambda row: (
        ...
        contexts.append(
            {
                ...
                "failed_baseline_repaired": bool(
                    ...
                    and any(
                        row["formal_contract_pass"] and row["source"] == "R8R15"
...
def finalize_primary(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R15 raw independent gates incomplete")
    formal = compute_formal_authority(ctx)
    integrity = bool(
        formal["formal_metric_equivalence_passed"]
        and formal["row_count"] == 176
...
def postprocess(ctx: Context) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R8R15 final independent agreement failed")
...
def execute(
    ...
    raise ValueError(f"unsupported R8R15 command: {command}")
