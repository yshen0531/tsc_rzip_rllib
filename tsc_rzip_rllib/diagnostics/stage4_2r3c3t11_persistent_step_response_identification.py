#!/usr/bin/env python3
"""Stage4.2R3c3T11 persistent transport/braking step identification."""
...
CONTROLLER_REVISION = "persistent_step_response_probe_v42r3c3t11_v1"
PACKAGE_REVISION = "r42r3c3t11_persistent_step_response_identification_v1"
RUN_NAME = "stage4_2r3c3t11_persistent_step_response_identification"
OBSERVATION_HORIZON = 50
BASELINE_PROBE_ID = "persistent_step_baseline"
PROBE_IDS = (
    "persistent_transport_step_mode0",
    "persistent_transport_step_mode1",
    "persistent_transport_step_mode2",
    "persistent_braking_step_mode0",
    "persistent_braking_step_mode1",
    "persistent_braking_step_mode2",
)
N_MODES = t1.N_MODES
N_COILS = t1.N_COILS
N_WIRES = t1.N_WIRES
...
class Stage42R3C3T11Context:
    ...
    paths: Stage42R3C3T11Paths


def _paths(run_dir: Path) -> Stage42R3C3T11Paths:
    root = run_dir.expanduser().resolve()
    control = root / "stage4_2r3c3t11_persistent_step_response_identification"
    return Stage42R3C3T11Paths(
        run_dir=root,
        control=control,
        raw=control / "raw",
        variants=root / "stage4_2r3c3t11_environment_variants",
        source_reference=root / "stage4_2r3c3t11_source_reference",
        analysis=root / "stage4_2r3c3t11_analysis",
        manifest=root / "stage4_2r3c3t11_manifest.json",
        state=root / "stage4_2r3c3t11_state.json",
    )
...
def _validate_config(cfg: Mapping[str, Any]) -> None:
    if (
        ...
        raise ValueError("Stage4.2R3c3T11 identity changed")
    probe = cfg["identification_probe"]
    matrix = cfg["control_matrix"]
    central = probe["central_symmetry"]
    ...
    if (
        ...
        or int(probe["required_issue_count_per_signed_probe"]) != 7
        ...
        or int(probe["transport_first_effect_state"]) != 3
        or int(probe["braking_first_effect_state"]) != 17
        or int(probe["response_condition_start_state"]) != 3
        or int(probe["required_response_rank"]) != 6
        or float(probe["maximum_response_velocity_condition_number"])
        != 25.0
        or dict(probe["formal_prefix_by_slew"])
        != {"1.0": 35, "0.9": 37}
        or not bool(probe["require_requested_and_applied_zero_net"])
        or float(probe["zero_net_atol"]) != 1.0e-12
        or float(
            central["maximum_even_velocity_rmse_m_per_s"]
        )
        != 0.004
        or float(central["maximum_even_position_rmse_m"]) != 0.0005
        or float(central["maximum_even_ip_rmse_A"]) != 20.0
        or float(
            history["maximum_odd_velocity_rmse_m_per_s"]
        )
        != 0.006
        or float(history["maximum_odd_position_rmse_m"]) != 0.001
        or float(history["maximum_odd_ip_rmse_A"]) != 40.0
        ...
        raise ValueError("Stage4.2R3c3T11 probe contract changed")
    if (
        int(matrix["expected_rollouts"]) != 416
        or int(matrix["expected_signed_probe_rollouts"]) != 384
        or int(matrix["expected_extended_baseline_rollouts"]) != 32
        or int(matrix["expected_rollouts_per_baseline_context"]) != 13
        or int(matrix["expected_response_groups"]) != 192
        or int(matrix["expected_matched_hidden_history_groups"]) != 96
        or int(matrix["expected_condition_number_groups"]) != 32
        or list(matrix["targets"]) != ["nominal", "RZ_p10_m10"]
        or list(matrix["history_members"])
        != ["plus_first", "minus_first"]
        or int(matrix["probe_basis_count"]) != 6
        ...
        raise ValueError("Stage4.2R3c3T11 task matrix changed")
    ...
    if (
        ...
        raise ValueError("Stage4.2R3c3T11 formal timing changed")
    if (
...
def _authenticate_preflight(
    ...
    if not path.is_file() or _sha256(path) != str(required["sha256"]):
        raise ValueError("T11 candidate preflight fingerprint mismatch")
    ...
    if (
        ...
        or not bool(payload["real_identification_implementation_authorized"])
        or bool(payload["controller_implementation_authorized"])
        or str(payload["design_config_sha256"])
        != str(required["design_config_sha256"])
        or len(payload["actuator_cases"]) != 2
    ):
        raise ValueError("T11 candidate preflight content mismatch")
    contract = payload["prospective_real_identification_contract"]
    if (
        int(contract["context_count"]) != 32
        or int(contract["extended_baseline_count"]) != 32
        or int(contract["probe_basis_count"]) != 6
        or int(contract["probe_sign_count"]) != 2
        or int(contract["signed_probe_count"]) != 384
        or int(contract["total_task_count"]) != 416
        or int(contract["required_response_rank"]) != 6
        or float(contract["maximum_response_velocity_condition_number"])
        != 25.0
        or bool(contract["formal_timing_changed"])
        or bool(contract["formal_tracking_is_identification_gate"])
    ):
        raise ValueError("T11 prospective identification contract mismatch")
    for case in payload["actuator_cases"]:
        if (
            not bool(case["passed"])
            or not bool(case["bounded_and_zero_net"])
            or int(case["existing_schedule_rank"])
            != int(required["required_existing_schedule_rank"])
            or int(case["new_schedule_rank"])
            != int(required["required_new_schedule_rank"])
            or
            ...
            or float(
                case[
                    "minimum_new_column_residual_norm_outside_existing_span"
                ]
            )
            < float(required["minimum_new_column_residual_norm"])
            or len(case["probe_schedules"]) != 6
        ):
            raise ValueError("T11 preflight actuator gate mismatch")
        for probe in case["probe_schedules"]:
            if (
                probe["probe_id"] not in PROBE_IDS
                or int(probe["nonzero_issue_count"]) != 7
                or len(probe["positive_schedule_by_task_issue_step"])
                != int(required["required_issue_count_per_signed_probe"])
                or int(probe["mode_index"]) not in {0, 1, 2}
                or int(probe["first_effect_state"]) not in {3, 17}
                ...
                raise ValueError("T11 frozen candidate schedule mismatch")
    return path, payload


def load_stage42r3c3t11_config(
    config_path: Path,
    *,
    ...
    if (
        ...
        raise ValueError("T11 frozen T2 base config mismatch")
    ...
    expected_bank_sha = str(
        preflight["source_hashes"]["t3_eight_basis_controller_bank"]
    )
    if not bank.is_file() or _sha256(bank) != expected_bank_sha:
        raise ValueError("T11 source T3 controller bank mismatch")
    return Stage42R3C3T11Context(
        cfg=cfg,
...
def _preflight_case(
    ctx: Stage42R3C3T11Context, *, delay: int, slew: float
) -> Mapping[str, Any]:
    matches = [
        case
    ...
    if len(matches) != 1:
        raise ValueError("T11 preflight actuator selection mismatch")
    return matches[0]
...
def _signed_schedule(
    ...
    if int(sign) not in {-1, 1}:
        raise ValueError("T11 signed schedule sign changed")
    ...
    for issue_step, values in probe[
        ...
        if (
            ...
            raise ValueError("T11 issue schedule invalid")
        schedule[issue_step] = value
    if len(schedule) != 7:
        raise ValueError("T11 issue count changed")
    net = np.sum(np.stack(list(schedule.values())), axis=0)
    if not np.allclose(net, np.zeros(N_MODES), rtol=0.0, atol=1e-12):
        raise ValueError("T11 signed schedule is not zero net")
    return schedule


def build_control_specs(
    ctx: Stage42R3C3T11Context,
    ...
    if len(templates) != 32:
        raise ValueError("T11 source context coverage mismatch")
    ...
    for key in sorted(templates):
        ...
        if set(probes) != set(PROBE_IDS):
            raise ValueError("T11 probe ID set changed")
        ...
        for probe_id, sign, schedule, frozen in members:
            ...
            spec.update(
                {
                    "kind": (
                        "stage4_2r3c3t11_persistent_step_response_"
                        "identification"
                    ),
                    "stage": STAGE,
                    "controller_revision": CONTROLLER_REVISION,
                    ...
                    "phase": "persistent_step_response_identification",
                    "category": "persistent_step_response_identification",
                    "environment_variant": f"stage4_2r3c3t11_{experiment_id}",
                    "horizon_steps": OBSERVATION_HORIZON,
                    "formal_horizon_steps": _formal_horizon(slew),
                    "identification_only": True,
                    "r3c3_probe_id": probe_id,
                    "r3c3_probe_mode": (
                        -1 if frozen is None else int(frozen["mode_index"])
                    ),
                    "r3c3_probe_sign": sign,
                    "r3c3_probe_first_effect_state": (
                        -1
                        if frozen is None
                        else int(frozen["first_effect_state"])
                    ),
                    ...
                    "r3c3t11_schedule_contract": (
                        "persistent_incremental_current_step_then_"
                        "effect_states_39_to_44_zero_net_v1"
                    ),
                    "candidate_preflight_sha256": _sha256(
                        ctx.preflight_path
                    ),
    ...
    if (
        ...
        raise ValueError("T11 control identity coverage mismatch")
    return specs
...
class PersistentStepResponseProbeController(
    t1.LongSeparationTransportProbeController
):
    """Exact R3c1 controller plus one frozen T11 persistent-step schedule."""
    ...
    def __init__(
        ...
        if baseline:
            ...
            actual_effects = sorted(
                int(issue_step) + delay + 1
                for issue_step in self.probe_schedule
            )
            valid = bool(
                self.probe_id in PROBE_IDS
                and self.probe_mode in {0, 1, 2}
                and self.probe_id.endswith(f"mode{self.probe_mode}")
                and self.probe_sign in {-1, 1}
                and self.probe_first_effect_state in {3, 17}
                and actual_effects == expected_effects
                and len(self.probe_schedule) == 7
                and max(
        ...
        if not valid:
            raise ValueError("Stage4.2R3c3T11 controller contract invalid")
        if self.probe_schedule:
            ...
            if not np.allclose(
                ...
                raise ValueError("Stage4.2R3c3T11 probe is not zero net")
    ...
    def action(
        ...
        trace.update(
            {
                "r3c3t11_identification_only": True,
                "r3c3t11_observation_horizon_steps": OBSERVATION_HORIZON,
                "r3c3t11_post_contract_neutralization": True,
            }
        )
        return action, trace
...
def _control_payload(
    ctx: Stage42R3C3T11Context, *, spec: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    payload["variant_id"] = f"stage4_2r3c3t11_{experiment_id}"
    payload["stage4_2r3c3t11_restart_snapshot_dir"] = str(
        spec["restart_snapshot_dir"]
    )
    payload["stage4_2r3c3t11_snapshot_manifest_digest"] = str(
...
class LocalPersistentStepProbeWorker:
    def __init__(
        self,
    ...
    def evaluate(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                raise ValueError("T11 observation/formal horizon changed")
            self.base.env.reset()
            ...
            trajectory = [initial]
            controller = PersistentStepResponseProbeController(
            ...
            for step in range(horizon):
                ...
                if truncated and step + 1 < horizon:
                    raise RuntimeError(
                        "environment truncated before T11 horizon"
                    )
            schedule = {
            ...
            expected_count = 0 if baseline else 7
            schedule_exact = bool(
            ...
            result.update(
                {
                    ...
                    "completed": True,
                    "failure_reason": (
                        "" if success else "incomplete or invalid T11 rollout"
                    ),
...
def _control_ray_actor_class():
    ...
    if _CONTROL_RAY_ACTOR is None:
        ...
        class Stage42R3C3T11ControlActor:
            def __init__(
                self, payload, library, bundle, worker_id, selector_cfg
            ):
                self.worker = LocalPersistentStepProbeWorker(
                    payload, library, bundle, worker_id, selector_cfg
                )
        ...
        _CONTROL_RAY_ACTOR = Stage42R3C3T11ControlActor
    ...
    *,
    ...
    if backend == "serial":
        for index, spec in enumerate(pending):
            worker = LocalPersistentStepProbeWorker(
                payloads[str(spec["experiment_id"])],
                library,
                bundle,
                f"stage42r3c3t11_serial_{index:03d}",
                selector,
            )
            try:
            ...
            print(f"[Stage4.2R3c3T11] {index + 1}/{len(pending)}", flush=True)
...
def _phase_trace_valid(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for row in trace:
        ...
        exact = bool(
            ...
            and bool(row.get("r3c3t11_identification_only"))
        )
        if issued:
            issued_count += 1
...
def _source_prefix_exact(
    ctx: Stage42R3C3T11Context, result: Mapping[str, Any]
) -> bool:
...
def summarize_control(
    ...
    *,
    ...
    if len(baselines) != 32:
        raise ValueError("T11 baseline coverage mismatch")
    ...
    response_condition_rows = []
    response_max = float(
        probe_cfg["maximum_response_velocity_condition_number"]
    )
    for key in sorted(new_by_context):
        ...
        rank, condition, passed = _condition_row(
            arrays,
            expected_rank=int(probe_cfg["required_response_rank"]),
            maximum=response_max,
        )
        response_condition_rows.append(
            {
                "pair_id": key[0],
                ...
                "response_velocity_condition_number": condition,
    ...
    summary = {
        ...
        "expected_response_group_count": 192,
        "central_symmetry_pass_count": count(
            response_rows, "central_symmetry_pass"
        ),
        "matched_hidden_history_group_count": len(history_rows),
        "expected_matched_hidden_history_group_count": 96,
        "matched_hidden_history_pass_count": count(
            history_rows, "matched_hidden_history_pass"
        ),
        "condition_number_group_count": len(response_condition_rows),
        "expected_condition_number_group_count": 32,
        "condition_number_pass_count": count(
            response_condition_rows, "condition_number_pass"
        ),
        "maximum_response_velocity_condition_number": max(
            (
                float(row["response_velocity_condition_number"])
                for row in response_condition_rows
                if row["response_velocity_condition_number"] is not None
            ),
            default=None,
        ),
    ...
    summary["central_symmetry_gate_passed"] = bool(
        len(response_rows) == summary["central_symmetry_pass_count"] == 192
    )
    ...
    if write_outputs:
        outputs = (
            ...
            ("condition_number_results", response_condition_rows),
        )
...
def _package_files(ctx: Stage42R3C3T11Context) -> list[Path]:
    ...
    names = [
        ...
        (
            "configs/stage4_2r3c3t11_persistent_step_response_"
            "identification_500ms.json"
        ),
        str(ctx.cfg["candidate_preflight"]["path"]),
        (
            "tsc_rzip_rllib/diagnostics/stage4_2r3c3t11_persistent_"
            "step_response_identification.py"
        ),
        (
            "scripts/stage4_2r3c3t11_persistent_step_response_"
            "identification.py"
        ),
        "scripts/stage4_2r3c3t11_server_postprocess.py",
        "scripts/stage4_2r3c3t11_shell_common.sh",
        "run_stage4_2r3c3t11_common.sh",
        "run_stage4_2r3c3t11_offline.sh",
        "run_stage4_2r3c3t11_server_postprocess.sh",
        (
            "run_stage4_2r3c3t11_persistent_step_response_"
        ...
        "run_stage4_2r3c3t11_self_test.sh",
        "run_stage4_2r3c3t11_identification_verify_package.sh",
        "run_stop_stage4_2r3c3t11_now.sh",
    ]
...
def _semantics_preserving_resume_compatibility(
    ...
    """Require byte-identical deployed source identity on resume."""

    if dict(original) != dict(active):
        raise ValueError(
            "Stage4.2R3c3T11 resume requires the exact deployed package "
            "fingerprint"
        )
    return None


def _prepare_dirs(paths: Stage42R3C3T11Paths) -> None:
    for path in (
        paths.run_dir,
        paths.control,
        paths.raw,
...
def prepare(
    ctx: Stage42R3C3T11Context, *, resume: bool
    ...
    if ctx.paths.manifest.is_file():
        if not resume:
            raise FileExistsError(
                "Stage4.2R3c3T11 run exists; use resume or a fresh run"
            )
        ...
        for key, value in manifest.items():
            ...
            if old.get(key) != value:
                raise ValueError(
                    f"T11 resume incompatibility in manifest field {key}"
                )
    ...
    t1.r3c3.atomic_write_json(
        ctx.paths.run_dir / "stage4_2r3c3t11_config.resolved.json", ctx.cfg
    )
    t1.r3c3.atomic_write_json(
    ...
    *,
    ...
    for index, key in enumerate(sorted(grouped)):
        ...
        plant = t1.r1.LocalPlantReplayWorker(
            ...
            library,
            bundle,
            f"stage42r3c3t11_offline_{index:03d}",
            selector,
        )
        try:
            for spec in context_specs:
                ...
                hidden_controller = PersistentStepResponseProbeController(
    ...
    summary = {
        "schema_version": SCHEMA_VERSION,
        "stage": STAGE,
        "phase": "offline_persistent_step_probe_audit",
    ...
    summary["passed"] = bool(
        ...
        and summary["signed_probe_spec_count"] == 384
    ...
    t1.r3c3.atomic_write_json(
        ctx.paths.source_reference / "offline_persistent_step_audit.json",
        summary,
    )
    return summary
...
def analyze(
    ...
    verdict = {
        ...
        "stage": STAGE,
        "verdict": (
            "STAGE4_2R3C3T11_PERSISTENT_STEP_IDENTIFICATION_PASS"
            if primary_pass
            else "STAGE4_2R3C3T11_PERSISTENT_STEP_IDENTIFICATION_FAIL"
        ),
        ...
        "next_if_pass": (
            "Build an authenticated six-basis persistent-step response "
            "bank and require prospective nonlinear local transport-MPC "
            "feasibility before any R3c4 controller implementation."
        ),
        "next_if_fail": (
            "Preserve raw and redesign time-localized plant-response "
            "identification without weakening timing or response gates."
        ),
    }
    t1.r3c3.atomic_write_json(
        ctx.paths.analysis / "stage4_2r3c3t11_summary.json",
        dict(control_summary),
    )
    t1.r3c3.atomic_write_json(
        ctx.paths.analysis / "stage4_2r3c3t11_verdict.json", verdict
    )
...
def execute(
    ...
    *,
    ...
    if not bool(offline.get("passed")):
        raise RuntimeError(
            "Stage4.2R3c3T11 offline gate failed; no real TSC started"
        )
    if command == "offline":
        state = t1.r3c3.read_json(ctx.paths.state)
        state.update(
            ...
            probes.append(
                {
                    ...
                    "mode_index": int(probe["mode_index"]),
                    "cancellation_effect_states": effects[1:],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage4.2R3c3T11 persistent-step identification"
    )
    ...
    ctx = load_stage42r3c3t11_config(
        args.config,
