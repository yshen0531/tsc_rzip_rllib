...
export STAGE2_RUN_DIR="${STAGE2_RUN_DIR:-stage2_runs/stage2_svd3_real_tsc_cem_100ms_$(date +%Y%m%d_%H%M%S)}"
LOG="logs/nohup/stage2_one_generation_$(date +%Y%m%d_%H%M%S).log"
...
nohup setsid env STAGE2_RUN_DIR="$STAGE2_RUN_DIR" SOURCE_STAGE1_1_RUN="${SOURCE_STAGE1_1_RUN:-}" \
  ...
  ./run_stage2_one_generation_native.sh >"$LOG" 2>&1 &
echo $! > stage2_runs/stage2_driver.pid
ln -sfn "$(basename "$LOG")" logs/nohup/latest_stage2_one_generation.log
