...
REV="${ID2Z33_SOURCE_REVISION:-}"
OUT="${ID2Z33_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z33_corrected_moving_center_d0.json"
fail() { printf 'ID2Z33_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }
test -n "$REV" || fail "ID2Z33_SOURCE_REVISION is required"
test -n "$OUT" || fail "ID2Z33_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z33_corrected_moving_center_d0.py \
  --config "$CONFIG" --source-revision "$REV" --output "${OUT}.offline.json" --offline || exit 2
set +e
python scripts/rgeo_zgeo_1ms_id2z33_corrected_moving_center_d0.py \
...
if test -f "$OUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z33_corrected_moving_center_d0_independent.py \
