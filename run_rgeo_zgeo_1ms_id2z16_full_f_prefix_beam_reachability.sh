...
SOURCE_REVISION="${ID2Z16_SOURCE_REVISION:-}"
OUTPUT="${ID2Z16_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z16_full_f_prefix_beam_reachability.json"

fail() { printf 'ID2Z16_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z16_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z16_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z16_full_f_prefix_beam_reachability.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z16_full_f_prefix_beam_reachability_independent.py \
