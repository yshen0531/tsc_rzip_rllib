...
command_name="${1:-}"
revision="${ID2R1_SOURCE_REVISION:-}"
output_dir="${ID2R1_OUTPUT_DIR:-}"
config="${PROJECT}/configs/rgeo_zgeo_1ms_id2r1_f03_exact_replay.json"

case "${command_name}" in
  offline)
    [[ -n "${revision}" ]] || { printf 'ERROR: set ID2R1_SOURCE_REVISION\n' >&2; exit 2; }
    python scripts/rgeo_zgeo_1ms_id2r1_f03_exact_replay.py offline \
      --stage-config "${config}" --source-revision "${revision}"
    ;;
  run)
    [[ -n "${revision}" && -n "${output_dir}" ]] || {
      printf 'ERROR: set ID2R1_SOURCE_REVISION and ID2R1_OUTPUT_DIR\n' >&2; exit 2
    }
    python scripts/rgeo_zgeo_1ms_id2r1_f03_exact_replay.py run \
  ...
  independent)
    ...
    python scripts/rgeo_zgeo_1ms_id2r1_f03_exact_replay_independent.py \
      --stage-config "${config}" --source-revision "${revision}" \
