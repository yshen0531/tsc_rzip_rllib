...
source "${PROJECT_DIR}/scripts/stage4_2r3c3t13s24_shell_common.sh"
...
for path in "${SOURCE_S21_RUN}" "${SOURCE_S23R1_OUTPUT}" "${SOURCE_R3B}" "${SOURCE_R3C3}" "${SOURCE_BANK}" "${SOURCE_T1}" "${SOURCE_T1_AUDIT}" "${Q1_RUN}" "${Q2_RUN}"; do
...
RUN_DIR="${STAGE4_2R3C3T13S24_RUN_DIR:-$(stage4_2r3c3t13s24_new_run_dir)}"
STATE_FILE="${RUN_DIR}/stage4_2r3c3t13s24_sequential_transition_identification/stage_state.json"
if [[ "${STAGE4_2R3C3T13S24_COMMAND}" == offline ]]; then
  [[ "${STAGE4_2R3C3T13S24_RESUME}" == 0 ]] || { echo "ERROR: offline phase cannot resume" >&2; exit 1; }
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]] || { echo "ERROR: fresh run directory not empty" >&2; exit 1; }
else
  [[ -f "${STATE_FILE}" ]] || { echo "ERROR: S24 frozen state missing" >&2; exit 1; }
fi
mkdir -p "${RUN_DIR}"
stage4_2r3c3t13s24_export_runtime
printf '[T13S24] user=%s host=%s pwd=%s\n' "$(id -un)" "$(hostname)" "$(pwd)"
printf '[T13S24] run_dir=%s command=%s backend=%s resume=%s workers=%s\n' "${RUN_DIR}" "${STAGE4_2R3C3T13S24_COMMAND}" "${STAGE4_2R3C3T13S24_BACKEND}" "${STAGE4_2R3C3T13S24_RESUME}" "${STAGE4_2R3C3T13S24_WORKERS}"
printf '[T13S24] frozen phased matrix: training 600, calibration 200, holdout 200; total 1000.\n'
printf '[T13S24] formal 250/270 ms arrival and 350/370 ms hold timing is unchanged.\n'
cd "${PROJECT_DIR}"
args=(
  --config "${STAGE4_2R3C3T13S24_CONFIG}" --source-s21-run "${SOURCE_S21_RUN}" --source-s23r1-output "${SOURCE_S23R1_OUTPUT}"
  ...
  --run-dir "${RUN_DIR}" --command "${STAGE4_2R3C3T13S24_COMMAND}" --backend "${STAGE4_2R3C3T13S24_BACKEND}"
)
[[ "${STAGE4_2R3C3T13S24_RESUME}" == 1 ]] && args+=(--resume)
exec "${STAGE4_2R3C3T13S24_PYTHON}" scripts/stage4_2r3c3t13s24_sequential_transition_identification.py "${args[@]}"
