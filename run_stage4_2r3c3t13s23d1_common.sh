...
source "${PROJECT_DIR}/scripts/stage4_2r3c3t13s23d1_shell_common.sh"
...
OUTPUT_DIR="${STAGE4_2R3C3T13S23D1_OUTPUT_DIR:-$(stage4_2r3c3t13s23d1_new_output_dir)}"
[[ ! -e "${OUTPUT_DIR}" ]] || {
  echo "ERROR: S23D1 output must be a new path: ${OUTPUT_DIR}" >&2
  exit 1
}
stage4_2r3c3t13s23d1_export_runtime
printf '[Stage4.2R3c3T13S23D1] user=%s host=%s pwd=%s\n' "$(id -un)" "$(hostname)" "$(pwd)"
printf '[Stage4.2R3c3T13S23D1] source_s21_run=%s\nsource_s22_output=%s\nsource_s23_output=%s\noutput=%s\n' \
  "${SOURCE_S21_RUN}" "${SOURCE_S22_OUTPUT}" "${SOURCE_S23_OUTPUT}" "${OUTPUT_DIR}"
printf '[Stage4.2R3c3T13S23D1] adaptive bounded search: zero plant/controller/TSC execution.\n'
printf '[Stage4.2R3c3T13S23D1] A candidate authorizes only a separately frozen S23R1 preflight.\n'
cd "${PROJECT_DIR}"
exec "${STAGE4_2R3C3T13S23D1_PYTHON}" \
  docs/codex/audit_tools/stage4_2r3c3t13s23d1_bounded_schedule_redesign_search.py \
  --config "${STAGE4_2R3C3T13S23D1_CONFIG}" \
  --design-document "${STAGE4_2R3C3T13S23D1_DESIGN}" \
