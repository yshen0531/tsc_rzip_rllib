...
PYTHON="${STAGE4_2R3C3T13S24D1R14R8R16_PYTHON:-/home/yangshen0711/tsc_all/tsc_simulation/venv_simu/bin/python}"
CONFIG="${STAGE4_2R3C3T13S24D1R14R8R16_CONFIG:-${PROJECT_DIR}/configs/stage4_2r3c3t13s24d1r14r8r16_temporal_affine_binary_cube_completion_preflight.json}"
COMMAND="${STAGE4_2R3C3T13S24D1R14R8R16_COMMAND:-primary}"
OUTPUT_ROOT="${STAGE4_2R3C3T13S24D1R14R8R16_OUTPUT_ROOT:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r16_runs}"
R8R7_RUN="${STAGE4_2R3C3T13S24D1R14R8R16_R8R7_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r7_runs/stage4_2r3c3t13s24d1r14r8r7_fresh_multipulse_static_observer_interaction_sentinel_20260807_d8d231e_v1}"
R8R12_RUN="${STAGE4_2R3C3T13S24D1R14R8R16_R8R12_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r12_runs/stage4_2r3c3t13s24d1r14r8r12_causal_cumulative_direction2_staircase_authority_sentinel_20260808_523c706_v2}"
R8R14_RUN="${STAGE4_2R3C3T13S24D1R14R8R16_R8R14_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r14_runs/stage4_2r3c3t13s24d1r14r8r14_cumulative_multidirection_staircase_authority_identification_20260808_9874068_v1}"
R8R15_RUN="${STAGE4_2R3C3T13S24D1R14R8R16_R8R15_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r15_runs/stage4_2r3c3t13s24d1r14r8r15_binary_temporal_switching_staircase_authority_sentinel_20260808_f23c96e_v1}"
...
case "${COMMAND}" in
  ...
  *) echo "ERROR: unsupported R8R16 command ${COMMAND}" >&2; exit 1 ;;
esac
...
if [[ -n "${STAGE4_2R3C3T13S24D1R14R8R16_RUN_DIR:-}" ]]; then
  RUN_DIR="${STAGE4_2R3C3T13S24D1R14R8R16_RUN_DIR}"
else
  mkdir -p "${OUTPUT_ROOT}"
  RUN_DIR="${OUTPUT_ROOT}/stage4_2r3c3t13s24d1r14r8r16_temporal_affine_binary_cube_completion_preflight_$(date -u +%Y%m%d_%H%M%S)"
fi
STAGE_DIR="${RUN_DIR}/stage4_2r3c3t13s24d1r14r8r16_temporal_affine_binary_cube_completion_preflight"
if [[ "${COMMAND}" == primary ]]; then
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]
else
  [[ -f "${STAGE_DIR}/stage_state.json" ]] || { echo "ERROR: R8R16 frozen state missing" >&2; exit 1; }
fi
mkdir -p "${RUN_DIR}"
...
cd "${PROJECT_DIR}"
printf '[R8R16] user=%s host=%s command=%s run_dir=%s\n' "$(id -un)" "$(hostname)" "${COMMAND}" "${RUN_DIR}"
printf '[R8R16] zero new TSC; all source trajectories remain forbidden from expert and learning data.\n'

args=(
  --config "${CONFIG}" --run-dir "${RUN_DIR}" --r8r15-run "${R8R15_RUN}"
...
if [[ "${COMMAND}" == independent ]]; then
  exec "${PYTHON}" docs/codex/audit_tools/stage4_2r3c3t13s24d1r14r8r16_independent_forensics.py "${args[@]}"
fi
exec "${PYTHON}" -m tsc_rzip_rllib.diagnostics.stage4_2r3c3t13s24d1r14r8r16_temporal_affine_binary_cube_completion_preflight "${args[@]}" --command "${COMMAND}"
