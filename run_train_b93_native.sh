...
echo "[run_train_b93_native] limit before set: ulimit -u=$(ulimit -u) ulimit -n=$(ulimit -n)"
ulimit -u 30000 || true
ulimit -n 4096 || true
echo "[run_train_b93_native] limit after set : ulimit -u=$(ulimit -u) ulimit -n=$(ulimit -n)"
cat /proc/$$/limits | egrep "processes|open files" || true

if [[ -n "${RESUME}" ]]; then
  if [[ ! -f "${RESUME}/mpo_checkpoint.pt" ]]; then
    echo "[run_train_b93_native] ERROR: resume checkpoint not found: ${RESUME}/mpo_checkpoint.pt" >&2
    exit 2
  fi
fi
...
if [[ -z "${RAY_TMPDIR:-}" ]]; then
  export RAY_TMPDIR="/tmp/ray_${USER}_b93_${RANDOM}"
fi
mkdir -p "${RAY_TMPDIR}"
...
echo "[run_train_b93_native] CONFIG=${CONFIG}"
echo "[run_train_b93_native] OVERRIDE=${OVERRIDE}"
echo "[run_train_b93_native] RESUME=${RESUME}"
echo "[run_train_b93_native] RAY_TMPDIR=${RAY_TMPDIR}"
echo "[run_train_b93_native] CMD=${cmd[*]}"
exec "${cmd[@]}"
