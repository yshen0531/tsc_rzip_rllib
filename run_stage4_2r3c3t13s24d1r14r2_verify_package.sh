...
PYTHON_BIN="${STAGE4_2R3C3T13S24D1R14R2_PYTHON:-/home/yangshen0711/tsc_all/tsc_simulation/venv_simu/bin/python}"
...
required = {
    ...
    "run_stage4_2r3c3t13s24d1r14r2_common.sh",
    "run_stage4_2r3c3t13s24d1r14r2_native.sh",
    "run_stage4_2r3c3t13s24d1r14r2_nohup.sh",
    "run_stage4_2r3c3t13s24d1r14r2_self_test.sh",
    "run_stage4_2r3c3t13s24d1r14r2_verify_package.sh",
    "run_stop_stage4_2r3c3t13s24d1r14r2_now.sh",
    "scripts/stage4_2r3c3t13s24d1r14r2_shell_common.sh",
    "scripts/stage4_2r3c3t13s24d1r14r2_mixed_basis_signed_excitation_sentinel.py",
    "tests/test_stage4_2r3c3t13s24d1r14r2_mixed_basis_signed_excitation_sentinel.py",
...
if not required.issubset(set(listed)):
    raise SystemExit(f"D1R14R2 required package files missing: {sorted(required - set(listed))}")
...
config = root / "configs/stage4_2r3c3t13s24d1r14r2_mixed_basis_signed_excitation_sentinel_370ms.json"
cfg = json.loads(config.read_text(encoding="utf-8"))
d1r14r2._validate_config(cfg, config)
if not d1r14r2.self_test(config)["passed"]:
    raise SystemExit("D1R14R2 self-test failed")
for path in (
    root / "run_stage4_2r3c3t13s24d1r14r2_native.sh",
    root / "run_stop_stage4_2r3c3t13s24d1r14r2_now.sh",
):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[T13S24D1R14R2 verify] Python/JSON/scientific guards passed.")
PY
mapfile -t SHELLS < <(find . -maxdepth 2 -type f -name '*.sh' -print | sort)
for script in "${SHELLS[@]}"; do bash -n "${script}"; done
"${PYTHON_BIN}" -m unittest -v \
  tests.test_stage4_2r3c3t13s24d1r14r2_mixed_basis_signed_excitation_sentinel
printf '[T13S24D1R14R2 verify] checksums, shell syntax and focused tests passed.\n'
