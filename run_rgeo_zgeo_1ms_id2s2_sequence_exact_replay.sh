...
command_name="${1:-}"
revision="${ID2S2_SOURCE_REVISION:-}"
output_dir="${ID2S2_OUTPUT_DIR:-}"
config="${PROJECT}/configs/rgeo_zgeo_1ms_id2s2_sequence_exact_replay.json"
[[ -n "${revision}" && -n "${output_dir}" ]] || {
  printf 'ERROR: set ID2S2_SOURCE_REVISION and ID2S2_OUTPUT_DIR\n' >&2; exit 2
}

case "${command_name}" in
  offline)
    python scripts/rgeo_zgeo_1ms_id2s2_sequence_exact_replay.py offline \
      --stage-config "${config}" --source-revision "${revision}"
    ;;
  run)
    python scripts/rgeo_zgeo_1ms_id2s2_sequence_exact_replay.py run \
      --stage-config "${config}" --source-revision "${revision}" \
      --output "${output_dir}"
    ;;
  independent)
    python scripts/rgeo_zgeo_1ms_id2s2_sequence_exact_replay_independent.py \
      --stage-config "${config}" --source-revision "${revision}" \
