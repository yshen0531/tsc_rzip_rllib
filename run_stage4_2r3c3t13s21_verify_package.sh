...
PYTHON_BIN="${STAGE4_2R3C3T13S21_PYTHON:-/home/yangshen0711/tsc_all/tsc_simulation/venv_simu/bin/python}"
...
from tsc_rzip_rllib.diagnostics import stage4_2r3c3t13s21_cumulative_exact_card15_pooled_observer_campaign as s21
...
if not s21.self_test(config)["passed"]:
    raise SystemExit("T13S21 self-test failed")
for path in (root / "run_stage4_2r3c3t13s21_native.sh", root / "run_stop_stage4_2r3c3t13s21_now.sh"):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[T13S21 verify] Python/JSON/scientific guards passed.")
PY
mapfile -t SHELLS < <(find . -maxdepth 2 -type f -name '*.sh' -print | sort)
for script in "${SHELLS[@]}"; do bash -n "${script}"; done
"${PYTHON_BIN}" -m unittest -v tests.test_stage4_2r3c3t13s21_cumulative_exact_card15_pooled_observer_campaign
printf '[T13S21 verify] checksums, shell syntax and focused tests passed.\n'
