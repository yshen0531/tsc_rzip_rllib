...
SOURCE_D1R13_AUDIT="$(stage4_2r3c3t13s24d1r14r2_find_source_d1r13_audit)"
...
for path in "${SOURCE_R1A_OUTPUT}" "${SOURCE_D1R13_RUN}" "${SOURCE_D1R11_RUN}" "${SOURCE_S21_RUN}" "${SOURCE_S23R1_OUTPUT}" "${SOURCE_S24_RUN}" "${SOURCE_D1R9_V1}" "${SOURCE_D1R9_V2}" "${SOURCE_D1R10_RUN}" "${SOURCE_R3B}" "${SOURCE_R3C3}" "${SOURCE_BANK}" "${SOURCE_T1}" "${SOURCE_T1_AUDIT}" "${Q1_RUN}" "${Q2_RUN}"; do
...
RUN_DIR="${STAGE4_2R3C3T13S24D1R14R2_RUN_DIR:-$(stage4_2r3c3t13s24d1r14r2_new_run_dir)}"
STATE_FILE="${RUN_DIR}/stage4_2r3c3t13s24d1r14r2_mixed_basis_signed_excitation_sentinel/stage_state.json"
if [[ "${STAGE4_2R3C3T13S24D1R14R2_COMMAND}" == offline ]]; then
  [[ "${STAGE4_2R3C3T13S24D1R14R2_RESUME}" == 0 ]] || { echo "ERROR: offline phase cannot resume" >&2; exit 1; }
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]] || { echo "ERROR: fresh run directory not empty" >&2; exit 1; }
else
  [[ -f "${STATE_FILE}" ]] || { echo "ERROR: D1R14R2 frozen state missing" >&2; exit 1; }
fi
mkdir -p "${RUN_DIR}"
stage4_2r3c3t13s24d1r14r2_export_runtime
printf '[T13S24D1R14R2] user=%s host=%s pwd=%s\n' "$(id -un)" "$(hostname)" "$(pwd)"
printf '[T13S24D1R14R2] run_dir=%s command=%s backend=%s resume=%s workers=%s\n' "${RUN_DIR}" "${STAGE4_2R3C3T13S24D1R14R2_COMMAND}" "${STAGE4_2R3C3T13S24D1R14R2_BACKEND}" "${STAGE4_2R3C3T13S24D1R14R2_RESUME}" "${STAGE4_2R3C3T13S24D1R14R2_WORKERS}"
printf '[T13S24D1R14R2] 72 fresh TSC fixed mixed-basis signed-excitation sentinels; formal tracking is diagnostic only.\n'
printf '[T13S24D1R14R2] formal 250/270 ms arrival and 350/370 ms hold timing is unchanged.\n'
cd "${PROJECT_DIR}"
args=(
  --config "${STAGE4_2R3C3T13S24D1R14R2_CONFIG}" --source-d1r13-run "${SOURCE_D1R13_RUN}" --source-d1r13-audit "${SOURCE_D1R13_AUDIT}" --source-r1a-output "${SOURCE_R1A_OUTPUT}"
  ...
  --run-dir "${RUN_DIR}" --command "${STAGE4_2R3C3T13S24D1R14R2_COMMAND}" --backend "${STAGE4_2R3C3T13S24D1R14R2_BACKEND}"
)
[[ "${STAGE4_2R3C3T13S24D1R14R2_RESUME}" == 1 ]] && args+=(--resume)
exec "${STAGE4_2R3C3T13S24D1R14R2_PYTHON}" scripts/stage4_2r3c3t13s24d1r14r2_mixed_basis_signed_excitation_sentinel.py "${args[@]}"
