...
SOURCE_REVISION="${ID2Z15_SOURCE_REVISION:-}"
OUTPUT="${ID2Z15_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z15_headroom_nominal_frontier.json"

fail() { printf 'ID2Z15_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z15_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z15_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z15_headroom_nominal_frontier.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z15_headroom_nominal_frontier_independent.py \
