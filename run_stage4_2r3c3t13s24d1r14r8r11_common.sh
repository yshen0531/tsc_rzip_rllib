...
PYTHON="${STAGE4_2R3C3T13S24D1R14R8R11_PYTHON:-/home/yangshen0711/tsc_all/tsc_simulation/venv_simu/bin/python}"
CONFIG="${STAGE4_2R3C3T13S24D1R14R8R11_CONFIG:-${PROJECT_DIR}/configs/stage4_2r3c3t13s24d1r14r8r11_sustained_exact_target_refresh_authority_sentinel_370ms.json}"
COMMAND="${STAGE4_2R3C3T13S24D1R14R8R11_COMMAND:-offline}"
BACKEND="${STAGE4_2R3C3T13S24D1R14R8R11_BACKEND:-ray}"
RESUME="${STAGE4_2R3C3T13S24D1R14R8R11_RESUME:-0}"
OUTPUT_ROOT="${STAGE4_2R3C3T13S24D1R14R8R11_OUTPUT_ROOT:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r11_runs}"
R8R7_RUN="${STAGE4_2R3C3T13S24D1R14R8R11_R8R7_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r7_runs/stage4_2r3c3t13s24d1r14r8r7_fresh_multipulse_static_observer_interaction_sentinel_20260807_d8d231e_v1}"
RAY_TMPDIR="${RAY_TMPDIR:-/tmp/stage4_2r3c3t13s24d1r14r8r11_${UID:-0}}"
...
case "${COMMAND}" in
  ...
  *) echo "ERROR: unsupported R8R11 command ${COMMAND}" >&2; exit 1 ;;
esac
...
if [[ -n "${STAGE4_2R3C3T13S24D1R14R8R11_RUN_DIR:-}" ]]; then
  RUN_DIR="${STAGE4_2R3C3T13S24D1R14R8R11_RUN_DIR}"
else
  mkdir -p "${OUTPUT_ROOT}"
  RUN_DIR="${OUTPUT_ROOT}/stage4_2r3c3t13s24d1r14r8r11_sustained_exact_target_refresh_authority_sentinel_$(date -u +%Y%m%d_%H%M%S)"
fi
STAGE_DIR="${RUN_DIR}/stage4_2r3c3t13s24d1r14r8r11_sustained_exact_target_refresh_authority_sentinel"
if [[ "${COMMAND}" == offline ]]; then
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]
else
  [[ -f "${STAGE_DIR}/stage_state.json" ]] || { echo "ERROR: R8R11 frozen state missing" >&2; exit 1; }
fi
mkdir -p "${RUN_DIR}"
...
cd "${PROJECT_DIR}"
printf '[R8R11] user=%s host=%s command=%s run_dir=%s\n' "$(id -un)" "$(hostname)" "${COMMAND}" "${RUN_DIR}"
printf '[R8R11] maximum 96 fresh safety/qualification trajectories; all are forbidden from expert and learning data.\n'

args=(
  --config "${CONFIG}" --run-dir "${RUN_DIR}" --r8r7-run "${R8R7_RUN}"
...
if [[ -n "${AUDIT_KIND}" ]]; then
  exec "${PYTHON}" docs/codex/audit_tools/stage4_2r3c3t13s24d1r14r8r11_independent_forensics.py "${args[@]}" --audit-kind "${AUDIT_KIND}"
fi
if [[ "${RESUME}" == 1 ]]; then args+=(--resume); fi
exec "${PYTHON}" -m tsc_rzip_rllib.diagnostics.stage4_2r3c3t13s24d1r14r8r11_sustained_exact_target_refresh_authority_sentinel "${args[@]}" --command "${COMMAND}" --backend "${BACKEND}"
