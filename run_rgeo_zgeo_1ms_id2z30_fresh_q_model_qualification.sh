...
REV="${ID2Z30_SOURCE_REVISION:-}"
OUT="${ID2Z30_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z30_fresh_q_model_qualification.json"
fail() { printf 'ID2Z30_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }
test -n "$REV" || fail "ID2Z30_SOURCE_REVISION is required"
test -n "$OUT" || fail "ID2Z30_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z30_fresh_q_model_qualification.py \
...
if test -f "$OUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z30_fresh_q_model_qualification_independent.py \
