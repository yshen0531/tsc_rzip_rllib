...
PYTHON_BIN="${STAGE4_2R3C3T13S19_PYTHON:-${PYTHON:-python3}}"
cd "${PROJECT_DIR}"
...
from tsc_rzip_rllib.diagnostics import stage4_2r3c3t13s19_prospective_pooled_observer_campaign as s19
config = root / "configs/stage4_2r3c3t13s19_prospective_pooled_observer_campaign_370ms.json"
cfg = json.loads(config.read_text(encoding="utf-8"))
s19._validate_config(cfg)
if not s19.self_test(config)["passed"]:
    raise SystemExit("T13S19 self-test failed")
for path in (root / "run_stage4_2r3c3t13s19_native.sh", root / "run_stop_stage4_2r3c3t13s19_now.sh"):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[T13S19 verify] Python/JSON/scientific guards passed.")
PY
mapfile -t SHELLS < <(find . -maxdepth 2 -type f -name '*.sh' -print | sort)
for script in "${SHELLS[@]}"; do bash -n "${script}"; done
"${PYTHON_BIN}" -m unittest -v tests.test_stage4_2r3c3t13s19_prospective_pooled_observer_campaign
printf '[T13S19 verify] checksums, shell syntax and focused tests passed.\n'
