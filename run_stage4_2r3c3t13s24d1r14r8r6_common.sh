...
RUN_DIR="${STAGE4_2R3C3T13S24D1R14R8R6_RUN_DIR:-$(stage4_2r3c3t13s24d1r14r8r6_new_run_dir)}"
STATE_FILE="${RUN_DIR}/stage4_2r3c3t13s24d1r14r8r6_causal_one_step_innovation_observer/stage_state.json"
if [[ "${STAGE4_2R3C3T13S24D1R14R8R6_COMMAND}" == offline ]]; then
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]
elif [[ "${STAGE4_2R3C3T13S24D1R14R8R6_COMMAND}" != self-test ]]; then
  [[ -f "${STATE_FILE}" ]] || { echo "ERROR: R8R6 frozen state missing" >&2; exit 1; }
fi
mkdir -p "${RUN_DIR}"
stage4_2r3c3t13s24d1r14r8r6_export
cd "${PROJECT_DIR}"
printf '[R8R6] user=%s host=%s command=%s run_dir=%s\n' "$(id -un)" "$(hostname)" "${STAGE4_2R3C3T13S24D1R14R8R6_COMMAND}" "${RUN_DIR}"
printf '[R8R6] zero new TSC; immutable formal timing retained; all stage trajectories remain forbidden from expert data.\n'
if [[ "${STAGE4_2R3C3T13S24D1R14R8R6_COMMAND}" == self-test ]]; then
  exec "${STAGE4_2R3C3T13S24D1R14R8R6_PYTHON}" -c "from pathlib import Path; from tsc_rzip_rllib.diagnostics.stage4_2r3c3t13s24d1r14r8r6_causal_one_step_innovation_observer import self_test; print(self_test(Path(r'${STAGE4_2R3C3T13S24D1R14R8R6_CONFIG}')))"
fi
if [[ "${STAGE4_2R3C3T13S24D1R14R8R6_COMMAND}" == independent ]]; then
  exec "${STAGE4_2R3C3T13S24D1R14R8R6_PYTHON}" docs/codex/audit_tools/stage4_2r3c3t13s24d1r14r8r6_independent_forensics.py \
    --config "${STAGE4_2R3C3T13S24D1R14R8R6_CONFIG}" --run-dir "${RUN_DIR}" \
    --r8r5-run "${STAGE4_2R3C3T13S24D1R14R8R6_R8R5_RUN}" --r8-run "${STAGE4_2R3C3T13S24D1R14R8R5_R8_RUN}" \
    --r8r4-run "${STAGE4_2R3C3T13S24D1R14R8R5_R8R4_RUN}" --source-r2-run "${SOURCE_R2}" \
    --source-r4-run "${SOURCE_R4}" --source-r6-run "${SOURCE_R6}"
fi
args=(
  --config "${STAGE4_2R3C3T13S24D1R14R8R6_CONFIG}" --run-dir "${RUN_DIR}"
  --r8r5-run "${STAGE4_2R3C3T13S24D1R14R8R6_R8R5_RUN}" --r8-run "${STAGE4_2R3C3T13S24D1R14R8R5_R8_RUN}"
  ...
  --command "${STAGE4_2R3C3T13S24D1R14R8R6_COMMAND}"
)
exec "${STAGE4_2R3C3T13S24D1R14R8R6_PYTHON}" scripts/stage4_2r3c3t13s24d1r14r8r6_causal_one_step_innovation_observer.py "${args[@]}"
