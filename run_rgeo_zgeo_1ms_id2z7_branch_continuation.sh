...
SOURCE_REVISION="${ID2Z7_SOURCE_REVISION:-}"
OUTPUT="${ID2Z7_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z7_branch_continuation.json"

fail() {
  printf 'ID2Z7_LAUNCH_FAIL: %s\n' "$1" >&2
  exit 2
}

test -n "$SOURCE_REVISION" || fail "ID2Z7_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z7_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z7_branch_continuation.py \
  --stage-config "$CONFIG" \
  --source-revision "$SOURCE_REVISION" \
  --offline-only > "${OUTPUT}.offline.json"
offline_rc=$?
if test "$offline_rc" -ne 0; then
  exit 2
fi
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z7_branch_continuation_independent.py \
    --stage-config "$CONFIG" \
    --run-dir "$OUTPUT" \
    --source-revision "$SOURCE_REVISION" \
