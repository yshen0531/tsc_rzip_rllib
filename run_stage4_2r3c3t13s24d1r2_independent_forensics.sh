...
RUN_DIR="${STAGE4_2R3C3T13S24D1R2_RUN_DIR:-$(cat "${STAGE4_2R3C3T13S24D1R2_OUTPUT_ROOT}/latest_stage4_2r3c3t13s24d1r2_run.txt")}"
COMPLETE_LOG="${STAGE4_2R3C3T13S24D1R2_LOG_FILE:-$(cat "${PROJECT_DIR}/logs/nohup/latest_stage4_2r3c3t13s24d1r2.log")}"
OUTPUT="${RUN_DIR}/stage4_2r3c3t13s24d1r2_geometry_restored_amplitude_safety_sentinel/analysis/independent_server_forensics.json"
[[ -f "${RUN_DIR}/stage4_2r3c3t13s24d1r2_geometry_restored_amplitude_safety_sentinel/final_result.json" ]] || { echo "ERROR: D1R2 final result missing" >&2; exit 1; }
[[ -f "${COMPLETE_LOG}" ]] || { echo "ERROR: D1R2 complete log missing" >&2; exit 1; }
stage4_2r3c3t13s24d1r2_export_runtime
cd "${PROJECT_DIR}"
exec "${STAGE4_2R3C3T13S24D1R2_PYTHON}" \
  ...
  --run-dir "${RUN_DIR}" --complete-log "${COMPLETE_LOG}" --output "${OUTPUT}"
