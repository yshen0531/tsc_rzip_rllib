...
def main():
    ...
    for i, line in enumerate(path.read_text().splitlines(), start=1):
        ...
        ret = (
            recursive_find(obj, "episode_return_mean")
            or recursive_find(obj, "episode_reward_mean")
        )
        length = (
            recursive_find(obj, "episode_len_mean")
            or recursive_find(obj, "episode_length_mean")
        )

        delta = None
        if last_steps is not None:
            delta = env_steps - last_steps
        last_steps = env_steps

        bad = list(walk_nonfinite(obj))
        total_nonfinite += len(bad)
        for k, _ in bad:
    ...
    print("\nNonfinite summary:")
    print("total_nonfinite_fields:", total_nonfinite)
    for k, n in sorted(nonfinite_keys.items(), key=lambda x: (-x[1], x[0]))[:50]:
        print(f"{n:4d}  {k}")
