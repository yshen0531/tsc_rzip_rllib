#!/usr/bin/env python3
"""Independent server-side raw audit for Stage4.2R3c3T11."""
...
def _snapshot_integrity(
    ...
    for state_id in sorted(by_state):
        ...
        try:
            manifest = t11.t1.r3c3.read_json(manifest_path)
            ...
            passed = bool(
                ...
                and t11.t1.r1._validate_snapshot_inventory(
                    directory, manifest
                )
            )
            row.update(
...
def _summary_fields(summary: Mapping[str, Any]) -> dict[str, Any]:
    keys = (
        ...
        "maximum_response_velocity_condition_number",
...
def main() -> None:
    ...
    ctx = t11.load_stage42r3c3t11_config(
        args.config,
    ...
    manifest = t11.t1.r3c3.read_json(ctx.paths.manifest)
    reported = t11.t1.r3c3.read_json(
        ctx.paths.control / "summary.json"
    )
    selected_pairs, _ = t11.t1.r3c3._recompute_selected_pairs(
        ctx.base_ctx.source_ctx.base_ctx
    )
    specs = t11.build_control_specs(ctx, selected_pairs)
    expected_by_id = {
    ...
    results = [t11.t1.r3c3.read_json_gz(path) for path in raw_paths]
    actual_by_id = {
    ...
    recomputed = t11.summarize_control(
    ...
    audit_package = t11._deployed_package_fingerprint(ctx)
    ...
    active_fingerprint_path = original_fingerprint_path
    control_specs_path = (
        ctx.paths.source_reference / "control_specs.json"
    )
    resolved_config_path = (
        ctx.paths.run_dir / "stage4_2r3c3t11_config.resolved.json"
    )
    ...
    evidence_files_exact = bool(
        ...
        and t11.t1.r3c3.read_json(original_fingerprint_path)
        == original_runtime_package
        and active_fingerprint_path.is_file()
        and t11.t1.r3c3.read_json(active_fingerprint_path)
        == runtime_package
        and control_specs_path.is_file()
        and t11.t1.r3c3.read_json(control_specs_path) == specs
        and resolved_config_path.is_file()
        and t11.t1.r3c3.read_json(resolved_config_path) == ctx.cfg
        and source_fingerprint_path.is_file()
        and t11.t1.r3c3.read_json(source_fingerprint_path)
        == ctx.base_ctx.source_t1_fingerprint
        and preflight_copy_path.is_file()
        and t11.t1.r3c3.read_json(preflight_copy_path) == ctx.preflight
    )
    manifest_compatibility = {
        "stage": manifest.get("stage") == t11.STAGE,
        "controller_revision": manifest.get("controller_revision")
        == t11.CONTROLLER_REVISION,
        "package_revision": manifest.get("package_revision")
        == t11.PACKAGE_REVISION,
        ...
        == t11._sha256(ctx.t3_controller_bank_path),
    ...
    audit = {
        ...
        "stage": t11.STAGE,
        "run_dir": str(run_dir),
        "controller_revision": t11.CONTROLLER_REVISION,
        "package_revision": t11.PACKAGE_REVISION,
        ...
        "source_stage4_2r3c3t3_controller_bank_sha256": t11._sha256(
            ctx.t3_controller_bank_path
        ),
        "candidate_preflight_sha256": t11._sha256(
            ctx.preflight_path
        ),
        ...
        "persistent_step_response_condition_number_gate_passed": bool(
    ...
    raw_path = audit_dir / "stage4_2r3c3t11_raw_inventory.json"
    run_path = audit_dir / "stage4_2r3c3t11_run_inventory.json"
    snapshot_path = audit_dir / "stage4_2r3c3t11_snapshot_audit.json"
    audit_path = audit_dir / "stage4_2r3c3t11_server_audit.json"
    t11.t1.r3c3.atomic_write_json(raw_path, raw_inventory)
    t11.t1.r3c3.atomic_write_json(run_path, run_inventory)
    t11.t1.r3c3.atomic_write_json(snapshot_path, snapshot_integrity)
    t11.t1.r3c3.atomic_write_json(audit_path, audit)
    print(
        json.dumps(
            {
