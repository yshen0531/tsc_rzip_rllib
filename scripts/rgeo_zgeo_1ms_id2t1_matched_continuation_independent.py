#!/usr/bin/env python3
"""Independent full-raw audit for ID-2T1."""
...
from scripts import rgeo_zgeo_1ms_id2t1_matched_continuation as primary  # noqa: E402

SCHEMA = "rgeo-zgeo-1ms-id2t1-matched-continuation-independent-v1"
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    try:
        stage, cfg, targets, source, references, base = primary.load(stage_path)
        expected = primary.campaign_streams(stage, cfg, references, base)
    except Exception as exc:
        failures.append(f"STAGE_LOAD:{type(exc).__name__}:{exc}")
        stage, cfg, references, expected = load_json(stage_path), None, {}, []
    ...
    raw_prefix_stage = dict(stage)
    raw_prefix_stage["semantic_artifacts"] = [
        name for name in stage["semantic_artifacts"] if name != "inputa"]
    prefix_ok = execution and all(not primary._prefix_reasons(
        row["states"][index], references[row["sequence_id"]]["states"][index],
        raw_prefix_stage, index)
        for row in raw_rows for index in range(31))
    metrics = primary.measured_metrics(raw_rows, references, stage) if execution else {"passed": False}
    if prefix_ok != result.get("prefix_checks_passed"):
        failures.append("PREFIX_RECOMPUTE")
    if metrics != result.get("matched_continuation_metrics"):
    ...
    if result.get("route") != route or bool(result.get("passed")) != (route == stage["routes"]["pass"]):
    ...
    return {"schema_version": SCHEMA, "source_revision": source_revision,
            ...
            "recomputed_prefix_checks_passed": prefix_ok,
            "recomputed_matched_continuation_metrics": metrics,
            "recomputed_route": route, "records_have_zero_fit_weight": True,
            "models_fit_or_updated": 0,
            "claim_boundary": "Independent full-raw ID2T1 matched continuation audit only."}
