#!/usr/bin/env python3
"""Fixed-1000 radial nominal ramp-and-hold development campaign."""
...
SCHEMA = "rgeo-zgeo-1ms-1000-radial-nominal-n0-v1"
CONFIG_SHA256 = "3072817a06075ddf48a9d219a2f687a0b5260ae6e5cb201b99c9e0e8a54b6634"
DEFAULT_CONFIG = ROOT / "configs/rgeo_zgeo_1ms_1000_radial_nominal_n0.json"
SEMANTIC_ARTIFACTS = b0.SEMANTIC_ARTIFACTS


class InputIntegrityError(ValueError):
    """Frozen N0 stage or evidence identity changed."""


def load(config_path: Path) -> tuple[dict[str, Any], TSCConfig, dict[str, Any]]:
    config_path = b0.inside_root(config_path, "N0 config")
    if b0.sha256(config_path) != CONFIG_SHA256:
        raise InputIntegrityError("N0 config SHA-256 mismatch")
    stage = json.loads(config_path.read_text(encoding="utf-8"))
    exact = {
        "schema_version": SCHEMA,
        "campaign_id": "rgeo_zgeo_1ms_1000_radial_nominal_n0_v1",
        "takeover_time_ms": 1000,
        "control_period_ms": 1,
        "horizon_steps": 64,
        "candidate_depths": [4, 8, 12, 16],
        "critical_replay_depth": 12,
        "rollouts": 5,
    ...
    if stage.get("axis_signs_tsc_order") != {"even": [1] * 14}:
        raise InputIntegrityError("N0 even coordinate changed")
    roles = stage.get("data_roles", {})
    if roles.get("critical_replay") != "integrity_only_zero_fit_weight" or any(
        roles.get(key) != "unopened" for key in ("calibration", "holdout")
    ) or roles.get("controller_or_recourse") != "forbidden":
        raise InputIntegrityError("N0 data roles changed")
    ...
    for section in ("baseline_evidence", "d1_evidence"):
...
def targets(stage: dict[str, Any], cfg: TSCConfig, source: dict[str, Any]) -> dict[str, Any]:
    ...
    q0 = frozen.q0
    q0_decimal = card15_target_decimal_a(q0, cfg.turns_tsc, name="n0.q0")
    if tuple(q0_decimal) != tuple(source["active_command_decimal_a_tsc"]):
        raise ContractError("N0 q0 is not the semantic source command")
    ...
    for level in range(1, stage["maximum_cumulative_level"] + 1):
        ...
        exact = card15_target_decimal_a(target, cfg.turns_tsc, name=f"n0.even_minus.level{level}")
        adjacent = Decimal(str(assert_exact_slew(previous, exact, name=f"n0.adjacent{level}")))
        maximum_adjacent = max(maximum_adjacent, adjacent)
        if adjacent > adjacent_limit:
            raise ContractError(f"N0 level {level} exceeds adjacent construction cap")
        if any(value < Decimal(str(low)) or value > Decimal(str(high))
            ...
            raise ContractError(f"N0 level {level} exceeds absolute current limits")
        if any(value >= base for value, base in zip(exact, q0_decimal)):
            raise ContractError(f"N0 level {level} lost even-minus sign")
    ...
    result["action_geometry"] = {
        ...
        "terminal_offset_linf_a": float(max(abs(value - base) for value, base in zip(previous, q0_decimal))),
    }
    return result
...
def action_stream(
    ...
    for issue, target in enumerate(sequence_for(spec, stage, target_map)):
        exact = card15_target_decimal_a(target, cfg.turns_tsc, name=f"n0.{spec['rollout_id']}.{issue}")
        maximum = assert_exact_slew(previous, exact, name=f"n0.{spec['rollout_id']}.{issue}")
        rows.append({
...
def offline(config_path: Path, source_revision: str) -> dict[str, Any]:
    ...
    return {
        ...
        "route": routes.get("offline_pass") if not failures else routes.get(
            "offline_fail", "ONE_MS_NR1000N0_OFFLINE_FAIL_NO_TSC"
        ),
        "failures": failures, "authorized_reset_calls": 5,
        "authorized_plant_advances": 320, "action_geometry": geometry,
...
def _window_metrics(states: Sequence[dict[str, Any]], start: int, end: int) -> dict[str, float]:
    ...
    distances = [1000.0 * math.hypot(s["r_geo_m"] - source["r_geo_m"], s["z_geo_m"] - source["z_geo_m"])
                 for s in window]
    speeds = [math.hypot(
    ...
    ip_fractions = [abs(s["ip_a"] - source["ip_a"]) / abs(source["ip_a"]) for s in window]
    return {
...
def scientific_metrics(
    primary_rows: Sequence[dict[str, Any]], baseline: Sequence[dict[str, Any]], stage: dict[str, Any],
) -> dict[str, Any]:
    start, end = stage["terminal_state_start"], stage["terminal_state_end"]
    baseline_metrics = _window_metrics(baseline, start, end)
    gates = stage["scientific_gates"]
    candidates = []
    for row in primary_rows:
        metrics = _window_metrics(row["states"], start, end)
        distance_improvement = 1.0 - metrics["maximum_source_distance_mm"] / baseline_metrics[
            "maximum_source_distance_mm"
        ]
        speed_improvement = baseline_metrics["maximum_speed_m_per_s"] - metrics["maximum_speed_m_per_s"]
        utility_pass = (
            distance_improvement >= gates["minimum_terminal_worst_source_distance_improvement_fraction"]
            and speed_improvement >= gates["minimum_terminal_max_speed_improvement_m_per_s"]
        )
        capture_pass = (
            metrics["maximum_source_distance_mm"] <= gates["capture_source_distance_mm"]
            and metrics["maximum_speed_m_per_s"] <= gates["capture_speed_m_per_s"]
            and metrics["maximum_ip_source_fraction"] <= gates["capture_ip_source_fraction"]
        )
        candidates.append({
            "rollout_id": row["rollout_id"], "depth": row["depth"], **metrics,
            "source_distance_improvement_fraction": distance_improvement,
            "speed_improvement_m_per_s": speed_improvement,
            "utility_pass": utility_pass, "capture_diagnostic_pass": capture_pass,
        })
    passed = any(row["utility_pass"] for row in candidates)
    return {
        "passed": passed, "baseline_terminal": baseline_metrics,
        "candidates": candidates,
        "best_candidate": min(candidates, key=lambda row: max(
            row["maximum_source_distance_mm"] / baseline_metrics["maximum_source_distance_mm"],
            row["maximum_speed_m_per_s"] / baseline_metrics["maximum_speed_m_per_s"],
        )),
...
def run(config_path: Path, source_revision: str, output_dir: Path) -> dict[str, Any]:
    ...
    if not preflight["passed"]:
        raise RuntimeError("N0 offline gate failed; TSC forbidden")
    ...
    for spec in rollout_specs(stage):
        row = d0._run_one(
            cfg, stage, spec, sequence_for(spec, stage, target_map), envelope, baseline["states"][0],
        )
    ...
    metrics = scientific_metrics(
        [row for row in rows.values() if row["repeat_index"] == 0], baseline["states"], stage,
    ...
    result = {
        "schema_version": SCHEMA,
        "kind": "authentic_fixed_1000_radial_nominal_development",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        ...
        "claim_boundary": "fixed-1000 radial nominal development only; no hold/recovery/control",
    }
