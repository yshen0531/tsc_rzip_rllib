#!/usr/bin/env python3
"""Independent raw audit for fixed-1000 joint allocator Authority G2."""
...
from scripts import rgeo_zgeo_1ms_1000_joint_allocator_authority_g2 as primary  # noqa: E402
...
def audit(config_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    run_dir = primary.b0.inside_root(run_dir, "G2 run directory")
    failures: list[str] = []
    ...
    source_command = decimal_single_turn_currents_a(
        ...
        cfg.turns_tsc,
        name="g2.independent.source_command",
    )
        ...
        try:
            ...
            for issue in range(stage["horizon_steps"]):
                ...
                maximum = assert_exact_slew(
                    ...
                    exact,
                    name=f"g2.independent.issued.{rollout_id}.{issue}",
                )
                ...
                if issue > 0:
                    try:
                        assert_exact_slew(
                            ...
                            name=f"g2.independent.observed.{rollout_id}.{issue}",
                        )
                        observed_checks += 1
