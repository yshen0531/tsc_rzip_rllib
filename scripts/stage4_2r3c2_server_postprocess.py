#!/usr/bin/env python3
"""Server-side Stage4.2R3c2 raw recomputation and compact audit."""
...
def _runtime_package_fingerprint(
    ...
    if not fingerprint:
        raise ValueError("Stage4.2R3c2 runtime package fingerprint missing")
    if run_manifest.get("semantics_preserving_resume_hotfix"):
        raise ValueError(
            "Stage4.2R3c2 has no registered runtime-hotfix package chain"
        )
    return fingerprint
...
def main() -> None:
    ...
    manifest_compatibility = {
        ...
        "restart_regulation": run_manifest.get("restart_regulation")
    ...
    r3c.atomic_write_json(
        output_dir / "stage4_2r3c2_run_inventory.json",
        run_inventory,
    )
    r3c.atomic_write_json(
        output_dir / "stage4_2r3c2_server_audit.json", audit
    )
    print(
        json.dumps(
            {
