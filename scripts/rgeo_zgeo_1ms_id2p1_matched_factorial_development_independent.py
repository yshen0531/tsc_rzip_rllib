#!/usr/bin/env python3
"""Independent full-raw audit for ID-2P1."""
...
from scripts import rgeo_zgeo_1ms_id2p1_matched_factorial_development as primary  # noqa: E402

SCHEMA = "rgeo-zgeo-1ms-id2p1-independent-raw-v1"
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    if cfg is not None:
        for row in compact:
            ...
            times = sorted(int(path.name[:-2]) for path in folder.iterdir() if path.is_dir() and path.name.endswith("ms"))
            ...
            for index, time_ms in enumerate(times):
                ...
                if len(state["wire_current_a"]) != 48 or any(abs(float(a)-float(b)) > 1e-9 for a,b in zip(state["wire_current_a"], saved["wire_current_a"])):
    ...
    prefixes = primary.matched_prefix_checks(raw_rows, stage) if len(raw_rows) == 42 else []
    replays = primary.replay_checks(raw_rows, stage) if len(raw_rows) == 42 else []
    metrics = primary.response_metrics(raw_rows, stage) if len(raw_rows) == 42 else {"passed": False}
    if prefixes != result.get("matched_prefix_checks"): failures.append("PREFIX_RECOMPUTE")
    if replays != result.get("critical_replay_checks"): failures.append("REPLAY_RECOMPUTE")
    if metrics != result.get("response_metrics"): failures.append("RESPONSE_RECOMPUTE")
    counters = {"rollouts_completed":len(compact), "unique_cells_completed":len({row.get("cell_id") for row in compact}),
    ...
    complete = (len(raw_rows)==42 and len(lines)==7350 and len(prefixes)==8 and all(x["passed"] for x in prefixes)
                and len(replays)==2 and all(x["passed"] for x in replays) and metrics.get("passed"))
    if complete and (not result.get("passed") or result.get("route") != stage["routes"]["pass"]):
        failures.append("PRIMARY_ROUTE_OR_VERDICT")
    return {"schema_version":SCHEMA,"source_revision":source_revision,"stage_config_sha256":primary.CONFIG_SHA256,
            "primary_sha256":sha256(result_path) if result_path.is_file() else None,
            "audit_passed":not failures,"failures":list(dict.fromkeys(failures)),"raw_rollouts":len(raw_rows),
            "raw_states":sum(len(row["states"]) for row in raw_rows),"recomputed_counters":counters,
            "recomputed_inventory_sha256":inventory,"recomputed_inventory_files":len(lines),
            "recomputed_inventory_bytes":total,"recomputed_matched_prefix_checks":prefixes,
            "recomputed_critical_replay_checks":replays,"recomputed_response_metrics":metrics,
            "development_fit_data_eligible":complete and not failures,"models_fit_or_updated":0,
            "claim_boundary":"Independent full-raw ID2P1 development audit only; no model or control qualification."}


def main(argv: Sequence[str] | None = None) -> int:
    parser=argparse.ArgumentParser(); parser.add_argument("--stage-config",type=Path,default=primary.CONFIG)
    ...
    result=audit(args.stage_config,args.run_dir,args.source_revision); write_new(inside(args.output,"output"),result)
    print(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)); return 0 if result["audit_passed"] else 2
