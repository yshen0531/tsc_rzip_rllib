#!/usr/bin/env python3
"""Independent raw reparse and route audit for ID-2Z13."""
...
from scripts import rgeo_zgeo_1ms_id2z13_early_p04_augmented_capture as primary  # noqa: E402


SCHEMA = "rgeo-zgeo-1ms-id2z13-independent-raw-v1"
...
def expected_streams(stage: dict[str, Any], cfg: Any, targets: dict[str, Any],
    ...
    roots = primary.build_matrix(stage, cfg, targets, parent, 49, 0, "root")
    root_arm = scientific.get("selected_root_arm_id")
    selected_root = next((v for v in roots if v["arm_id"] == root_arm), None)
    if selected_root is None:
        return roots
    mains = primary.build_matrix(stage, cfg, targets, selected_root, 53, 1, "main")
    main_arm = scientific.get("selected_main_arm_id")
    selected_main = next((v for v in mains if v["arm_id"] == main_arm), None)
    values = [*roots, *mains]
    if selected_main is not None:
        values.append(primary.replay_stream(selected_main))
    return values
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    for arm in primary.ARM_IDS:
        ...
        if row:
            prefix_values.append(primary.z6.prefix_check(
                row, source_reference, 50, 49, stage["semantic_artifacts"]))
    for arm in primary.ARM_IDS:
    ...
    scientific = {
        ...
        "selected_root_committed_tokens": (
            next((v["tokens"][:4] for v in expected
                  if v.get("rollout_id") == f"root__{root_arm}"), None)),
    ...
    return {
        ...
        **counters, "models_fit_or_updated": 0,
        ...
        "claim_boundary": "Independent raw audit of finite ID2Z13 only.",
    }
