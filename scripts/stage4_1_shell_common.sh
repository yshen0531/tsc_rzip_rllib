#!/usr/bin/env bash
# Shared helpers for the complete standalone Stage4.1 launchers.
# No Git, network access, or external source-tree recovery is used.

stage41_die() { echo "ERROR: $*" >&2; exit 2; }
...
stage41_find_python() {
  ...
  else stage41_die "no Python interpreter found; set PYTHON_BIN=/absolute/path/to/python"
  fi
  [[ -x "${PYTHON_BIN}" ]] || stage41_die "Python interpreter is not executable: ${PYTHON_BIN}"
  export PYTHON_BIN
}
...
stage41_detect_source() {
  ...
  if [[ -n "${SOURCE_STAGE4_0_RUN:-}" ]]; then
    candidate="$(stage41_abspath "${SOURCE_STAGE4_0_RUN}")"
    stage41_validate_source "${candidate}" || stage41_die "SOURCE_STAGE4_0_RUN is incomplete or unsupported: ${candidate}"
  ...
  if [[ -f "${PROJECT_DIR}/stage4_0_runs/latest_stage4_0_run.txt" ]]; then
    ...
    [[ -n "${candidate}" ]] && candidate="$(stage41_abspath "${candidate}")"
    if [[ -n "${candidate}" ]] && stage41_validate_source "${candidate}"; then
      SOURCE_STAGE4_0_RUN="${candidate}"; export SOURCE_STAGE4_0_RUN; return
    fi
  fi
...
PY
  ...
  [[ -n "${candidate}" ]] || stage41_die "no complete Stage4.0 source found. Set SOURCE_STAGE4_0_RUN=/absolute/path/to/run"
  SOURCE_STAGE4_0_RUN="${candidate}"; export SOURCE_STAGE4_0_RUN
}

stage41_source_from_existing_run() {
  local run_dir="$1" candidate
  candidate="$("${PYTHON_BIN}" - "${PROJECT_DIR}" "${run_dir}" <<'PY'
from pathlib import Path
import json,sys
project=Path(sys.argv[1]).resolve(); run=Path(sys.argv[2]).resolve(); p=run/'stage4_1_manifest.json'
...
stage41_ray_stop() {
  local ray_cli; ray_cli="$(stage41_find_ray_cli)"
...
stage41_validate_tmp_path() {
  local value="$1" label="$2"
  [[ -n "${value}" && "${value}" == /tmp/* && "${value}" != /tmp && "${value}" != /tmp/ ]] || stage41_die "${label} must be below /tmp; got ${value}"
}

stage41_cleanup_runtime() {
  local workspace="${STAGE4_1_TSC_WORKSPACE_ROOT:-/tmp/tsc_workspace}"
  local run_root="${STAGE4_1_TSC_RUN_ROOT:-${workspace}/episode_runs}"
  local ray_tmp="${RAY_TMPDIR:-/tmp/stage4_1_$(id -u)}"
  stage41_validate_tmp_path "${workspace}" STAGE4_1_TSC_WORKSPACE_ROOT
  stage41_validate_tmp_path "${run_root}" STAGE4_1_TSC_RUN_ROOT
  stage41_validate_tmp_path "${ray_tmp}" RAY_TMPDIR
  mkdir -p "${workspace}" "${run_root}"
  find "${workspace}" -mindepth 1 -maxdepth 1 -type d \( -name 'stage4_1*' -o -name 'stage41*' -o -name 'stage2_*' \) -exec rm -rf -- {} + 2>/dev/null || true
...
stage41_runtime_env() {
  ...
  export STAGE4_1_WORKERS="${STAGE4_1_WORKERS:-${STAGE4_WORKERS:-128}}"
  export STAGE4_WORKERS="${STAGE4_1_WORKERS}"
  export STAGE2_WORKERS="${STAGE4_1_WORKERS}"
  export STAGE4_1_TSC_WORKSPACE_ROOT="${STAGE4_1_TSC_WORKSPACE_ROOT:-/tmp/tsc_workspace}"
  export STAGE4_1_TSC_RUN_ROOT="${STAGE4_1_TSC_RUN_ROOT:-${STAGE4_1_TSC_WORKSPACE_ROOT}/episode_runs}"
  export STAGE3_4_TSC_WORKSPACE_ROOT="${STAGE4_1_TSC_WORKSPACE_ROOT}" STAGE3_4_TSC_RUN_ROOT="${STAGE4_1_TSC_RUN_ROOT}"
  export STAGE2_TSC_WORKSPACE_ROOT="${STAGE4_1_TSC_WORKSPACE_ROOT}" STAGE2_TSC_RUN_ROOT="${STAGE4_1_TSC_RUN_ROOT}"
  export RAY_TMPDIR="${RAY_TMPDIR:-/tmp/stage4_1_$(id -u)}"
  export TMPDIR="${TMPDIR:-${RAY_TMPDIR}/tmp}"
  stage41_validate_tmp_path "${STAGE4_1_TSC_WORKSPACE_ROOT}" STAGE4_1_TSC_WORKSPACE_ROOT
  stage41_validate_tmp_path "${STAGE4_1_TSC_RUN_ROOT}" STAGE4_1_TSC_RUN_ROOT
  stage41_validate_tmp_path "${RAY_TMPDIR}" RAY_TMPDIR
  mkdir -p "${STAGE4_1_TSC_WORKSPACE_ROOT}" "${STAGE4_1_TSC_RUN_ROOT}" "${TMPDIR}"
