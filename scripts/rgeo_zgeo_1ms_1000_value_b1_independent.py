#!/usr/bin/env python3
"""Independent raw reconstruction for fixed-1000 B1 blind evaluation."""
...
from scripts import rgeo_zgeo_1ms_1000_value_b1 as primary  # noqa: E402
...
def audit(config_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    run_dir = primary.b0.inside_root(run_dir, "B1 run directory")
    failures: list[str] = []
    ...
    source_command = decimal_single_turn_currents_a(
        ...
        cfg.turns_tsc, name="b1.independent.source_command")
    ...
    for spec in specs:
        ...
        try:
            ...
            if target_map is None:
                ...
                target_map = primary.c0.d2.d1.targets(d1_stage, cfg, source)
            ...
            for issue, target in enumerate(sequence):
                ...
                exact = card15_target_decimal_a(target, cfg.turns_tsc,
                                                 name=f"b1.audit.{rollout_id}.{issue}")
                assert_exact_slew(active, exact, name=f"b1.audit.issue.{rollout_id}.{issue}")
                active = exact
                if issue > 0:
                    assert_exact_slew(states[issue]["current_decimal_a_tsc"],
                                      states[issue + 1]["current_decimal_a_tsc"],
                                      name=f"b1.audit.observed.{rollout_id}.{issue}")
                    observed_checks += 1
    ...
    eval_stage = primary.evaluation_stage(stage)
    scientific = primary.c0.d2.scientific_metrics(primary_rows, eval_stage) if complete else None
    blind = primary.c0.calibration_metrics(primary_rows, eval_stage, artifact) if complete else None
    try:
    ...
    if blind is None or not d0audit._close(blind, reported.get("model_blind_metrics")):
        failures.append("PRIMARY_BLIND_METRICS")
    ...
    result = {
        ...
        "passed": not failures,
        "route": "ONE_MS_NR1000B1_INDEPENDENT_PASS" if not failures else "ONE_MS_NR1000B1_INDEPENDENT_FAIL",
