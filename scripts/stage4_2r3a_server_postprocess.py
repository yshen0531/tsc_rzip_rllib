#!/usr/bin/env python3
"""Server-side Stage4.2R3a raw-evidence recomputation and compact audit.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Recompute Stage4.2R3a evidence on the server"
    )
    ...
    r3.atomic_write_json(output_dir / "stage4_2r3a_server_audit.json", audit)
    r3.atomic_write_json(
        output_dir / "stage4_2r3a_snapshot_checks.json", snapshot_checks
    )
    r3.atomic_write_json(
        output_dir / "stage4_2r3a_run_inventory.json", inventory
    )
