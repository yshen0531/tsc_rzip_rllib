#!/usr/bin/env python3
"""Independent full-raw audit for ID-2S1."""
...
from scripts import rgeo_zgeo_1ms_id2s1_measured_sequence_discriminator as primary  # noqa: E402


SCHEMA = "rgeo-zgeo-1ms-id2s1-measured-sequence-discriminator-independent-v1"
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    try:
        stage, cfg, targets, source, baseline, s0_result = primary.load(stage_path)
        expected = primary.campaign_streams(stage, cfg, targets, source, s0_result)
    except Exception as exc:
        failures.append(f"STAGE_LOAD:{type(exc).__name__}:{exc}")
        stage, cfg, baseline, s0_result, expected = load_json(stage_path), None, {}, {}, []
        ...
        for row in compact:
            ...
            for index, time_ms in enumerate(times):
                ...
                if index >= len(row.get("states", [])):
                    failures.append(f"COMPACT_STATE_MISSING:{row['rollout_id']}:{time_ms}")
                else:
    ...
    execution = len(raw_rows) == 4 and len(compact) == 4 and all(row.get("passed") for row in compact)
    prefixes = primary.matched_prefix_checks(raw_rows, baseline) if execution else []
    metrics = primary.measured_metrics(raw_rows, baseline, s0_result, stage) if execution else {"passed": False}
    if prefixes != result.get("matched_prefix_checks"):
        failures.append("PREFIX_RECOMPUTE")
    if metrics != result.get("measured_sequence_metrics"):
    ...
    raw_ok = (len(lines) == stage.get("required_artifact_files_if_complete")
              and not any(reason.startswith(("RAW_STATE", "MISSING_ARTIFACT", "STATE_DIRECTORY",
                                             "COMPACT_STATE_MISSING")) for reason in failures))
    prefix_ok = len(prefixes) == 4 and all(row["passed"] for row in prefixes)
    if not execution:
        recomputed_route = stage["routes"]["execution_or_interface_fail"]
    elif not raw_ok:
        recomputed_route = stage["routes"]["raw_integrity_fail"]
    elif not prefix_ok:
        recomputed_route = stage["routes"]["prefix_mismatch"]
    elif not metrics["passed"]:
        recomputed_route = stage["routes"]["measured_utility_fail"]
    else:
    ...
    return {
        ...
        "claim_boundary": "Independent full-raw ID2S1 sequence audit only; no model or control qualification.",
    }
