#!/usr/bin/env python3
"""Run the staged fresh ID2Z35 event-set qualification."""
...
CONFIG = ROOT / "configs/rgeo_zgeo_1ms_id2z35_fresh_event_set_qualification.json"
CONFIG_SHA256 = "de8cc644cb8f205f29715d54fb662bdcb5ba38ae60f460ccf287eb65f86916af"
SCHEMA = "rgeo-zgeo-1ms-id2z35-fresh-event-set-qualification-result-v1"
ROW_SCHEMA = "rgeo-zgeo-1ms-id2z35-fresh-event-set-qualification-row-v1"
OFFLINE_SCHEMA = "rgeo-zgeo-1ms-id2z35-fresh-event-set-qualification-offline-v1"
...
def _require(stage: dict[str, Any]) -> None:
    if _sha(CONFIG) != CONFIG_SHA256 or stage != _read(CONFIG):
        raise ValueError("ID2Z35 frozen config changed")
    exact = {
        "schema_version": "rgeo-zgeo-1ms-id2z35-fresh-event-set-qualification-v1",
        "identity": "rgeo-zgeo-1ms-id2z35-fresh-event-set-qualification-v1",
        "stage": "ID-2Z35", "common_horizon_steps": 73,
        "calibration_phase_issue": 52, "blind_phase_issue": 54,
    ...
    for key, expected in exact.items():
        if stage.get(key) != expected:
            raise ValueError(f"ID2Z35 frozen field changed: {key}")
    ...
    if (roles.get("calibration_family_count") != 4
        ...
        raise ValueError("ID2Z35 data/qualification contract changed")


def load(config: Path = CONFIG):
    if config.resolve() != CONFIG.resolve():
        raise ValueError("ID2Z35 alternate config forbidden")
    stage = _read(config)
    _require(stage)
    for name, spec in stage["evidence"].items():
        path = ROOT / spec["path"]
        if _sha(path) != spec["sha256"]:
            raise ValueError(f"ID2Z35 evidence hash mismatch: {name}")
    model_result = _read(ROOT / stage["evidence"]["id2z34_result"]["path"])
    model_audit = _read(ROOT / stage["evidence"]["id2z34_independent"]["path"])
    if (model_result.get("passed") is not True
            or model_result.get("route") != stage["evidence"]["id2z34_result"]["required_route"]
            or model_result.get("selected_candidate") != "candidate_b"
            or model_result.get("model_payload", {}).get("payload_sha256")
            != stage["evidence"]["id2z34_result"]["required_payload_sha256"]
            or model_audit.get("audit_passed") is not True
            or model_audit.get("primary_sha256") != stage["evidence"]["id2z34_result"]["sha256"]):
        raise ValueError("ID2Z34 frozen model identity mismatch")
    ...
    for spec in stage["rollout_specs"]:
        ...
        if spec["kind"] == "replay":
            ...
            row = {
                ...
                "round_index": 0, "round_id": "fresh_event_set_qualification",
...
def action_separation(streams: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    for role, phase in (("calibration", 52), ("blind", 54)):
        for axis in ("q_r", "q_z"):
            plus = by_id[f"{'cal52' if role == 'calibration' else 'blind54'}__{axis}__plus_then_return"]
            minus = by_id[f"{'cal52' if role == 'calibration' else 'blind54'}__{axis}__minus_then_return"]
