#!/usr/bin/env python3
"""Independent raw audit for fixed-1000 deadband feedback F1."""
...
from scripts import rgeo_zgeo_1ms_1000_feedback_f0_independent as f0audit  # noqa: E402
from scripts import rgeo_zgeo_1ms_1000_feedback_f1 as primary  # noqa: E402
...
def audit(config_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    run_dir = primary.f0.b0.inside_root(run_dir, "F1 run directory")
    failures = []
    ...
    source_command = decimal_single_turn_currents_a(
        ...
        cfg.turns_tsc, name="f1.audit.source")
    rows = {}
    ...
    for spec in primary.specs(stage):
        ...
        times = sorted(int(path.name[:-2]) for path in root.iterdir()
        ...
        try:
            ...
            for index, state in enumerate(states):
                ...
                state["artifact_sha256"] = {
                    name: (f0audit._sha256(root / f"{1000 + index}ms" / name)
                           if name != "inputa" else
                           compact["states"][index]["artifact_sha256"][name])
                    for name in primary.f0.b0.SEMANTIC_ARTIFACTS
                }
            compact["states"] = states
            rows[rollout_id] = compact
            failures.extend(
                f"SOURCE:{rollout_id}:{reason}" for reason in
                f0audit.a0audit.d0audit._source_reasons(
            ...
            if target_map is None:
                ...
                target_map = primary.f0.a0.b1.c0.d2.d1.targets(d1_stage, cfg, source)
            sequence = [target_map["q0"]] * 64
            expected_decisions = []
            for ordinal, issue in enumerate(stage["decision_issues"] if spec["path"] else []):
                current = primary.f0._rz_mm(states[issue])
                reference = primary.f0._rz_mm(baseline["states"][issue])
                ...
                if candidate is None:
                    failures.append(f"UNEXPECTED_DECISION_REFUSAL:{rollout_id}:{issue}")
                    break
                if candidate != stage["noop_candidate"]:
                    primary.f0.a0.b1.c0.d2._apply_macro(sequence, issue, candidate, target_map)
                expected_decisions.append((issue, candidate, before, after))
            if len(compact.get("decisions", [])) != len(expected_decisions):
                failures.append(f"DECISION_COUNT:{rollout_id}")
            for got, expected in zip(compact.get("decisions", []), expected_decisions):
            ...
            for issue, target in enumerate(sequence):
                ...
                exact = card15_target_decimal_a(target, cfg.turns_tsc,
                                                name=f"f1.audit.{rollout_id}.{issue}")
                assert_exact_slew(active, exact, name=f"f1.audit.issue.{rollout_id}.{issue}")
                active = exact
                if issue > 0:
                    assert_exact_slew(states[issue]["current_decimal_a_tsc"],
                                      states[issue + 1]["current_decimal_a_tsc"],
                                      name=f"f1.audit.observed.{rollout_id}.{issue}")
                    observed_checks += 1
            for index, state in enumerate(states):
    ...
    if complete:
        failures.extend(
            f"Q0_REFERENCE:{reason}" for reason in f0audit._reference_failures(
                run_dir, "q0_baseline", rows["q0_baseline"]["states"], baseline)
        )
    replay = f0audit.a0audit._semantic_compare(
        run_dir, "path_a", "path_a_replay", rows.get("path_a", {}).get("states", []),
    ...
    if measured is None or not f0audit.a0audit.d0audit._close(measured, reported.get("metrics")):
    ...
    result = {"schema_version": f"{primary.SCHEMA}-independent", "kind": "independent_raw_audit",
        ...
        "passed": not failures, "route": "ONE_MS_NR1000F1_INDEPENDENT_PASS" if not failures else
        "ONE_MS_NR1000F1_INDEPENDENT_FAIL", "failures": failures,
    ...
    primary.f0.b0.write_new(run_dir / "independent_audit.json", result)
    return result
