#!/usr/bin/env python3
"""Independent full-raw audit for ID-2S2."""
...
from scripts import rgeo_zgeo_1ms_id2s2_sequence_exact_replay as primary  # noqa: E402


SCHEMA = "rgeo-zgeo-1ms-id2s2-sequence-exact-replay-independent-v1"
...
def raw_replay_checks(raw_rows: Sequence[dict[str, Any]], originals: dict[str, dict[str, Any]],
    ...
    raw_stage["semantic_artifacts"] = [name for name in stage["semantic_artifacts"]
                                       if name != "inputa"]
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    try:
        stage, cfg, targets, source, baseline, frozen_s0, originals = primary.load(stage_path)
        expected = primary.campaign_streams(
            stage, cfg, targets, source, frozen_s0, originals)
    except Exception as exc:
        failures.append(f"STAGE_LOAD:{type(exc).__name__}:{exc}")
        stage, cfg, baseline, frozen_s0, originals, expected = (
    ...
    for row in compact:
        if row.get("schema_version") != primary.SCHEMA or row.get("source_revision") != source_revision:
            failures.append(f"COMPACT_SCHEMA_OR_REVISION:{row.get('rollout_id')}")
    ...
    if cfg is not None:
        for row in compact:
            ...
            if row.get("actions") and list(_fields(folder / "1134ms" / "inputa")) != row["actions"][-1]["expected_card15_fields"]:
    ...
    execution = len(raw_rows) == len(compact) == 4 and all(row.get("passed") for row in compact)
    prefixes = s1.matched_prefix_checks(raw_rows, baseline) if execution else []
    replays = raw_replay_checks(raw_rows, originals, stage) if execution else []
    responses = primary.response_checks(raw_rows, originals, baseline, stage) if execution else []
    s1_stage = load_json(ROOT / stage["evidence"]["id2s1_config"]["path"])
    metrics = s1.measured_metrics(raw_rows, baseline, frozen_s0, s1_stage) if execution else {"passed": False}
    original_metrics = load_json(ROOT / stage["evidence"]["id2s1_result"]["path"])[
    ...
    if metrics != result.get("measured_sequence_metrics") or metrics_exact != result.get(
            "measured_metrics_exactly_reproduced"):
        failures.append("METRICS_RECOMPUTE")
    counters = {
    ...
    raw_ok = (len(lines) == stage.get("required_artifact_files_if_complete")
              and not any(reason.startswith(("RAW_STATE", "MISSING_ARTIFACT", "STATE_DIRECTORY"))
                          for reason in failures))
    replay_ok = (len(prefixes) == len(replays) == len(responses) == 4
    ...
    if (result.get("route") != recomputed_route
            or bool(result.get("passed")) != (recomputed_route == stage["routes"]["pass"])):
    ...
    return {"schema_version": SCHEMA, "source_revision": source_revision,
            ...
            "recomputed_measured_sequence_metrics": metrics,
            "recomputed_measured_metrics_exactly_reproduced": metrics_exact,
            "recomputed_route": recomputed_route,
            "fresh_records_have_zero_fit_weight": True,
            "two_repeats_do_not_define_a_probabilistic_tube": True,
            "models_fit_or_updated": 0,
            "claim_boundary": "Independent full-raw ID2S2 exact sequence replay audit only; no model or control qualification."}
