#!/usr/bin/env python3
"""Independent raw audit for the fixed-1000 signed action-basis G3 campaign."""
...
from scripts import rgeo_zgeo_1ms_1000_signed_basis_g3 as primary  # noqa: E402
...
def audit(config_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    run_dir = primary.b0.inside_root(run_dir, "G3 run directory")
    failures: list[str] = []
    ...
    source_command = decimal_single_turn_currents_a(
        ...
        name="g3.independent.source_command",
    )
        ...
        try:
            ...
            if target_map is None:
                ...
                if tuple(card15_target_decimal_a(
                    frozen.q0, cfg.turns_tsc, name="g3.independent.q0"
            ...
            for issue, target in enumerate(sequence):
                ...
                exact = card15_target_decimal_a(
                    target, cfg.turns_tsc, name=f"g3.independent.{rollout_id}.{issue}"
                )
                assert_exact_slew(active, exact, name=f"g3.independent.issue.{rollout_id}.{issue}")
                active = exact
                if issue > 0:
                    assert_exact_slew(
                        states[issue]["current_decimal_a_tsc"],
                        states[issue + 1]["current_decimal_a_tsc"],
                        name=f"g3.independent.observed.{rollout_id}.{issue}",
                    )
                    observed_checks += 1
    ...
    try:
        ...
        primary_result = {}
    execution_pass = len(rows) == stage["rollouts"] and all(len(value) == 41 for value in rows.values())
    replay_pass = len(replay_rows) == len(stage["critical_replays"]) and all(row["passed"] for row in replay_rows)
    ...
    expected_counters = {
        "rollout_count": stage["rollouts"],
        "reset_calls": stage["maximum_reset_calls"],
        "advance_attempts": stage["maximum_advance_attempts"],
        "gotsc_calls": stage["maximum_gotsc_calls"],
