#!/usr/bin/env python3
"""Server-side Stage4.2R3c3 raw recomputation and compact audit."""
...
def _runtime_package_fingerprint(
    ...
    if not fingerprint:
        raise ValueError("Stage4.2R3c3 runtime package fingerprint missing")
    if run_manifest.get("semantics_preserving_resume_hotfix"):
        raise ValueError(
            "Stage4.2R3c3 has no registered runtime-hotfix package chain"
        )
    return fingerprint
...
def main() -> None:
    ...
    reported_summary = (
        r3c.read_json(reported_summary_path)
    ...
    manifest_compatibility = {
        ...
        "identification_probe": run_manifest.get("identification_probe")
    ...
    certified_primary_pass = bool(
        ...
        and recomputed_identification_gate_pass
    )
    ...
    audit = {
        ...
        "source_stage4_2r3c1_fingerprint_digest": (
            ctx.source_r3c1_fingerprint["digest"]
        ),
        "source_stage4_2r3c2_run": str(ctx.source_r3c2_run),
        "source_stage4_2r3c2_fingerprint_digest": (
            ctx.source_r3c2_fingerprint["digest"]
        ),
        ...
        "recomputed_identification_gate_pass": (
            recomputed_identification_gate_pass
        ),
    ...
    r3c.atomic_write_json(
        output_dir / "stage4_2r3c3_run_inventory.json",
        run_inventory,
    )
    r3c.atomic_write_json(
        output_dir / "stage4_2r3c3_server_audit.json", audit
    )
    print(
        json.dumps(
            {
                ...
                "condition_number_gate_passed": control_summary[
