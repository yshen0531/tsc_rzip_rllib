#!/usr/bin/env python3
"""Independent raw reparse and route audit for ID-2Z11."""
...
from scripts import rgeo_zgeo_1ms_id2z11_bounded_capture_teacher as primary  # noqa: E402


SCHEMA = "rgeo-zgeo-1ms-id2z11-independent-raw-v1"
...
def expected_streams(stage: dict[str, Any], cfg: Any, targets: dict[str, Any],
    ...
    root = primary.build_matrix(stage, cfg, targets, parent, 49, 0, "root")
    ...
    if root_selected is None:
        return root, {"root_reference": parent}
    main = primary.build_matrix(stage, cfg, targets, root_selected, 53, 1, "main")
    main_arm = scientific.get("selected_main_arm_id")
    main_selected = next((v for v in main if v["arm_id"] == main_arm), None)
    values = [*root, *main]
    refs: dict[str, dict[str, Any]] = {"root_reference": parent,
                                      "root_selected": root_selected}
    if main_selected is None:
        return values, refs
    alt_parent_id = primary.alternate_parent_arm(str(root_selected["tokens"]))
    alt_parent = next(v for v in root if v["arm_id"] == alt_parent_id)
    selected_tokens = next(v["tokens"] for v in stage["candidate_specs"]
                           if v["arm_id"] == main_arm)
    alternate = [
        primary.build_stream(stage, cfg, targets, alt_parent, 53, 2,
                             "alternate__h8", "h8", "HHHHHHHH"),
        primary.build_stream(stage, cfg, targets, alt_parent, 53, 2,
                             f"alternate__{main_arm}", main_arm, selected_tokens),
    ]
    replay = primary.replay_stream(main_selected)
    values.extend(alternate)
    values.append(replay)
    refs.update({"main_selected": main_selected, "alt_parent": alt_parent,
                 "replay": replay})
    return values, refs
...
def compact_rows(run_dir: Path, expected: Sequence[dict[str, Any]],
    ...
    rows = [load_json(path) for path in paths if "rollout_id" in load_json(path)]
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    try:
        stage, runtime, cfg, targets, parent, root_reference = primary.load(stage_path)
        expected, _ = expected_streams(stage, cfg, targets, parent, result)
    except Exception as exc:
        failures.append(f"STAGE_LOAD:{type(exc).__name__}:{exc}")
        stage, runtime, cfg, targets, parent, root_reference, expected = (
    ...
    root_rows = [raw_by_id[v] for v in (f"root__{arm}" for arm in primary.ARM_IDS)
                 if v in raw_by_id]
    root_metrics = primary._metrics(root_rows, stage, primary.ARM_IDS)
    ...
    main_rows = [raw_by_id[v] for v in (f"main__{arm}" for arm in primary.ARM_IDS)
                 if v in raw_by_id]
    main_metrics = primary._metrics(main_rows, stage, primary.ARM_IDS)
    main_arm = main_metrics.get("selected_arm_id")
    main_ref = compact_by_id.get(str(main_arm and f"main__{main_arm}"), {})
    alternate_ids = ["alternate__h8", f"alternate__{main_arm}"] if main_arm else []
    alternate_rows = [raw_by_id[v] for v in alternate_ids if v in raw_by_id]
    alt_metrics = primary._metrics(alternate_rows, stage,
                                   ("h8", main_arm) if main_arm else ("h8",))
    ...
    replay_ok = primary.z6.replay_check(
        raw_by_id.get(f"main__{main_arm}"), replay_row,
        stage["semantic_artifacts"])["passed"] if main_arm else False
    ...
    for arm in primary.ARM_IDS:
        ...
        if row:
            prefix_values.append(primary.z6.prefix_check(
                row, root_reference, 50, 49, stage["semantic_artifacts"]))
    for arm in primary.ARM_IDS:
        row = compact_by_id.get(f"main__{arm}")
        if row:
            prefix_values.append(primary.z6.prefix_check(
                row, root_ref, 54, 53, stage["semantic_artifacts"]))
    if main_arm and root_arm:
        alt_parent_id = primary.alternate_parent_arm(
            next(v["tokens"] for v in stage["candidate_specs"]
                 if v["arm_id"] == root_arm))
        alt_ref = compact_by_id.get(f"root__{alt_parent_id}", {})
        for rollout_id in alternate_ids:
            row = compact_by_id.get(rollout_id)
            if row:
                prefix_values.append(primary.z6.prefix_check(
                    row, alt_ref, 54, 53, stage["semantic_artifacts"]))
    ...
    route = primary.route_for(stage, execution, raw_ok, prefixes_ok,
                              root_metrics, main_metrics, alt_ok, replay_ok, capture)
    scientific = {
        "root_metrics": root_metrics, "main_metrics": main_metrics,
        "alternate_metrics": alt_metrics,
        "alternate_parent_arm_id": (
            primary.alternate_parent_arm(next(v["tokens"] for v in stage["candidate_specs"]
                                              if v["arm_id"] == root_arm))
            if root_arm else None),
        ...
        "selected_root_committed_tokens": (
            next(v["tokens"] for v in stage["candidate_specs"]
                 if v["arm_id"] == root_arm)[:4] if root_arm else None),
    ...
    audit_value = {
        ...
        **counters, "models_fit_or_updated": 0,
        ...
        "claim_boundary": "Independent raw audit of finite ID2Z11 only.",
    }
    return audit_value
