#!/usr/bin/env python3
"""Structurally separate final-raw audit for NR2R2C2aA1."""
...
from scripts.rgeo_zgeo_1ms_nr2r2c2aa1_authority import (  # noqa: E402
    FAIL_ROUTE, PASS_ROUTE, SCHEMA, load, target_from_fields, targets_for,
)
...
def independent_authority_metrics(
    states: list[dict[str, Any]], q0_states: list[dict[str, Any]], stage: dict[str, Any]
) -> dict[str, Any]:
    first = stage["authority_window_start_state"]
    last = stage["authority_window_end_state"]
    opposition = [
        (states[index]["r_geo_m"] - q0_states[index]["r_geo_m"])
        - (states[index]["z_geo_m"] - q0_states[index]["z_geo_m"])
        for index in range(first, last + 1)
    ]
    ip = [states[index]["ip_a"] - q0_states[index]["ip_a"]
          for index in range(first, last + 1)]
    values = {
        "window_states": list(range(first, last + 1)),
        "opposition_m": opposition,
        "mean_opposition_m": sum(opposition) / len(opposition),
        "positive_opposition_fraction": sum(value > 0.0 for value in opposition) / len(opposition),
        "maximum_opposition_m": max(opposition),
        "maximum_absolute_ip_deviation_from_q0_a": max(abs(value) for value in ip),
    }
    values["passed"] = (
        ...
        and values["maximum_absolute_ip_deviation_from_q0_a"]
        <= stage["maximum_absolute_ip_deviation_from_q0_a"]
    )
    return values
...
def metric_difference(left: dict[str, Any], right: dict[str, Any]) -> float:
    if (left.get("window_states") != right.get("window_states")
            or len(left.get("opposition_m", ())) != len(right.get("opposition_m", ()))):
        return float("inf")
    scalar_keys = (
        "mean_opposition_m", "positive_opposition_fraction", "maximum_opposition_m",
        "maximum_absolute_ip_deviation_from_q0_a",
    )
    values = [abs(float(left[key]) - float(right[key])) for key in scalar_keys]
    values.extend(abs(float(a) - float(b))
                  for a, b in zip(left["opposition_m"], right["opposition_m"]))
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    source_command = decimal_single_turn_currents_a(
        ...
        name="c2aa1.independent.source_command",
    )
    for index in range(primary["rollouts_completed"]):
        rollout_id = f"level2_authority_r{index}"
        ...
        try:
            ...
            level1 = target_from_fields(stage["level1_card15_fields"], cfg, "c2aa1.independent.level1")
            level2 = target_from_fields(stage["level2_card15_fields"], cfg, "c2aa1.independent.level2")
            ...
            for step in range(len(states) - 1):
                ...
                exact = card15_target_decimal_a(
                    expected, cfg.turns_tsc, name=f"c2aa1.independent.{rollout_id}.{step}"
                )
                issued = assert_exact_slew(
                    previous_command, exact, name=f"c2aa1.independent.issue.{rollout_id}.{step}"
                )
                assert_exact_slew(
                    states[step]["current_decimal_a_tsc"], states[step + 1]["current_decimal_a_tsc"],
                    name=f"c2aa1.independent.readback.{rollout_id}.{step}",
                )
                actions.append({
                    "issue_step": step, "issue_time_ms": 1100 + step,
            ...
            metrics = independent_authority_metrics(states, q0_record["states"], stage) if len(states) == 33 else None
            ...
            rows.append({
                ...
                "pair_id": "level2_authority", "passed": compact["passed"],
    ...
    authority = bool(repeatable and all(row["authority_metrics"]["passed"] for row in rows))
    expected_route = (
        "ONE_MS_NR2R2C2AA1_EXECUTION_FAIL_STOP" if not execution else
    ...
    result = {
        "schema_version": SCHEMA + "-independent-v1", "source_revision": source_revision,
        "audit_passed": audit_passed,
        "route": ("ONE_MS_NR2R2C2AA1_INDEPENDENT_AUDIT_PASS" if audit_passed
                  else "ONE_MS_NR2R2C2AA1_INDEPENDENT_AUDIT_FAIL_STOP"),
