#!/usr/bin/env python3
"""Independent full-raw audit for ID-2Z31."""
...
from scripts import rgeo_zgeo_1ms_id2z31_event_phase_discriminator as primary  # noqa: E402
...
SCHEMA = "rgeo-zgeo-1ms-id2z31-event-phase-discriminator-independent-v1"
...
def _event_map(rows: Sequence[dict[str, Any]], stage: dict[str, Any]) -> dict[str, Any]:
    window = [int(value) for value in stage["event_window_effect_states"]]
    threshold = float(stage["positive_r_event_threshold_m"])
    mapped = []
    for row in rows:
        states = row.get("states", []); events = []
        if row.get("passed") and len(states) == 74:
            for index in window:
                delta = float(states[index]["r_geo_m"]) - float(states[index - 1]["r_geo_m"])
                if delta > threshold:
                    events.append({"effect_state_index": index, "delta_r_m": delta})
        complete = bool(row.get("passed") and len(states) == 74)
        mapped.append({"family_id": row.get("family_id"), "complete": complete,
                       "events": events, "event_count": len(events),
                       "passed": bool(complete and len(events) == 1)})
...
def audit(config: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    centered = semantic.get("fresh_event_baseline_transition_center")
    prefixes = []
    for frozen in expected[:len(compact)]:
        ...
        prefixes.append(primary.z30.z27.z6.prefix_check(
    ...
    replay = (primary.z30.z27.z6.replay_check(
        semantic.get("event_issue44__q_z__plus_then_return"),
        semantic.get("replay_event_issue44__q_z__plus_then_return"),
        stage["semantic_artifacts"]) if len(raw_rows) >= 4
              else {"passed": False, "failures": ["NOT_RUN"]})
    mapping = _event_map(raw_rows, stage)
    if not execution:
    ...
    if result.get("event_map") != mapping:
        failures.append("EVENT_MAP_RECOMPUTE")
    ...
    return {"schema_version": SCHEMA, "source_revision": source_revision,
            ...
            "recomputed_event_map": mapping, "recomputed_replay_metrics": replay,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    primary.z30.z27.io.write_new(args.output, value)
