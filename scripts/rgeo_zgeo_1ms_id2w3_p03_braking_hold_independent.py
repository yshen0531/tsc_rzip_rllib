#!/usr/bin/env python3
"""Independent full-raw audit for ID-2W3."""
...
from scripts import rgeo_zgeo_1ms_id2w3_p03_braking_hold as primary  # noqa: E402


SCHEMA = "rgeo-zgeo-1ms-id2w3-independent-raw-v1"
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    try:
        ...
        expected = primary.action_streams(stage, cfg, targets)
    ...
    paths = sorted(path for path in run_dir.glob("*.json") if path.name not in allowed)
    compact = [load_json(path) for path in paths]
    compact.sort(key=lambda row: order.get(row.get("rollout_id"), 999))
    expected_ids = [row["rollout_id"] for row in expected[:len(compact)]]
    if [row.get("rollout_id") for row in compact] != expected_ids:
    ...
    execution = len(raw_rows) == 2 and all(
        row.get("passed") and len(row["states"]) == 97
        and len(row.get("actions", [])) == 96
        and int(row.get("verified_plant_advances", 0)) == 96 for row in raw_rows)
    raw_ok = bool(execution and len(lines) == 970 and not any(
        failure.startswith("MISSING_ARTIFACT") for failure in failures))
    prefixes = ([primary.prefix_check(row, reference, stage["semantic_artifacts"])
    ...
    return {
        ...
        "claim_boundary": (
            "Independent full-raw ID2W3 audit; a scientific PASS/FAIL remains finite "
            "source-local nominal braking/hold design evidence only."),
    }
