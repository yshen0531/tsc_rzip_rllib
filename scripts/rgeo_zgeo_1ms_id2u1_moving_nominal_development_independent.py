#!/usr/bin/env python3
"""Independent full-raw audit for ID-2U1."""
...
from scripts import rgeo_zgeo_1ms_id2u1_moving_nominal_development as primary  # noqa: E402


SCHEMA = "rgeo-zgeo-1ms-id2u1-independent-raw-v1"
...
def audit(stage_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    try:
        stage, cfg, expected = primary.load(stage_path)
    ...
    prefixes = primary.matched_prefix_checks(raw_rows, stage) if len(raw_rows) == 20 else []
    metrics = primary.response_metrics(raw_rows, stage) if len(raw_rows) == 20 else {"passed": False}
    ...
    complete = (len(raw_rows) == 20 and len(lines) == 4100 and len(prefixes) == 4
                and all(row["passed"] for row in prefixes) and metrics.get("passed"))
    ...
    if result.get("calibration_or_holdout_records_read") != 0:
        failures.append("CALIBRATION_OR_HOLDOUT_READ")
    return {
        ...
        "claim_boundary": "Independent full-raw ID2U1 development audit only; no model or control qualification.",
    }
