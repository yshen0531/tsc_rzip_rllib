#!/usr/bin/env python3
"""Independent raw reconstruction for fixed-1000 D2."""
...
from scripts import rgeo_zgeo_1ms_1000_value_d2 as primary  # noqa: E402
...
def audit(config_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    stage, cfg, baseline, d1_stage = primary.load(config_path)
    run_dir = primary.b0.inside_root(run_dir, "D2 run directory")
    failures: list[str] = []
    ...
    source_command = decimal_single_turn_currents_a(
        ...
        cfg.turns_tsc, name="d2.independent.source_command",
    )
    ...
    for spec in specs:
        ...
        try:
            ...
            if target_map is None:
                ...
                target_map = primary.d1.targets(d1_stage, cfg, source)
            ...
            for issue, target in enumerate(sequence):
                ...
                exact = card15_target_decimal_a(target, cfg.turns_tsc, name=f"d2.audit.{rollout_id}.{issue}")
                assert_exact_slew(active, exact, name=f"d2.audit.issue.{rollout_id}.{issue}")
                active = exact
                if issue > 0:
                    assert_exact_slew(states[issue]["current_decimal_a_tsc"],
                                      states[issue + 1]["current_decimal_a_tsc"],
                                      name=f"d2.audit.observed.{rollout_id}.{issue}")
                    observed_checks += 1
    ...
    for family in stage["critical_replays"]:
        comparison = d1audit.semantic_replay(
            run_dir, family, f"{family}_replay", rows.get(family, []), rows.get(f"{family}_replay", [])
        )
    ...
    metrics = primary.scientific_metrics(
        [{**spec, "states": rows[spec["rollout_id"]]} for spec in specs if spec["repeat_index"] == 0], stage
    ...
    expected_pass = execution_pass and replay_pass and scientific_pass
    ...
    result = {
        ...
        "passed": not failures,
        "route": "ONE_MS_NR1000D2_INDEPENDENT_PASS" if not failures else "ONE_MS_NR1000D2_INDEPENDENT_FAIL",
