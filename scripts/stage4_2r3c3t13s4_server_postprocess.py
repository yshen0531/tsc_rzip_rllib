#!/usr/bin/env python3
"""Independent server-side authentication of the T13S4 blind holdout."""
...
def main() -> None:
    ...
    ctx = s4.load_stage42r3c3t13s4_config(
        args.config,
    ...
    raw_identity_exact = bool(
        ...
        and len(list(ctx.paths.raw.glob("*.json.gz"))) == 52
    ...
    audit = {
        ...
        "scientific_route": (
            ...
            if primary_pass else (
                "LATTICE_HOLDOUT_FAIL_REDESIGN" if certified
    ...
    for name, value in (
        ("stage4_2r3c3t13s4_raw_inventory.json", raw_inventory),
        ("stage4_2r3c3t13s4_run_inventory.json", run_inventory),
        ("stage4_2r3c3t13s4_snapshot_audit.json", snapshots),
        ("stage4_2r3c3t13s4_server_audit.json", audit),
    ):
        s4.t11.t1.r3c3.atomic_write_json(audit_dir / name, value)
    audit_path = audit_dir / "stage4_2r3c3t13s4_server_audit.json"
    print(json.dumps({
