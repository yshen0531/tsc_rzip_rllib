#!/usr/bin/env python3
"""Independent raw reconstruction for fixed-1000 D3."""
...
from scripts import rgeo_zgeo_1ms_1000_signed_basis_g3_independent as g3audit  # noqa: E402
from scripts import rgeo_zgeo_1ms_1000_discrete_value_d3 as primary  # noqa: E402
...
def audit(config_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    run_dir = primary.b0.inside_root(run_dir, "D3 run directory")
    failures: list[str] = []
    ...
    source_command = decimal_single_turn_currents_a(
        ...
        cfg.turns_tsc, name="d3.independent.source_command")
    ...
    for spec in specs:
        ...
        actual_times = sorted(int(p.name[:-2]) for p in root.iterdir()
                              if p.is_dir() and p.name.endswith("ms") and p.name[:-2].isdigit()) if root.is_dir() else []
        ...
        try:
            ...
            failures.extend(f"SOURCE:{rollout_id}:{reason}" for reason in g3audit._source_reasons(
            ...
            for issue, target in enumerate(primary.sequence_for(spec, stage, target_map)):
                ...
                exact = card15_target_decimal_a(target, cfg.turns_tsc, name=f"d3.audit.{rollout_id}.{issue}")
                assert_exact_slew(active, exact, name=f"d3.audit.issue.{rollout_id}.{issue}")
                active = exact
                if issue > 0:
                    assert_exact_slew(states[issue]["current_decimal_a_tsc"], states[issue+1]["current_decimal_a_tsc"],
                                      name=f"d3.audit.observed.{rollout_id}.{issue}")
                    observed_checks += 1
            for index, state in enumerate(states):
                if state["time_ms"] != 1000+index: failures.append(f"TIME:{rollout_id}:{index}")
                if state["abnormal"]: failures.append(f"ABNORMAL:{rollout_id}:{index}")
                if not state["r_inner_m"] <= state["r_geo_m"] <= state["r_outer_m"]:
                    failures.append(f"LIMITER:{rollout_id}:{index}")
                if abs(state["r_geo_m"]-states[0]["r_geo_m"]) > 0.05: failures.append(f"R_ENVELOPE:{rollout_id}:{index}")
                if abs(state["z_geo_m"]-states[0]["z_geo_m"]) > 0.05: failures.append(f"Z_ENVELOPE:{rollout_id}:{index}")
                if state["ip_a"]*states[0]["ip_a"] <= 0 or abs(state["ip_a"]-states[0]["ip_a"]) > .10*abs(states[0]["ip_a"]):
                    failures.append(f"IP_ENVELOPE:{rollout_id}:{index}")
                if any(v < lo or v > hi for v, lo, hi in zip(state["current_a_tsc"], cfg.min_current_a_tsc, cfg.max_current_a_tsc)):
    ...
    for family in stage["critical_replays"]:
        comparison = d1audit.semantic_replay(run_dir, family, f"{family}_replay",
                                              rows.get(family, []), rows.get(f"{family}_replay", []))
    ...
    metrics = primary.scientific_metrics([{**spec, "states": rows[spec["rollout_id"]]}
                                          for spec in specs if spec["repeat_index"] == 0], stage) if complete else None
    try:
        reported = json.loads((run_dir / "result.json").read_text(encoding="utf-8"))
    except Exception as exc:
        failures.append(f"PRIMARY_RESULT:{type(exc).__name__}:{exc}")
        reported = {}
    execution = complete and len(rows) == stage["rollouts"]
    replay_pass = len(replay_rows) == 2 and all(r["passed"] for r in replay_rows)
    science = metrics is not None and metrics["passed"]
    expected_pass = execution and replay_pass and science
    expected_route = stage["routes"]["pass"] if expected_pass else (stage["routes"]["execution_fail"] if not execution
        else stage["routes"]["replay_fail"] if not replay_pass else stage["routes"]["scientific_fail"])
    for key, expected in {"rollout_count":16, "reset_calls":16, "advance_attempts":768,
                          "gotsc_calls":768, "verified_plant_advances":768}.items():
        if reported.get(key) != expected: failures.append(f"PRIMARY_COUNTER:{key}")
    if reported.get("source_revision") != source_revision: failures.append("PRIMARY_SOURCE_REVISION")
    if reported.get("passed") is not expected_pass or reported.get("route") != expected_route:
        failures.append("PRIMARY_VERDICT")
    if metrics is None or not g3audit._close(metrics, reported.get("scientific_metrics")):
        failures.append("PRIMARY_SCIENTIFIC_METRICS")
    if target_map is None or not g3audit._close(target_map["action_geometry"], reported.get("action_geometry")):
    ...
    result = {"schema_version": f"{primary.SCHEMA}-independent", "kind": "independent_raw_audit",
              ...
              "raw_rollouts": sum(len(v)==49 for v in rows.values()), "raw_states": raw_states,
              "action_checks": action_checks, "observed_slew_checks": observed_checks,
