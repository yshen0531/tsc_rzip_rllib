#!/usr/bin/env python3
"""Independent full-raw audit for frozen ID-2Z25."""
...
from scripts.rgeo_zgeo_1ms_id0_vector_tail import write_new
from scripts import rgeo_zgeo_1ms_id2z9_late_root_branch_utility_support_independent as rawio
from scripts import rgeo_zgeo_1ms_id2z25_centered_coallocation_campaign as primary

SCHEMA = "rgeo-zgeo-1ms-id2z25-independent-raw-v1"
...
def _geometry(vectors: Sequence[np.ndarray], count: int) -> dict[str, float]:
    angles = sorted((math.degrees(math.atan2(float(v[1]), float(v[0]))) + 360.0) % 360.0
                    for v in vectors)
    gap = max(angles[(i + 1) % len(angles)] + (360.0 if i == len(angles) - 1 else 0.0)
              - angles[i] for i in range(len(angles)))
    weak = min(max(float(np.dot(v, np.asarray([math.cos(a), math.sin(a)]))) for v in vectors)
               for a in (2.0 * math.pi * i / count for i in range(count)))
...
def scientific_metrics(rows: Sequence[dict[str, Any]], stage: dict[str, Any]) -> dict[str, Any]:
    ...
    branches: list[dict[str, Any]] = []; all_signal = baseline_complete
    ...
    if baseline_complete:
        for phase in stage["phase_issue_steps"]:
            ...
            for h in (4, 8):
                value = _geometry(grouped[h], gates["direction_grid_count"])
                value.update({"phase_issue": phase, "horizon": h,
                              "diagnostic_only": True})
                geometry.append(value)
    replay = _replay(by_id.get("issue24__p05_plus__plus_then_minus"),
    ...
    if baseline_complete:
        ...
        for row in rows:
            ...
            capture.append({"family_id": row["family_id"],
                            ...
                            "diagnostic_only": True})
    return {"baseline_complete": baseline_complete, "branch_metrics": branches,
...
def audit(stage_path: Path, run_dir: Path, source_revision: str,
    ...
    raw_rows: list[dict[str, Any]] = []; inventory_lines: list[str] = []; inventory_bytes = 0
    if cfg is not None:
    ...
    for frozen in expected[:len(compact)]:
        ...
        prefixes.append(primary.z6.prefix_check(
            compact_by_id.get(family, {}), reference, state_count, action_count,
            stage["semantic_artifacts"]))
    ...
    prefix_ok = bool(len(prefixes) == len(compact) and all(row.get("passed") for row in prefixes))
    ...
    if result.get("route") != route or result.get("passed") != (route == stage["routes"]["data_pass"]):
    ...
    return {"schema_version": SCHEMA, "source_revision": source_revision,
            ...
            "claim_boundary": "Independent full-raw audit of finite ID2Z25 D0 only."}
