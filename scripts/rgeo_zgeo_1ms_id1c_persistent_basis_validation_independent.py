#!/usr/bin/env python3
"""Independent raw reparse for the frozen 1 ms ID-1C discriminator."""
...
SCHEMA = "rgeo-zgeo-1ms-id1c-persistent-basis-validation-v1-independent"
...
def _metrics(rows: Sequence[dict[str, Any]], stage: dict[str, Any]) -> dict[str, Any]:
    ...
    for direction in direction_ids:
        ...
        pair_metrics.append({
            ...
            "even_rz_norm_m": float(math.hypot(even[0], even[1])),
            "odd_rz_norm_m": float(math.hypot(odd[0], odd[1])),
        })
    keys = [f"{direction}:{sign}" for direction in direction_ids for sign in ("plus", "minus")]
    vectors = []
    for key in keys:
    ...
    return {
        "arm_metrics": arms, "pair_metrics": pair_metrics, "vector_keys": keys,
        ...
        "signal_passed": signal, "symmetry_passed": symmetry, "ip_passed": ip_ok,
...
def audit(
    ...
    try:
        ...
        for spec in specs:
            ...
            expected_dirs = [f"{time}ms" for time in range(1100, 1119)]
            if state_dirs != expected_dirs:
                raise ValueError(f"state directory set:{spec['rollout_id']}")
            states = [_state(folder / f"{time}ms", cfg) for time in range(1100, 1119)]
            ...
            for issue, target in enumerate(stream["targets"]):
                ...
                exact = decimal_single_turn_currents_a(
                    ...
                    name=f"id1c.audit.{spec['rollout_id']}.{issue}",
                )
                maximum = assert_exact_slew(previous_command, exact, name=f"id1c.audit.issue.{issue}")
                observed = assert_exact_slew(
                    states[issue]["actual_current_decimal_a_tsc"],
                    states[issue + 1]["actual_current_decimal_a_tsc"],
                    name=f"id1c.audit.observed.{issue}",
                )
    ...
    if len(rows) == stage["rollouts"] and not any(item.startswith("RAW:") for item in failures):
        ...
        if primary.get("reset_calls") != 10 or primary.get("advance_attempts") != 180 or primary.get("plant_advance_gotsc_calls") != 180 or primary.get("verified_plant_advances") != 180:
    ...
    result = {
        ...
        "route": "ONE_MS_ID1C_INDEPENDENT_AUDIT_PASS" if not failures else "ONE_MS_ID1C_INDEPENDENT_AUDIT_FAIL",
...
def main() -> int:
    ...
    parser.add_argument("--stage-config", type=Path, default=ROOT / "configs/rgeo_zgeo_1ms_id1c_persistent_basis_validation.json")
