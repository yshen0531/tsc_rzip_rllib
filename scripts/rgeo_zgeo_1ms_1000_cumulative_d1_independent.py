#!/usr/bin/env python3
"""Independent raw audit for the fixed-1000 cumulative D1 campaign."""
...
from scripts import rgeo_zgeo_1ms_1000_cumulative_d1 as primary  # noqa: E402
...
def semantic_replay(
    ...
    if len(left) != 49 or len(right) != 49:
...
def audit(config_path: Path, run_dir: Path, source_revision: str) -> dict[str, Any]:
    ...
    run_dir = primary.b0.inside_root(run_dir, "D1 run directory")
    failures: list[str] = []
    ...
    source_command = decimal_single_turn_currents_a(
        ...
        name="d1.independent.source_command",
    )
        ...
        try:
            ...
            for issue, target in enumerate(sequence):
                ...
                exact = card15_target_decimal_a(
                    target, cfg.turns_tsc, name=f"d1.independent.{rollout_id}.{issue}"
                )
                assert_exact_slew(active, exact, name=f"d1.independent.issue.{rollout_id}.{issue}")
                active = exact
                if issue > 0:
                    assert_exact_slew(
                        states[issue]["current_decimal_a_tsc"],
                        states[issue + 1]["current_decimal_a_tsc"],
                        name=f"d1.independent.observed.{rollout_id}.{issue}",
                    )
                    observed_checks += 1
    ...
    for family in stage["critical_replays"]:
        comparison = semantic_replay(
            run_dir, family, f"{family}_replay",
    ...
    metrics = None
    if all(len(rows.get(spec["rollout_id"], [])) == 49 for spec in specs):
    ...
    try:
        ...
        primary_result = {}
    execution_pass = len(rows) == 10 and all(len(value) == 49 for value in rows.values())
    ...
    for key, expected in {
        "rollout_count": 10, "reset_calls": 10, "advance_attempts": 480,
        "gotsc_calls": 480, "verified_plant_advances": 480,
    }.items():
