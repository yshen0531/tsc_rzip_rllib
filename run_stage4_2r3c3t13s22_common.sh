...
# shellcheck source=scripts/stage4_2r3c3t13s22_shell_common.sh
source "${PROJECT_DIR}/scripts/stage4_2r3c3t13s22_shell_common.sh"
stage4_2r3c3t13s22_validate_common
SOURCE_S21_RUN="$(stage4_2r3c3t13s22_find_s21_run)"
...
OUTPUT_DIR="${STAGE4_2R3C3T13S22_OUTPUT_DIR:-$(stage4_2r3c3t13s22_new_output_dir)}"
[[ ! -e "${OUTPUT_DIR}" ]] || {
  echo "ERROR: S22 output must be a new path: ${OUTPUT_DIR}" >&2
  exit 1
}
stage4_2r3c3t13s22_export_runtime
printf '[Stage4.2R3c3T13S22] user=%s host=%s pwd=%s\n' "$(id -un)" "$(hostname)" "$(pwd)"
printf '[Stage4.2R3c3T13S22] source_s21_run=%s\noutput=%s\n' "${SOURCE_S21_RUN}" "${OUTPUT_DIR}"
printf '[Stage4.2R3c3T13S22] read-only S21 raw affine audit: zero plant/controller/snapshot execution.\n'
printf '[Stage4.2R3c3T13S22] Formal 250/270 ms arrival and 350/370 ms hold timing is unchanged.\n'
cd "${PROJECT_DIR}"
exec "${STAGE4_2R3C3T13S22_PYTHON}" \
  docs/codex/audit_tools/stage4_2r3c3t13s22_full_horizon_affine_authority.py \
  --config "${STAGE4_2R3C3T13S22_CONFIG}" \
  --source-s21-config "${STAGE4_2R3C3T13S22_S21_CONFIG}" \
