...
RUN_DIR="${STAGE4_2R3C3T13S5_RUN_DIR:-}"
if [[ -z "${RUN_DIR}" ]]; then
  LATEST="${STAGE4_2R3C3T13S5_OUTPUT_ROOT}/latest_stage4_2r3c3t13s5_run.txt"
  [[ -f "${LATEST}" ]] || { echo "ERROR: T13S5 latest-run pointer missing" >&2; exit 1; }
  RUN_DIR="$(tr -d '\r\n' < "${LATEST}")"
fi
[[ -d "${RUN_DIR}" ]] || { echo "ERROR: T13S5 run directory missing" >&2; exit 1; }
AUDIT_ROOT="${STAGE4_2R3C3T13S5_AUDIT_ROOT:-${PROJECT_DIR}/stage4_2r3c3t13s5_audits}"
AUDIT_DIR="${STAGE4_2R3C3T13S5_AUDIT_DIR:-${AUDIT_ROOT}/$(basename "${RUN_DIR}")}"
mkdir -p "${AUDIT_DIR}"
export PYTHONPATH="${PROJECT_DIR}${PYTHONPATH:+:${PYTHONPATH}}"
exec "${STAGE4_2R3C3T13S5_PYTHON}" "${PROJECT_DIR}/scripts/stage4_2r3c3t13s5_server_postprocess.py" \
