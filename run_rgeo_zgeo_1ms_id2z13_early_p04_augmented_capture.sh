...
SOURCE_REVISION="${ID2Z13_SOURCE_REVISION:-}"
OUTPUT="${ID2Z13_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z13_early_p04_augmented_capture.json"

fail() { printf 'ID2Z13_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z13_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z13_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z13_early_p04_augmented_capture.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z13_early_p04_augmented_capture_independent.py \
