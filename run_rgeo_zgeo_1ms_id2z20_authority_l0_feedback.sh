...
SOURCE_REVISION="${ID2Z20_SOURCE_REVISION:-}"
OUTPUT="${ID2Z20_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z20_authority_l0_feedback.json"

fail() { printf 'ID2Z20_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z20_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z20_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z20_authority_l0_feedback.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z20_authority_l0_feedback_independent.py \
