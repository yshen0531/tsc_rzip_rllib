...
> **2026-08-21 post-ID2Z27 amendment:** use the authenticated q_R/q_Z D0
> response as a bounded local event/value learning seed, not as Authority or
> capture. The immediate model target is 1--8 ms action-conditioned response,
> terminal one-step velocity and constraint/value margins with exact current
> observation and Card15 actions; full free-rollout world-state prediction is
> secondary. Authority-L0 design proceeds in parallel and must be validated
> by fresh sequence-level TSC evidence. Real feedback remains gated by fresh
> model calibration/blind holdout AND Authority-L0 AND Recourse-L1 AND the
> hard interface. Capture remains a stronger diagnostic and cannot be
> inferred from D0's input/output rank or positive span.

> **2026-08-20 historical route amendment:** before another learned predictor or
