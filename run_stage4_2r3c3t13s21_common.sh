...
RUN_DIR="${STAGE4_2R3C3T13S21_RUN_DIR:-$(stage4_2r3c3t13s21_new_run_dir)}"
if [[ "${STAGE4_2R3C3T13S21_RESUME}" == 1 ]]; then
  [[ -f "${RUN_DIR}/stage4_2r3c3t13s21_state.json" ]] || { echo "ERROR: resume state missing" >&2; exit 1; }
else
...
printf '[T13S21] user=%s host=%s pwd=%s\n' "$(id -un)" "$(hostname)" "$(pwd)"
printf '[T13S21] run_dir=%s command=%s backend=%s resume=%s workers=%s\n' "${RUN_DIR}" "${STAGE4_2R3C3T13S21_COMMAND}" "${STAGE4_2R3C3T13S21_BACKEND}" "${STAGE4_2R3C3T13S21_RESUME}" "${STAGE4_2R3C3T13S21_WORKERS}"
printf '[T13S21] fixed phased matrix: training 216, calibration 72, holdout 72.\n'
cd "${PROJECT_DIR}"
args=(
  --config "${STAGE4_2R3C3T13S21_CONFIG}"
  ...
  --run-dir "${RUN_DIR}" --command "${STAGE4_2R3C3T13S21_COMMAND}" --backend "${STAGE4_2R3C3T13S21_BACKEND}"
)
[[ "${STAGE4_2R3C3T13S21_RESUME}" == 1 ]] && args+=(--resume)
exec "${STAGE4_2R3C3T13S21_PYTHON}" scripts/stage4_2r3c3t13s21_cumulative_exact_card15_pooled_observer_campaign.py "${args[@]}"
