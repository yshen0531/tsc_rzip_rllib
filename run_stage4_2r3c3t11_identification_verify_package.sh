...
expected = {
    ...
    "controller_revision": (
        "persistent_step_response_probe_v42r3c3t11_v1"
    ),
    "package_revision": (
        "r42r3c3t11_persistent_step_response_identification_v1"
    ),
    "run_name": (
        "stage4_2r3c3t11_persistent_step_response_identification"
    ),
}
...
if not t11.self_test()["passed"]:
    raise SystemExit("T11 self-test failed")
cfg = json.loads(
    (
        root
        / "configs/stage4_2r3c3t11_persistent_step_response_identification_500ms.json"
    ).read_text(encoding="utf-8")
)
t11._validate_config(cfg)
for path in (
    root
    / "run_stage4_2r3c3t11_persistent_step_response_identification_native.sh",
    root / "run_stop_stage4_2r3c3t11_now.sh",
):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[Stage4.2R3c3T11 verify] Python/JSON/scientific guards passed.")
PY
...
"${PYTHON_BIN}" -m unittest \
  tests.test_stage4_2r3c3t11_persistent_step_response_identification
printf '[Stage4.2R3c3T11 verify] checksums, shell syntax and focused tests passed.\n'
