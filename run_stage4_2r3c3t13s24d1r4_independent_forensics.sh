...
RUN_DIR="${STAGE4_2R3C3T13S24D1R4_RUN_DIR:-$(cat "${STAGE4_2R3C3T13S24D1R4_OUTPUT_ROOT}/latest_stage4_2r3c3t13s24d1r4_run.txt")}"
COMPLETE_LOG="${STAGE4_2R3C3T13S24D1R4_LOG_FILE:-$(cat "${PROJECT_DIR}/logs/nohup/latest_stage4_2r3c3t13s24d1r4.log")}"
OUTPUT="${RUN_DIR}/stage4_2r3c3t13s24d1r4_causal_split_return_safety_sentinel/analysis/independent_server_forensics.json"
[[ -f "${RUN_DIR}/stage4_2r3c3t13s24d1r4_causal_split_return_safety_sentinel/final_result.json" ]] || { echo "ERROR: D1R4 final result missing" >&2; exit 1; }
[[ -f "${COMPLETE_LOG}" ]] || { echo "ERROR: D1R4 complete log missing" >&2; exit 1; }
[[ ! -e "${OUTPUT}" ]] || { echo "ERROR: D1R4 independent output must be new" >&2; exit 1; }
stage4_2r3c3t13s24d1r4_export_runtime
cd "${PROJECT_DIR}"
exec "${STAGE4_2R3C3T13S24D1R4_PYTHON}" \
  docs/codex/audit_tools/stage4_2r3c3t13s24d1r4_independent_forensics.py \
  --config "${STAGE4_2R3C3T13S24D1R4_CONFIG}" --source-d1r2-run "${SOURCE_D1R2_RUN}" \
