...
SOURCE_REVISION="${ID2Z11_SOURCE_REVISION:-}"
OUTPUT="${ID2Z11_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z11_bounded_capture_teacher.json"

fail() { printf 'ID2Z11_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z11_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z11_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z11_bounded_capture_teacher.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z11_bounded_capture_teacher_independent.py \
