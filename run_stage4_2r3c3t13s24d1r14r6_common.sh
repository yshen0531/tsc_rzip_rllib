...
source "${PROJECT_DIR}/scripts/stage4_2r3c3t13s24d1r14r6_shell_common.sh"
...
SOURCE_R4_RUN="$(stage4_2r3c3t13s24d1r14r6_find_source_r4)"
SOURCE_R5_OUTPUT="$(stage4_2r3c3t13s24d1r14r6_find_source_r5)"
...
RUN_DIR="${STAGE4_2R3C3T13S24D1R14R6_RUN_DIR:-$(stage4_2r3c3t13s24d1r14r6_new_run_dir)}"
STATE_FILE="${RUN_DIR}/stage4_2r3c3t13s24d1r14r6_direction0_replacement_sentinel/stage_state.json"
if [[ "${STAGE4_2R3C3T13S24D1R14R6_COMMAND}" == offline ]]; then
  [[ "${STAGE4_2R3C3T13S24D1R14R6_RESUME}" == 0 ]] || { echo "ERROR: offline phase cannot resume" >&2; exit 1; }
  [[ ! -e "${RUN_DIR}" || -z "$(find "${RUN_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]] || { echo "ERROR: fresh run directory not empty" >&2; exit 1; }
else
  [[ -f "${STATE_FILE}" ]] || { echo "ERROR: R6 frozen state missing" >&2; exit 1; }
fi
mkdir -p "${RUN_DIR}"
stage4_2r3c3t13s24d1r14r6_export_runtime
printf '[T13S24D1R14R6] user=%s host=%s pwd=%s\n' "$(id -un)" "$(hostname)" "$(pwd)"
printf '[T13S24D1R14R6] run_dir=%s command=%s backend=%s resume=%s workers=%s\n' "${RUN_DIR}" "${STAGE4_2R3C3T13S24D1R14R6_COMMAND}" "${STAGE4_2R3C3T13S24D1R14R6_BACKEND}" "${STAGE4_2R3C3T13S24D1R14R6_RESUME}" "${STAGE4_2R3C3T13S24D1R14R6_WORKERS}"
printf '[T13S24D1R14R6] 48 fresh TSC direction-0 replacement sentinels; formal tracking is diagnostic only.\n'
printf '[T13S24D1R14R6] formal 250/270 ms arrival and 350/370 ms hold timing is unchanged.\n'
cd "${PROJECT_DIR}"
args=(
  --config "${STAGE4_2R3C3T13S24D1R14R6_CONFIG}" --source-d1r13-run "${SOURCE_D1R13_RUN}" --source-d1r13-audit "${SOURCE_D1R13_AUDIT}" --source-r1a-output "${SOURCE_R1A_OUTPUT}"
  ...
  --source-r4-run "${SOURCE_R4_RUN}" --source-r5-output "${SOURCE_R5_OUTPUT}"
  ...
  --run-dir "${RUN_DIR}" --command "${STAGE4_2R3C3T13S24D1R14R6_COMMAND}" --backend "${STAGE4_2R3C3T13S24D1R14R6_BACKEND}"
)
[[ "${STAGE4_2R3C3T13S24D1R14R6_RESUME}" == 1 ]] && args+=(--resume)
exec "${STAGE4_2R3C3T13S24D1R14R6_PYTHON}" scripts/stage4_2r3c3t13s24d1r14r6_direction0_replacement_sentinel.py "${args[@]}"
