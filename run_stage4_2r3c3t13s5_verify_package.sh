...
PYTHON_BIN="${STAGE4_2R3C3T13S5_PYTHON:-${PYTHON:-python3}}"
cd "${PROJECT_DIR}"
...
from tsc_rzip_rllib.diagnostics import stage4_2r3c3t13s5_lattice_native_split_holdout as s4
if not s4.self_test()["passed"]:
    raise SystemExit("T13S5 self-test failed")
s4._validate_config(json.loads((root / "configs/stage4_2r3c3t13s5_lattice_native_split_holdout_370ms.json").read_text()))
for path in (root / "run_stage4_2r3c3t13s5_native.sh", root / "run_stop_stage4_2r3c3t13s5_now.sh"):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[T13S5 verify] Python/JSON/scientific guards passed.")
PY
mapfile -t SHELLS < <(find . -maxdepth 2 -type f -name '*.sh' -print | sort)
for script in "${SHELLS[@]}"; do bash -n "${script}"; done
"${PYTHON_BIN}" -m unittest -v tests.test_stage4_2r3c3t13s5_lattice_native_split_holdout
printf '[T13S5 verify] checksums, shell syntax and focused tests passed.\n'
