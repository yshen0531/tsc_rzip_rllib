...
SOURCE_REVISION="${ID2Z23_SOURCE_REVISION:-}"
OUTPUT="${ID2Z23_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z23_moving_nominal_discrete_vertex_campaign.json"

fail() { printf 'ID2Z23_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }

test -n "$SOURCE_REVISION" || fail "ID2Z23_SOURCE_REVISION is required"
test -n "$OUTPUT" || fail "ID2Z23_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z23_moving_nominal_discrete_vertex_campaign.py \
...
if test -f "$OUTPUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z23_moving_nominal_discrete_vertex_campaign_independent.py \
