...
PYTHON_BIN="${STAGE4_2R3C3T13S24D1R14R4_PYTHON:-/home/yangshen0711/tsc_all/tsc_simulation/venv_simu/bin/python}"
...
required = {
    ...
    "run_stage4_2r3c3t13s24d1r14r4_common.sh",
    "run_stage4_2r3c3t13s24d1r14r4_native.sh",
    "run_stage4_2r3c3t13s24d1r14r4_nohup.sh",
    "run_stage4_2r3c3t13s24d1r14r4_self_test.sh",
    "run_stage4_2r3c3t13s24d1r14r4_verify_package.sh",
    "run_stop_stage4_2r3c3t13s24d1r14r4_now.sh",
    "scripts/stage4_2r3c3t13s24d1r14r4_shell_common.sh",
    "scripts/stage4_2r3c3t13s24d1r14r4_time_shifted_sign_split_sentinel.py",
    "tests/test_stage4_2r3c3t13s24d1r14r4_time_shifted_sign_split_sentinel.py",
...
if not required.issubset(set(listed)):
    raise SystemExit(f"D1R14R4 required package files missing: {sorted(required - set(listed))}")
...
config = root / "configs/stage4_2r3c3t13s24d1r14r4_time_shifted_sign_split_sentinel_370ms.json"
cfg = json.loads(config.read_text(encoding="utf-8"))
d1r14r4._validate_config(cfg, config)
if not d1r14r4.self_test(config)["passed"]:
    raise SystemExit("D1R14R4 self-test failed")
for path in (
    root / "run_stage4_2r3c3t13s24d1r14r4_native.sh",
    root / "run_stop_stage4_2r3c3t13s24d1r14r4_now.sh",
):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[T13S24D1R14R4 verify] Python/JSON/scientific guards passed.")
PY
mapfile -t SHELLS < <(find . -maxdepth 2 -type f -name '*.sh' -print | sort)
for script in "${SHELLS[@]}"; do bash -n "${script}"; done
"${PYTHON_BIN}" -m unittest -v \
  tests.test_stage4_2r3c3t13s24d1r14r4_time_shifted_sign_split_sentinel
printf '[T13S24D1R14R4 verify] checksums, shell syntax and focused tests passed.\n'
