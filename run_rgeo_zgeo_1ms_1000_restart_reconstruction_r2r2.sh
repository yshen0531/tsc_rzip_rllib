...
cd "$ROOT"
: "${NR1000_RECON_R2R2_SOURCE_REVISION:?set NR1000_RECON_R2R2_SOURCE_REVISION}"
...
ARGS=(
  --config configs/rgeo_zgeo_1ms_1000_restart_reconstruction_r2r2.json
  --source-revision "$NR1000_RECON_R2R2_SOURCE_REVISION"
  --output "$OUTPUT"
)
if [[ "$OFFLINE" == 1 ]]; then
  ARGS+=(--offline)
