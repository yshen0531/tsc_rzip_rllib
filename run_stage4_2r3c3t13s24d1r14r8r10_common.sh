...
PYTHON="${STAGE4_2R3C3T13S24D1R14R8R10_PYTHON:-/home/yangshen0711/tsc_all/tsc_simulation/venv_simu/bin/python}"
CONFIG="${STAGE4_2R3C3T13S24D1R14R8R10_CONFIG:-${PROJECT_DIR}/configs/stage4_2r3c3t13s24d1r14r8r10_replacement_scale_formal_authority_audit.json}"
COMMAND="${STAGE4_2R3C3T13S24D1R14R8R10_COMMAND:-primary}"
OUTPUT_ROOT="${STAGE4_2R3C3T13S24D1R14R8R10_OUTPUT_ROOT:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8r10_runs}"
RUN_DIR="${STAGE4_2R3C3T13S24D1R14R8R10_RUN_DIR:-${OUTPUT_ROOT}/stage4_2r3c3t13s24d1r14r8r10_replacement_scale_formal_authority_audit_$(date -u +%Y%m%d_%H%M%S)}"
R8_RUN="${STAGE4_2R3C3T13S24D1R14R8R10_R8_RUN:-${PROJECT_DIR}/stage4_2r3c3t13s24d1r14r8_runs/stage4_2r3c3t13s24d1r14r8_partitioned_broad_response_identification_20260804_5e57f60_v2}"
...
if [[ "${COMMAND}" == primary ]]; then
  ...
  [[ -f "${RUN_DIR}/stage4_2r3c3t13s24d1r14r8r10_replacement_scale_formal_authority_audit/analysis/primary_detailed.json" ]]
fi

mkdir -p "${RUN_DIR}"
stage4_2r3c3t13s24d1r14r8r8_export
cd "${PROJECT_DIR}"
printf '[R8R10] user=%s host=%s command=%s run_dir=%s zero_new_tsc=true\n' "$(id -un)" "$(hostname)" "${COMMAND}" "${RUN_DIR}"

args=(
  --config "${CONFIG}" --run-dir "${RUN_DIR}" --r8-run "${R8_RUN}"
...
if [[ "${COMMAND}" == primary ]]; then
  exec "${PYTHON}" tsc_rzip_rllib/diagnostics/stage4_2r3c3t13s24d1r14r8r10_replacement_scale_formal_authority_audit.py "${args[@]}"
fi
exec "${PYTHON}" docs/codex/audit_tools/stage4_2r3c3t13s24d1r14r8r10_independent_forensics.py "${args[@]}"
