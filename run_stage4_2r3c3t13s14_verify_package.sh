...
PYTHON_BIN="${STAGE4_2R3C3T13S14_PYTHON:-${PYTHON:-python3}}"
cd "${PROJECT_DIR}"
...
from tsc_rzip_rllib.diagnostics import stage4_2r3c3t13s14_active_calibration_sentinel as s14
cfg = json.loads((root / "configs/stage4_2r3c3t13s14_active_calibration_sentinel_370ms.json").read_text())
s14._validate_config(cfg)
if not s14._self_test(root / "configs/stage4_2r3c3t13s14_active_calibration_sentinel_370ms.json")["passed"]:
    raise SystemExit("T13S14 self-test failed")
for path in (root / "run_stage4_2r3c3t13s14_native.sh", root / "run_stop_stage4_2r3c3t13s14_now.sh"):
    text = path.read_text(encoding="utf-8")
    if "ray stop --force" in text or "pkill" in text:
        raise SystemExit("broad process termination is forbidden")
print("[T13S14 verify] Python/JSON/scientific guards passed.")
PY
mapfile -t SHELLS < <(find . -maxdepth 2 -type f -name '*.sh' -print | sort)
for script in "${SHELLS[@]}"; do bash -n "${script}"; done
"${PYTHON_BIN}" -m unittest -v tests.test_stage4_2r3c3t13s14_active_calibration_sentinel
printf '[T13S14 verify] checksums, shell syntax and focused tests passed.\n'
