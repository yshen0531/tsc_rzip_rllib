...
REV="${ID2Z32_SOURCE_REVISION:-}"
OUT="${ID2Z32_OUTPUT:-}"
CONFIG="$PROJECT_ROOT/configs/rgeo_zgeo_1ms_id2z32_post_event_delayed_tail_d0.json"
fail() { printf 'ID2Z32_LAUNCH_FAIL: %s\n' "$1" >&2; exit 2; }
test -n "$REV" || fail "ID2Z32_SOURCE_REVISION is required"
test -n "$OUT" || fail "ID2Z32_OUTPUT is required"
...
python scripts/rgeo_zgeo_1ms_id2z32_post_event_delayed_tail_d0.py \
...
if test -f "$OUT/result.json"; then
  python scripts/rgeo_zgeo_1ms_id2z32_post_event_delayed_tail_d0_independent.py \
