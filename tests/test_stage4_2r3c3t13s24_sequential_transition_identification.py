...
class Stage42R3C3T13S24Tests(unittest.TestCase):
    ...
    def test_frozen_config_and_self_test(self):
        s24._validate_config(copy.deepcopy(self.cfg), CONFIG)
        result = s24.self_test(CONFIG)
        ...
        for value in cases:
            with self.subTest(value=value):
                with self.assertRaises(ValueError):
                    s24._validate_config(value, CONFIG)
        ...
        for pair_index, partition in enumerate(partitions):
            for history in ("plus_first", "minus_first"):
                ...
                specs.append(
                    {
                        **copy.deepcopy(row),
                        "r3c3_probe_id": s24.s21.BASELINE_PROBE_ID,
    ...
    def test_spec_matrix_is_exactly_1000_and_whole_pair_partitioned(self):
        ...
        with mock.patch.object(s24.s21, "build_specs", return_value=source_specs):
            specs = s24.build_specs(ctx, table)
    ...
    def test_issue_and_cancel_use_current_center_and_exact_return(self):
        controller = object.__new__(s24.SequentialAmplitudeCodedProbeController)
        ...
        center_fields = [s24.s21.s16.s9.format_number(0.0) for _ in range(14)]
        target_fields = [
            s24.s21.s16.s9.format_number(0.25 if coil < 4 else 0.0)
        ...
        actual = tuple(
            s24.Decimal("0.25") if coil < 4 else s24.Decimal("0")
            for coil in range(14)
        )
        chosen = {
    ...
    def test_phase_guard_and_incomplete_resume_fail_closed(self):
        ...
        with mock.patch.object(s24, "_read_json", return_value={"phase_status": "wrong", "finished": False}):
            with self.assertRaises(ValueError):
                s24._require_phase(ctx, "offline_ready")
        path = mock.Mock()
        path.is_file.return_value = False
        self.assertFalse(s24._result_complete(path, {"horizon_steps": 35}))
