...
from docs.codex.audit_tools import (
    stage4_2r3c3t13s24d1r14r8r17_independent_forensics as independent,
)
from tsc_rzip_rllib.diagnostics import (
    stage4_2r3c3t13s24d1r14r8r17_adjacent_switch_interaction_binary_cube_preflight as preflight,
)


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/stage4_2r3c3t13s24d1r14r8r17_adjacent_switch_interaction_binary_cube_preflight.json"


class AdjacentSwitchInteractionBinaryCubePreflightTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_matrix_geometry_and_frozen_feature_order(self) -> None:
        ...
        self.assertEqual(np.linalg.matrix_rank(x_full), 6)
        self.assertEqual(np.linalg.matrix_rank(x_cube), 6)
        self.assertLessEqual(np.linalg.cond(x_dev), 6.70)
        self.assertLessEqual(np.linalg.cond(x_full), 2.62)
        self.assertLessEqual(np.linalg.cond(x_cube), 1.74)
        self.assertTrue(np.allclose(preflight.code_row("UVVU"), [1, 1, -1, -1, 1, -1 / 3]))
        self.assertTrue(np.array_equal(preflight.code_row("UVUV"), [1, 1, -1, 1, -1, -1]))

    def test_svd_fit_exactly_recovers_affine_calibration_and_missing_codes(self) -> None:
        development = self.cfg["code_contract"]["development_codes"]
        rng = np.random.default_rng(1608)
        coefficients = rng.normal(size=(6, 7, 3))
    ...
    def test_contract_rejects_gate_code_and_learning_mutation(self) -> None:
        mutations = (
            ...
            ("model_contract", "nonlinear_terms_allowed", False),
    ...
    def test_primary_and_independent_model_implementations_are_separate(self) -> None:
        ...
        self.assertNotIn(
            "stage4_2r3c3t13s24d1r14r8r17_adjacent_switch_interaction_binary_cube_preflight as",
            source,
        )
        self.assertIn("np.linalg.lstsq", source)
        self.assertIn("np.linalg.svd", inspect.getsource(preflight.fit_svd))
        self.assertIn("np.dot(q[:-1], q[1:])", source)
        self.assertNotIn("np.dot(q[:-1], q[1:])", inspect.getsource(preflight.code_row))
    ...
    def test_launcher_uses_existing_server_venv_and_zero_tsc_commands(self) -> None:
        launcher = (ROOT / "run_stage4_2r3c3t13s24d1r14r8r17_common.sh").read_text(encoding="utf-8")
