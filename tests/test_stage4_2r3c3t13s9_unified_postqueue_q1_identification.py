...
from scripts import stage4_2r3c3t13s9_server_postprocess as postprocess
...
class Stage42R3C3T13S9Tests(unittest.TestCase):
    def test_frozen_config_and_exact_component_count(self):
        cfg = _cfg()
        s4._validate_config(cfg)
        self.assertEqual(
            cfg["lattice_probe"]["expected_current_components"], 34272
        )
        self.assertEqual(34 * 37 * 14 + 34 * 35 * 14, 34272)

    def test_q1_sources_and_postqueue_effect_states_are_frozen(self):
        cfg = _cfg()
        self.assertEqual(
            [row["pair_id"] for row in cfg["holdout_contexts"]],
            [
        ...
        weak = cfg["lattice_probe"]["schedule_by_delay"]["2"]
        self.assertEqual(
            tuple(weak["transport"][key] for key in (
        ...
        self.assertEqual(
            tuple(weak["braking"][key] for key in (
                "issue_step", "cancel_step", "first_effect_state",
                "cancel_effect_state",
            )),
            (14, 15, 15, 16),
    ...
    def test_dynamic_lattice_moves_required_steps_and_reproduces_fields(self):
        _, model, current, plan = _plan()
        self.assertTrue(plan["passed"])
        self.assertGreaterEqual(plan["minimum_significant_grid_steps_actual"], 1)
    ...
    def test_native_split_directions_and_coil8_amplitude(self):
        cfg = _cfg()["lattice_probe"]
        modes = np.zeros((14, 3), dtype=float)
        modes[:, 0] = 1.0 / math.sqrt(14)
        modes[::2, 1] = 1.0
        modes[1::2, 1] = -1.0
        modes[:, 1] /= np.linalg.norm(modes[:, 1])
        modes[:7, 2] = 1.0
        modes[7:, 2] = -1.0
        modes[:, 2] /= np.linalg.norm(modes[:, 2])
        split = int(cfg["split_coil_index_tsc"])
        without = s4.lattice_native_direction(
            modes, "mode0_without_coil8", split_coil=split
        )
        coil_only = s4.lattice_native_direction(
            modes, "mode0_coil8_component", split_coil=split
        )
        self.assertEqual(without[split], 0.0)
        self.assertEqual(np.count_nonzero(coil_only), 1)
        self.assertEqual(coil_only[split], 1.0)
        fields = [format_number(0.0)] * 14
        direction_cfg = s4.direction_lattice_config(
            center_fields=fields,
            turns_tsc=[100.0] * 14,
            direction="mode0_coil8_component",
            cfg=cfg,
        )
        plan = s4.choose_lattice_displacement(
            center_fields=fields,
            measured_current_a_tsc=[0.0] * 14,
            baseline_action_norm_tsc=[0.0] * 14,
            mode_vector_tsc=coil_only,
            turns_tsc=[100.0] * 14,
            max_slew_step_a=1000.0,
            minimum_current_a_tsc=[-20000.0] * 14,
            maximum_current_a_tsc=[20000.0] * 14,
        ...
        self.assertTrue(plan["passed"])
        target_a = (
            float(plan["delta_field_kAt_tsc"][split]) * 1000.0 / 100.0
        )
        self.assertGreaterEqual(target_a, 0.08 - 1e-12)
    ...
    def test_return_first_candidate_reproduces_stored_issue_center(self):
        cfg, model, current, plan = _plan(1.234)
        center = model.apply(current, [0.0] * 14)
        issued = model.apply(current, plan["positive_action_norm_tsc"])
        returned = s4.exact_stored_center_action(
            stored_fields=center.card15_fields,
            measured_current_a_tsc=issued.nominal_readback_current_a_tsc,
            baseline_action_norm_tsc=[0.0] * 14,
            turns_tsc=[100.0] * 14,
            max_slew_step_a=1000.0,
            minimum_current_a_tsc=[-20000.0] * 14,
        ...
        selected = model.apply(
            ...
            returned["action_norm_tsc"],
        )
        self.assertTrue(returned["passed"])
        self.assertTrue(returned["exact_stored_issue_center"])
        self.assertEqual(list(selected.card15_fields), list(center.card15_fields))

    def test_cancellation_policy_audit_enforces_return_first(self):
        base = {"controller_trace": []}
        self.assertTrue(
            s4._cancellation_policy_trace(base, baseline=True)[
                "cancellation_policy_pass"
            ]
        )
        fields = [format_number(0.0)] * 14
        trace = {
            "r3c3t13s9_lattice_event": "cancel",
            "r3c3t13s9_lattice": {
                "selected_method": "exact_negative_relative_current_center",
                "return_candidate": {"passed": True, "target_fields": fields},
                "inverse_candidate": {"passed": True, "target_fields": fields},
            },
            "r3c3t13s9_actuator_prediction": {"card15_fields": fields},
        }
        audited = s4._cancellation_policy_trace(
            {"controller_trace": [trace]}, baseline=False
        )
        self.assertFalse(audited["cancellation_policy_pass"])
        trace["r3c3t13s9_lattice"]["selected_method"] = (
            "exact_return_to_stored_issue_center"
        )
        audited = s4._cancellation_policy_trace(
            {"controller_trace": [trace]}, baseline=False
        )
        self.assertTrue(audited["cancellation_policy_pass"])
    ...
    def test_blind_guard_rejects_late_development_and_bad_hash(self):
        ...
        with self.assertRaisesRegex(ValueError, "64 hexadecimal"):
            guard.freeze("bad")
        guard.freeze("b" * 64)
        with self.assertRaisesRegex(ValueError, "after T13S9 model freeze"):
            guard.before_open("development")
    ...
    def test_controller_spec_strips_identity_and_history_labels(self):
        source = {
            ...
            "phase": "forbidden",
            "category": "forbidden",
            "r3c3t13s9_offline_role": "development",
            "r3c3t13s9_stratum": "hard",
            "r3c3t13s9_source_r3c1_experiment_id": "forbidden",
            "target_id": "nominal",
        }
    ...
    def test_reporting_finalizer_bypasses_control_evaluation(self):
        ...
        final = {"verdict": "UNIFIED_POSTQUEUE_Q1_IDENTIFICATION_FAIL_REDESIGN"}
        with (
