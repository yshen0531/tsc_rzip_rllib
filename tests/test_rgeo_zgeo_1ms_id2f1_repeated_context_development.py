...
class ID2F1FrozenContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.path = ROOT / "configs/rgeo_zgeo_1ms_id2f1_repeated_context_development.json"
    ...
    def test_frozen_config_hash_and_budget(self):
        ...
        self.assertEqual(self.stage["maximum_advance_attempts"], 78 * 32)
        self.assertEqual(self.stage["maximum_retained_states"], 78 * 33)
...
class ID2F1ExecutableContractTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.stage, cls.cfg, cls.targets, cls.id2c1 = load(
            ROOT / "configs/rgeo_zgeo_1ms_id2f1_repeated_context_development.json"
        )
