...
from docs.codex.audit_tools import (
    stage4_2r3c3t13s24d1r14r8r20_independent_forensics as independent,
)
from tsc_rzip_rllib.diagnostics import (
    stage4_2r3c3t13s24d1r14r6_direction0_replacement_sentinel as r6,
    stage4_2r3c3t13s24d1r14r8r20_direct_boolean_cube_completion_authority_sentinel as sentinel,
)
...
CONFIG = (
    ROOT
    / "configs/stage4_2r3c3t13s24d1r14r8r20_direct_boolean_cube_completion_authority_sentinel_370ms.json"
)


class DirectBooleanCubeCompletionAuthoritySentinelTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_frozen_design_contract(self) -> None:
        ...
        self.assertEqual(
            schedule["reused_sequences"],
            ["UUUU", "VVVV", "UVVV", "UUVV", "UUUV", "VUUU", "VVUU", "VVVU", "UVUV", "VUVU"],
        )
        self.assertEqual(
            schedule["source_r8r15_sequences"],
        ...
        self.assertEqual((schedule["safety_spec_count"], schedule["qualification_spec_count"], schedule["total_spec_count"], schedule["final_atlas_spec_count"]), (24, 72, 96, 272))
        self.assertEqual(
            set(schedule["reused_sequences"]) | set(schedule["new_sequences"]),
            {"".join(bits) for bits in itertools.product("UV", repeat=4)},
        )
    ...
    def test_build_specs_freezes_six_missing_sequences_per_context(self) -> None:
        ...
        self.assertEqual(sum(row["partition"] == "safety" for row in specs), 24)
        self.assertEqual(sum(row["partition"] == "qualification" for row in specs), 72)
        expected = set(self.cfg["schedule_contract"]["new_sequences"])
        for index in range(16):
            group = specs[index * 6 : (index + 1) * 6]
            self.assertEqual({row["r8r20_sequence_code"] for row in group}, expected)
            self.assertTrue(all(len(row["r8r20_sequence"]) == 4 for row in group))
        self.assertTrue(all(row["r8r20_allowed_in_expert_dataset"] is False for row in specs))
    ...
    def test_controller_freezes_binary_code_and_four_decisions(self) -> None:
        ...
        with mock.patch.object(r6.MixedBasisSignedExcitationController, "__init__", return_value=None) as parent:
            controller = sentinel.DirectBooleanCubeCompletionController(
                object(), {}, {}, {}, {}, {}, {}, {}, contract, spec=spec
            )
        self.assertEqual(controller.r8r20_decisions, (10, 14, 18, 22))
        self.assertEqual(controller.r8r20_sequence_code, code)
    ...
    def test_issue_symbol_switches_only_to_the_frozen_next_coordinate(self) -> None:
        ...
        self.assertEqual((event["sequence_code"], event["sequence_level"], event["symbol"]), ("UVUU", 2, "V"))

    def test_formal_audit_uses_complete_sixteen_code_cube(self) -> None:
        ...
        for source in (primary, audited):
            ...
            self.assertIn("len(candidates) != 16", source)
            self.assertIn("held_oracle", source)
        self.assertIn("row_count", primary)
        self.assertIn("len(rows) == 272", audited)
    ...
    def test_independent_implementation_does_not_import_primary(self) -> None:
        ...
        self.assertNotIn(
            "stage4_2r3c3t13s24d1r14r8r20_direct_boolean_cube_completion_authority_sentinel as",
            source,
        )
        self.assertIn("_source_r8r12_candidates", source)
        self.assertIn("_source_r8r14_candidates", source)
        self.assertIn("_source_r8r15_candidates", source)

    def test_launcher_uses_existing_server_venv_and_direct_file_workflow(self) -> None:
        source = (ROOT / "run_stage4_2r3c3t13s24d1r14r8r20_common.sh").read_text(encoding="utf-8")
        ...
        self.assertIn("--r8r15-run", source)
        self.assertIn("--r8r19-run", source)
