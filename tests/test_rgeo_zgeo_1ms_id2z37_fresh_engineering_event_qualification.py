...
from scripts import rgeo_zgeo_1ms_id2z37_fresh_engineering_event_qualification as m


ROOT = Path(__file__).resolve().parents[1]


class ID2Z37Tests(unittest.TestCase):
    @classmethod
    ...
    def test_identity_model_and_staged_roles_are_frozen(self) -> None:
        ...
        self.assertEqual(self.model["payload_sha256"],
                         self.stage["evidence"]["id2z36_result"]["required_payload_sha256"])
        self.assertEqual(self.stage["calibration_phase_issue"], 48)
    ...
    def test_new_event_age_fails_calibration_without_relabelling(self) -> None:
        ...
        target = next(row for row in rows if row["family_id"] == "cal48__q_r__minus_then_return")
        target["states"][48 + 11]["r_geo_m"] += .0002
    ...
    def test_launcher_runs_independent_and_forbids_resume(self) -> None:
        launcher = (ROOT / "run_rgeo_zgeo_1ms_id2z37_fresh_engineering_event_qualification.sh").read_text(
            encoding="utf-8")
        self.assertIn("ID2Z37_SOURCE_REVISION", launcher)
