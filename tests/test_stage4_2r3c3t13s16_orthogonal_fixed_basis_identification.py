...
from tsc_rzip_rllib.diagnostics import (
    stage4_2r3c3t13s16_orthogonal_fixed_basis_identification as s15,
)


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/stage4_2r3c3t13s16_orthogonal_fixed_basis_identification_370ms.json"


class Stage42R3C3T13S16Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_frozen_config_and_local_design(self):
        ...
        self.assertEqual(
            self.cfg["active_calibration"]["orthogonal_basis_scale_factors"],
            [1.0, 1.0, 1.0, 0.6],
        )
    ...
    def _exact_trace(self):
        ...
        trace = [
            {"r3c3t13s16_fixed_basis_delta_field_kAt_tsc": copy.deepcopy(basis)}
            for _ in range(12)
        ]
        for index, (issue, cancel, direction) in enumerate(
            ...
            delta = basis[index]
            trace[issue].update({
                "r3c3t13s16_lattice_event": "calibration_issue",
                "r3c3t13s16_event_index": index,
                ...
                "r3c3t13s16_requested_net_kAt_tsc": [0.0] * 14,
            })
            trace[cancel].update({
                "r3c3t13s16_lattice_event": "calibration_cancel",
                "r3c3t13s16_event_index": index,
                "r3c3t13s16_event_direction": direction,
                "r3c3t13s16_lattice": {
    ...
    def test_calibration_trace_requires_constant_rank4_exact_signed_pairs(self):
        ...
        broken[7]["r3c3t13s16_lattice"]["signed_delta_field_kAt_tsc"][3] = -2.0
        self.assertFalse(s15._calibration_trace_audit({"controller_trace": broken})["passed"])
        drifting = copy.deepcopy(trace)
        drifting[4]["r3c3t13s16_fixed_basis_delta_field_kAt_tsc"][0][0] = 2.0
    ...
    def test_controller_freezes_once_and_uses_contemporaneous_centers(self):
        freeze_source = inspect.getsource(s15.OrthogonalFixedBasisProbeController._freeze_fixed_basis)
        signed_source = inspect.getsource(s15.OrthogonalFixedBasisProbeController._fixed_signed_calibration)
        self.assertIn("if self._fixed_basis_delta", freeze_source)
        self.assertIn("np.linalg.qr(native_basis", freeze_source)
        self.assertIn("np.where(np.diag(r) < 0.0", freeze_source)
        self.assertIn("_nearest_exact_symmetric_count", freeze_source)
        self.assertIn("orthogonal_basis_scale_factors", freeze_source)
    ...
    def test_offline_preflight_executes_basis_without_plant_step(self):
        source = inspect.getsource(s15._preaction_basis_preflight)
        self.assertIn("controller.action(initial)", source)
        self.assertNotIn("env.step", source)
        self.assertIn('"plant_advance_count": 0', source)
        prepare = inspect.getsource(s15.prepare_offline)
        self.assertIn("basis_preflight = _preaction_basis_preflight", prepare)
        self.assertIn('basis_preflight["passed"]', prepare)
    ...
    def test_stop_launcher_never_broadly_kills_processes(self):
        text = (ROOT / "run_stop_stage4_2r3c3t13s16_now.sh").read_text(encoding="utf-8")
