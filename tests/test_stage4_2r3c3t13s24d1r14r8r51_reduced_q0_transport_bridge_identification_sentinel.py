...
from docs.codex.audit_tools import (
    stage4_2r3c3t13s24d1r14r8r51_independent_forensics as independent,
)
from tsc_rzip_rllib.diagnostics import (
    stage4_2r3c3t13s24d1r14r6_direction0_replacement_sentinel as r6,
    stage4_2r3c3t13s24d1r14r8r51_reduced_q0_transport_bridge_identification_sentinel as r51,
)


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs" / (
    "stage4_2r3c3t13s24d1r14r8r51_"
    "reduced_q0_transport_bridge_identification_sentinel_370ms.json"
)


class R8R51Q0TransportBridgeTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_frozen_config_and_design_hash(self) -> None:
        ...
        self.assertTrue(all(
            len(value) == 64
            for section in ("source_r8r48", "source_r8r49")
    ...
    def test_build_specs_is_exact_16_by_13_reduced_nonzero_matrix(self) -> None:
        ...
        self.assertNotIn("q0", {row["r8r51_candidate_id"] for row in specs})
        self.assertEqual(
            {row["r8r51_candidate_index"] for row in specs},
            {1, 2, 3, 4, 5, 6, 9, 10, 11, 13, 14, 15, 16},
        )
        self.assertTrue(all(row["r8r51_allowed_in_expert_dataset"] is False for row in specs))
        for row in specs:
            expected = self.matrix @ np.asarray(row["r8r51_candidate_q"], dtype=float)
            self.assertTrue(np.array_equal(expected, np.asarray(row["r8r51_requested_coordinate"])))

    def test_controller_freezes_steps_and_explicit_coordinate(self) -> None:
        spec = {
            "r8r51_candidate_id": "u1p25",
            "r8r51_requested_coordinate": (self.matrix @ np.asarray([0.0, 0.0, 1.25, 0.0])).tolist(),
        }
        ...
        self.assertEqual(controller.r8r51_candidate_id, "u1p25")
    ...
    def test_controller_uses_exact_issue_return_and_refresh_primitives(self) -> None:
        ...
        self.assertNotIn("-self.r8r51", source)
    ...
    def test_runner_uses_exported_source_locator_names(self) -> None:
        runner = (ROOT / "run_stage4_2r3c3t13s24d1r14r8r51_common.sh").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn("r8r51_reduced_q0_transport_bridge_identification_sentinel", runner)
