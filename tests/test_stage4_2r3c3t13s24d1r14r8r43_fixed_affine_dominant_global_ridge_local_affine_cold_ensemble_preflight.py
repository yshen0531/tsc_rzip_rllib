...
class R8R43FixedAffineDominantLocalAffineColdEnsembleTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.config_path = (
            cls.root
            / "configs"
            / "stage4_2r3c3t13s24d1r14r8r43_fixed_affine_dominant_global_ridge_local_affine_cold_ensemble_preflight.json"
        )
        cls.cfg = json.loads(cls.config_path.read_text(encoding="utf-8"))
        cls.trajectories = cls._make_trajectories()
        cls.primary_model = r8r43.fit_model(
            cls.trajectories, lambda _row: True, cls.cfg
        )
        cls.independent_model = independent43._fit_model(
            cls.trajectories, lambda _row: True, cls.cfg
        )
    ...
    def test_frozen_config_and_source_contract(self):
        r8r43.validate_config(self.cfg, project_root=self.root)
        ...
        self.assertEqual(model["global_weight"], 0.25)
        self.assertEqual(model["local_weight"], 0.75)
    ...
    def test_primary_and_independent_expert_predictions_agree(self):
        ...
        for left, right in zip(primary[:3], independent[:3]):
            self.assertLessEqual(
                float(np.max(np.abs(left - right) * r8r43.FACTORS / scales)),
                1e-9,
            )

    def test_ensemble_is_exact_fixed_affine_dominant_sum(self):
        row = self.trajectories[1]["intervals"][4]
        global_prediction, local_prediction, ensemble, _neighbors = r8r43.predict(
            self.primary_model, row, self.cfg
        )
        np.testing.assert_array_equal(
            ensemble, 0.25 * global_prediction + 0.75 * local_prediction
        )

    def test_local_affine_head_is_not_a_local_constant_mean(self):
        row = self.trajectories[3]["intervals"][5]
        local_prediction, _neighbors = r8r43._predict_local_affine(
            self.primary_model["local_model"], row, self.cfg
        )
        group = self.primary_model["local_model"]["intervals"][5]["groups"][0]
        query = (
            r8r43.r8r34.coordinate62(row)
    ...
    def test_local_expert_is_cold_and_target_response_blind(self):
        ...
        for left_row, right_row in zip(left["intervals"], right["intervals"]):
            left_prediction = r8r43.predict(
                self.primary_model, left_row, self.cfg
            )[2]
            right_prediction = r8r43.predict(
    ...
    def test_held_targets_cannot_influence_either_expert_fit(self):
        ...
        left = r8r43.fit_model(self.trajectories, selected, self.cfg)
        right = r8r43.fit_model(changed, selected, self.cfg)
        self.assertEqual(r8r43.model_evidence(left), r8r43.model_evidence(right))

    def test_cardinality_uses_both_frozen_exclusion_families(self):
        evidence = independent43._cardinality(self.trajectories, self.cfg)
    ...
    def test_primary_and_independent_model_parameters_agree(self):
        primary = {
            "outer_model_evidence": [
                {"held_pair": "p", **r8r43.model_evidence(self.primary_model)}
            ],
            "schedule_model_evidence": [
                {"held_schedule": "s", **r8r43.model_evidence(self.primary_model)}
            ],
        }
        independent = {
            "outer_model_evidence": [
                {
                    "held_pair": "p",
                    **independent43._model_evidence(self.independent_model),
                }
            ],
            "schedule_model_evidence": [
                {
        ...
        self.assertLessEqual(
            independent43._scaled_model_difference(primary, independent), 1e-9
        )
        independent["outer_model_evidence"][0]["local_model_evidence"][
            "intervals"
        ][0]["groups"][0]["training_row_count"] += 1
        self.assertEqual(
            independent43._scaled_model_difference(primary, independent), np.inf
        )
    ...
    def test_prediction_comparison_checks_both_experts_and_blend(self):
        ...
        self.assertEqual(
            independent43._scaled_prediction_difference(
    ...
    def test_independent_owns_local_fit_predict_ensemble_and_aggregators(self):
        self.assertIsNot(independent43._fit_model, r8r43.fit_model)
        self.assertIsNot(independent43._predict_local, r8r43._predict_local_affine)
        self.assertIsNot(independent43._predict, r8r43.predict)
        self.assertIsNot(independent43._residuals, r8r43._residuals)
        self.assertIsNot(independent43._outer, r8r43.outer_model_evaluation)
        self.assertIsNot(independent43._schedule, r8r43.schedule_jackknife)
        self.assertIsNot(independent43._compute, r8r43.compute_from_bank)
    ...
    def test_summary_blocks_controller_gate_a_and_learning(self):
        ...
        summary = r8r43._summary(detailed, "model")
    ...
    def test_finalizer_requires_all_agreements_and_seals_zero_tsc(self):
        with tempfile.TemporaryDirectory() as directory:
            stage = Path(directory) / r8r43.RUN_NAME
            paths = r8r43.Paths(
                stage=stage,
            ...
            r8r43._write(paths.analysis / "primary_summary.json", summary)
            r8r43._write(paths.analysis / "primary_detailed.json", detailed)
            r8r43._write(paths.model / "preflight_model.json", {"model": "primary"})
            r8r43._write(paths.manifest, {"stage": r8r43.STAGE})
            r8r43._write(paths.state, {"primary_completed": True})
            independent = {
                "passed": True,
                **{field: True for field in r8r43.AGREEMENT_FIELDS},
                **{field: 0.0 for field in r8r43.DIFFERENCE_FIELDS},
                "route": summary["route"],
                "real_tsc_executed": False,
                "plant_step_count": 0,
                "new_raw_count": 0,
                "primary_summary_sha256": r8r43._sha(paths.analysis / "primary_summary.json"),
                "primary_detailed_sha256": r8r43._sha(paths.analysis / "primary_detailed.json"),
                "primary_model_sha256": r8r43._sha(paths.model / "preflight_model.json"),
            }
            r8r43._write(paths.analysis / "independent.json", independent)
            ctx = r8r43.Context(
                cfg=self.cfg,
