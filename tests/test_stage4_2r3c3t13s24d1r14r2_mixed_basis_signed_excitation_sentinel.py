...
class Stage42R3C3T13S24D1R14R2Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.cfg = json.loads(CONFIG.read_text(encoding="utf-8"))

    def test_frozen_config_and_self_test(self):
        d1r14r2._validate_config(copy.deepcopy(self.cfg), CONFIG)
        result = d1r14r2.self_test(CONFIG)
        ...
        for value in mutations:
            with self.subTest(value=value):
                with self.assertRaises(ValueError):
                    d1r14r2._validate_config(value, CONFIG)
    ...
    def test_load_config_forwards_complete_d1r13_source_closure(self):
        ...
        with mock.patch.object(d1r14r2.d1r13, "load_config", return_value=source_ctx) as load:
            ctx = d1r14r2.load_config(
                CONFIG,
    ...
    def test_zero_action_boundary(self):
        with self.assertRaises(ValueError):
            d1r14r2.zero_action(9)
        for step in (10, 12, 36):
            action = d1r14r2.zero_action(step)
    ...
    def test_prefix_delegates_and_baseline_zero_reads_only_step(self):
        controller = object.__new__(d1r14r2.MixedBasisSignedExcitationController)
        controller.step = 9
        controller.d1r14r2_role = "baseline"
        parent_action = np.linspace(-0.2, 0.2, 14)
        with mock.patch.object(
            d1r14r2.d1r11.SequentialAmplitudeCodedProbeController,
            "action",
        ...
        self.assertTrue(trace["r3c3t13s24d1r14r2_delegated_prefix"])
        ...
        controller.step = 10
        with mock.patch.object(
            d1r14r2.d1r11.SequentialAmplitudeCodedProbeController, "action"
        ) as parent:
            action, trace = controller.action(current)
        parent.assert_not_called()
        self.assertEqual(current.reads, ["step_index"])
        self.assertTrue(np.array_equal(action, np.zeros(14)))
        self.assertTrue(trace["r3c3t13s24d1r14r2_zero_increment"])

    def test_signed_issue_and_cancel_are_fixed_and_causal(self):
        controller = object.__new__(d1r14r2.MixedBasisSignedExcitationController)
        controller.d1r14r2_role = "signed_probe"
        controller.d1r14r2_direction_index = 2
        controller.d1r14r2_sign = -1
        controller.d1r14r2_requested_coordinate = -np.asarray(
            self.cfg["controller_contract"]["requested_coordinate_matrix_columns"],
            dtype=float,
        )[:, 2]
        ...
        self.assertEqual(trace["r3c3t13s24d1r14r2_event"], "signed_issue")
        ...
        self.assertEqual(trace["r3c3t13s24d1r14r2_event"], "stored_center_cancel")
    ...
    def test_mixed_issue_enforces_preregistered_action_and_projection_gates(self):
        ...
        controller.d1r14r2_controller_contract = copy.deepcopy(
    ...
    def test_controller_source_spec_strips_labels_and_has_no_future_sequence(self):
        source = {
            ...
            "d1r14r2_direction_name": "pooled_mixed_1",
            "d1r14r2_sign": 1,
            "d1r14r2_requested_coordinate": np.asarray(
                self.cfg["controller_contract"]["requested_coordinate_matrix_columns"],
                dtype=float,
            )[:, 1].tolist(),
            "d1r14r2_issue_task_step": 10,
            "d1r14r2_cancel_task_step": 11,
            "d1r14r2_zero_after_task_step": 12,
        ...
        with mock.patch.object(
            ...
            output = d1r14r2._controller_source_spec(source, schedule)
        ...
        self.assertNotIn("d1r14r2_direction_name", output)
        self.assertFalse(
            any(
                key.startswith(("d1r13_", "d1r14r2_", "source_d1r13_"))
                for key in output
            )
        )
    ...
    def test_visible_outputs_construct_backward_velocity_and_state10_origin(self):
        ...
        values = d1r14r2._visible_outputs5(trajectory, scales)
    ...
    def test_geometry_uses_fresh_baseline_signed_pairs_and_passes_rank_four(self):
        ...
        result = d1r14r2._response_geometry(ctx, specs, results)
        self.assertTrue(result["passed"])
    ...
    def test_forbidden_trace_keys_and_ray_batching_are_fail_closed(self):
        trace = d1r14r2._trace_template(12, np.zeros(14))
        self.assertTrue(all(not bool(trace.get(key)) for key in d1r14r2.FORBIDDEN_TRACE_KEYS))
        source = inspect.getsource(d1r14r2.evaluate_specs)
    ...
    def test_resume_identity_reauthenticates_package_sources_and_specs(self):
        source = inspect.getsource(d1r14r2._assert_run_identity)
        for token in (
        ...
        self.assertIn("_assert_run_identity(ctx)", inspect.getsource(d1r14r2.run_real))
        self.assertIn("_assert_run_identity(ctx)", inspect.getsource(d1r14r2.postprocess))
    ...
    def test_independent_forensic_is_structurally_separate_and_r1a_authenticated(self):
        ...
        self.assertNotIn(
            "from tsc_rzip_rllib.diagnostics import (\n"
            "    stage4_2r3c3t13s24d1r14r2",
            source,
        )
        self.assertIn("def _r1a_authentication", source)
        self.assertIn("--source-r1a-output", source)
        self.assertIn("MIXED_BASIS_SENTINEL_PASS_TIME_DISTRIBUTED_ID_DESIGN_REQUIRED", source)
        self.assertIn("requested_mixed_coordinate_exact", inspect.getsource(d1r14r2.MixedBasisSignedExcitationController._issue))

    def test_launchers_pin_server_venv_capacity_and_exact_pid_stop(self):
        shell_common = (
            ROOT / "scripts/stage4_2r3c3t13s24d1r14r2_shell_common.sh"
        ).read_text(encoding="utf-8")
        common = (ROOT / "run_stage4_2r3c3t13s24d1r14r2_common.sh").read_text(
            encoding="utf-8"
        )
        stop = (ROOT / "run_stop_stage4_2r3c3t13s24d1r14r2_now.sh").read_text(
            encoding="utf-8"
        )
        self.assertIn("/tsc_simulation/venv_simu/bin/python", shell_common)
        self.assertIn('WORKERS:-72', shell_common)
        self.assertIn('WORKERS}" == 72', shell_common)
        self.assertIn("--source-r1a-output", common)
        self.assertIn("72 fresh TSC", common)
