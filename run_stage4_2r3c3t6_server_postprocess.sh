...
if [[ -z "${RUN_DIR}" ]]; then
  LATEST="${STAGE4_2R3C3T6_OUTPUT_ROOT}/latest_stage4_2r3c3t6_run.txt"
  [[ -f "${LATEST}" ]] || {
    echo "ERROR: Stage4.2R3c3T6 latest-run pointer missing" >&2
    exit 1
  }
  RUN_DIR="$(tr -d '\r\n' < "${LATEST}")"
fi
[[ -d "${RUN_DIR}" ]] || {
  echo "ERROR: Stage4.2R3c3T6 run directory missing: ${RUN_DIR}" >&2
  exit 1
}
AUDIT_ROOT="${STAGE4_2R3C3T6_AUDIT_ROOT:-${PROJECT_DIR}/stage4_2r3c3t6_audits}"
AUDIT_DIR="${STAGE4_2R3C3T6_AUDIT_DIR:-${AUDIT_ROOT}/$(basename "${RUN_DIR}")}"
mkdir -p "${AUDIT_DIR}"
export PYTHONPATH="${PROJECT_DIR}${PYTHONPATH:+:${PYTHONPATH}}"
exec "${STAGE4_2R3C3T6_PYTHON}" \
  "${PROJECT_DIR}/scripts/stage4_2r3c3t6_server_postprocess.py" \
