...
# Fresh Ray session for this guarded resume run.
...
if [[ -z "$B99_6_BEST_ERROR_CKPT" || ! -f "$B99_6_BEST_ERROR_CKPT/mpo_checkpoint.pt" ]]; then
  echo "[run_train_b99_8_resume_b99_6_best_error_native] ERROR: B99.6 best_error_score checkpoint not found." >&2
...
echo "[run_train_b99_8_resume_b99_6_best_error_native] user=$(whoami) host=$(hostname) pwd=$(pwd)"
echo "[run_train_b99_8_resume_b99_6_best_error_native] ulimit -n=$(ulimit -n) ulimit -u=$(ulimit -u)"
echo "[run_train_b99_8_resume_b99_6_best_error_native] RAY_TMPDIR=$RAY_TMPDIR"
echo "[run_train_b99_8_resume_b99_6_best_error_native] nproc=$(nproc) nproc_all=$(nproc --all)"
grep Cpus_allowed_list /proc/self/status || true
taskset -pc $$ || true
echo "[run_train_b99_8_resume_b99_6_best_error_native] SLURM_CPUS_PER_TASK=${SLURM_CPUS_PER_TASK:-unset}"
echo "[run_train_b99_8_resume_b99_6_best_error_native] config=$CONFIG"
echo "[run_train_b99_8_resume_b99_6_best_error_native] resume=$B99_6_BEST_ERROR_CKPT"
